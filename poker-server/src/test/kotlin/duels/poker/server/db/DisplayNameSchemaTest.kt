...
class DisplayNameSchemaTest {
    ...
    fun anEmptyNameIsRefused() {
        ...
        // Paired acceptance: one character (the floor) is accepted
        val singleCharPlayerId = insertPlayerWithName("a")
        val storedName = readDisplayName(singleCharPlayerId)
        assertEquals("a", storedName, "One-character name should be stored")
    }
    ...
    fun aNameWithLeadingOrTrailingSpaceIsRefused() {
        ...
        // Paired acceptance: the same letters without spaces are accepted
        val trimmedPlayerId = insertPlayerWithName("bob")
        val storedName = readDisplayName(trimmedPlayerId)
        assertEquals("bob", storedName, "Trimmed name should be stored as-is")
    }
