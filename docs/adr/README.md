...
| DEC-022 | **The architect's** — what is `web-client`'s toolchain, and how do its checks enter CI? `ADR-0003` fixes React, TypeScript and Tailwind and this does not reopen that; everything around it is open — the build tool and dev server, the package manager and whether a lockfile is committed, the Node version (CI pins 20, the workstation has 26), the unit and component test runner, the lint and format tools **and how they are kept off the generated `protocol.gen.ts`**, how the dev server reaches `/ws` and `/api` on the Kotlin server (proxy or CORS), and whether the client's typecheck/test/build runs as its own CI job or through Gradle. Excludes the browser end-to-end question, which is `DEC-024`. | [`STORY-0301`](../../tasks/stories/STORY-0301-web-client-toolchain.md) | before EPIC-03 starts — it blocks every story in it |
...
| DEC-026 | **The human's, not an architect's** — what does a claim convert, and what happens when a device that already has an anonymous profile with coins signs into a *different* existing account? Merge both balances and both histories, keep the account's and abandon the device's, or refuse the sign-in while a local profile has history. `ADR-0014` defines a balance as `wins − losses`, so every answer except "refuse" either mints coins that no duel paid for or destroys coins a duel did — which is the same class of property as chip conservation and deserves the same care. | [`EPIC-04`](../../tasks/epics/EPIC-04-identity-and-profiles.md) | before STORY-0407 |
| DEC-027 | **The human's, not an architect's** — does an account carry an email address, what may it be used for (recovery only, or contact), and can a password be recovered without one? `docs/vision.md` asks for quiet and minimal, which argues against asking for an email; but a password that cannot be recovered re-creates the lost-profile problem `EPIC-04` exists to fix, one step further along. This also decides whether social sign-in is on the table at all. | [`EPIC-04`](../../tasks/epics/EPIC-04-identity-and-profiles.md) | before EPIC-04 starts — it blocks the credential chain |
| DEC-028 | **The architect's** — what is the credential, session and handshake model once a device id is no longer the only credential a player holds? What is stored and hashed with what; what a session is, where it lives and how long it lasts; what the socket presents in `Hello` and what HTTP presents beside `X-Device-Id`; whether a device id and a session can both be present and which wins; and whether `PROTOCOL_VERSION` moves. `ADR-0012` makes `DeviceId` the sole credential today and `ADR-0002` forbids the client asserting its own identity, so both constrain the answer. | [`EPIC-04`](../../tasks/epics/EPIC-04-identity-and-profiles.md) | before EPIC-04 starts — it blocks the credential chain |
