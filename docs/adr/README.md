...
| DEC-035 | **The architect's** — how does `design/check-drift.sh` read a CSS value? Its value reader is the awk regex `--pd-NAME:[^;]*;`, which knows nothing about strings, so three shapes misbehave: a declaration without its terminating semicolon vanishes or swallows the text after it; a semicolon inside a quoted value (`url("data:…;utf8,…")`) truncates sheet and card identically so real drift passes green; and a quoted string wrapped across sheet lines yields a **false** drift — live-adjacent, since `tokens.css` already wraps `--pd-font-ui`/`--pd-font-mono` and 13 cards declare them. The options: a third quote-aware walker beside the two the file already carries (`SYMEXTRACT`, `ANATOMY`), a convention lint that refuses shapes the reader cannot read, or accepting and recording the limit. Three guesses were built and all three withdrawn under review (#511) | [`TASK-060121`](../../tasks/tasks/TASK-060121-semicolonless-declaration-enters-the-gate.md) | before the sheet's quoted values are reflowed |
