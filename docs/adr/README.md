...
| DEC-038 | **The architect's** — which epic ships `ADR-0028`'s server half, and where does its client half land? `ADR-0028` answered `DEC-018` and was accepted, but `OpponentPresence`, `SeatPresence` and `ActedForAbsentSeat` exist nowhere: no Kotlin type, no row in `docs/protocol.md`, nothing in `protocol.gen.ts`, and no ticket under `STORY-0208` builds them. `EPIC-03` writes no Kotlin, so `STORY-0310` was split with no pause state at all — the wire it would render is not there. `DEC-023` is the precedent for the shape of the answer: `ADR-0044` put the rematch's server half in `EPIC-02` as `STORY-0213`. Open: does the presence half likewise reopen `EPIC-02`, or wait for a later epic; does it take a `PROTOCOL_VERSION` step of its own (`ADR-0028` §8 now names three unlanded bumps sharing one rule); and does the client half re-open `STORY-0310` or become its own story under `EPIC-03`? | [`STORY-0310`](../../tasks/stories/STORY-0310-reconnect-and-resume.md) | before EPIC-03 closes |
