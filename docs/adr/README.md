...
| [0045](ADR-0045-presence-belongs-to-the-table.md) | Presence belongs to the table, not to reconnect, and EPIC-02 ships the server half | Accepted |
...
| DEC-040 | **The architect's** — what mechanism holds the `PROTOCOL_VERSION` lock, so that two branches cannot both claim the same number and both merge green? [`ADR-0045`](ADR-0045-presence-belongs-to-the-table.md) §3 accepted this as a known cost and called enforcing it *"its own decision"*: a three-way merge takes the identical `2` → `3` edit from both sides **without a conflict**, `ProtocolDocumentationTest` compares the document to the constant and both moved together, and `verifyProtocolTypes` only ever sees one branch — so one integer ends up naming two wire shapes with every gate green. The candidates differ in where the gate lives and what a bump costs every future story: a CI check comparing against `origin/develop`; a checked-in version→wire-fingerprint ledger, in the shape the existing protocol gates already use, where two claims on one number conflict textually; a branch-protection setting outside the repository. Three unlanded bumps share the lock today (`STORY-0213`, `STORY-0214`, `STORY-0405`). | [`TASK-000104`](../../tasks/tasks/TASK-000104-a-second-branch-cannot-claim-the-same-protocol-version.md) | before the first of `STORY-0213` / `0214` / `0405` lands its bump |
