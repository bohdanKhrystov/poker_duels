...
| DEC-037 | **The architect's** — after a `Rejected`, is the decision point still open on the client, and when does a rejection stop being shown? `DuelAction.act` returns **only** the `Rejected` frame, so no fresh `YourTurn` follows one; `duel-state.ts` clears `pendingTurn` on `Rejected` (pinned by `TASK-030404`) and never clears `rejection`. Together those mean one rejected action ends that player's hand — the bar goes off with nothing able to re-open it, and its sentence stays on screen for the rest of the duel — while both players are still connected. Three shapes answer it and they are not equivalent: the reducer keeps `pendingTurn` through a `Rejected` and clears `rejection` on the next `YourTurn`/`Snapshot`; the bar keeps its own copy, putting *"it is still your turn"* inside a component no store test can see; or the **server** re-prompts, which is the server/protocol change `EPIC-03` forbids itself and would need another epic's ticket. It is the client's reading of `ADR-0002`, so it is not a component diff's to settle | [`TASK-030712`](../../tasks/tasks/TASK-030712-after-a-rejection-the-player-can-act-again.md) | before STORY-0307 closes |
