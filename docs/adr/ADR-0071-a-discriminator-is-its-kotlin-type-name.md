...
- **`TASK-021402` unblocks at thirteen files with no merged gate edited.** The wire step goes back to
  ...
  edits. `status` moves off `blocked` to **`ready`**: `TASK-021401` landed as `done` while this ADR
  was being written, so `lint_tickets.py`'s rule that a `ready` task has no open `depends_on` is
  satisfied and the ticket is startable the moment this merges.
