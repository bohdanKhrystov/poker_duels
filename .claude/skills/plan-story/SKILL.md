...
- [ ] every `atomic:` ticket's `files_touched` **equals** its own *Files* table's edit-row count (the linter checks this; there is no ceiling — `ADR-0069`), and its file list came from **a probe run until the gate set exits 0** (`ADR-0070`) — not from a single red run, whose output is only a prefix, and not from another ticket's list
