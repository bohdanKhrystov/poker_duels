...
Two ways the gate passes something it should refuse, both found by the #511 review
and both confirmed uncaught on today's gate:
...
- **The lockup clause can go quiet.** `anat_copies` has no floor, so if the one card
  ...
  copies" and exits 0 — the vacuous pass every sibling clause guards against.
...
- The name gate keeps accepting a token *printed* on a card (clause 1's stated
  contract) but resolves names against the sheet's declared set rather than against
  every string in the sheet, comments included.
...
- Whatever `DEC-035` answers about reading values, the name gate's set is the same
  set: land this after that ADR if the two touch, and say so in the PR.

## Out of scope

- How the gate reads values — `DEC-035` and `TASK-060121`.
