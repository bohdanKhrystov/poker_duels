...
verify:
  ...
  - sh -c 'T=$(mktemp -d) && cp -R design "$T/design" && perl -0777 -pi -e "s/\.mark \.coin \{/.lockup .disc {/" "$T/design/screens/create-duel.html" || exit 2; "$T/design/check-drift.sh" >/dev/null 2>&1; [ $? -eq 1 ]'
...
review after the value reader landed:
...
- **The name gate reads comments.** A card may inline a `--pd-` token the sheet only
  *mentions* in prose; clause 1 vouches for the name from that comment and clause 3
  has nothing to compare, so an undeclared token passes both.
...
- Both negatives are in `verify:`, each red on today's gate.
