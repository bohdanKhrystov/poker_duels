...
depends_on: []
verify:
  - test -f design/screens/sign-in.html
  - grep -qF 'design/tokens/tokens.css' design/screens/sign-in.html
  - test "$(grep -o -- '--pd-[a-z0-9-]*' design/screens/sign-in.html | sort -u | wc -l)" -ge 20
...
`design/screens/sign-in.html` exists: a rendered reference for the `sign-in` screen, composed from
the settled vocabulary, so the next UAT round can judge that screen's conformance instead of
recording `BLOCKED — no card`.

## The defect

Round 1 of `/qa-cycle uat regression`, 2026-08-30, commit `c05ee695`. **The `sign-in` screen — the sign-in form reached from account — has no merged card.**
...
The screen is in scope — `docs/test-plan.md` §UAT's inventory lists it, routed by `04-03`, `04-04` and `04-05` — and it
...
| File | Action |
| --- | --- |
| `design/screens/sign-in.html` | create |
...
- **Draw the states this screen actually has**: the form, its refusal state, and the `Forgot your password?` route out.
- **Copy is transcribed, never invented.** Take every string from the module that owns it or from
  the merged ADR that settled it — `web-client/src/account/`, `ADR-0083` and `docs/protocol.md`. A string with no merged source is not drawn; it is a
...
- [ ] `design/screens/sign-in.html` exists and opens in a browser as a rendered card
