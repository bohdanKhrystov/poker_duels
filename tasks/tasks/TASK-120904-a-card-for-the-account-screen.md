...
schema: 2
id: TASK-120904
title: A card for the `account` screen
type: task
status: ready
parent: STORY-1209
module: design
...
depends_on: []
verify:
  - test -f design/screens/account.html
  - grep -qF 'design/tokens/tokens.css' design/screens/account.html
  - test "$(grep -o -- '--pd-[a-z0-9-]*' design/screens/account.html | sort -u | wc -l)" -ge 20
...
`design/screens/account.html` exists: a rendered reference for the `account` screen, composed from
the settled vocabulary, so the next UAT round can judge that screen's conformance instead of
recording `BLOCKED — no card`.

## The defect

Round 1 of `/qa-cycle uat regression`, 2026-08-30, commit `c05ee695`. **The `account` screen — claiming a profile, or the signed-in profile's own page — has no merged card.**
...
The screen is in scope — `docs/test-plan.md` §UAT's inventory lists it, routed by `04-02`, `04-03` and `04-05` — and it
...
| File | Action |
| --- | --- |
| `design/screens/account.html` | create |
...
- **Draw the states this screen actually has**: the claim form an unclaimed profile sees, the signed-in page that states the password route, and the recovery-address section.
- **Copy is transcribed, never invented.** Take every string from the module that owns it or from
  the merged ADR that settled it — `web-client/src/account/AccountScreen.tsx`, `ADR-0050`, `ADR-0083` and `docs/protocol.md`. A string with no merged source is not drawn; it is a
...
- [ ] `design/screens/account.html` exists and opens in a browser as a rendered card
