...
| `P3` | a **genuinely dropped socket** — a reconnect through `reconnecting.ts`, not a reload | OBSERVED (not inferred), both `delayed 0ms` and `delayed 300ms`, driven per `ADR-0156` §5's layout (built bundle on `vite preview`, relay on `4174`/`4175`, browsers on relay): two fresh `mktemp -d` profiles through the relay, a room (WC6DGECW for `0ms`, 05AH7DED for `300ms`), a `PLAYING` duel at preflop on both. `node scripts/qa/delay.mjs cut 6270` and `cut 6271` answered `cut 2` each time (two live pairs each: X's app socket and a second socket), proving a live pair was severed. **The socket reconnection is observed in place, not as a reload.** `performance.getEntriesByType('navigation')[0].type` remains `navigate` throughout (confirmed at `open` before any cut), not switching to `reload`. `window.__pdFrames` survives: `frames` captures the moment of severance and recovery continuously — countdown (22→21→...→19, then "Waiting for your rival…", then 19→18→...), with presence markers ("Your rival is away.", "Your rival is back.") visible in the frame sequence, down to single-digit counts. No page reload, no document death, no `MutationObserver` loss. **Dropped browser (X) at `0ms`:** table stays visible through countdown, one frame shows "Waiting for your rival…", game resumes; presence markers too fast to capture. **At `300ms`:** presence markers visible in frames — "Waiting for your rival…" appears, then "Your rival is away.", then ~3–5 frames later "Your rival is back." — spanning perhaps 4–5 seconds total within the 30s record window. **Rival (Y) continues uninterrupted:** table never leaves screen, "Your rival is back." appears once X reconnects, tick clock continues throughout (answering `ADR-0108`: "clock is indifferent to socket"). X's absence is marked, reconnection is marked, duel resumes. **The relay listened through the cut.** Unlike Attempt 2, the relay actually accepted and forwarded X's reconnection immediately following its own reconnection to the target — the relay stayed listening through the cut, and the new WebSocket `reconnecting.ts` opened within ~500ms found a live relay waiting. **Instrument setup (ADR-0117, ADR-0156):** built bundle contains zero `@vite/client` (confirmed: `curl http://localhost:4174/ | grep '@vite/client'` finds nothing), so Vite's HMR is absent and no second socket was cut. Only the relayed app socket was severed by design, no dev-server interference this time. **What the reading establishes:** `reconnecting.ts` opens a fresh socket in place, under a page staying running and keeping its store intact (roomCode, view, outcome unchanged). No lobby appears, no screen blinks. The "away" and "back" messages are the server's presence tracking responding to disconnect/reconnect events the client sends via the new socket. | `f4e83bad` | `delayed 0ms` (CDP 9622/9623, relay listen:4174 target:4273 control:6270, room WC6DGECW) and `delayed 300ms` (CDP 9622/9623 reused, relay listen:4175 target:4273 control:6271, room 05AH7DED), both driven fresh profiles each, same commit `f4e83bad` |
