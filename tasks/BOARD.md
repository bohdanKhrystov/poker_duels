...
| DEC-151 | **The architect's** — by what mechanism does a display name **change**? Registered 2026-09-07 by [`ADR-0130`](../docs/adr/ADR-0130-a-name-can-be-changed-and-the-name-it-leaves-is-spent.md), which fixes what a player sees and designs nothing: what `player_display_name_is_permanent()` becomes now that `ADR-0029` §4's *`name → a different name` raises* is superseded; what statements a change runs, in what transaction, and what rolls back; whether the vacated `name_registry` row moves `TAKEN → RETIRED` and whether `retired_from` is populated — under `ADR-0130` §4's constraint that `ADR-0052` §6's derived bit reads **false** for a player who renamed themselves; what `PUT /api/me/name` answers now that `403 AlreadyNamed` has no cause, and what `docs/protocol.md` says instead; and **whether the write needs a budget**, which `ADR-0029` §5 already named as additive and `ADR-0022`-shaped, keyed by `PlayerId`. It may not change what `ADR-0130` §§1–7 say a player sees | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1409` ships — the namespace has no brake until it lands |
| DEC-152 | **The architect's** — by what mechanism does a browser **stop owning the profile it owned**, at a sign-out and only there, and how does the confirmation learn **before** it acts which of the two sign-outs this is? Registered open 2026-09-07 by [`ADR-0131`](../docs/adr/ADR-0131-signing-out-of-your-own-account-hands-the-browser-a-new-profile.md) §7, which fixes the product half and writes no repair — `ADR-0105` §6's precedent. Six product constraints the mechanism may not move: it **revokes nothing** (`ADR-0049` §2's `UPDATE` and its finality trigger stay out of this path, §3 is not entered, and the `device_binding` row left behind stays **live**); it writes nothing to `player`, `credential`, `duel` or `duel_result` and moves no coin; the abandoned profile stays reachable by its credential so `ADR-0131` §2's invariant holds for every path it creates; **the case must be knowable to the confirmation before the press**, which is a server fact — `DuelSocket.kt:168` answers a session-identified handshake with `deviceId = null` and `ADR-0027` §4 does not even validate a device id presented beside a token, so the client cannot compare the session's player to its own — of the same class as `DEC-147`'s and answerable by neither `signedIn` nor the presence of a token; `ADR-0030` §8's write-once rule **stands everywhere else** and no longer forbids the change at this one moment; and `ADR-0002` holds, so a browser may forget a credential it holds and may not assert which player it is. Three shapes named without choosing, each with a different answer to *what the old live binding means afterwards*: the client forgets `pd.deviceId` and `ADR-0027` path 3 mints on the next `Hello` (no schema, no wire, no server code); the sign-out response carries a new device id; or a server act the client only obeys | [`ADR-0131`](../docs/adr/ADR-0131-signing-out-of-your-own-account-hands-the-browser-a-new-profile.md) §7 | before `STORY-1410`'s **implementing** ticket — its design card and its copy wait on nothing |
...
**Registers `DEC-152`**, the architect's, fenced by §7. `STORY-1410` is splittable.
...
architect's, for the mechanism — due **before** `STORY-1409` ships, not after.
...
| `STORY-1409` | A name can be changed in account settings — *item 1c; [`ADR-0130`](../docs/adr/ADR-0130-a-name-can-be-changed-and-the-name-it-leaves-is-spent.md) is the answer* | `DEC-151` (the mechanism; the product half is answered — the name given up is **retired**, history prints the name as it **is**, the form is **one** form on the **account screen**, and `PERMANENCE_LINE` leaves the product) |
