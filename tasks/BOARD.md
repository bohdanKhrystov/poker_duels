...
**`STORY-1401`, `STORY-1403` and `STORY-1414` are written and split; the other thirteen seams are
not.** The planner writes each one once the decisions it waits on are merged, and the first ticket
of every story that puts a new surface in front of a player is its **design card** (`ADR-0091` §2)
— which `STORY-1401` owes none of, because the merged card already draws `Timebank 3:00` and the
client is what fails to transcribe it, and which `STORY-1414` owes none of either, because it puts
no new surface up: it corrects four nodes on cards that already exist, and `ADR-0122` §7 says so.
...
| | [TASK-140202](tasks/TASK-140202-the-front-doors-primary-control-says-play-duel.md) The front door's primary control says `Play duel` — *`Lobby.tsx` ×1, `App.test.tsx` ×18, `Lobby.test.tsx` ×10, and **no `atomic:`**: the probe showed `npm run check` refuses every proper subset of exactly these three, which is the ordinary cap. `Play duel` is nine characters shorter, so Prettier reflows the call sites and `format:check` fails until it is run — the ticket says so, having measured it. A gate renames the source back and requires `17 failed \| 121 passed (138)`* | S | backlog |
