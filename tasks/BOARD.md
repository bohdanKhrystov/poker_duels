...
| DEC-154 | **The architect's** — by what call, at what point in `replyToJoinRoom`, and under what lock order is a held `WAITING` room **released** when its holder takes a seat in another room? Registered 2026-09-08 by [`ADR-0141`](../docs/adr/ADR-0141-taking-a-seat-elsewhere-releases-the-room-you-were-holding.md) §8, which fixes the product half and refuses to choose the mechanism. What makes it a real question: `RoomRegistry.mutate` runs its block under the **joined** room's mutex and that block is a plain function type, not a `suspend` one, so a second room's mutex **cannot** be acquired inside it (`ADR-0133` §4). The release therefore sits outside that critical section, and each side has a price — **before**, and a refused join has already destroyed a room, which `ADR-0141` §4 forbids; **after**, and a window opens in which both rooms are live, which `ADR-0141` §3 bounds but does not close. `ADR-0141` §8 fixes what it must satisfy: found from the **player**, only a still-`WAITING` room, only on a join that seated somebody, nothing else moves, and **no new frame**, so no `PROTOCOL_VERSION` step is in question. **Blocks** the ticket that implements `ADR-0141` | [`ADR-0141`](../docs/adr/ADR-0141-taking-a-seat-elsewhere-releases-the-room-you-were-holding.md) §8 | before the ticket that releases a room is written |
| DEC-157 | **The product owner's** — does *give this profile a password* owe an irreversibility sentence of its own? Registered 2026-09-09 by [`ADR-0143`](../docs/adr/ADR-0143-irreversibility-is-said-last-and-never-coloured.md) §4, which found it by measuring rather than by being asked. Signing up claims a handle into `credential`'s `UNIQUE (kind, identifier)` (`V4__credential_and_auth_session.sql:17`), and [`ADR-0039`](../docs/adr/ADR-0039-v01-offers-no-account-deletion.md) refuses account deletion in v0.1, so **no path a player has gives a handle back** — while `SIGN_UP_LABEL`, `HANDLE_LABEL` and `PASSWORD_LABEL` say nothing about it and the form has no second press. That makes it the one act on the account screen that fails `ADR-0143` §2's first obligation. It is **not** answered there on purpose: it is an obligation question of `ADR-0130` §5's kind about a **different act**, and folding it in would have settled two decisions because they looked related. The answer must also say whether a handle is *spent* in the sense `ADR-0130` §2 gives the word, or merely held for as long as the credential row exists — the schema does not decide that, because nothing deletes the row. **Blocks nothing** | [`ADR-0143`](../docs/adr/ADR-0143-irreversibility-is-said-last-and-never-coloured.md) §4 | before the sign-up form's words are next opened, or before the first player who is not the author holds an account — whichever comes first |
...
changes. **Registers `DEC-154`**, the architect's, for the release's call site and lock order. Costs
