...
| | [TASK-140914](tasks/TASK-140914-the-account-screen-carries-the-name-form.md) The account screen carries the name form — *an **optional** prop, so the ticket is green with nothing passing it. Three tests: the same profile with and without the prop (differing in one input, so neither constant passes both), and all four non-`profile` states enumerated rather than sampled* | XS | backlog |
| | [TASK-140915](tasks/TASK-140915-the-front-door-hands-the-form-to-the-account-screen.md) The front door hands the form to the account screen — *the front-door render **stays** for one ticket: deleting it here drags the e2e arc in, and deleting it without the wiring fails `TS6133` — both measured. The test drives a real submit and asserts the spy took the typed string, because a handler that ignored its argument satisfies `toHaveBeenCalled`* | XS | backlog |
| | [TASK-140916](tasks/TASK-140916-the-arc-names-its-profile-where-the-form-now-stands.md) The arc names its profile where the form now stands — *ten call sites move behind the `Account` door. The pattern was **run on this tree** against one of the four cases that break, with the front-door render already deleted. Green before and after the deletion, which is the risk: a query against the whole container would find the front door's copy today and fail tomorrow* | S | backlog |
