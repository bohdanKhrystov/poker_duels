...
| DEC-079 | **The product owner's** — is `ADR-0036`'s *"not again"* a fact about the **player** or about **this browser**, and what **spends** the offer? Raised on 2026-08-27 when `STORY-0415` was split, because a merged ADR and the story that applies it say opposite things. [`ADR-0036`](../docs/adr/ADR-0036-an-account-is-offered-never-required.md) §Consequences: the dismissal *"cannot live in `localStorage` alone… **It belongs on the profile**"*, so it follows the player to a second browser and survives a storage clear. `STORY-0415` §Design notes: *"stored under a key this module owns"*, asserted *"through the injected storage"*, so it does neither. Second half: `DEC-049` records that a `429` *"spends nothing, including `ADR-0036`'s offer, **which only 'Not now' dismisses**"* — so does a player who takes the offer, reaches `#/account` and never signs up see it again on their next win, against this story's *"It does not appear a second time after a second win"*? Both halves are what a player experiences and derive from `docs/vision.md`; **neither touches the vision's *What it is / What it is not***, so this is not the human's. Blocks `STORY-0415`'s fifth, sixth and seventh tickets — the persistence, the wiring and the arc — and none of the four already written | [`STORY-0415`](stories/STORY-0415-the-offer-after-a-first-win.md) | before STORY-0415's fifth ticket |
| DEC-080 | **The architect's**, downstream of `DEC-079` — what carries the offer's state on the wire, and what tells the client this win is the **first**? `GET /api/me` declares exactly `playerId`, `coinBalance`, `displayName`, `displayNameRemoved`, `deviceRouteLive`, `hasRecoveryEmail` and carries neither fact; `ADR-0036` §Consequences calls the first-win fact *"a read-path question rather than new state"* and names no field, endpoint or column. If `DEC-079` answers *the player*: the column, the field, the endpoint that records a dismissal, and whether `STORY-0415` — `module: web-client`, depending only on `STORY-0412` — grows a server half or a server story precedes it. If it answers *this browser*: whether the flag is one client-owned key, in which case `web-client/src/protocol/one-module-owns-each-storage-key.test.ts` gains a third entry, and `ADR-0036`'s *belongs on the profile* is **amended rather than quietly ignored**. Blocks exactly what `DEC-079` blocks | [`STORY-0415`](stories/STORY-0415-the-offer-after-a-first-win.md) | before STORY-0415's fifth ticket |
...
| [STORY-0415](stories/STORY-0415-the-offer-after-a-first-win.md) The offer — an account after a first win, dismissed for good | | | **in progress** — **partially split** on 2026-08-27 into **four**, with `TASK-041501` the head and the other three depending on it for sequencing only, their `Files` tables pairwise disjoint so all three run in one batch. The split stopped at four on purpose. `ADR-0036` §Consequences says the dismissal flag *"belongs on the profile"*; this story's own design notes say *"stored under a key this module owns"* and its criterion asserts it *"through the injected storage"*. Those cannot both be built, and `GET /api/me` carries neither that field nor any first-win fact today — so **`DEC-079` (the product owner's)** asks whether *"not again"* is a fact about the player or about this browser, and what **spends** the offer, since `DEC-049` says *"only 'Not now' dismisses"* while this story says a second win never shows it; **`DEC-080` (the architect's, downstream)** asks what carries it on the wire and whether this `module: web-client` story grows a server half. Neither blocks the four written tickets, which hold under either answer; the persistence, the `Lobby` wiring and the whole-client arc are deliberately unwritten. **The split's most useful measurement:** the accept control **cannot** be `useScreen().open("account")` — `ADR-0076` §3 is enforced by a `Lobby.tsx` effect that replaces any non-`first` screen back to `/` while `state.outcome !== null`, and rendering `Lobby` over a store holding a `DuelFinished` and then setting `#/account` measured `hash=""`, no account screen and *Victory* still on screen, while the same navigation with no outcome reached it. So the control is `<a href="/#/account">`, a real page load, which `ADR-0076` §6 requires of that screen's links anyway. Every ticket's Proof section was run rather than predicted: twelve mutations, each reddening the test it names, plus two vacuity checks on `queryBy…`-shaped negatives that both reddened. Suite **811/103**, and **822/106** with all four | v0.2 |
| | [TASK-041501](tasks/TASK-041501-the-words-the-offer-says.md) The words the offer says, and the one word ADR-0036 already chose | XS | backlog |
| | [TASK-041502](tasks/TASK-041502-whether-the-offer-is-made-at-all.md) Whether the offer is made — a win, no credential, and not already settled | S | backlog |
| | [TASK-041503](tasks/TASK-041503-the-offer-and-the-page-load-that-reaches-the-account-screen.md) The offer itself, and the page load that reaches the account screen | S | backlog |
| | [TASK-041504](tasks/TASK-041504-the-result-screen-carries-an-offer-it-does-not-make.md) The result screen carries an offer it does not make, and gives nothing up for it | S | backlog |
