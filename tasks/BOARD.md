...
| | [TASK-140916](tasks/TASK-140916-the-arc-names-its-profile-where-the-form-now-stands.md) The arc names its profile where the form now stands — *ten call sites move behind the `Account` door. The pattern was **run on this tree** against one of the four cases that break, with the front-door render already deleted. Green before and after the deletion, which is the risk: a query against the whole container would find the front door's copy today and fail tomorrow* | S | ready |
| | [TASK-140917](tasks/TASK-140917-the-form-leaves-the-front-door.md) The form leaves the front door — *it owns the two `Lobby.test.tsx` cases its change invalidates and rewrites both rather than deleting them; the `TestPlayer` count moves 2 → 1, and 1 is not a value the bug leaves unchanged. `TASK-140915`'s wiring test is named must-stay-green, because a deletion ticket whose gates are all negative can delete a feature instead of moving it* | XS | backlog |
| | [TASK-140918](tasks/TASK-140918-a-player-who-holds-a-name-is-offered-the-form-that-changes-it.md) A player who holds a name is offered the form that changes it — *last on purpose: landing it before `TASK-140917` would put a rename control on the front door, the one placement `ADR-0130` §6 refuses. The no-suggestion gate renders a **named and a nameless** profile and asserts the field is empty in both — on the nameless render alone, an echoing field and an empty one are the same. The fake answers a different string from the one typed, or the canonical-answer test is vacuous* | S | backlog |
