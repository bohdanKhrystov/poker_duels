...
| | [TASK-130803](tasks/TASK-130803-the-turn-allowance-and-the-timebank-are-configuration.md) The two numbers are read from configuration at startup — *`duel.turnMillis` / `TURN_MILLIS` and `duel.timebankMillis` / `TIMEBANK_MILLIS`, resolved through the helper the window already uses. The environment-outranks-key test states **both** numbers at **different** values, because one resolver serving both cannot be told from two by a single value* | S | ready |
