...
| | [TASK-041226](tasks/TASK-041226-the-sign-in-screens-word-and-its-slug.md) The sign-in screen's word, and the address that word becomes | XS | **blocked** — `DEC-077` **answered** on 2026-08-26 by [`ADR-0083`](../docs/adr/ADR-0083-the-second-account-screen-is-sign-in-and-its-address-is-never-refused.md) (*Sign in*, `#/sign-in`); the ticket file still reads `blocked` until the planner folds the answer in, and its `^[a-z]+$` criterion widens |
| | [TASK-041227](tasks/TASK-041227-the-sign-in-screen-at-its-address-and-the-door-to-it.md) The sign-in screen at its address, and the door to it from the account screen | S | **blocked** — `DEC-077` **answered** on 2026-08-26 by [`ADR-0083`](../docs/adr/ADR-0083-the-second-account-screen-is-sign-in-and-its-address-is-never-refused.md); the ticket file still reads `blocked` until the planner folds the answer in, and it gains §5's landing rule, which reaches a fourth file |
