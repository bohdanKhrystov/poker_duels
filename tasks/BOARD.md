...
decisions. **`STORY-1301` is cut into three tickets**; the other ten are not — `/plan-story <STORY-ID>` does
that one story at a time, and the first ticket of every story that puts a new surface in front of a
player is its **design card**, which merges before the ticket that implements it is startable
(`ADR-0091` §2, `EPIC-13` *Design first*).
