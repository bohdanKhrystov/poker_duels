...
| | [TASK-130504](tasks/TASK-130504-the-bar-carries-a-typed-total-and-a-refused-one-sends-nothing.md) The bar carries a typed total, and a refused one sends nothing and says why — *the entry becomes the bar's only amount state, presets write into it, and a `BET`/`RAISE` press with an unsendable entry returns before `setSent` and before `send` — the other four buttons are untouched, which its fourth test proves by folding through a garbage entry. The button prints the proposal or nothing, never a different amount (`ADR-0111` §7, `ADR-0100` §2). `<Notice>` moves inside `Live` and gains an `entryRefusal` that wins while it stands: no fourth row, so the phone fit stays out of the diff. Owns and replaces `ActionBar.test.tsx:141`. 26 → **30*** | S | ready |
