...
source, and six of its eight items are questions rather than instructions. Three are now **answered**:
`DEC-114` by [`ADR-0107`](../docs/adr/ADR-0107-pot-names-every-chip-committed-to-the-hand.md) — `Pot`
names the total, every chip committed to the hand so far; `DEC-115`, the turn clock and the largest of
the six, by
...
`ADR-0013`'s grace window, its mechanism registered as `DEC-120`, **the architect's**; and `DEC-117` by
[`ADR-0109`](../docs/adr/ADR-0109-the-table-marks-the-last-act-and-the-next-deal-clears-it.md) — one
mark, the most recent act, cleared only by the next act or the next painted deal.
`DEC-116`, `DEC-118` and `DEC-119` stay registered open for the **product owner**, who makes them
concrete before any story is split — splitting first would be inventing the answers. Two items need no decision at all, because *pulsing or running
