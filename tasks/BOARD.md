...
| | [TASK-121301](tasks/TASK-121301-the-runout-arrives-street-by-street-on-the-screen-too.md) The runout arrives street by street on the screen, not only in the log — *audit fix set 1 of 3, `R1` at beat 5; `manual-verify`; **registers a `DEC` for the architect before any diff exists** ([ADR-0096](../docs/adr/ADR-0096-the-audit-judges-a-whole-duel-against-a-frozen-rubric.md) §2 routes the pacing to the architect, since no still can hold it)* | S | ready |
