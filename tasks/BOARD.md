...
| DEC-142 | **The architect's** — by what mechanism is a display-name **suggestion** produced: what generates the string, does the generator consult `name_registry` before offering it, and does the suggestion cross the wire at all (a server endpoint) or is it a client string? Registered open 2026-09-06 by [`ADR-0119`](../docs/adr/ADR-0119-the-name-is-asked-at-the-first-press-and-skipping-plays.md) §4, which fixes the product rules the mechanism must satisfy — a suggestion is the field's initial value and promises nothing, nothing is written unless the player takes it, it encodes nothing about the player (`ADR-0029` §6, `ADR-0067`), and its refusal must be rare enough that accepting it normally works first try — and chooses no generator. `ADR-0029` §5's refusal of an availability-check endpoint is applied, not reopened, and `ADR-0051` §1's *a string that enters the registry never leaves it* is why pre-registering a suggestion is already forbidden by `ADR-0119` §5 | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1407` is split |
...
| `STORY-1407` | A player names themselves before their first duel — *item 1a; [`ADR-0119`](../docs/adr/ADR-0119-the-name-is-asked-at-the-first-press-and-skipping-plays.md) is the answer* | `DEC-142`, and nothing else |
