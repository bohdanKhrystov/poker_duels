...
| DEC-133 | **The product owner's** — **how much of a showdown does the table show?** Raised 2026-09-06 by `EPIC-14` item 2a, on the human's *"if we went to showdown villan card shoud be show"* and their drawing's *"villan card shown if required"*. The floor costs nothing: the engine already emits `HandRevealed` for every hand actually shown and passes it to both seats unfiltered (`StreetProgression.kt:124-133`, `EventRedaction.kt:13`), it is on the wire, and **no client file outside `protocol.gen.ts` mentions it** — a shown hand is sent today and drawn by nothing. The ceiling is [`ADR-0008`](ADR-0008-loser-mucks-at-showdown.md), which never reveals a mucked hand and names the **only** sanctioned widening: *"a post-hand disclosure written by the server after the hand is settled — never a loosening of what the engine publishes mid-hand."* The engine non-negotiable is not in play either way | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1411` is split |
...
nothing startable, because **eleven product decisions** stand in front of it.
...
against `ADR-0012`'s device binding; `DEC-133` widens what a showdown shows against `ADR-0008`'s
ceiling; `DEC-136` would replace the reflow `ADR-0103` §2 fixed with scaling. `DEC-139` carries
...
to both seats, it is on the wire, and no client file outside `protocol.gen.ts` mentions it —
while `BestHand.cards`, whose KDoc says it exists *"because the client highlights the winning five
at showdown"*, reaches nothing outside `poker-engine`. Chips never cross the table: `ChipPile` is
three discs that animate on mount and stand still. The timebank's `1:58.623999999999995` has one
cause — `turn-clock.ts:105` hands `bankFigure` a fraction where its own KDoc asks for whole seconds,
on the plate of the seat the clock does not name. The `Call` button does no arithmetic, because
`action-text.ts` refuses it in as many words. And the room the human wants alive for *"3-5 min"*
already lives **ten**; what does not survive is the tab's memory of it.
...
| `STORY-1411` | The showdown shows the hands it reached — *item 2a* | `DEC-133` |
