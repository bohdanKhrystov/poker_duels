...
| | [TASK-131003](tasks/TASK-131003-p1-a-refresh-on-the-result-screen.md) `P1` — a refresh on the result screen — *the path `EPIC-13`'s five did not cover. `open`'s own output is the first paint and the only pre-frame observation a navigation allows; `record` is armed immediately after it. Driven bare **and** delayed, because a negative on loopback is a statement about the sampler* | XS | ready |
