...
| | [TASK-140703](tasks/TASK-140703-the-second-list-is-a-hundred-substances.md) The second list is a hundred substances — *the middle word of `Quiet Iron Raven`. The five shape tests range over the whole vocabulary and are **not edited**; one line moves, to `toEqual([50, 100])`* | S | backlog |
| | [TASK-140704](tasks/TASK-140704-the-third-list-takes-its-beasts.md) The third list takes its beasts — *a hundred birds, animals and fish; `toEqual([50, 100, 100])`, a product of `500_000`, which is correct at this point because the floor is not asserted until the vocabulary is whole* | S | backlog |
| | [TASK-140705](tasks/TASK-140705-the-third-list-takes-its-offices-and-the-arity-reaches-a-million.md) The third list takes its offices, and the arity reaches a million — *a hundred roles, ranks and trades; `50 × 100 × 200 = 1_000_000`, **exactly** `ADR-0137` §4's floor, so one deleted word puts the vocabulary under it. The fold test now spans both halves of the list* | S | backlog |
| | [TASK-140706](tasks/TASK-140706-the-floor-and-the-draw.md) The floor and the draw — *`SUGGESTION_SPACE_FLOOR = 1_000_000` and `suggestName(random)`: one entry per list, in order, joined by one `U+0020`, with `ADR-0137` §2's clamp written verbatim so a source returning `1` cannot join `undefined` into a permanent name. The floor becomes a test; a fourth test runs the server's whole rule set over the **joined** string. The module takes no player fact — gated at one import and zero storage or globals* | S | backlog |
