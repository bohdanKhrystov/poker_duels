...
| DEC-109 | **The product owner's** — may one player hold seats in **two live rooms at once**? Registered open 2026-09-01 by [`ADR-0104`](../docs/adr/ADR-0104-a-frame-reaches-the-connection-in-the-room-it-is-about.md) §10, which answered `DEC-107` and split this off rather than deciding it. Read from source, not theorised: `DuelSocket.replyToCreateRoom` overwrites the connection's `RoomMembership.code` **unconditionally**, `replyToJoinRoom` does the same, and neither vacates the seat the player already holds — nothing can, because `ClientMessage` has five subtypes and none of them is `LeaveRoom` (`ADR-0072` §Context, force 3). So a player who creates or joins a second room stays seated in the first, `ADR-0025`'s ticker expires their grace window there, `ADR-0023` plays that seat's hands for them, and `ADR-0014` settles a **coin** on the result of a duel they walked out of. Whether that is a second duel, a **forfeit** of the first, or a request the server should refuse outright is a question about what a duel is and what a coin is worth — `docs/vision.md`'s, not an architect's, which is why `ADR-0104` refused it by name. **It blocks nothing**, and in particular not `TASK-121403`: `ADR-0104` §1's room-scoped delivery is correct under every answer — if two rooms are allowed their frames must not cross, and if they are not, the second request is refused and delivery is scoped anyway for the ordinary disconnect-and-reconnect case. Not raised by a QA round, so **not** one of `ADR-0092` §5's three slots | [`ADR-0104`](../docs/adr/ADR-0104-a-frame-reaches-the-connection-in-the-room-it-is-about.md) §10 | before a player can reach a second room from a screen |
...
gains no row, and **`TASK-121403` is not `atomic:`**. **Production is unchanged** — `disconnect()` still
...
| | [TASK-121403](tasks/TASK-121403-presence-is-about-the-room-the-reader-is-in.md) Presence is about the room the reader is sitting in — *the blocker; `DEC-107` is **answered** by [`ADR-0104`](../docs/adr/ADR-0104-a-frame-reaches-the-connection-in-the-room-it-is-about.md), so the decision no longer blocks it — it stays `blocked` because its *Files* table and `verify:` block are still the placeholders that ADR's §§1, 2, 4 and 7 tell the planner what to re-cut them into, and the ticket file still carries its own §Blocked note until that re-cut lands. Not `atomic:` — `ADR-0104` §6: `PROTOCOL_VERSION` does not move* | S | blocked |
