...
stand in front of it, `DEC-129`–`DEC-139`, none answered. Five of them ask to change something a
merged ADR decided: the post-win account offer
...
`ADR-0103` §2 fixed continuous reflow — and `DEC-139` **re-opens `DEC-111`**, which has sat open
since 2026-09-01 and is answered *yes, silently* by the shipped product: `Back to the lobby` calls
...
have sent. The epic's *What is already true* is what keeps that honest:
...
| DEC-130 | **The product owner's** — does the **account screen name the anonymous state** (the human's *"Anonymous Account"*) and become the **only** door to promotion, retiring the post-win offer entirely? Raised 2026-09-06 by `EPIC-14` item 1b, on an annotation that strikes the whole offer block through. It asks to reverse the central mechanism of [`ADR-0036`](ADR-0036-an-account-is-offered-never-required.md) — *after a player's first win, the client offers one* — and to delete the surface `ADR-0085`, `ADR-0086` and `ADR-0116` were all written about, so the answering ADR must say which clause of each it supersedes. **`DEC-089` asks how that surface should look** and would be mooted; the answering PR strikes it or states why it is kept. Answer it against `DEC-132`: an offer that protects coins and a sign-out that abandons them cannot both be decided alone | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1408` is split |
...
| DEC-132 | **The product owner's** — does **signing out abandon the anonymous profile and issue a new one**? Raised 2026-09-06 by `EPIC-14` item 1d. [`ADR-0012`](ADR-0012-device-bound-anonymous-profiles.md) binds a profile to a device id, `ADR-0027` puts the session above it, and `ADR-0037` calls the device *a credential until revoked* — so today a sign-out returns the browser to the profile it already had, coins included. A browser handed a **fresh** profile at every sign-out is the first mechanism in this product that can strand a duel coin, which is the exact loss the offer `DEC-130` may delete exists to prevent. `ADR-0049`'s *a device binding is a row and revoking is final* and `ADR-0050`'s *signs out everywhere but here* are what the mechanism has to be written against | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1410` is split |
| DEC-134 | **The product owner's** — may the table **mark the five cards that won**, in the winner's hand and on the board? Raised 2026-09-06 by `EPIC-14` item 2b. [`ADR-0095`](ADR-0095-the-table-states-who-took-the-pot-and-never-names-a-hand.md) says the table *never names a hand*, and a highlight names one without words, so this is a question about that rule rather than an application of it. `BestHand.kt:8` says in its own KDoc that its `cards` field *"exists because the client highlights the winning five at showdown"* — a consumer that has never been built — and **nothing outside `poker-engine` references `BestHand`**. The client may not compute the five itself: which cards won is a game fact and `ADR-0002` forbids a client asserting one, so a yes hands the wire question to the architect | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1412` is split |
| DEC-135 | **The product owner's** — when it is not the player's turn, does the action bar stand as **disabled controls** rather than the sentence `Waiting for your rival…` (`ActionBar.tsx:290`)? Raised 2026-09-06 by `EPIC-14` item 3c, on the annotation *"show disabled controllers"*. The height is not at stake — `ADR-0103` quotes the merged card's *"the bar reserves both rows in its `off` state"* — what is at stake is what an unpressable bar says to a player. The open **`DEC-108`** asks the adjacent question for a *paused* duel (may the bar stay **enabled**), and the two answers must agree about what a bar that cannot be pressed means | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1406` is split |
| DEC-138 | **The product owner's** — **does a rematch offer follow the rival off the result screen, and as what?** Raised 2026-09-06 by `EPIC-14` item 5, on the human's *"it shoud appear on any screen"*. [`ADR-0044`](../docs/adr/ADR-0044-a-rematch-is-one-intent-and-one-room-fact.md) already settles the intent and the room fact and does not change; what is unowned is the **surface**, and this product has no modal of any kind — `RematchControl.tsx` is rendered by `DuelResult.tsx` and by nothing else. The situation it addresses is one `EPIC-13` created on purpose: [`ADR-0112`](../docs/adr/ADR-0112-only-a-running-duel-refuses-another-screen.md) rules that a `FINISHED` room **honours** an ask for another screen, so a rival may now be anywhere when the offer arrives. Delivery is not the hard half — `ADR-0104` puts the frame on the connection in the room it is about, and that connection is the same one — the hard half is what a player may miss, ignore or refuse, and whether a modal is a thing *"dark, quiet, fast, minimal."* admits | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1415` is split |
| DEC-139 | **The product owner's** — **does pressing play again return the host to the room they already hold?** Raised 2026-09-06 by `EPIC-14` item 6, and it carries **`DEC-111`** with it — *may one player hold more than one `WAITING` room at once?* — which has been open since 2026-09-01 and is answered *yes, silently* by the shipped product. The lifetime the human asks for already exists and is longer than they asked: `RoomTimeouts.DEFAULT_WAITING_MILLIS` is ten minutes (`RoomTimeouts.kt:35`), and `ADR-0073`'s *"The room stays open. That link still works for your rival, and it brings you back."* is true on the server. What is not true is the browser's half: [`ADR-0072`](../docs/adr/ADR-0072-a-tab-remembers-its-room-until-the-player-leaves-it.md) §3 makes `forgetRoom()` — the `Back to the lobby` control — one of exactly two things that clear `pd.roomCode`, and [`ADR-0105`](../docs/adr/ADR-0105-one-duel-at-a-time-and-the-refusal-hands-back-the-duel.md) §2 refuses a second room only while a duel is `PLAYING`. So a host who leaves and presses play again **opens a second room** while the first lives out its ten minutes holding the code they may already have sent. The answer decides what the browser remembers after *Back to the lobby*, what a second press means, and it **strikes `DEC-111`** in the same PR or says why that row stays | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1416` is split |
...
nothing startable, because **eleven product decisions** stand in front of it — one of them,
`DEC-133`, now answered.
...
**Five decisions contradict a merged ADR, and one re-opens an open one.** `DEC-130` would delete the
surface `ADR-0036`, `ADR-0085`, `ADR-0086` and `ADR-0116` were written about; `DEC-131` would make
...
own text asks for. `DEC-139` carries
**`DEC-111`**, open since 2026-09-01. Each answering ADR must name the clause it supersedes, and the
PR that answers `DEC-130` and `DEC-139` strikes `DEC-089` and `DEC-111` or says why they stay.
...
highlights the winning five at showdown"*, reaches nothing outside `poker-engine`. Chips never cross
...
| `STORY-1408` | The account screen says what an anonymous account is — *item 1b* | `DEC-130` |
...
| `STORY-1412` | The winning five is marked, in hand and on board — *item 2b* | `DEC-134` |
| `STORY-1413` | The pot travels to the winner — *item 2c* | `STORY-1412` |
