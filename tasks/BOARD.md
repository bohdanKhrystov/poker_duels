...
| DEC-159 | **The architect's** — `ADR-0135` requires `GET /api/me/device`; the merged `DuelServerRoutesTest.theDeviceRouteIsNotInstalledOnAnyOtherVerb` forbids it, citing `ADR-0049` §5. Repairing that test makes `TASK-141003` four files, which the linter permits only with `atomic:` — and `ADR-0135` forbids `atomic:` in as many words. Three merged sources jointly unsatisfiable; one must yield. Also owed: §4's formula contradicts §5's own truth table. **Blocks `TASK-141003`** | open |
