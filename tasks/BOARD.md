...
| DEC-035 | **Technical.** How does `check-drift.sh` read a CSS value? Its reader is `--pd-NAME:[^;]*;`, which knows nothing about strings, so a semicolon inside a quoted value truncates both sides identically (real drift passes green) and a declaration without its terminating semicolon either vanishes or swallows the text after it. The options each cost something: a real parser forfeits the stock-tools property `ADR-0024` relies on; a convention lint refuses legal CSS the reader cannot read; accepting the limit means recording what the gate does **not** guarantee. Three attempts to guess this shipped three worse defects (#511, blocked [`TASK-060121`](tasks/TASK-060121-semicolonless-declaration-enters-the-gate.md)) | [`STORY-0601`](stories/STORY-0601-design-foundations.md) | before a token holds a quoted `;`, `{` or `}` |
...
| | [TASK-030614](tasks/TASK-030614-the-reserved-line-states-what-the-rival-has-committed.md) The reserved line states what the rival has committed this street | XS | ready |
