...
| | [TASK-141401](tasks/TASK-141401-the-cards-draw-the-call-mark-bare.md) The cards draw the call mark bare — *the card goes first (`ADR-0091` §2, `ADR-0107` §6's shape). `ADR-0122` §7 named one node; **four** were measured — `duel-table-states.html:298`, `seat-and-pot.html:252` and its caption, and the `.last-act` comment saying two of six acts are bare in **three** cards. Gates pin the eight `Call` **button** nodes as unchanged, because those frames commit nothing and 400 is already the price* | XS | ready |
