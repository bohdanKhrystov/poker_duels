...
| DEC-146 | **The architect's** — by what mechanism does the client's step queue give **one beat a length different from a step**? Registered open 2026-09-06 by [`ADR-0120`](../docs/adr/ADR-0120-a-showdown-shows-the-hands-the-rules-showed-and-the-beat-that-shows-them-stands.md) §3, which settles the product half: a showdown's **last** beat stands **2,000 ms** while every other ending keeps `ADR-0102` §4's **600 ms**. That section pins the rest — *a step is 600 ms, named once, at the boot seam* — reaching the store as a **single** parameter, and `web-client/src/e2e/drive-duel.tsx` boots it at `0` so `ADR-0100` §3's four recorded-frame suites are neither edited nor re-recorded. Where the second number is named, whether a step carries its own length or the store branches on what the beat paints, and what `0` means for both | [`ADR-0120`](../docs/adr/ADR-0120-a-showdown-shows-the-hands-the-rules-showed-and-the-beat-that-shows-them-stands.md) §3, [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) item 2a | before `STORY-1411`'s hold is implemented |
...
| `STORY-1411` | The showdown shows the hands it reached, and stands long enough to read — *item 2a* | `DEC-146` (the hold's mechanism; the product half is [`ADR-0120`](../docs/adr/ADR-0120-a-showdown-shows-the-hands-the-rules-showed-and-the-beat-that-shows-them-stands.md)) |
