...
split: `DEC-120` (the clock's mechanism, by `ADR-0108`) and `DEC-123` (the navigation guard's, by
`ADR-0112`). **The epic is split: eleven stories, written 2026-09-02, none yet cut into tickets.** The eight items
...
wait behind the half that does. **`STORY-1301` is the one story startable now**; `STORY-1308` and
`STORY-1309` wait on `DEC-120` and `STORY-1311` on `DEC-123`. The order is one chain and it is not
...
animated surface owes a still form to a player whose system asks for reduced motion. It blocks the
implementing tickets of `STORY-1303` and `STORY-1306` and nothing else — not their cards, not the
chip's minting, not the epic. The epic writes down what it measured so no story re-discovers it: the pot is a display
...
| DEC-124 | **The product owner's** — does a surface this product **animates** owe a **still form** for a player whose system asks for reduced motion, and what governs it? Registered open 2026-09-02 by the planner splitting `EPIC-13` into stories. Its items 1 (the acting seat's mark — the human's own words were *"some animation like pulsing or running circle"*) and 6 (chips that move to the pot and back to a stack) are the **first continuous motion this product has ever had**, and nothing settles what it does for a player who asked their system for less of it: no `prefers-reduced-motion` rule anywhere in the repository, no motion token in `design/tokens/tokens.css`, no accessibility stance in `docs/vision.md` or any ADR. [`ADR-0102`](../docs/adr/ADR-0102-a-hand-ends-in-steps-and-the-client-owns-the-clock.md) §4 licensed a client-owned display *schedule* and says in as many words that `R1` *"fixes no duration, no animation and no transition"*, so it does not reach this. **Not taste, and therefore not `ADR-0024` §3's**: *pulsing or running circle* is a choice between two drawings the human looks at, and a media query is not a drawing a card can render. Open: a still form under the OS preference, a player-set preference (the Lichess reference point ships one), or no policy at all — and whether it binds every future animated surface or only these two. **Blocks the implementing tickets of [`STORY-1303`](stories/STORY-1303-the-acting-seat-is-marked-and-the-mark-moves.md) and [`STORY-1306`](stories/STORY-1306-a-stack-is-chips-and-chips-move.md)** — not their design cards, not the chip's minting, and not `EPIC-13` | [`EPIC-13`](epics/EPIC-13-the-living-table.md) §Open decisions | before EPIC-13 items 1 and 6 reach a client ticket |
...
decisions. None is cut into tickets yet: `/plan-story <STORY-ID>` does that one story at a time, and
the first ticket of every story that puts a new surface in front of a player is its **design card**,
which merges before the ticket that implements it is startable (`ADR-0091` §2, `EPIC-13`
*Design first*).
...
| Story | Task | Est | Status |
| --- | --- | --- | --- |
| **[STORY-1301](stories/STORY-1301-pot-names-every-chip-committed-to-the-hand.md)** `Pot` names every chip committed to the hand — *item 2; `ADR-0107`; a two-node card correction, no new surface* | | | **ready — the one story startable now** |
| **[STORY-1302](stories/STORY-1302-the-host-waits-at-the-table.md)** The host waits at the table, and both promises move with them — *item 5; `ADR-0110`; card owes four host-alone variants plus the arrival* | | | ready — waits on `STORY-1301` |
| **[STORY-1303](stories/STORY-1303-the-acting-seat-is-marked-and-the-mark-moves.md)** The acting seat is marked, and the mark moves — *item 1; no ADR, the human's eye; raises `DEC-124`* | | | ready — card startable, implementing tickets blocked on `DEC-124` |
| **[STORY-1304](stories/STORY-1304-the-table-marks-the-last-act.md)** The table marks the last act, and the next deal clears it — *item 3; `ADR-0109`; card owes six states, four with a figure* | | | ready — waits on `STORY-1303` |
...
| **[STORY-1310](stories/STORY-1310-the-refresh-paths-nobody-drove.md)** The refresh paths nobody drove, driven and written down — *item 8a; `ADR-0112` §6's six paths; no browser drive may be a `verify:` gate* | | | ready — decision-free, waits on `STORY-1301` |
