...
| | [TASK-141513](tasks/TASK-141513-the-module-stops-offering-the-label-the-adr-says-to-copy.md) The module stops offering the label the ADR says to copy — *filed 2026-09-10 from a reviewer's aside while reviewing `TASK-141511`, that `rematch-text.ts`'s `NOT_NOW` is defined but unused. Measured: nothing imports it, `RematchNotice.tsx:17` declares its own, and `ADR-0138` §4 says outright *"`Not now` is copied, not imported"* while listing five constants that do not include it. **Named cost:** the dead line is harmless; what is not is that the shared module now offers exactly the import §4 closed off, so a later coder reaching for the dismiss label finds it in the module both surfaces share and couples them by the obvious route rather than the forbidden one* | XS | backlog |
