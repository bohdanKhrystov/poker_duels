...
| | [TASK-140717](tasks/TASK-140717-a-refused-player-may-press-again.md) A refused player may press again, and a test says so — *found while reviewing `TASK-140712` and **measured, not suspected**: deleting the in-flight guard's reset leaves all eight of that ticket's tests green, so a build in which one refusal locks a player out of naming themselves ships on a full green run. That ticket is not at fault — its `Tests` table names no retry case, and its *Out of scope* refuses an **automatic** resend, which is a different thing from the player pressing again. One test, `NameAsk.tsx` byte-unchanged* | XS | backlog |
