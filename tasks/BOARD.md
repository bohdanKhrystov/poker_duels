...
| DEC-127 | **The product owner's** — a browser recovering a room paints a confident **`Waiting for your rival`** screen, room code and invite link and all, over a room that is actually `PLAYING` or already finished, and then corrects itself. Is a self-correcting false screen **acceptable-because-late**, or **wrong-because-false** and owed a neutral interstitial? Raised 2026-09-04 by `TASK-131009` reading `STORY-1310` whole, on **three independent observations by three different routes** — `P2` reloading mid-runout, `P4` reloading a `PLAYING` duel, `P6b` opening a mailed link over a held room — so it is not one path's quirk. It contradicts `ADR-0112` §6's *no lobby on the way* directly. The distinction the drives were careful to preserve is that `ADR-0102` §5 arguably still holds: each screen is a state the server **did** state, arriving late, rather than a fact the client invented — which is why this is a product question about what a player may be shown in the meantime, and not a mechanism question. `ADR-0114` §5's `roomAwaited` distinguishes *no room* from *not told yet* and `STORY-1311` lands it, but §2 leaves the branch order untouched, so **`STORY-1311` does not resolve this on its own**. *Acceptable* is a complete answer and needs saying out loud rather than falling out. Blocks nothing today — every drive that found it also watched it correct itself | [`STORY-1310`](stories/STORY-1310-the-refresh-paths-nobody-drove.md) | before `STORY-1311` draws its held state |
