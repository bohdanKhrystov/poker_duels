...
| | [TASK-131103](tasks/TASK-131103-boot-says-whether-this-tab-is-awaiting-a-room.md) Boot says whether this tab is awaiting a room — *computed **once at construction**, not in the `Welcome` handler, and the load-bearing test is the fourth: a later `RoomJoined` does not move it. `readRoomCode`, never `localStorage` — `ADR-0086` §2 keeps the global in `main.tsx`* | XS | ready |
