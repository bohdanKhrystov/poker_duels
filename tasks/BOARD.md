...
| | [TASK-140917](tasks/TASK-140917-the-form-leaves-the-front-door.md) The form leaves the front door — *it owns the two `Lobby.test.tsx` cases its change invalidates and rewrites both rather than deleting them; the `TestPlayer` count moves 2 → 1, and 1 is not a value the bug leaves unchanged. `TASK-140915`'s wiring test is named must-stay-green, because a deletion ticket whose gates are all negative can delete a feature instead of moving it* | XS | ready |
