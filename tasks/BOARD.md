...
| DEC-147 | **The architect's** — by what means does the client learn that the profile it holds carries **no credential**? Registered 2026-09-06 by [`ADR-0125`](../docs/adr/ADR-0125-the-account-screen-names-the-anonymous-profile-and-owns-the-door.md) §4, which rules that the account screen may call a profile *anonymous* only when it has been told, and never from the absence of a session token. Measured there: `GET /api/me` carries `playerId`, `coinBalance`, `displayName`, `displayNameRemoved`, `deviceRouteLive` and `hasRecoveryEmail`, and none of them is this fact; `signedIn` supports only the sound direction — a token implies a password, which is `AccountScreen.tsx`'s own written derivation — and is a **false positive for anonymity** in two reachable states: a `201` sign-up whose follow-up sign-in failed (`sign-up.ts` answers `signed-up` regardless, deliberately, while `SignUpForm` is showing *"This profile now has a password."*), and a player who signed out in this browser (`ADR-0085` §Consequences' named-not-solved case, which `ADR-0086` §4 kept `signOut` away from on purpose). It is the same converse-of-`signedIn` mistake `TASK-120601` filed as a defect on this screen on 2026-08-29 — *"the screen states a falsehood about the account"* — repaired then by making the browser hold a token rather than by telling the client the fact. Whether the answer is a seventh field on `GET /api/me` or something else, and what it may be called without becoming an enumeration surface, is the architect's — `ADR-0125` chooses no mechanism and moves no wire. `ADR-0031` §6.3 and `ADR-0027` §6 are the posture it is written against | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1408`'s anonymous-state ticket is startable |
...
registers **`DEC-147`** for the architect and holds §3's block until it lands, while §§1, 2, 5 and 6
need no fact the client does not already hold. **§5: the front door stops describing an absence** —
...
| `STORY-1408` | The account screen says what an anonymous account is, the post-win offer goes, and the front door stops printing `No profile yet.` — *item 1b; [`ADR-0125`](../docs/adr/ADR-0125-the-account-screen-names-the-anonymous-profile-and-owns-the-door.md) is the answer* | `DEC-147` for the anonymous block only — the deletions wait on nothing |
...
| `STORY-1416` | Play again returns the host to the room they still hold — *item 6* | `DEC-110` — `DEC-139` and `DEC-111` are answered by [`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md), whose §6 widens the architect's open mechanism question to the `WAITING` case |
