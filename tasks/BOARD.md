...
| | [TASK-141108](tasks/TASK-141108-the-read-beat-is-spent-once-and-a-repeat-is-a-step.md) The read beat is spent once, and a repeat of the same hand is a step — *added 2026-09-07, **`blocked` on `DEC-153`**. `ADR-0120` §3's rule is *a hand face up the viewer **has not been shown before***, but the sentence restating it *as the client can evaluate it* drops the word **before**, and `TASK-141104` merged the restatement: every delivery carrying rival hole cards answers `"read"`, because `PlayerView.of` refills a revealed seat's `holeCards` on every projection of the hand, `duel-state.ts` re-lays a reveal on every `COMPLETE`, and `duel-store.ts` keeps one state across sockets. **Latent, not live, and measured rather than argued:** a throwaway `poker-server` probe over 60 duels found **0** hand-completing `Snapshot`s in **2,220** resume frames and **0** repeats across **604** hand endings — `act` calls `advance` in the same call, so a live hand is never over — while the same probe, asked what a runner would hand back without that call, found **602**. So the guard contradicts `ADR-0136` §1's *no previous view is remembered* to fix something no path reaches, which is the architect's call and not a ticket's. Prototyped and reverted: 2 files, **92 → 95** tests, mutations `2 failed \| 93 passed (95)` and `1 failed \| 94 passed (95)`, each gate proving with `! cmp -s` that the literal it mutates was there* | S | blocked |
...
| | [TASK-141304](tasks/TASK-141304-the-flight-has-a-side-and-the-built-sheet-proves-it.md) The flight has a side, and the built stylesheet proves it — *two keyframes, two utilities, `ChipPile`'s optional `from`, and gates that `npm run build` then `grep -F` over `dist/assets/*.css`, because a class-name assertion passes identically for a rule that generates nothing — this epic's third encounter with that defect class. Three mutations, all run at planning time: the utility renamed away (built rule gone, **`5 passed (5)`** still green), the reduce block turned to `no-preference` (reduce grep red, `src/table` **253** green), and the side table collapsed (**`1 failed | 4 passed (5)`**). A **trap is written into the ticket**: spelling the class dynamically does **not** drop the rule, because Tailwind scans the test file's literals too* | S | backlog |
| | [TASK-141305](tasks/TASK-141305-the-rivals-seat-takes-its-chips-from-below.md) The rival's seat takes its chips from below — *one expression, `from={props.isViewer ? "above" : "below"}`, read off the plate's own prop rather than threaded from `DuelTable`, whose KDoc already fixes the arrangement. The test renders **both** seats — one input cannot tell an expression from a constant — and **both** hard-codings redden it, each `1 failed | 19 passed (20)`. `PotStrip` and `DuelTable` are gated as unopened* | XS | backlog |
