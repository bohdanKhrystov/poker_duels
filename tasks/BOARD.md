...
| DEC-156 | **The product owner's** — on a screen carrying both reversible and irreversible acts, does the irreversible one get weight the others do not? Registered 2026-09-09 while landing `TASK-140909`: the account screen's name form is the only act on it that spends something forever, and its *gone for good* sentence sits at the same register as everything beside it — same box, same button, same muted type. The distinction lives in the words and nowhere else, so a player skimming loses a name permanently. **Blocks nothing** | open |
