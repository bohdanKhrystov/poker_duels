...
| | [TASK-041611](tasks/TASK-041611-erasing-an-address-is-one-statement-and-so-is-forgetting-a-stale-one.md) Erasing an address is one statement, and so is forgetting a stale one | S | **ready** |
...
`TASK-041638` is the only other ticket that touches `RecoveryMailerShapeTest.kt`. The count is
**still 38** after `ADR-0080` and `ADR-0081`: both are corrections to tickets that already existed,
and neither adds or removes one. **`V3`'s
...
`blocked`. **`TASK-041624` was deliberately left alone and is the one ticket still saying it is
blocked.** `ADR-0078` answers its `DEC-071`, but that ticket is a *shell*: its two fixture tables are
empty by design and were written to be filled from the answering ADR's §6, so the fold-in is a
re-plan of its Tests rather than a prose correction, and half of it would read as all of it. It is
owed a planner pass of its own before its turn comes at `TASK-041627 → 041625`.
