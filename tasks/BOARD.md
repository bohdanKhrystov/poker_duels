...
| DEC-131 | **The product owner's** — may a display name be **changed after it is set**, and what becomes of the one given up: released for anyone to take, held by its former owner, or retired? Raised 2026-09-06 by `EPIC-14` item 1c. [`ADR-0029`](ADR-0029-a-display-name-is-unique-and-permanent.md) is titled *unique and **permanent***, and its permanence is not a convention but an index — *"the index is the reservation"* — while `ADR-0038` can take a name away and `ADR-0051` registers one before it is held. The leaderboard, the duel record and every finished duel print a name that would now be able to move, so the answer decides what those rows mean as much as what the settings screen offers | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1409` is split |
...
repository has made; `DEC-131` would make
changeable a name whose ADR title says **permanent**; `DEC-132` would hand a browser a new profile
