...
| DEC-162 | **The human's** — does a standing rematch offer follow a rival who presses *Back to the lobby*? [`ADR-0123`](../docs/adr/ADR-0123-a-standing-rematch-offer-follows-the-rival.md) records the *whether* as *"the human's, stated verbatim"* and quotes it: *"it shoud appear on any screen, like if opp come back to lobby it or any other page in still shoud be shown"*. §1 then carves that case out — *"A player who has left the room is not followed, and cannot be"* — reading *lobby* as *leaving the room*. Measured on `develop` at `ea4bd05c` while triaging `STORY-1216`, the carve-out is total rather than partial: `RematchNotice` renders only while `shown !== "first"` (`ADR-0138` §2), the three controls that open a chosen screen are rendered **only** in `Lobby.tsx`'s front-door branch, the result branch (`Lobby.tsx:340`) renders `DuelResult` and nothing else, and `grep -rn 'href='` finds exactly **two** links in the whole client, both `href="/"` — so the set of screens a player can reach while holding a finished room is **empty**, and `STORY-1415`'s thirteen tickets ship a surface no press can produce. `ADR-0123` §Context rests the whole decision on a premise the markup does not carry: *"the product now invites a player to leave the only screen on which a rematch can be seen"*. Two repairs are available and they are different products — the result screen gains a route to the chosen screens, or *Back to the lobby* stops ending the room. **Not the product owner's**: the sentence being reinterpreted is the human's own. **Found outside round 1's report**, so `EPIC-12` §Termination rule 1 puts it in the ordinary backlog and no `B(N)` moves. **Blocks nothing** | [`STORY-1216`](stories/STORY-1216-round-1-epic-14-the-stack-served-a-tree-three-days-old.md) §Found outside the round | before the next round retests `EPIC14-4` |
