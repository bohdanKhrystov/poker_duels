...
| DEC-129 | **The product owner's** — is a player **asked for a name before their first duel**, where does the **suggestion** come from, and may the dialog be dismissed without naming? Raised 2026-09-06 by `EPIC-14` item 1a, from the human's own instruction after playing the product on two devices. `ADR-0036` holds that *"no screen gates on having a credential"* and a name is not a credential, but a dialog between a player and their first duel is still a gate on something, and `ADR-0058` decided what the client prints where a name would be — a rule that mostly stops mattering if every player has one. The suggestion is the sharp half: `ADR-0029` §1 makes a name's freedom a **database** fact under a unique index, so a suggested name cannot be a client string, and one that is free when suggested may not be free when taken | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1407` is split |
...
| `STORY-1407` | A player names themselves before their first duel — *item 1a* | `DEC-129` |
