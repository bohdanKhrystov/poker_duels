...
| DEC-105 | **The architect's** — how is a runout's pacing produced, and what does a step cost? One question, two halves: does `poker-server` send one `Snapshot` per street during a runout — changing `poker-server` and possibly the frame contract — or does `web-client` hold the `Snapshot` it already has and reveal the board in steps — changing only `web-client`, but holding back a fact the server has already sent (`CLAUDE.md`: *"the server is authoritative"*)? And however it is produced: how long is a street held, and does a returning or reconnecting client replay the pacing or jump to the end? Raised 2026-09-01 by `TASK-121301` (`STORY-1213`), routed by [`ADR-0096`](../docs/adr/ADR-0096-the-audit-judges-a-whole-duel-against-a-frozen-rubric.md) §2 — *"How a beat is paced is settled by the ticket that repairs it — with a card where a still can hold it (`ADR-0091` §3), with the architect where it cannot."* A still cannot hold it: a three-card board and a four-card board are `design/screens/duel-table.html`'s existing anatomy at two fills, and no new element appears at any point in a runout, so what is undecided is temporal and structural. The ticket established the gap is two-layer: `Addressed.kt`'s `broadcast` does emit all three `StreetDealt` events before the terminal `Snapshot`, `duel-state.ts` does append them to `narration`, and nothing anywhere turns one into a paint — while the `Snapshot` already carries the post-runout state. **Blocks `TASK-121301` entirely** — no production diff lands until this merges. Recorded in [`STORY-1213`](stories/STORY-1213-round-1-audit-three-criteria-unmet-and-a-table-that-names-the-wrong-winner.md) | [`STORY-1213`](stories/STORY-1213-round-1-audit-three-criteria-unmet-and-a-table-that-names-the-wrong-winner.md) | before `TASK-121301` can implement a fix |

`DEC-106` → [`ADR-0102`](../docs/adr/ADR-0102-the-table-fits-the-phone-and-the-cards-give-before-the-numbers.md)
...
| **[STORY-1213](stories/STORY-1213-round-1-audit-three-criteria-unmet-and-a-table-that-names-the-wrong-winner.md)** Round 1 (audit) — three criteria unmet, and a table that names the wrong winner — *schema 2; **the first round under the `audit` focus**; `A(1) = 3` of a five-criterion rubric, `A(0)` n/a, verdict `PROCEED`, fix set 3 in the rubric's own order* | | | **ready** |
| | [TASK-121301](tasks/TASK-121301-the-runout-arrives-street-by-street-on-the-screen-too.md) The runout arrives street by street on the screen, not only in the log — *audit fix set 1 of 3, `R1` at beat 5; `manual-verify`; **registers a `DEC` for the architect before any diff exists** ([ADR-0096](../docs/adr/ADR-0096-the-audit-judges-a-whole-duel-against-a-frozen-rubric.md) §2 routes the pacing to the architect, since no still can hold it)* | S | blocked |
| | [TASK-121302](tasks/TASK-121302-the-decision-fits-a-390-by-664-screen.md) The decision fits a 390 by 664 screen, because the column is budgeted to the viewport — *audit fix set 2 of 3, `R2` at beats 2/3, 4, 5 and 6; `manual-verify` — the failure is a measured geometry and [ADR-0089](../docs/adr/ADR-0089-a-browser-drives-this-client-for-a-qa-round-never-for-a-gate.md) §2b forbids a browser gate; **blocked on 2026-09-01 by [`ADR-0102`](../docs/adr/ADR-0102-the-table-fits-the-phone-and-the-cards-give-before-the-numbers.md), and to be rewritten, not amended** — two of its sentences are now false (the card is `read` in its *Files* table, and "No `DEC` is needed here — this is conformance to a merged card"), because the merged card itself measures **732 against 664** at 390 × 664 and so describes no phone that any conformance could reach. It gains a predecessor — a `module: design` ticket amending `design/screens/duel-table.html`, where the hardcoded `--w:96px` and `--w:40px` narrow like `--bw` and a same-markup 390 × 664 frame is added, boxed in height as well as width — and **design precedes client**. Its height-budget half survives as necessary but not sufficient, and its file set is measured, not copied* | S | blocked |
| | [TASK-121303](tasks/TASK-121303-the-front-doors-three-doors-are-three-doors.md) The front door's three doors read as three doors, not as one word — *audit fix set 3 of 3, `R4` at beat 1; `manual-verify`; separates them and must **not** dress them — `DEC-094` stays open and unanswered* | XS | ready |
