...
| DEC-101 | **The product owner's** — what amount does each named sizing preset set: what does a chip labelled `pot` (and `⅓`, `½`) promise a player in a heads-up duel? Registered open 2026-08-31 by [`ADR-0100`](../docs/adr/ADR-0100-the-driver-reaches-an-amount-by-pressing-what-a-player-presses.md) §6, which settles the technical half — **a named preset computes the quantity it is named for**, and the pot reaches `ActionBar` as `view.pot + both seats' committedThisStreet` through `Lobby.tsx`, needing no wire change — and refuses to guess this half. `TASK-120908`'s coder sized the fractions off the server's own minimum (`floor × 4/3`, `× 1.5`, `× 2`) because no pot reached the bar, so a chip reading *½* sizes half of nothing; that arithmetic does not ship. **The merged card admits two formulas that agree on the one frame it draws and disagree in play**: `duel-table.html` draws `Pot 2,450`, the rival `committed 400`, `Call 400`, the `pot` chip selected, `Raise to 3,250` — which is both `view.pot + 2 × callTo` and `(view.pot + both committed) + callTo` — and it **excludes** the textbook pot-sized raise a poker player expects, 3,650 there. No technical fact chooses between them, and the choice decides what a player believes when they press, which is what a label promises. Raised by a ticket, not promoted at a triage, so **not** one of `ADR-0092` §5's three slots. **Gates the rewritten [`TASK-120908`](tasks/TASK-120908-the-tables-sizing-control-is-the-cards-presets.md)**, which is `backlog` in a cycle that ended `PASS`, so no round waits and `STOP_BLOCKED` does not fire (`ADR-0092` §5) | [`ADR-0100`](../docs/adr/ADR-0100-the-driver-reaches-an-amount-by-pressing-what-a-player-presses.md) §6 | before `TASK-120908` is rewritten |
...
`TASK-120908` grows from a two-file `S` into a **six-file atomic ticket that is blocked** on
`DEC-101` — `ActionBar.tsx`, `ActionBar.test.tsx`, `drive-duel.tsx`, `Lobby.tsx`,
...
| | [TASK-120908](tasks/TASK-120908-the-tables-sizing-control-is-the-cards-presets.md) The table's sizing control is the card's presets, not a range slider — *product; `medium`; **to be rewritten, not amended**, by the planner over the six files [`ADR-0100`](../docs/adr/ADR-0100-the-driver-reaches-an-amount-by-pressing-what-a-player-presses.md) §7 names (`ActionBar.tsx`, `ActionBar.test.tsx`, `drive-duel.tsx`, `Lobby.tsx`, `bar-no-derivation.test.tsx`, `turn-fixture.ts`), `atomic:` on `whole-duel.test.tsx`; as written it cannot go green — its two-file budget excludes the driver `DEC-100` settles, and its three-button actions row breaks the recorded `AllIn` frame independently of the amounts. The rewrite is **blocked on `DEC-101`**, whose answer the preset arithmetic needs; the row stays `backlog` until the planner writes it* | S | blocked |
