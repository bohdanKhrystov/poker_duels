...
| | [TASK-041229](tasks/TASK-041229-a-successful-sign-in-lands-on-the-account-screen.md) A successful sign-in starts the next boot on the account screen, with no way back to sign-in | S | **blocked** — 2026-08-27, after three dispatches, on **three defects in the ticket rather than in the code**. Its `## Tests` preamble specifies the rendered tree over a stubbed `window.fetch`, *"driven the same way as `TASK-041223`'s binding tests"* — but that ticket was **amended precisely because that fixture does not exist**: `App.test.tsx` line 35 mocks `./main` wholesale for every test in the file. This ticket was written against `TASK-041223`'s **pre-amendment** shape and never updated. Worse, its third row names `sends sign-in with no credential of its own, even holding a session` as *"existing"* and greps it in `verify:`, and **that test exists nowhere** — it was the pre-amendment name, replaced by `refuses to wrap sign-in…` (`App.test.tsx:238`), with the request-level guarantee in `sends no device id and no authorization of its own` (`sign-in.test.ts:135`). Both merged, both holding. So **a `verify:` line greping a test that must already exist is a gate satisfiable by creating it**, and it manufactured one. Third, `leaves a refused sign-in exactly where it was` cannot be honest here: measured on the third dispatch, mutating `sign-in.ts` to call the injected `reload` on **every** outcome reddens **nothing in the whole suite** — the property is *genuinely unguarded*, not covered elsewhere, and a source assertion on `main.tsx` cannot see which branch calls `reload`. **One test is good and should survive**: `lands the next boot…` reddens by name both when `replaceState` is deleted and when it is swapped for a hash assignment, and the `reloadAtAccount` implementation is sound — `replaceState` fires neither event in this jsdom and creates no history entry. All five gated `main.tsx` counts held. Three routes are recorded in the ticket; the acceptance test for any amendment is that mutating `sign-in.ts` to reload on every outcome must redden a test **by name** |
