...
| | [TASK-141012](tasks/TASK-141012-the-boot-reads-the-device-standing-once.md) The boot reads the device standing once, beside the profile — *`App.test.tsx` is in the budget because it pins a **count of `main.tsx`'s own source**: `fetch: plainFetch` is **7** at `c6a41e6d` and this binding makes it 8, while `fetch: apiFetch` stays at 5. `plainFetch` is not a preference — `readDeviceStanding` sets its own bearer header and the wrapper would attach a second. Two-sided regex, because a count alone would also pass for some third fetch. 36 → 37* | S | ready |
| | [TASK-141013](tasks/TASK-141013-the-account-screen-is-told-which-sign-out-it-offers.md) The account screen is told which sign-out it offers — *probed: a required prop on `AccountScreen` names **nine** render sites in its own test and one in `Lobby.tsx`, and nothing else; `App.test.tsx` escapes because it wraps the component rather than rendering it. The new test holds `signedIn` fixed and varies only the new prop, because the two are adjacent booleans and confusing them is the mistake. 20 → 21* | S | backlog |
