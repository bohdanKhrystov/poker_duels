...
| | [TASK-141505](tasks/TASK-141505-the-accept-is-one-press-one-frame-and-the-span-it-opens.md) The accept is one press, one frame, and the span it opens — *`accepted` gets `ADR-0138` §3's clear even though §4 only says "exactly as `RematchControl.tsx:32` does", because that sentence is about where the fact lives and `RematchControl`'s own flag is stale. Two harness facts measured against a prototype: `getByText(DEALING_TAIL)` **throws** — the two lines share one element across the `<br />`, so the merged regex over the whole node is the matcher — and the three offer-ending frames must arrive in **three separate `act()` calls**, or React batches them, the clear never fires, and the test fails against a correct component* | S | ready |
| | [TASK-141506](tasks/TASK-141506-a-gone-room-retires-the-accept-and-says-so.md) A gone room retires the accept and says so, and no other refusal touches it — *`UNKNOWN_ROOM` gated at 1 and `REMATCH_UNAVAILABLE` at 0. `a transient refusal leaves the accept standing` is the positive control that stops the branch being written as `refusal !== null`, and `the panel is still there after the room is gone` is what stops it being written as `return null`* | XS | backlog |
| | [TASK-141507](tasks/TASK-141507-not-now-hides-the-surface-and-the-next-offer-brings-it-back.md) Not now hides the surface, sends nothing, and the next offer brings it back — *one `useState` whose lifetime is the mount, cleared during render, **no `eslint-disable`** and a gate refusing one. `Not now sends nothing` asserts a **count of 0 on both sides** rather than *not called with a decline*, because sending `OfferRematch` on dismiss is the plausible mistake and it passes the weaker form. The reload proof is a fresh `render()`, deliberately **not** an assertion over `Storage` keys — Node's `localStorage` shadows jsdom's under vitest and reads `undefined`* | S | backlog |
