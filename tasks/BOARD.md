...
| DEC-161 | **The architect's** — what does **minting** actually require now? [`ADR-0091`](../docs/adr/ADR-0091-design-gets-no-agent-a-new-screen-owes-a-card.md) §3 says a minting card *"is worked interactively with the human, because taste does not survive a verify block"*. Measured across all **86** `module: design` tickets on `develop` at `702408ed`, four carry the `minting` label — `TASK-130301`, `TASK-130601`, `TASK-130701`, `TASK-141501` — and **all four are `review: standard`, were dispatched to a coder and merged as ordinary PRs**. §3's interactive branch has not been practised once since `ADR-0091` merged on 2026-08-30, and nothing failed; `.claude/agents/qa-manager.md` independently reads §3 as prescribing a dispatched `module: design`, `review: light` ticket for a whole missing card. So either the merged practice is a standing deviation owing a repair, or §3's minting branch means something narrower than its words — a stricter tier and a labelled ticket, not a different authorship channel. **Not in scope**: `ADR-0024` §3 assigns the visual verdict to the human on every card, and `ADR-0091` names delegating taste as the one thing only the human can give away — an answer that moved the **verdict** would be the human's, not the architect's. Registered 2026-09-09 by [`ADR-0145`](../docs/adr/ADR-0145-a-cards-scaffolding-is-composing-and-the-transcriber-names-the-mint.md) §5, which answers which **side** of the line scaffolding falls on and deliberately neither ratifies nor repairs what the other side costs. **Blocks nothing** | [`ADR-0145`](../docs/adr/ADR-0145-a-cards-scaffolding-is-composing-and-the-transcriber-names-the-mint.md) §5 | before the next ticket a planner would label `minting` |
