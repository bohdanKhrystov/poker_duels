...
| `STORY-1416` | Play again returns the host to the room they still hold — *item 6* | nothing — `DEC-139` and `DEC-111` by [`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md), `DEC-110` by [`ADR-0133`](../docs/adr/ADR-0133-the-room-a-player-holds-is-scanned-for.md), whose §9 says what this story builds (`heldRoom`, the per-player stripes, `heldOrOpen`, `replyToCreateRoom`'s `WAITING` branch), that it moves **no wire** and is **not** `atomic:`, and that it **must leave the `PLAYING` case falling through to `create`** — that half is `ADR-0105` §1's own ticket, and it **is** `atomic:`, because its refusal spends a new `ProtocolError` value, `ALREADY_IN_DUEL`, which moves `ADR-0047` §2's fingerprint and forces a `PROTOCOL_VERSION` step; that is the one place `ADR-0133` **disagrees** with `ADR-0124` §6's prediction |

**`STORY-1401`–`STORY-1404`, `STORY-1411` and `STORY-1414` are written and split; the other nine
seams are not.** The planner writes each one once the decisions it waits on are merged, and the
