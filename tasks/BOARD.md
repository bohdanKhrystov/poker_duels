...
source, and six of its eight items are questions rather than instructions. `DEC-114` is answered by [`ADR-0107`](../docs/adr/ADR-0107-pot-names-every-chip-committed-to-the-hand.md)
— `Pot` names the total, every chip committed to the hand so far — and `DEC-115`–`DEC-119` are registered open for the **product owner**, who makes them concrete before any story is split — splitting
