...
| DEC-137 | **The product owner's** — does the `Call` button name the **total** (`callTo`, what ships) or **what the player must add** — the human's *"diff i need to add"*? Raised 2026-09-06 by `EPIC-14` item 4, reported as a bug and registered as a decision because there is no arithmetic to be wrong: `actionText` returns `{ verb: "Call", amount: actions.callTo }` (`action-text.ts:48-49`) under a KDoc that refuses the netting **by name** — *"Every figure here is the server's or the player's own … Nothing is priced, netted or worked out."* The client is not missing the other number (`PlayerView.committedThisStreet` is on the view and `Lobby.tsx:154` already sums both seats' copies for the sizing row); it is forbidden to subtract them, by that KDoc and by the `no-derivation` gates. An increment therefore either arrives as a second field or licenses exactly one subtraction, the way [`ADR-0107`](../docs/adr/ADR-0107-pot-names-every-chip-committed-to-the-hand.md) §5 admitted exactly one sum. The answer must also rule on [`ADR-0109`](../docs/adr/ADR-0109-the-table-marks-the-last-act-and-the-next-deal-clears-it.md)'s last-act mark, which prints *"the event's own `to` total … nothing computed"* for the same act a tick later, and on whether `Raise to` keeps saying *to* | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1414` is split |
...
| `STORY-1414` | `Call` says what it costs — *item 4* | `DEC-137` |
