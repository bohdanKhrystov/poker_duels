...
| DEC-165 | **The product owner's** — does a rematch offer owe its recipient a **minimum time to answer**, and if so how long? Registered 2026-09-10 by [`ADR-0152`](../docs/adr/ADR-0152-the-finished-rooms-five-minutes-stand.md) §5. Measured there: a finished room dies at `finish + finishedMillis` (five minutes by default) and a standing offer **does not restart that clock** (`Room.offerRematch`'s `Offered` branch, `Room.kt:330`), so what a recipient actually gets is `finishedMillis - (offer - finish)` — **floor zero, ceiling five minutes** — and no screen states any of it. Since [`ADR-0150`](../docs/adr/ADR-0150-back-to-the-lobby-keeps-the-room.md) the recipient may be reading another screen when the panel appears, and a `Rematch` press after the room is reaped answers `Failure(UNKNOWN_ROOM)` — `ADR-0044`'s *"the button can lie"*. **The mechanism for any floor is the architect's and is one field in one pure copy; the floor itself is a duration a player waits on.** ***Nothing changes* is a complete answer.** **Blocks nothing** — the window as it stands is what ships | [`ADR-0152`](../docs/adr/ADR-0152-the-finished-rooms-five-minutes-stand.md) §5 | before a QA round reports a `Rematch` press that answered `UNKNOWN_ROOM`, or the first time a player says an offer expired under them |
