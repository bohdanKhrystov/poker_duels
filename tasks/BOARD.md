...
| DEC-155 | **The architect's** — by what mechanism does a text module stay in step with the design card it was transcribed from? Registered 2026-09-08 while landing `TASK-140811`, on a gap `TASK-140710` and `TASK-140811` reported independently: nothing in `design/check-drift.sh`, `design/check-frame-cards.sh` or CI compares a transcribed module to its card, so the approved words and the shipped words are checked once by a ticket's own greps and never again. Half the mechanism already exists — `account.html` carries `<!-- ANON-BLOCK: -->` markers, `name-ask.html` and every other card carry none. **Blocks nothing** | open |
