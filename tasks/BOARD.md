...
| DEC-136 | **The product owner's** — **does the duel table scale to the viewport, or keep reflowing — and against which viewport is the fit measured?** Raised 2026-09-06 by `EPIC-14` item 3d, on the first photograph of this product on a real phone: the table scrolled and both seat plates were clipped at the screen edge. [`ADR-0103`](ADR-0103-the-table-fits-the-phone-and-the-cards-give-before-the-numbers.md) §1 promises the opposite as a number — at 390 × 664, at every beat, `scrollHeight ≤ clientHeight` with every control and number unscrolled — and §2 forbids a different table at a different width, fixing **continuous reflow** as the mechanism. The human proposes scaling the table area with a floor below which scrolling is allowed, which is a different mechanism; and iOS Safari's chrome makes the real viewport smaller and **variable**, so 390 × 664 may be why a promise that holds headless fails on the device. A defect against `ADR-0103` and a reason to amend it are both live readings, and the answer says which | [`EPIC-14`](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | before `STORY-1405` is split |
...
against `ADR-0012`'s device binding; `DEC-133` asked to widen what a showdown shows against
`ADR-0008`'s ceiling and was answered without widening it; `DEC-136` would replace the reflow
`ADR-0103` §2 fixed with scaling. `DEC-139` carries **`DEC-111`**, open since 2026-09-01. Each
answering ADR must name the clause it supersedes, and the PR that answers `DEC-130` and `DEC-139`
strikes `DEC-089` and `DEC-111` or says why they stay.
...
| `STORY-1405` | The table fits the phone it is played on — *item 3d* | `DEC-136` |
