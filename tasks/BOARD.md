...
| | [TASK-141509](tasks/TASK-141509-the-dismissal-outlives-the-screen-and-the-agreement-takes-it.md) The dismissal outlives a walk through the lobby, and the agreement takes the screen — *test-only, `review: deep`, and four `git diff --quiet` gates make repairing what it tests a failure rather than a choice. The **trip through the room's own screen** is `ADR-0138` §8's own assertion that discriminates the mount point: a panel inside `Lobby`'s cascade passes the first half and fails this one. The three awaits are positive controls, not waits. The agreement's three literals are the merged ones from `App.test.tsx:967`* | S | ready |
