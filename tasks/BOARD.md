...
| DEC-164 | **The architect's** — does the finished room's five minutes still cover the window [`ADR-0150`](../docs/adr/ADR-0150-back-to-the-lobby-keeps-the-room.md) §1 opens, now that the room's holder may spend the whole of it on another screen? Registered 2026-09-10 by that ADR §7; its §Consequences carries this as the cost **the human accepted with the call**. Measured: reaping is idleness on `lastActivityAt` (`RoomRegistry.kt:841`) and a wandering holder generates none; the default is five minutes (`RoomTimeouts.DEFAULT_FINISHED_MILLIS`); a deployment may set its own (`ROOM_FINISHED_TIMEOUT_MILLIS`). ***Nothing changes* is a complete answer**, and so is a refresh on the holder's presence. **If the answer is a different number rather than a different mechanism, the number is a promise made to a player and comes back to the product owner.** **Blocks nothing** — the window as it stands is what ships until this is answered | [`ADR-0150`](../docs/adr/ADR-0150-back-to-the-lobby-keeps-the-room.md) §7 | before a player who is not the author holds a finished room, or the first time a QA round reports a dead one |
| DEC-165 | **The product owner's** — when the room behind a standing rematch offer is gone and its holder is standing on the **front door**, which of two merged sentences does the player read: the front door's *No duel room has that code.* (`Lobby.tsx:494`, rendered whenever `state.refusal !== null`) or the panel's *That duel room is gone.* (`ADR-0138` §4)? Registered 2026-09-10 by [`ADR-0151`](../docs/adr/ADR-0151-the-press-is-one-fact-and-the-way-back-stops-navigating.md) §7. Until `ADR-0150` §2 the two could not share a screen, because the front door was unreachable while a room was held; they can now — a refused rematch press, or a press of the way back after one — and the first is addressed to a player who typed no code. `Failure` sets one field and the client cannot see which frame it answered (`ADR-0072` §7, `ADR-0044` §3), so this is a choice between two merged sentences and not a mechanism. **Blocks nothing**: the mechanism ships either way and the repair is one conditional wherever the answer puts it | [`ADR-0151`](../docs/adr/ADR-0151-the-press-is-one-fact-and-the-way-back-stops-navigating.md) §7 | with the story that implements `ADR-0150`, or the first UAT round that reaches the front door with a room held |
...
press, and `Play duel` abandoning a standing offer in silence. Registers **`DEC-165`**, **the product
