...
duel end to end, and is **`backlog` on purpose**: the feedback is quoted verbatim in the epic as the
<<<<<<< HEAD
source, and six of its eight items are questions rather than instructions. Two are now **answered**:
`DEC-114` by [`ADR-0107`](../docs/adr/ADR-0107-pot-names-every-chip-committed-to-the-hand.md) — `Pot`
names the total, every chip committed to the hand so far — and `DEC-115`, the turn clock and the
largest of the six, by
[`ADR-0108`](../docs/adr/ADR-0108-expiry-plays-the-seat-and-the-timebank-replaces-the-grace-window.md):
an expiry checks or folds the one decision and never forfeits the duel, and the timebank **replaces**
`ADR-0013`'s grace window; its mechanism is `DEC-120`, **the architect's**, in the table below.
`DEC-116`–`DEC-119` stay registered open for the **product owner**, who makes them concrete before any
story is split — splitting first would be inventing the answers. Two items need no decision at all, because *pulsing or running
=======
source, and six of its eight items are questions rather than instructions. `DEC-114`–`DEC-119` are
registered for the **product owner** — `DEC-117` (the act just made) answered on 2026-09-02 by
[`ADR-0109`](../docs/adr/ADR-0109-the-table-marks-the-last-act-and-the-next-deal-clears-it.md) —
who makes them concrete before any story is split — splitting
first would be inventing the answers. Two items need no decision at all, because *pulsing or running
>>>>>>> f74d5c5c (docs(adr): ADR-0109 — the table marks the last act, and the next deal clears it (DEC-117))
circle* is a choice between two drawings and `ADR-0024` §3 puts that in front of the human's eye rather
than in an ADR. The epic writes down what it measured so no story re-discovers it: the pot is a display
...
<<<<<<< HEAD
| [EPIC-13](epics/EPIC-13-the-living-table.md) | The living table — the turn clock, the chips, and the act just made | **backlog** — opened 2026-09-02 on the human's raw feedback after playing a duel end to end, and deliberately **not specified**: eight items, of which two need only a design card and the human's eye — the acting seat's mark, and chips that move — and six are blocked on the **product owner**. `DEC-114` and `DEC-115` are **answered**, `DEC-116`–`DEC-119` still open. `DEC-114` is answered by [`ADR-0107`](../docs/adr/ADR-0107-pot-names-every-chip-committed-to-the-hand.md) — `Pot` names the total — and `DEC-115` by [`ADR-0108`](../docs/adr/ADR-0108-expiry-plays-the-seat-and-the-timebank-replaces-the-grace-window.md) — an expiry checks or folds the one decision, never forfeits the duel, and the timebank replaces the grace window — with the mechanism open as `DEC-120`, the architect's. The largest item is the **turn clock** — 30 s a move plus a 3 m timebank that also covers disconnection — which closes a cost [`ADR-0105`](../docs/adr/ADR-0105-one-duel-at-a-time-and-the-refusal-hands-back-the-duel.md) accepted by name (*"no resign, **no turn clock**"*) and made sharper by letting *Create a duel room* refuse; it is the one item that moves the wire, so it is `atomic:` by `ADR-0070`'s probe and serialised by `ADR-0047`'s lock. **The pot item is a display decision, not a defect**: `PlayerView` already carries `committedThisStreet` and `Lobby.tsx:154` already sums it for the sizing row, so the same screen shows both numbers and prints the smaller one — no wire change is implied either way, which is why it was `DEC-114` and not an `EPIC-12` bug, and `ADR-0107` answers it: `Pot` names the total. Design-first is not re-legislated: [`ADR-0091`](../docs/adr/ADR-0091-design-gets-no-agent-a-new-screen-owes-a-card.md) §2 already makes the card the split's first ticket, and the epic adds only that a card draws **every** state it has and merges before its implementing ticket is startable. **An eighth item was added the same day** — *a refresh should leave the player where they were* — and **its reported symptom does not reproduce**: a host in a duel on a bare `/`, a rival on `?room=`, the waiting screen and a hash route on a room-free browser all survive `location.reload()`. What does reproduce is the inverse — a browser **holding a room** has its fragment **erased**, so `duels`, `leaderboard` and `account` are unreachable mid-duel — which is `DEC-119`, and the story that lands it owes a reproduction attempt on the paths not yet tried | v0.4 |
=======
| [EPIC-13](epics/EPIC-13-the-living-table.md) | The living table — the turn clock, the chips, and the act just made | **backlog** — opened 2026-09-02 on the human's raw feedback after playing a duel end to end, and deliberately **not specified**: eight items, of which two need only a design card and the human's eye — the acting seat's mark, and chips that move — and six are blocked on the **product owner**. `DEC-114`–`DEC-119` are registered; `DEC-117` (the act just made) was answered on 2026-09-02 by [`ADR-0109`](../docs/adr/ADR-0109-the-table-marks-the-last-act-and-the-next-deal-clears-it.md) — one mark, the most recent of the six acts, at the seat that made it, in the bar's own words with the event's `to` total on the four money acts; only the next act replaces it, the next painted deal removes it, and no timer, fade or dismiss ever does — leaving item 3 waiting only on its six-state card. The largest item is the **turn clock** — 30 s a move plus a 3 m timebank that also covers disconnection — which closes a cost [`ADR-0105`](../docs/adr/ADR-0105-one-duel-at-a-time-and-the-refusal-hands-back-the-duel.md) accepted by name (*"no resign, **no turn clock**"*) and made sharper by letting *Create a duel room* refuse; it is the one item that moves the wire, so it is `atomic:` by `ADR-0070`'s probe and serialised by `ADR-0047`'s lock. **The pot item is a display decision, not a defect**: `PlayerView` already carries `committedThisStreet` and `Lobby.tsx:154` already sums it for the sizing row, so the same screen shows both numbers and prints the smaller one — no wire change is implied either way, which is why it is `DEC-114` and not an `EPIC-12` bug. Design-first is not re-legislated: [`ADR-0091`](../docs/adr/ADR-0091-design-gets-no-agent-a-new-screen-owes-a-card.md) §2 already makes the card the split's first ticket, and the epic adds only that a card draws **every** state it has and merges before its implementing ticket is startable. **An eighth item was added the same day** — *a refresh should leave the player where they were* — and **its reported symptom does not reproduce**: a host in a duel on a bare `/`, a rival on `?room=`, the waiting screen and a hash route on a room-free browser all survive `location.reload()`. What does reproduce is the inverse — a browser **holding a room** has its fragment **erased**, so `duels`, `leaderboard` and `account` are unreachable mid-duel — which is `DEC-119`, and the story that lands it owes a reproduction attempt on the paths not yet tried | v0.4 |
>>>>>>> f74d5c5c (docs(adr): ADR-0109 — the table marks the last act, and the next deal clears it (DEC-117))
...
| DEC-120 | **The architect's** — by what mechanism is [`ADR-0108`](../docs/adr/ADR-0108-expiry-plays-the-seat-and-the-timebank-replaces-the-grace-window.md)'s turn clock carried, enforced and resumed? Registered open 2026-09-02 by `ADR-0108` §6, which fixes what must be true and writes no repair — `ADR-0105` §6's precedent. Open: which frame states the deadline the client counts down to (a field on an existing message or a new one — either way a server-sent fact, `ADR-0002`, with the client interpolating ticks between frames and asserting nothing, `ADR-0102`'s shape); whether an expiry is a **server-synthesised act** through the ordinary act path (`foldAbsent`'s shape) or a **new room event**, with `ADR-0028`'s mark reaching both seats either way; what schedules expiry and at what resolution — the enforced expiry may **trail** the stated deadline by a sweep period (`ADR-0025`) and may never precede it; how a resuming client learns the live deadline; and what becomes of `disconnectGraceMillis`, `GraceExpiry`, the `DUEL_PAUSED` path and `graceRemainingMillis` as `presence-countdown.ts`'s input. **It moves the wire**, so it is a `PROTOCOL_VERSION` step claimed in `ADR-0047`'s ledger under its one-bumping-branch lock, and the implementing ticket is `atomic:` sized by `ADR-0070`'s probe run to green. `poker-engine` stays closed — no clock, no new action — and timer tests inject time rather than sleep (`ADR-0013`'s rule). **Blocks `EPIC-13` item 4**, and nothing else | [`ADR-0108`](../docs/adr/ADR-0108-expiry-plays-the-seat-and-the-timebank-replaces-the-grace-window.md) §6 | before EPIC-13 item 4 is split into tickets |
...
new room event — is registered as **`DEC-120`, the architect's**, and blocks `EPIC-13` item 4 alone.
