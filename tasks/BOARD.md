...
| | [TASK-120908](tasks/TASK-120908-the-tables-sizing-control-is-the-cards-presets.md) The table's sizing control is the card's presets, not a range slider — *product; `medium`; **to be rewritten, not amended**, by the planner over the six files [`ADR-0100`](../docs/adr/ADR-0100-the-driver-reaches-an-amount-by-pressing-what-a-player-presses.md) §7 names (`ActionBar.tsx`, `ActionBar.test.tsx`, `drive-duel.tsx`, `Lobby.tsx`, `bar-no-derivation.test.tsx`, `turn-fixture.ts`), `atomic:` on `whole-duel.test.tsx`; as written it cannot go green — its two-file budget excludes the driver `DEC-100` settles, and its three-button actions row breaks the recorded `AllIn` frame independently of the amounts. **`DEC-101` is answered** — [`ADR-0101`](../docs/adr/ADR-0101-pot-means-a-pot-sized-raise-and-the-fractions-share-its-base.md) fixes the preset arithmetic (`pot → callTo + base`, the fractions over the same base, out-of-range chips absent rather than clamped), so nothing gates the rewrite but the planner; the file set is unchanged and `DEC-102`, the stepper's step, does not block it* — **unblocked 2026-08-31** by [`ADR-0101`](../docs/adr/ADR-0101-pot-means-a-pot-sized-raise-and-the-fractions-share-its-base.md) (`DEC-101`); it now awaits the planner rewrite `ADR-0100` §7 specifies — six files, atomic, gated by `whole-duel.test.tsx`| S | backlog |
