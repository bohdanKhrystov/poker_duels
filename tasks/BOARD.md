...
| | [TASK-041222](tasks/TASK-041222-the-account-screen-has-an-address-and-the-lobby-has-the-door.md) The account screen has an address, and the lobby has the door | S | **ready** — amended 2026-08-26: `## Scope` authorised a `main.tsx` edit its *Files* table forbids, the seventh disagreement of that shape this run. The flag now arrives through `useSignedIn()` and `main.tsx` is refused in `## Out of scope`; `depends_on` moves to `TASK-041231` |
