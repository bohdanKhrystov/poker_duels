...
| DEC-080 | **The architect's**, and **narrowed on 2026-08-27** by [`ADR-0085`](../docs/adr/ADR-0085-not-again-is-this-browser-and-an-answer-spends-the-offer.md), which answered `DEC-079` as *this browser*: **which key, and which module owns it** — and therefore the third entry `web-client/src/protocol/one-module-owns-each-storage-key.test.ts` gains, that file being the merged gate making *one module owns it* enforceable. Closed by that answer: nothing crosses the wire, `GET /api/me` gains no field, no column records the dismissal, no endpoint accepts it, and `STORY-0415` grows no server half (§1); and nothing needs to say *this win is the first*, since the offer is made on a win this browser has not answered for (§5). Blocks exactly what `DEC-079` blocked — the persistence, the wiring and the arc — and none of the four already written | [`STORY-0415`](stories/STORY-0415-the-offer-after-a-first-win.md) | before STORY-0415's fifth ticket |
...
`module: web-client`. **Which key and which module is `DEC-080`'s**, still the architect's.
