...
| DEC-107 | **The architect's** — where is a presence frame scoped to the room it is about, so that a client cannot apply one about a room it has left? Raised 2026-09-01 by [`STORY-1214`](stories/STORY-1214-a-duel-played-by-hand-deadlocked-on-presence.md), out of a duel the human played by hand and could not play: both tables rendered correct, agreeing game state and **each seat was told the other one had vanished**. Measured on the live stack, four reproductions and a controlled negative — the wire shows `{"type":"OpponentPresence","presence":"AWAY","graceRemainingMillis":60000}` arriving **320 ms before the recipient sends `CreateRoom`**, and with no stale frame in flight the same create-and-join is clean with **zero** presence frames on either wire. **Three merged sources make this unfixable without a decision, and each is deliberate.** `SeatDelivery.kt`'s `deliver` resolves `Addressed.seat` through the room's seating and then takes `connections.writerFor(player)` — that player's *current* connection, whatever room it is in — and its KDoc states it reads `Addressed.seat` *"and nothing else"*. `ServerMessage.OpponentPresence` carries **no room and no seat**, because `ADR-0028` §1 held that *"a seat field would be a second thing to get wrong at the one place that already decides where a frame goes"*. And `duel-state.ts` applies the frame unconditionally, because it has nothing to test it against. **So no layer can currently recognise a stale frame**, and the four places one could be given that power differ in cost: a room check in `deliver` (contradicts its stated contract and needs a connection to know its room); a room on the wire (**moves `PROTOCOL_VERSION` 5 → 6**, making the repair `atomic:` across the ledger, and reopens `ADR-0028` §1); binding presence production to the seat a connection currently occupies; or a client-side reset on `RoomJoined` (cheapest, but still delivers the frame to a lobby, and does nothing for a player who has moved to a *different* live room). Part of the same question: should `disconnect()` produce a frame at all when the other seat is no longer connected to that room? **Blocks [`TASK-121403`](tasks/TASK-121403-presence-is-about-the-room-the-reader-is-in.md)**, whose *Files* table and `verify:` block are placeholders until this lands, and which the planner re-cuts whole with an `ADR-0069` probe once it does. Not raised by a QA round, so **not** one of `ADR-0092` §5's three slots | [`STORY-1214`](stories/STORY-1214-a-duel-played-by-hand-deadlocked-on-presence.md) | before `TASK-121403` starts |
| DEC-108 | **The product owner's** — when the table says *The duel is paused.*, may the action bar stay enabled? Raised 2026-09-01 by [`STORY-1214`](stories/STORY-1214-a-duel-played-by-hand-deadlocked-on-presence.md). Measured: under that notice every control read `disabled: false` — `Fold`, `Call 100`, `Raise to 200`, `All in 10,000` and all four sizing chips — and the human obeyed the sentence and waited while the duel was in fact playable. A script clicking through sees nothing wrong; a person reading the screen stops. **This is not filed as a defect, and deliberately so**: [`ADR-0046`](ADR-0046-the-table-says-away-timed-out-and-back.md) §Out of scope declines the question **by name** — *"Whether the action bar's controls look disabled while the duel is paused"* — recording that `ADR-0028` §6 keeps `YourTurn` standing with `DUEL_PAUSED` as the refusal and that `STORY-0313`'s criteria *"already assume a live bar and an explained refusal"*. A live bar therefore contradicts no merged source, so a bug ticket would contradict a merged ADR and a QA case asserting the opposite would have no source (`ADR-0092` §5). What is genuinely open is whether *wait* and *you may act* should be sayable at the same moment at all — and, if not, whether the bar dims or the notice changes its words. **Blocks nothing**; `TASK-121401` writes `CORE-23` against the half that *is* merged (a client may never assert a game fact — the screen said paused while the server accepted the action) and leaves this untouched. The case that checks this answer is written when the answer merges | [`STORY-1214`](stories/STORY-1214-a-duel-played-by-hand-deadlocked-on-presence.md) | before the next `/qa-cycle audit` invocation |
...
| **[STORY-1214](stories/STORY-1214-a-duel-played-by-hand-deadlocked-on-presence.md)** A duel played by hand deadlocked on presence, and the catalogue could not have caught it — *schema 2; **not a round story** — found by the human playing by hand, outside every round, so `EPIC-12` §Termination rule 1 puts it in the ordinary backlog and no `A(N)` or `B(N)` moves; registers `DEC-107` (architect) and `DEC-108` (product owner)* | | | **ready** |
| | [TASK-121401](tasks/TASK-121401-the-catalogue-sees-a-present-player-marked-away.md) The catalogue sees a present player marked away — *`CORE-06` gains presence, a false Reconnect preamble goes, and `CORE-21`–`CORE-23` are added **red on today's product*** | S | ready |
| | [TASK-121402](tasks/TASK-121402-the-duel-table-column-fits-the-phone-it-is-nested-in.md) The duel table's column fits the phone it is nested in — *`manual-verify`; `TASK-121302` merged with its measurement undischarged and the client reads **712 / 664**; a **new** ticket, `STORY-1213` not reopened* | XS | backlog |
| | [TASK-121403](tasks/TASK-121403-presence-is-about-the-room-the-reader-is-in.md) Presence is about the room the reader is sitting in — *the blocker; **blocked on `DEC-107`**, *Files* and `verify:` are placeholders the planner re-cuts once it lands* | S | blocked |
