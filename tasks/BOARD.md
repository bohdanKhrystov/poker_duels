...
| | [TASK-121606](tasks/TASK-121606-the-harness-stops-what-it-starts.md) The harness stops what it starts — *filed 2026-09-10 on the human's question "why did they not finish?". Five agents were listed as running; all five had merged their tickets, the newest three days earlier, and each was held open by a child it had started and could not stop — two by the identical wedged `vitest list`, one from the main checkout rather than its worktree. Every one released within seconds of its child being stopped. Also standing: six headless Chrome roots from rounds that never called `chrome-down`, and `agent-a4b7dd77…-postgres-1` holding `0.0.0.0:5432`, which left this project's own database with no published port — the state `TASK-121603` prevents for new containers but cannot evict from one that predates it. Adds `down` and `reap` to `stack.sh`, **reversing that file's own header**: `kill` stays denied and is replaced by a narrower rule that takes no pid from its caller, reaches nothing outside `$ROOT`, and excludes its own ancestry. **Named cost:** a build running under another tool call is inside `$ROOT` and therefore in reach of `down`, so the verb is for a machine between rounds, not one mid-build. **The mutation has not been run** — two attempts were correctly refused by the permission layer, and the ticket names the three mutations the coder owes, which is why it is `review: deep`* | S | ready |
