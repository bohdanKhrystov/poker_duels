...
`EPIC-14` (the name, the showdown and the fit) **opened 2026-09-06**, on the first report from a
player rather than from a driver: the human brought the stack up on the local network and played
the product on a **laptop and an iPhone**. **Six items** — three in the first message, three added
the same day — sixteen story seams, and it opens `backlog` because **eleven product decisions**
stand in front of it, `DEC-129`–`DEC-139` — **five of them were answered on 2026-09-06: `DEC-138`
by [`ADR-0123`](../docs/adr/ADR-0123-a-standing-rematch-offer-follows-the-rival.md), `DEC-139` by
[`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md), `DEC-130` by
[`ADR-0125`](../docs/adr/ADR-0125-the-account-screen-names-the-anonymous-profile-and-owns-the-door.md),
`DEC-134` by [`ADR-0126`](../docs/adr/ADR-0126-the-table-shows-the-cards-and-marks-none-of-them.md)
and `DEC-135` by
[`ADR-0127`](../docs/adr/ADR-0127-a-control-stands-only-for-a-decision-the-server-has-opened.md),
which registered `DEC-144`, `DEC-145` and `DEC-147` in their place.** Five of them ask to change
something a merged ADR decided: the post-win account offer
(`ADR-0036`, `ADR-0085`, `ADR-0086`, `ADR-0116`) would go, a display name whose ADR title says
**permanent** (`ADR-0029`) would become changeable, a sign-out would hand the browser a **new**
anonymous profile against `ADR-0012`'s device binding, and the phone's table would **scale** where
`ADR-0103` §2 fixed continuous reflow — and `DEC-139` **re-opened `DEC-111`**, which had sat open
since 2026-09-01 and was answered *yes, silently* by the shipped product: `Back to the lobby` calls
`ADR-0072` §3's `forgetRoom()` while `ADR-0105` §2 refuses only a `PLAYING` room, so a host who
leaves and presses play again opens a **second** waiting room and orphans the code they may already
have sent. [`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md)
struck both on 2026-09-06 — one waiting room per player, and the second press hands back the first,
answered on the server so the browser can go on forgetting. The epic's *What is already true* is
what keeps that honest:
the coins the human asked to see *transferred* on promotion never move at all (`ADR-0030` §1, and
the story owes a test rather than a migration); the showdown the human never saw is already emitted
by the engine and drawn by no client file; `BestHand.cards` has said in its own KDoc since August
that it exists for a highlight nothing has ever rendered; and the timebank figure photographed as
`1:58.623999999999995` has one cause, one call site and one line; the `Call` button reported as a
bug does no arithmetic at all, because `action-text.ts` refuses it by name — *"Nothing is priced,
netted or worked out"* — so what looked like a calculation error is a decision about which number a
button may say; and the room lifetime the human asked for at *"3-5 min"* already ships at **ten**.
Its five annotated screenshots
are **transcribed into the epic** rather than committed, by the human's choice — the repository
holds no binaries and keeps none.
...
| [EPIC-13](epics/EPIC-13-the-living-table.md) | The living table — the turn clock, the chips, and the act just made | **ready** — **split into eleven stories on 2026-09-02**; **`STORY-1301` was cut into three tickets the same day** and was the one startable then; `STORY-1301`–`STORY-1308` are now done, **`STORY-1309` was split into twelve tickets on 2026-09-04** once `ADR-0113` answered `DEC-120`, `STORY-1311` still waits on `DEC-123`, and the split raised `DEC-124` (the product owner's — does an animated surface owe a still form?), which blocks only the implementing tickets of `STORY-1303` and `STORY-1306`. Opened 2026-09-02 on the human's raw feedback after playing a duel end to end and deliberately **not specified**; its six product decisions were all answered the same day by the `product-owner` agent, and the epic was split the same day. Eight items: two needed only a design card and the human's eye (the acting seat's mark, and chips that move) and six needed a merged answer first. [`ADR-0107`](../docs/adr/ADR-0107-pot-names-every-chip-committed-to-the-hand.md) makes `Pot` the **total** — `PlayerView` already carries `committedThisStreet` and `Lobby.tsx:154` already summed it for the sizing row, so the same screen was showing both numbers and printing the smaller one; no wire moves. [`ADR-0108`](../docs/adr/ADR-0108-expiry-plays-the-seat-and-the-timebank-replaces-the-grace-window.md) is the largest: 30 s a move plus a 3 m timebank, an expiry that **plays the seat and never the duel**, and a timebank that **replaces** [`ADR-0013`](../docs/adr/ADR-0013-disconnect-grace-period.md)'s grace window — so the duel never pauses and `DUEL_PAUSED` loses its occasion. It closes a cost [`ADR-0105`](../docs/adr/ADR-0105-one-duel-at-a-time-and-the-refusal-hands-back-the-duel.md) accepted by name (*"no resign, **no turn clock**"*), and it is the one item that moves the wire, so it is `atomic:` by `ADR-0070`'s probe and serialised by `ADR-0047`'s lock. [`ADR-0109`](../docs/adr/ADR-0109-the-table-marks-the-last-act-and-the-next-deal-clears-it.md) gives the table **one** mark, cleared only by the next act or the next painted deal — never a timer. [`ADR-0110`](../docs/adr/ADR-0110-creating-a-duel-seats-the-host-at-the-table.md) **retires the waiting screen**: the host waits at the table, and `ADR-0073` §3's promises move with them so `ADR-0105` §2 still cites a sentence a surface renders. [`ADR-0111`](../docs/adr/ADR-0111-an-illegal-typed-amount-is-refused-in-the-servers-own-numbers.md) refuses an illegal typed amount in the server's own numbers — never clamps, never knowingly sends — with `ADR-0100` §5's driver guarantee explicitly intact. [`ADR-0112`](../docs/adr/ADR-0112-only-a-running-duel-refuses-another-screen.md) answers the eighth item, added by the human the same day: **the reported refresh symptom did not reproduce** — a host in a duel on a bare `/`, a rival on `?room=`, the waiting screen and a hash route on a room-free browser all survive `location.reload()` — and what did reproduce is the inverse, a browser **holding a room** having its fragment erased. So only a **running** duel refuses another screen, the refusal restores the address, and `WAITING`/`FINISHED`/`ABANDONED` let the player look away; the story that lands it still owes a reproduction attempt on the six paths not yet driven. Two **architect's** decisions were registered along the way and gate the wire work rather than the split: `DEC-120` (the clock's mechanism) and `DEC-123` (the navigation guard's). Design-first is not re-legislated: [`ADR-0091`](../docs/adr/ADR-0091-design-gets-no-agent-a-new-screen-owes-a-card.md) §2 already makes the card the split's first ticket, and the epic adds only that a card draws **every** state it has and merges before its implementing ticket is startable | **done 2026-09-05** — eleven stories, 90 tickets, every one merged on one dispatch |
| [EPIC-14](epics/EPIC-14-the-name-the-showdown-and-the-fit.md) | The player has a name, the showdown shows its hand, and every screen fits | **backlog** — opened 2026-09-06 on the human's feedback after playing the product on a **laptop and an iPhone** against a local-network stack, with five annotated screenshots transcribed into the epic as its source (the images are deliberately not committed). **Six items** — three in the first message, three added the same day — sixteen story seams, **eleven decisions, all the product owner's** — **all eleven answered, nine on 2026-09-06 and `DEC-131` and `DEC-132` on 2026-09-07**: `DEC-129` by [`ADR-0119`](../docs/adr/ADR-0119-the-name-is-asked-at-the-first-press-and-skipping-plays.md), `DEC-133` by [`ADR-0120`](../docs/adr/ADR-0120-a-showdown-shows-the-hands-the-rules-showed-and-the-beat-that-shows-them-stands.md), `DEC-136` by [`ADR-0121`](../docs/adr/ADR-0121-the-table-reflows-never-scales-and-the-shape-is-measured-on-the-device.md), `DEC-137` by [`ADR-0122`](../docs/adr/ADR-0122-call-names-the-price-and-raise-to-names-the-total.md), `DEC-138` by [`ADR-0123`](../docs/adr/ADR-0123-a-standing-rematch-offer-follows-the-rival.md), `DEC-139` by [`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md), `DEC-130` by [`ADR-0125`](../docs/adr/ADR-0125-the-account-screen-names-the-anonymous-profile-and-owns-the-door.md), `DEC-134` by [`ADR-0126`](../docs/adr/ADR-0126-the-table-shows-the-cards-and-marks-none-of-them.md) and `DEC-135` by [`ADR-0127`](../docs/adr/ADR-0127-a-control-stands-only-for-a-decision-the-server-has-opened.md), and on 2026-09-07 **`DEC-131`** by [`ADR-0130`](../docs/adr/ADR-0130-a-name-can-be-changed-and-the-name-it-leaves-is-spent.md) and **`DEC-132`** by [`ADR-0131`](../docs/adr/ADR-0131-signing-out-of-your-own-account-hands-the-browser-a-new-profile.md). With **`DEC-140`** by [`ADR-0128`](../docs/adr/ADR-0128-the-copy-control-is-never-absent-and-a-press-that-cannot-copy-hands-over-the-selection.md) and **`DEC-141`** by [`ADR-0129`](../docs/adr/ADR-0129-the-host-is-told-the-duel-starts-by-itself.md), the two the planner raised while splitting item 3b, **all thirteen of this epic's decisions are answered**; of the follow-ups those answers registered, **`DEC-144`** is answered by [`ADR-0138`](../docs/adr/ADR-0138-the-panel-mounts-beside-the-lobby-and-the-dismissal-lives-in-the-mount.md) and **`DEC-150`** by [`ADR-0140`](../docs/adr/ADR-0140-the-clipboard-api-is-the-only-copy-this-client-attempts.md), **`DEC-145`** is answered by [`ADR-0141`](../docs/adr/ADR-0141-taking-a-seat-elsewhere-releases-the-room-you-were-holding.md), **so none of them is still open and no story waits on one**. **`DEC-142` is answered on 2026-09-07** by [`ADR-0137`](../docs/adr/ADR-0137-a-name-suggestion-is-drawn-in-the-browser.md) — a name suggestion is **drawn in the browser** by `suggestName(random, replacing?)`, a pure function that takes no player fact at all, from an **import-free, bundled vocabulary module** whose arity is data; **`name_registry` is never consulted** and **nothing crosses the wire**, so rarity is bought with the size of the set (a tested floor of `1_000_000` combinations) rather than with a check, three tests make a `400` on a suggestion impossible while leaving the `409` that `ADR-0119` §4 already priced at one press, the reroll is free because `ADR-0134` §6 refunds a `409`, and **`STORY-1407` waits on nothing and its tickets are not `atomic:`**. **`DEC-147` is answered on 2026-09-07** by [`ADR-0132`](../docs/adr/ADR-0132-the-profile-says-whether-it-holds-a-password.md) — `GET /api/me` gains `hasPassword: Boolean`, computed on every response carrying a `ProfileResponse` and told only to the player it is about, a missing field reads `unavailable` rather than `false`, and the provider re-reads after `signUp`'s `signed-up`; no wire moves, so `STORY-1408` is startable whole and its ticket is not `atomic:`. `DEC-110`, widened to the `WAITING` case, is **answered on 2026-09-07** by [`ADR-0133`](../docs/adr/ADR-0133-the-room-a-player-holds-is-scanned-for.md) — the room a player holds is found by a **lock-free scan** of the registry (`heldRoom`), opening is find-or-create under a **per-player stripe**, the `WAITING` hand-back is `RoomJoined` alone and moves no wire, and only the `PLAYING` refusal spends a `ProtocolError` value and a `PROTOCOL_VERSION` step — so **`STORY-1416` is unblocked, is not `atomic:`, and the `PLAYING` half stays `ADR-0105` §1's own `atomic:` ticket, outside this epic**. **`DEC-146` is answered on 2026-09-07** by [`ADR-0136`](../docs/adr/ADR-0136-a-beat-declares-its-own-length-and-zero-silences-every-beat.md) — a beat carries its **kind** (`RevealStep.hold: "step" | "read"`, set in `layOutReveal` from `ADR-0120` §3's own predicate over `view.viewerSeat` and `seat.holeCards`, so the classification is a pure function of one frame) while the **store** owns both millisecond figures (`readMillis` absent meaning `stepMillis`, `REVEAL_READ_MS = 2000` beside `REVEAL_STEP_MS = 600` at `ADR-0102` §4's boot seam), and **`stepMillis === 0` silences every beat**, gated in the store — so `drive-duel.tsx` is **not edited**, `ADR-0100` §3's four recorded-frame suites are neither edited nor re-recorded, no component learns the hold, the hold has **no CSS** so `ADR-0115` §3 holds by construction, and **nothing crosses the socket**, leaving `STORY-1411`'s implementing ticket a plain ticket rather than an `atomic:` one. **`DEC-152` is answered the same day** by [`ADR-0135`](../docs/adr/ADR-0135-the-server-says-which-sign-out-this-is-and-the-browser-forgets-one-key.md) — the server answers one boolean on a new `GET /api/me/device`, the browser forgets `pd.deviceId` and nothing else, the replacement profile is minted by the `Identity.Anonymous` handshake that already ships, and **nothing crosses the socket**, so `STORY-1410`'s implementing ticket is startable and is **not** `atomic:` — but **`STORY-1408` and `STORY-1410` must be serialised**, because `ADR-0132`'s `hasPassword` and this ADR's device bit both edit `docs/protocol.md`'s `/api/me` section and `web-client/src/profile/profile.ts`. Item 1 gives the player an identity they chose — a name asked before the first duel from a **free** suggestion (`DEC-129`), an account screen that says *anonymous* and owns the promotion in place of the post-win offer (`DEC-130`, **answered** by `ADR-0125` — the offer is retired whole, nothing replaces it, the front door stops printing `No profile yet.`, and the open `DEC-089` is struck by deletion), a name that can be changed (`DEC-131`, **answered** by `ADR-0130` — *that* it can is the human's verbatim, while **what happens to the string it leaves is derived**: retired, not released and not held; history prints the name as it **is**; one form on the **account screen**; `PERMANENCE_LINE` leaves the product; and **`DEC-151`** is registered for the mechanism and **answered** by [`ADR-0134`](../docs/adr/ADR-0134-a-rename-spends-before-it-replaces.md) — the permanence trigger becomes `player_display_name_never_released` with `ADR-0051` §3's body minus one disjunct, the write is **four statements** under `SELECT … FOR UPDATE`, the vacated row moves **`TAKEN → REPLACED`** with `retired_from` set, `PUT /api/me/name` loses `403` and gains `429`, the write is **budgeted** by `PlayerId` metering spending rather than trying, and nothing crosses the socket, so `STORY-1409` waits on nothing — and `ADR-0134` §7 hands the planner an ordering, since the migration is **behaviour-preserving on its own**: the schema and the write path may land as **separate tickets**) and a sign-out that hands the browser a **new** profile (`DEC-132`, **answered** by `ADR-0131` — a new profile when the account being left is the one this browser owns, the profile it already had when it is somebody else's, nothing revoked or destroyed either way, and the player told on the confirmation that already asks). Four of those contradict merged ADRs by name — `ADR-0036`/`ADR-0085`/`ADR-0086`/`ADR-0116`, `ADR-0029`'s *permanent*, and `ADR-0012`/`ADR-0027`/`ADR-0037` — so each answering ADR must say which clause it supersedes, and three now do: `ADR-0125` was the first — `ADR-0085`, `ADR-0086` and `ADR-0116` in full, plus `ADR-0036`'s offer block, with its *never required* first paragraph left standing and re-applied — `ADR-0130` supersedes `ADR-0029` §4's rename clause, its title's *permanent* and §5's `403`, plus `ADR-0051` §3's *no exception at all* and `ADR-0119` §3's obligation 2, and `ADR-0131` supersedes `ADR-0030` §3's outcome clause and §8's *not on sign-out*, **for one case only**, while superseding nothing at all in the three the epic named. **Item 1e is already true and owes only a test**: `ADR-0030` §1 makes a promotion move no row, so the coins the human wants *transferred* were never anywhere else — and `ADR-0131` §2 keeps it true, because the only act that abandons a profile abandons one that holds a password. Item 2 is the showdown, and it is mostly a **client** gap: the engine emits `HandRevealed` for every shown hand and `EventRedaction` passes it to both seats, yet no client file outside `protocol.gen.ts` mentions it, and `BestHand.cards` — whose KDoc says it exists *"because the client highlights the winning five at showdown"* — reaches nothing outside `poker-engine`. `DEC-133` asks how far the reveal goes against `ADR-0008`'s ceiling, `DEC-134` is answered with it — **the table marks no card**, so `ADR-0095` §3's refusal now covers pictures as well as words, item 2b moves no wire and `BestHand` stays inside the engine — and chips crossing the table is a card under `ADR-0115`. Item 3 is the screens: the timebank's `1:58.623999999999995` is a **defect with one cause** (`turn-clock.ts:105` hands `bankFigure` a fraction where its KDoc asks for whole seconds, on the plate of the seat the clock does not name), the front door covers its own wordmark, and the phone contradicts [`ADR-0103`](../docs/adr/ADR-0103-the-table-fits-the-phone-and-the-cards-give-before-the-numbers.md) §1's promise that the table never scrolls at 390 × 664 — **`DEC-136` is answered**: [`ADR-0121`](../docs/adr/ADR-0121-the-table-reflows-never-scales-and-the-shape-is-measured-on-the-device.md) rules it a **defect against §1's instrument**, keeps the reflow and refuses the scale on `R3`, adds the width axis (`scrollWidth ≤ clientWidth`), and makes 390 × 664 a stand-in that a device reading may correct **downward** and nobody may raise, so `STORY-1405` is startable. The three items added later are each one decision wide: `DEC-137` asks whether `Call` names the total or the price, and is a decision rather than a bug because `action-text.ts` states *"Nothing is priced, netted or worked out"* and the client is forbidden the subtraction; `DEC-138` asked what surface a rematch offer takes now that `ADR-0112` lets the rival walk off the result screen, in a product with no modal of any kind, and is **answered** by `ADR-0123` — the offer follows the rival onto every screen as a **panel that never takes the screen**, dismissal hides the surface and never the offer, and the mechanism is registered as **`DEC-144`**, the architect's; and `DEC-139` asked whether pressing play again returns the host to the room they already hold — carrying the open **`DEC-111`** with it, since the ten-minute `WAITING` lifetime already ships and it is `forgetRoom()` plus `ADR-0105` §2's `PLAYING`-only refusal that quietly opens a second room. **Both are answered** by [`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md) on 2026-09-06: a second `CreateRoom` from a holder of a `WAITING` seat hands back the room they hold — no second room, no refusal, no new string, no client change — and the browser goes on forgetting, because the return is the server's. `STORY-1416` now waits on the architect's `DEC-110`, widened to the `WAITING` case, and `DEC-145` is registered for the one route left into two duels at once. **`DEC-145` is answered on 2026-09-08** by [`ADR-0141`](../docs/adr/ADR-0141-taking-a-seat-elsewhere-releases-the-room-you-were-holding.md) — the player **takes the seat** (`ADR-0094` §1 gains no second qualification) and the room they were holding is **released in the same act**, its code answering `UNKNOWN_ROOM` so that a released room is indistinguishable from a reaped one; **no string, no wire, no client file and nothing stored**, only a room still `WAITING` and holding nobody else is ever released, a join that seats nobody changes nothing, and **a duel can still begin into a seat that is already away** — that stays `ADR-0073` §3's promise working. It supersedes two sentences of `ADR-0073`, amends `ADR-0105` §2's `WAITING` row on its `JoinRoom` half, and registers **`DEC-154`** for the architect. So of the follow-ups this epic's answers registered, none is open: `DEC-144` and `DEC-150` are answered too, and the one decision this answer *adds* — **`DEC-154`**, the release's call site and lock order — **is answered on 2026-09-10** by [`ADR-0147`](../docs/adr/ADR-0147-the-held-room-is-abandoned-after-the-seat-under-the-joiners-own-stripe.md): the release is `Room.abandon` through `mutate` — never a removal, since `Room.join` already answers `UNKNOWN_ROOM` for an `ABANDONED` room — found by a new `waitingRoomsHeldBy(player, except)` rather than by `heldRoom`, which sorts `PLAYING` first and would answer with the room just joined, and taken under the joining player's own stripe outermost with **no two room mutexes ever held at once**, so `replyToJoinRoom` changes by exactly one identifier. **So no decision of this epic's is open**, and the ticket that implements `ADR-0141` — never written — is now writable | v0.2 |
...
| DEC-126 | **The architect's** — in the deployment this product is actually offered on, what serves the built bundle, on what origin, and does the Ktor server serve it or does something in front of it? Registered open 2026-09-04 by [`ADR-0117`](../docs/adr/ADR-0117-the-proofs-of-record-load-the-built-bundle.md) §8, which answers the **proof** half of `DEC-087` with `vite preview` on a workstation and **deliberately declines the deployment half** rather than letting a QA instrument imply an answer: `vite preview` is not a statement that anything ships behind Vite. It is genuinely open — [`docs/architecture.md`](../docs/architecture.md#deployment-later) says *"Not decided yet; not needed before v0.2"* — and `ADR-0117` §Alternatives 1 states the strongest case for the Ktor server serving `dist/` (one origin, no proxy, and `socket-url.ts`'s KDoc already claiming *"production is same-origin"*) and rejects it **only as the proof mechanism**, on cost and on refusing to pre-decide this. Two things turn on the answer: `ADR-0088` §2 step 3's origin, which becomes the deployment's once one exists and supersedes `ADR-0117` §4; and `ServerConfig.BASE_URL`, whose default still names the old dev origin. Registered as a `DEC` rather than a prose pointer at `EPIC-07` **on purpose** — twenty-odd ADRs name an epic that is not written and collects none of them, and a row in an open table with a due event is the one form this repository strikes | [`ADR-0117`](../docs/adr/ADR-0117-the-proofs-of-record-load-the-built-bundle.md) | **before anything is offered to real users** — the same event as `ADR-0093` §1b's bound transport, and beside it |
| DEC-128 | **The product owner's** — when a hand ends with an **uncalled bet returned**, should the table's award banner state the amount **awarded from the pot** (`PotAwarded.amount`, what ships) or the **total chips the winner receives at the resolution** (the award plus the returned uncalled bet, which is exactly the `Pot` figure the same slot printed a tick earlier)? Raised 2026-09-05 at the triage of [`EPIC-13`](epics/EPIC-13-the-living-table.md)'s Definition-of-done drive — 31 hands, two fresh Chrome profiles, `866ebc21`. **Measured, three cases:** at 50/100 an uncontested fold made `Pot 150` → `wins 100` with the winner's stack up 150; at 75/150, `Pot 225` → `wins 150`, stack up 225; at a real 100/200 showdown, `Pot 400` → `wins 400`, stack up 400. The gap is exactly the uncalled blind excess the folder never matched, so the shipped banner is a **third** quantity — neither the pot nor net profit, which is +50 and +75 — and *"the banner states the big blind"* is a coincidence of `BB = 2 × SB` that the showdown row already falsifies. **Not a defect, and deliberately not filed:** [`ADR-0095`](../docs/adr/ADR-0095-the-table-states-who-took-the-pot-and-never-names-a-hand.md) §2 names the number by name — *"a `PotAwarded.amount` this client received … **not** a sum of what the seats committed, **not** a stack delta, **not** the view's `pot`"* — which refuses all three of the values the drive proposed, and `PotStrip.awardLineFor` prints `formatChips(award.amount)` with no arithmetic, so a bug ticket would contradict a merged ADR. **What is genuinely open** is that `ADR-0095` §3 and its Alternative 4 **deleted** the card's fold-ending second line (*"Nobody shows — your river bet of 800 goes uncalled and returns"*) on the stated ground that *"the printed amount and the stack change already agree to the chip"* — true against the strip of 2026-08-30, which printed `view.pot` (`0` preflop), and **false since** [`ADR-0107`](../docs/adr/ADR-0107-pot-names-every-chip-committed-to-the-hand.md) made the strip print the total, its §3 accepting by name that the figure *"includes chips that a fold would hand back to the raiser"*. A player now watches 150 become *wins 100* while 150 arrives, which is the sentence `ADR-0095` deleted because nobody would need it. The answer must also settle whether the fold ending owes that line back. It is cheap either way — `UncalledBetReturned` carries `seat` and `amount` and is already in `narration`, so either shape is a **transcription** and no wire moves — but a banner printing award + return would be a client-computed figure, which `ADR-0095` §2 forbids and `no-derivation.test.tsx` gates, so a **change** hands the *how* to the architect. **Blocks nothing**, and it does not block `EPIC-13` from closing: the banner is not one of the DoD's five visible claims, and *a pot that matches the sizing row's base* was observed directly on the same drive | [`EPIC-13`](epics/EPIC-13-the-living-table.md) §Definition of done | before the first duel played by someone who is not the author — [`ADR-0095`](../docs/adr/ADR-0095-the-table-states-who-took-the-pot-and-never-names-a-hand.md) §6's own named trigger |
| DEC-168 | **The product owner's** — does the rematch panel go on saying *That duel room is gone.*, and retiring `Rematch`, for a refusal the player **brought on themselves**? Registered 2026-09-10 by [`ADR-0154`](../docs/adr/ADR-0154-the-gone-room-is-the-rematch-surfaces-news.md) §6, which answered `DEC-166` for the **front door** and left the panel exactly as merged. `RematchNotice` branches on the same one `state.refusal` field (`ADR-0138` §4), so on the held-room front door a **mistyped room code** now flips the panel to the gone-room sentence and retires the control over an offer that may still be live — for the rest of the room's five minutes, and for at least the sixty seconds `ADR-0153` §1 guarantees it. Newly reachable for exactly the reason `DEC-166` was. It cannot be answered by tracking which frame the `Failure` answered — `ADR-0072` §7 refuses that — and `RematchControl` shares the branch and the words, so any answer must also say what the **result screen** does. **Blocks nothing**: `ADR-0154` §1's conditional ships either way | [`ADR-0154`](../docs/adr/ADR-0154-the-gone-room-is-the-rematch-surfaces-news.md) §6 | with the story that implements `ADR-0150`, or the first UAT round that reaches the front door with a room held |

`DEC-169` → [`ADR-0156`](../docs/adr/ADR-0156-the-relay-dials-by-name-and-holds-both-loopback-families.md)
on 2026-09-10 — **the relay dials its target by name, and holds both loopback families.** The
architect's, **registered and answered in the same pull request**: it was found by `TASK-131006`'s
drive rather than by a planner, has never stood in an open table, and gated `TASK-131006` —
`EPIC-13`'s last ticket. The question: *by what change does the QA relay reach the preview origin
[`ADR-0117`](../docs/adr/ADR-0117-the-proofs-of-record-load-the-built-bundle.md) §1 established?*
`scripts/qa/delay.mjs:66` dials `connect(targetPort, "127.0.0.1")` while
`web-client/vite.config.ts:7` sets `preview: { port: 4173, strictPort: true }` with **no `host`**,
so Vite binds `localhost` — `::1` first on this machine — and the relay accepts the browser but
cannot forward it. The drive read `cut 0`: no app socket to sever, a number that looks like a
reading. **The answer changes the instrument, not the origin.** `vite.config.ts` is untouched,
`ADR-0117` is neither amended nor superseded, and the origin of a proof of record stays
`http://localhost:4173`. **The question's premise was falsified while answering it**: dev does *not*
bind both families — dev and preview resolve their host through one `resolveHostname` and both bound
`::1` when run against this repository's own config — so how the delayed `dev` drives ever reached
Vite on `5273` is recorded nowhere, and those readings passed on a condition nothing states,
requires or checks. **§1** dials the target by name with `autoSelectFamily: true` written
explicitly, because with it off the name reaches `::1` only and is `ECONNREFUSED` against a
`127.0.0.1` listener, and because a default is a Node-version fact a flag can flip. **§2** keeps the
other four `connect` sites literal under one rule — *dial by literal address what you bound, by name
what somebody else bound* — since a `localhost` `cut` could reach a different process on `[::1]` and
report `cut 0` all over again. **§3** holds both loopback families on the listen port or refuses to
start: measured, `[::1]:P` does **not** collide with `127.0.0.1:P`, so a `preview` left on its
default `4173` is reached directly by a browser that prefers `::1` and the relay is bypassed in
silence — the same fault that cost `P1` a reading. **§4 is what stops this recurring**: a start-up
dial that exits 1 naming the target and prints the resolved `remoteAddress`, a per-connection
failure message where the `error` handler swallows one today, and `--selftest-unreachable` proving
both. **§5** pins the relayed layout — `npm run preview -- --port 4273` behind the relay on `4173`,
so the served port moves and the origin a record names does not — with readiness taken as
`ADR-0117` §2's `/@vite/client` marker fetched **through** the relay. **§6 writes the follow-up
ticket for the planner**: two files, `scripts/qa/delay.mjs` and `drive.mjs`'s `PD_APP_ORIGIN`
(`ADR-0117` §3 exactly as merged), gated by the three self-tests. Costs named, the sharpest first:
**a machine without IPv6 loopback cannot run the relay at all**, deliberately, because a silently
skipped bind is the silence being deleted; the relay's port becomes exclusive on the machine, so
bring-ups that used to work by bypassing it now refuse and `kill` is denied here; dialling by name
spends certainty about which server answered, recorded for the start-up dial only; the path is
proved at bring-up and not continuously; and `drive.mjs` defaulting to `:4173` while `stack.sh`
still probes `5173` leaves two truths about the origin until `ADR-0117` §3's `stack.sh` half lands —
owed, and blocking nothing. Registers no decision: the LAN proof origin
[`ADR-0140`](../docs/adr/ADR-0140-the-clipboard-api-is-the-only-copy-this-client-attempts.md) §6
wants is argued as a rejected alternative and left to the ticket that needs it.

`DEC-167` → [`ADR-0155`](../docs/adr/ADR-0155-the-floor-is-its-own-instant-on-the-room.md)
on 2026-09-10 — **the floor is its own instant on the room, and `lastActivityAt` keeps its meaning.**
All four parts of [`ADR-0153`](../docs/adr/ADR-0153-a-rematch-offer-owes-its-recipient-a-minute.md)
§6, and every one **derived**: `ADR-0153` §1 fixed the promise and §4 fixed the silence, so nothing
left was a product call. **§1 — where it lives.** `Room` gains a sixteenth property,
`rematchFloorEndsAt: Long?`, in the defaulted tail immediately after `lastActivityAt` — an absolute
instant in `ADR-0113` §3's idiom (*"a value nobody has to remember to decrement is a value nobody
forgets to"*), named for the rule it serves rather than the effect it has, so that a second reason to
hold a room is a rename and an ADR rather than an extra call. **§2 — who arms it.**
`Room.offerRematch`'s `Offered` branch (`Room.kt:330`) becomes `copy(rematchOffers = offers,
rematchFloorEndsAt = now + RoomTimeouts.REMATCH_FLOOR_MILLIS)`; the `Agreed` branch and `Room.abandon`
clear it in the `copy` that already clears the offers; and a sixteenth `init` `require` —
`rematchFloorEndsAt == null || rematchOffers.isNotEmpty()` — makes `ADR-0153` §3's **one arming**
structural instead of conventional: past the `ALREADY_OFFERED` guard (`Room.kt:315`) the same seat
cannot re-arm, a `PLAYING` room can never carry a floor so agreement cannot leak one into the next
duel's `finish`, and a room with a floor and no standing offer **cannot be constructed at all**.
**§3 — a constant, not a knob.** `RoomTimeouts.REMATCH_FLOOR_MILLIS = 60 * 1000L`, on the companion
beside the four `DEFAULT_` constants and deliberately without that prefix, because there is no field
for it to be the default of: no fifth `RoomTimeouts` field, no `ServerConfig` key, no
`ROOM_REMATCH_FLOOR_MILLIS`. The sixty is `ADR-0153` §1's promise in the product's voice; an env
variable makes a promise per-deployment and silently unmakeable, and no operator has asked.
**§4 — how the reaper reads it.** `isReapable` gains one conjunct,
`(room.rematchFloorEndsAt == null || now >= room.rematchFloorEndsAt)`, with `null` reading as *no
floor was ever armed* rather than *a floor of zero*. No new locking and no race: `reap` re-checks
under `holder.mutex` and `offerRematch` arms inside `mutate`, which holds the same mutex
(`ADR-0016`), so an offer that wins the lock is visible to the re-check and a reaping that wins
answers the press `UNKNOWN_ROOM` exactly as today. **§5 — the dead-but-unswept room.** The floor arms
**unconditionally**, deadline or no deadline: the promise attaches to the offer the server accepted
and answered `RematchOffered`, not to a room's remaining life the wire never carried. §3's bound is
preserved as §3 states it — `finish + 6:00` plus up to a sweep, exactly as `finish + 5:00` always
meant plus up to a sweep (`ServerConfig.DEFAULT_SWEEP_PERIOD_MILLIS`). **§6 — the test.**
[`ADR-0152`](../docs/adr/ADR-0152-the-finished-rooms-five-minutes-stand.md) §6's characterisation
test is **superseded and replaced, not amended** — it pins the opposite of a merged rule, and such a
test may only be removed. **Measured at `faf14216`: it was never written** — `RoomReapTest` holds
nine cases, none mentioning a rematch — so no file loses a case and the replacement is filed
*instead of*. Six cases: the floor holding past the room's own deadline (both sides of
`offer + 60_000`), the floor never lowering a generous deadline, a repeat press not moving it, a
reload not shortening it, the untouched no-offer case, and a `Room`-level pin that `Offered` carries
`now + 60_000` while the `Agreed` room that follows carries `null`. **Two fixtures, not one** —
`TEST_TIMEOUTS`' `finishedMillis = 4_000` sits *under* the floor, so with it alone the floor always
dominates and *never lowers* is unobservable — and the sixty asserted as a **literal** `60_000`, never
as the constant, since a test that reads the constant follows it wherever it goes.
**§8 — the sentence that replaces `ADR-0152` §4's.** Written into `RoomTimeouts`' KDoc and
`isReapable`'s, because a sentence living only in an ADR is one a reader of the code never meets:
***a room's life is five minutes after the duel, or one minute after a rematch offer, whichever is
later — and one rival press can pull the second, once.*** **Leaves `ADR-0152` §3 standing,
byte-unchanged, and returns the permission it granted unspent:** the write set at `lastActivityAt` is
**still exactly two**, because the floor is a different field — so §3 goes on forbidding a third, with
this ADR as the worked example of not needing one. **Named cost:** the constant **outranks the
operator** — a deployment that sets `ROOM_FINISHED_TIMEOUT_MILLIS` below sixty seconds gets rooms that
outlive its own number, with no validation at boot, no log line, no metric and no knob short of a code
change. Beside it: `isReapable` stops being four lines, which `ADR-0152` called *"the whole virtue"* of
it; a nullable field is an invitation nothing but its name guards; the worst case per room rises a
fifth against a `ConcurrentHashMap` still unbounded and uncounted; the `init` guard takes construction
flexibility away from future callers on purpose; and the replacement test is **a promise on paper until
a planner files it** — nothing goes red if it is forgotten, which is the same failure the superseded
test already suffered. Moves no wire type, no `PROTOCOL_VERSION`, no schema, no stored key, no engine
file and no word on any screen, and registers nothing.
`DEC-166` → [`ADR-0154`](../docs/adr/ADR-0154-the-gone-room-is-the-rematch-surfaces-news.md)
on 2026-09-10 — **the panel's *That duel room is gone.*, and the front door says nothing at all.**
Two licensing sentences, one for each half. That the player reads **one** sentence and not two is
[`docs/vision.md`](../docs/vision.md)'s Positioning — *"Dark, quiet, fast, minimal."* — since two
error lines for one event is neither. That the surviving one is the **panel's** is
[`ADR-0150`](../docs/adr/ADR-0150-back-to-the-lobby-keeps-the-room.md) §2 read literally —
*"Nothing on the front door says a room is held … The only thing the held room may put on a screen
is `ADR-0123`'s panel"* — because a room that is **gone** is news about the room this tab holds; and
`ADR-0044` §6 had already given that frame to the rematch surface (*"the client says so and offers
the way back to the lobby"*). **§1**: on the front door of a tab standing there **under a room it
still holds** — `ADR-0151` §2's `"front-door"` answer — no `UNKNOWN_ROOM` line is rendered at all.
**§2**: every other front door is byte-unchanged, which is the ordinary path — a mistyped code, an
invite link naming a reaped room, and boot's stale-code correction (`ADR-0072` §6, whose failed
rejoin leaves the standing `none`). **§3**: only `UNKNOWN_ROOM` is withheld and only there;
`ROOM_FULL` and the default *The server refused that.* are untouched everywhere, because the panel
states neither and on that screen each can only answer a code typed in the field in front of the
player. **§4**: `RematchNotice` and `RematchControl` are **not edited**, and `Not now` does not give
the front door its sentence back. **§5**: a table of what is on screen for each of the four ways in.
**What decides it**, and it is not taste: the panel's sentence does a second job — it **retires
`Rematch`** (`ADR-0138` §4), which is how `RematchControl`'s *"A button that can only fail is not
offered"* stays true once the room is known to be gone — so silencing the panel would leave a
live-looking button over a dead room, while silencing the front door costs only a sentence.
**Named cost:** a press can now go unanswered — a player holding a finished room who types a room
code naming a room that does not exist reads **nothing**, and each retry spends one of `ADR-0022`'s
ten failed joins a minute; the same silence covers a refusal that lands a round trip after the way
back with no rival's offer standing. Beside it: `Lobby.tsx:494` stops being a function of one field
and now reads which screen the room puts up; `UNKNOWN_ROOM` becomes the one refusal with a
per-screen rule, so the front door's three sentences no longer behave alike; neither sentence is
made **true**, only non-contradictory — the panel's `ROOM_GONE` still fires on a refusal that may
have answered something else; and the result screen and the front door now say different things
about one event by design. No wire, no `PROTOCOL_VERSION`, no schema, no stored key, **no new
string** — one conditional, over a fact `Lobby` already computes for `ADR-0151` §2's cascade, and
one conjunct to reverse. Registers `DEC-168` — **the product owner's** — whether the panel goes on
saying *That duel room is gone.* for a refusal the player brought on themselves, which blocks
nothing.

`DEC-165` → [`ADR-0153`](../docs/adr/ADR-0153-a-rematch-offer-owes-its-recipient-a-minute.md)
on 2026-09-10 — **yes: a rematch offer owes its recipient sixty seconds, as a floor and never as a
ceiling.** Licensed by [`docs/vision.md`](../docs/vision.md)'s first success condition, whose last
beat is the press this decision is about — *"Send a link. She opens it in a browser. We play a full
heads-up match. Someone wins. **We hit Rematch.**"*, followed by *"Everything else is downstream of
that moment."* [`ADR-0152`](../docs/adr/ADR-0152-the-finished-rooms-five-minutes-stand.md) measured the
recipient's window as `finishedMillis - (offer - finish)` — **floor zero** — and named the resulting
control *"that can lie to a player it invited to press it"* as its first cost; a control the product
invites a player to press must not be able to fail before their hand reaches the mouse.
**§1**: a `FINISHED` room on which an offer stands does not end **before sixty seconds have passed
since that offer was recorded** — the window becomes `max(finish + finishedMillis, offer + 60s) -
offer`. **§2**: a floor only, never a ceiling — an offer at `finish + 0:10` still leaves four
minutes fifty, and nothing expires because the minute is up while the room's own deadline is ahead.
**§3**: armed **once**, by the `Offered` transition (`Room.kt:330`) and by nothing else — a repeat
press by the same seat answers `ALREADY_OFFERED` (`Room.kt:315`) and never reaches it, and the other
seat's press **agrees** the rematch and takes the room to `PLAYING` — so a finished room's worst case
moves from `finish + 5:00` to `finish + 6:00`, **once**, and neither player can push it further.
**§4**: **nothing on any screen** — no countdown, no *"you have a minute"*, no change to
`RIVAL_OFFERS`, `REMATCH_LABEL`, `ROOM_GONE` or `Not now`; `ADR-0044` §7 already declined a countdown
by name, one would need a deadline on the wire and a `PROTOCOL_VERSION` bump the floor does not, and
a running clock in a corner panel is neither *quiet* nor *minimal*. **§5**: `RematchNotice` and
`RematchControl` ship exactly as merged — the floor is how `RematchControl`'s own
*"A button that can only fail is not offered"* becomes true of the panel, without the client ever
learning a deadline. **§6**: registers `DEC-167`.
**Why sixty.** It is twice the turn allowance (`RoomTimeouts.DEFAULT_TURN_MILLIS`, `ADR-0108` §1),
and the doubling is the whole argument: thirty seconds is what the product gives a decision the
player is **watching for** — their move, at the table, with the clock visibly running — while
[`ADR-0150`](../docs/adr/ADR-0150-back-to-the-lobby-keeps-the-room.md) deliberately built a
recipient who is reading the ladder or their account when the panel appears in the corner, with no
clock anywhere. It is bounded above by what a stranger's press may cost: one fifth of the room's own
five minutes, pulled once. **Amends** `ADR-0152` §4 — the branch it named as *"exactly where any
minimum answering window would land"* — and §3's closure at two writes, in the one way §3 provides
for. **Named cost:** a rival now holds a lever over a room that is not only theirs, and the product
hands it to them — one press extends a stranger's finished room by up to a minute, spending
`ADR-0152` §4's *"a room's life … independent of what either of them does"*. Beside it: this buys
**sixty seconds of honesty, not honesty** — a player who notices after seventy seconds presses a dead
button exactly as before, and §4 tells them nothing; the room is held sixty seconds for a recipient
who may not be there, because the floor is armed by the offer being *recorded* and not *seen*; a
rematch is slightly likelier to be agreed into a room whose offerer has walked away, which
`foldAbsent` then plays out; the number is derived from a neighbouring number rather than evidence,
because the counter `ADR-0152` named still does not exist; and a finished room now carries **two**
durations where it carried one, a permanent tax on four lines of `isReapable`. Registers `DEC-167` —
**the architect's** — the mechanism, which may not move the wire, add a stored key, change a word on
any screen, make a room's life a function of presence, or break §3's bound.

`DEC-163` → [`ADR-0151`](../docs/adr/ADR-0151-the-press-is-one-fact-and-the-way-back-stops-navigating.md)
on 2026-09-10 — **the press sets one fact above the cascade, and the way back stops navigating.** All
four parts of `ADR-0150` §7, and every one **derived**: nothing here is a product call, and
`ADR-0150` §§1–5 are what a player sees, unchanged. **§1 — the fact.** One `useState<boolean>` in a
new `ResultScreenProvider`, which `App` wraps `<Lobby />` and `<RematchNotice />` in. Neither gains a
prop, so `ADR-0138` §1's *takes no props* survives verbatim; the mount is in `App` and not
`main.tsx`, because `ADR-0138` §8's proofs render `App` and a provider above it would leave those
proofs testing a client nobody runs. It outlives a screen change, dies with the document and mints no
key — `ADR-0150` §5's lifetime, obtained with no storage question — and it is **cleared during the
provider's own render whenever the standing is not `finished`**, which is `ADR-0138` §3's exact
construction applied to the press. The clear is not a nicety: a flag that outlived the result it was
pressed on would swallow **the next duel's verdict**, and an effect would show one painted frame of
the front door before it. **§2 — the cascade.** `Lobby.tsx`'s three store branches are keyed on
`ADR-0114`'s `standing` rather than on `outcome`, `view` and `roomCode`, and one new pure function in
`room-standing.ts` — `finishedRoomScreen(standing, leftTheResult)`, answering `"result"`,
`"front-door"` or `null` — says which of a finished room's two screens shows. It renders identically
for every state that exists today, because `roomStanding` *is* that cascade in that order; what it
buys is the **skip**, which the raw fields can never do — a finished room's `view` and `roomCode` are
both still set (`Lobby.tsx:338` says so), so the obvious `&& !left` on the result branch lands the
player back at the duel table they just finished. That is the whole reason `DEC-163` was a decision.
The function deliberately does **not** take `shown`, which is what preserves `ADR-0114` §2's
fall-through: a player whose provider is absent still lands on the room they hold. **§3 — the
control.** The result screen's way back becomes a `<button type="button">` with the same four words
and the same class list; an anchor would re-boot the tab, the resume would deliver the finished
duel's frames, and the press would undo its own effect. Not an anchor with `preventDefault` either —
the middle-click, the *open in new tab* and the status bar all still promise a document that returns
the player to the result screen. `onLeave` becomes a **required** `onBack` (with a button, an absent
handler is a screen with no way off; and *leave* is the word `ADR-0150` §1 denies), `forgetRoom` is
**not** called, and the handler calls `leave()` **before** recording the fact — because `href="/"`
was quietly doing a second job, stripping an invite link's `?room=ABCDEFGH`, which `main.tsx:176`
reads at every boot and `boot.ts:113` prefers over the room this tab holds. **§4 — the gate.**
`RematchNotice` withholds the panel when `shown === "first"` and `finishedRoomScreen(…) !==
"front-door"` — the front door named **positively** rather than *the result screen is not showing*
negated, because the negation is also true over the **duel table**, and `ADR-0138` §2's *"no frame of
the product ever shows a rematch panel over a duel table"* would then be held by `duel-state.ts:307`
clearing `rematchOffers` on a `Snapshot` instead of by the gate. It stays a second **call** of one
function, never a second rule. **Supersedes three sentences of `ADR-0138`** — §1's *"one import and
one element"* and *"no context is added"*, and §2's `shown !== "first"` gate — **and one of
`ADR-0072` §5**, the anchor, as a statement about the result screen's control only; the waiting
screen keeps its link, its `forgetRoom` and its promise sentence. `roomStanding`, `rulingOn`,
`use-screen.ts`, `screen.ts`, the store, the schema and the wire are **not edited**, and no
client-only value enters `DuelState`. **Costs named:** `App` gains a store-reading provider and stops
being one import and one element; six of the thirteen `getByRole("link", { name: "Back to the lobby" })`
assertions move to `role: "button"` and the waiting screen's other seven must not, on a string that is
identical in both; ~20 `onLeave` call sites become compile errors; the press
leaves no history entry, so the browser's own Back stops returning to the result screen; the
cascade's correctness now rests on `roomStanding`'s internal ordering, which nothing enforces; and
`room-standing.ts` holds its first function whose argument is a press rather than a frame. Of
`ADR-0150`'s four named costs it **resolves** the first — *holds a room, shows the front door*, the
debt `DEC-163` was registered to pay — and **inherits the other three unchanged**: one label with two
behaviours (now sharper, an anchor that forgets beside a button that remembers), a reload undoing the
press, and `Play duel` abandoning a standing offer in silence. Registers **`DEC-166`**, **the product
owner's** — two sentences about one dead room — which **blocks nothing**.
`DEC-164` → [`ADR-0152`](../docs/adr/ADR-0152-the-finished-rooms-five-minutes-stand.md)
on 2026-09-10 — **nothing changes: the finished room's five minutes cover the window `ADR-0150` §1
opens, because §1 opened no new interval.** Measured on `develop` at `d5ef7df8`: `isReapable` reads
`now - lastActivityAt >= timeouts.finishedMillis` (`RoomRegistry.kt:836-844`), and **exactly two**
things write that field on a `FINISHED` room — `Room.finish(now)` (`Room.kt:238`) and
`RoomRegistry.resume`'s `reconnect(seat).touch(now)` (`RoomRegistry.kt:390`), the only `touch(` call
site in the whole server. **A wander is neither, and neither was sitting on the result screen**:
screens are the URL hash (`use-screen.ts:58-75`) over a socket booted once per tab (`ADR-0032`), and
the client's whole message set is `Act | CreateRoom | Hello | JoinRoom | OfferRematch` — no ping, no
keepalive, no *leave*. Nor does a standing offer restamp (`Room.offerRematch`'s `Offered` branch is
`copy(rematchOffers = offers)`, `Room.kt:330`, and `mutate` adds no implicit stamp), nor a socket
dropping (`Room.disconnect` copies the away marks alone), nor the sweep (`giveUpTurn` answers `null`
off `PLAYING`, and nothing in production calls `RoomRegistry.abandon`). So the room's life is
`[finish, finish + F)` before `ADR-0150` and after it; what §1 changed is how much of that interval a
**presser** can reach — from none of it, since `forgetRoom()` left the tab holding no room, to all of
it, which is the window a player who never pressed always had. **§1 spends room life already
allocated and asks for none.** The ADR therefore leaves the rule, `DEFAULT_FINISHED_MILLIS` and
`ROOM_FINISHED_TIMEOUT_MILLIS` exactly as merged and mints **no production change**: **§3** closes
the write set at two so no ticket may add a third; **§4** rules that a standing offer goes on not
restamping and names that branch as where any floor would land; **§6** mints one characterisation
test in `RoomReapTest` — a `FINISHED` room with one seat's offer standing is still reaped at
`finish + finishedMillis` — and nothing else. **Named cost:** the product ships a `Rematch` control
that **can lie to a player it invited to press it** — an offer at `finish + 4:59` leaves under a
second to answer and the press returns `Failure(UNKNOWN_ROOM)` — and this ADR declines to make that
impossible; before `ADR-0150` the presser could not be offered to at all, so `ADR-0150` made the
failure visible and this one leaves it visible. Beside it: the answering window's **floor is zero**;
finished rooms accumulate a little more, since more tabs now hold one and every reload restamps
through `resume` against an unbounded `ConcurrentHashMap` with no cap; and the answer rests on
structure, not evidence, because no counter exists and this ADR does not build one. Registers
**`DEC-165`** — **the product owner's** — whether a rematch offer owes its recipient a minimum time
to answer, the number being a promise rather than a mechanism.

`DEC-162` → [`ADR-0150`](../docs/adr/ADR-0150-back-to-the-lobby-keeps-the-room.md)
on 2026-09-10 — ***Back to the lobby* stops ending the room.** **The human's own call**, stated that
day and recorded rather than derived: a player who presses it keeps their finished room while they
browse the lobby, the leaderboard and their account, so a rival's standing rematch offer reaches them
on any of those screens. It is the reading closest to the human's original sentence, the one
`ADR-0123` quotes — *"it shoud appear on any screen, like if opp come back to lobby it or any other
page in still shoud be shown"* — and the ADR **does not choose it and argues for neither repair**:
`DEC-162` named two, said they are different products, and only the person whose sentence it was
could say which reading stands. **§1 supersedes `ADR-0123` §1's carve-out** on two named sentences —
*"A player who has left the room is not followed, and cannot be"*, and *"`Back to the lobby` calls
`forgetRoom()` and loads `/`; after it this browser holds no room"* **as a statement about the result
screen's control**, which stays true of the waiting screen's. §1's **rule** is untouched and is what
now carries the decision: *"the offer follows a player who is still in the room, and nobody else"* —
and after this ADR the presser **is** still in the room. `ADR-0123` §Context's premise — *"the product
now invites a player to leave the only screen on which a rematch can be seen"* — is **replaced by this
ADR**, which creates the invitation that premise assumed; the invitation was `ADR-0112` §3's honouring
of chosen screens for a `FINISHED` room, and the client offered no way to accept it. **What the press
was ending, precisely**: not the room — `ADR-0072` §4's *"It tells the server nothing"* — but this
**browser's hold** on it, and that is the only thing that stops. The room is still reaped on
`lastActivityAt` idleness (`RoomRegistry.kt:841`). One clause of `ADR-0072` §5 goes with it, the
wiring of `Lobby.tsx`'s `state.outcome !== null` branch to `forgetRoom`; §§1–4 and 6–9 stand and
`forgetRoom` keeps both remaining callers, the waiting screen (`Lobby.tsx:439`) and sign-out
(`sign-out.ts:70`). **§2**: the press shows the **front door** — `Play duel`, the room-code field,
`Your duels`, `Leaderboard`, `Account` — with the room still held, and nothing on it says so: no
banner, no badge, no string (`ADR-0124` §3's precedent, and the vision's *"Dark, quiet, fast,
minimal."*). No control returns to the result screen, and nothing is lost with it, since the panel's
own `Rematch` sends the same frame. **§3**: the panel stands on the front door as well, and
`ADR-0123` §3's rule — *one fact never has two live surfaces at once* — is unchanged; only its test
moves, from `shown !== "first"` (`ADR-0138` §2) to **the result screen not showing**, because those
stopped being the same fact here. **§4** enumerates what still ends a room — the five-minute reaping,
an agreed rematch, starting another duel from the front door (measured: `heldRoom` names only
`WAITING` and `PLAYING`, `RoomRegistry.kt:151-155`, and a `RoomJoined` naming a different room resets
the store, `duel-state.ts:261-282`), and sign-out — and adds none of them. **§5** mints no storage
key, so the press survives a screen change and not a reload, `ADR-0123` §7's own trade applied to the
press. **The cost the human accepted with the call**: rooms now outlive the result screen, so the
three-to-five-minute lifetime has to cover a player who has wandered off rather than one sitting on
the result. Named beside it: the client gains a state it has never had — *holds a room, shows the
front door* — which is `ADR-0124` §2's *"the forget is what keeps the door open"* losing its second
job; one label with two behaviours, since the waiting screen's `Back to the lobby` goes on forgetting;
a reload undoing the press; and `Play duel` abandoning a standing offer in silence. Registers
**`DEC-163`** (the mechanism, which blocks every implementing ticket) and **`DEC-164`** (whether five
minutes still fits), both **the architect's**, and answers neither.

`DEC-161` → [`ADR-0149`](../docs/adr/ADR-0149-minting-is-dispatched-and-a-ladder-value-waits-for-the-human.md)
on 2026-09-10 — **minting is a dispatched ticket, and only a value that joins a ladder waits for the
human.** `ADR-0091` §3's minting branch is **superseded, and that branch alone**; the rest of §3,
`ADR-0091` §§1, 2, 4, 5 and `ADR-0145` §§1–4 stand byte-unchanged, and **no verdict moves** —
`ADR-0024` §3 keeps the human's visual verdict, and what changes is who **drafts** a value, which no
merged source ever assigned to the human. Measured on `develop` at `cee03146`: the four `minting`
tickets are all `review: standard` and all four were dispatched and merged as ordinary PRs, with
`TASK-130601` quoting §3's interactive sentence in its own body and being dispatched anyway;
`ADR-0145` §3's floor has fired **12** times, **9** of them `EPIC-06` tickets under its scoped
deviation and the other three dispatched. **§1: the practice is ratified and nothing is owed as
repair** — the interactive session is **un-mandated, not abolished**, and the human may still work
any card in a session and land it as a ticketed diff. Nothing could execute the old branch: there is
no `await human` state, and `ADR-0091` §2 makes the card the split's **first** ticket, so the stall
is the whole story's — the trade §3 itself refused one sentence later for the verdict. **§2: a
dispatched mint may not add a colour, or a step in a named ladder** (`--pd-fs-*`, `--pd-space-*`,
`--pd-radius-*`, `--pd-weight-*`, `--pd-lh-*`, `--pd-track-*`), because inserting into a ladder
changes what every existing step means while a **new group** falsifies nothing — measured, the eight
tokens minted by dispatch contain **no colour at all**, zero hex literals have entered the sheet
since 2026-08-15, and both refusals on record citing §3 (`ADR-0143`'s hue, `STORY-1309`'s `1rem`,
still standing on three cards with no repair ticket) are ladder insertions. **It blocks nothing**:
the ticket draws at the nearest declared step and says so, as `TASK-130906` did. **§3** mints only
what the same diff draws; **§4** makes every minted value state what it derives from in the sheet;
**§5** gives `lint_tickets.py` one clause — `module: design` plus `minting` ⇒ `review: standard` or
`deep`, firing on four tickets and failing none — and **defers** the Files-keyed clause with a
trigger, since it would fail **nine merged `EPIC-06` tickets** whose review level was right when
they were written. **§6: a story that merges a mint records whether the pane verdict has been
given**, `EPIC-06`'s own acceptance line used four times and by no story since — measured, **15 of
the 21 cards** on `develop` have been created or modified and **no verdict is recorded anywhere**
after 2026-08-15, and the outstanding set is named in the ADR so no closed epic reopens to hold it.
**§7** states what `.claude/agents/planner.md` (lines 159–160, still carrying the superseded
sentence verbatim), `.claude/agents/qa-manager.md` and `lint_tickets.py` owe: one ticket, three
files, no shipped behaviour, due before the next split that mints. Costs named: a drafted value is
now transcribed with no eye on it **by rule**, and `ADR-0145`'s blast radius is the card plus every
client file that copied it; the ladder/group line would not have stopped `--pd-motion-turn-period`,
the product's whole animation grammar; §6 records debt without collecting it; the gate holds the
tier only after a planner has already made the judgment; a **fourth** classification vocabulary; and
nine historical tickets are left unlabelled rather than rewritten.
`DEC-158` → [`ADR-0148`](../docs/adr/ADR-0148-a-module-names-every-card-that-draws-it-and-every-card-is-registered.md)
on 2026-09-10 — **no, the `card text ⇒ module` direction gets no register; `ADR-0142` §7's third
bullet stands, and what gets one is the card side of the pairing.** The trigger fired on a different
wall than §7 was watching. Sorted by hole, the three findings on `design/screens/rematch-states.html`
are **two cardinality failures and one direction failure**: `RIVAL_OFFERS` and `ROOM_GONE` belong to
`result/rematch-text.ts`, which spends `ADR-0142` §4's **one card** on `rematch-panel.html` while the
same five sentences are drawn on `rematch-states.html`. Replayed at `f20d07ed` against the five
values `TASK-141503` later moved out of `RematchControl.tsx` verbatim, **three pass and two fail** —
so the merged forward direction catches **two of the three findings on its own**, aimed at the card
it was not allowed to name. Only `Rematch offered — waiting for ImKate` is §7's, and it is a bare
JSX literal at `RematchControl.tsx:71` that is no module's export. **The cardinality bug is already
producing a passing lie**: `account-text.ts` registers `SIGN_IN_REFUSED` `notCarded` with the reason
*"no frame on this card draws a refused sign-in"* — true of `account.html`, **false** of
`design/screens/sign-in.html`, whose second frame draws *"That handle and password do not match an
account."* and whose own note names the constant. **§1**: no reverse register — priced rather than
feared at **746 text units** across twelve cards (153 on the three paired ones), of which **386 sit
on `duel-table.html` and `duel-table-states.html`, whose modules export 0 string constants**, so for
more than half the corpus there is no export name to point at. **§2 amends `ADR-0142` §4**: a `PAIRS`
row is one **(module, card)** pair and a module may hold as many rows as there are cards drawing its
words — six rows, adding `account-text.ts` × `sign-in.html`, `rematch-text.ts` × `rematch-states.html`
and `rematch-text.ts` × `duel-end.html`. **§3**: a row may give one `notCardedByDefault` reason, and
**every `notCarded` export, listed or defaulted, is asserted absent from that row's card** by
**whole-unit equality** — which is what makes the default safe, and turns thirteen merged reasons
from prose into assertions. The asymmetry with §5c's substring is deliberate and costs nothing:
measured, **all 26 carded values match their card as whole units**. **§4**: coverage becomes total
over **cards** — `design/screens/*.html` is swept, six enter `NO_MODULE`, and an unpaired card may
carry **no** carded value as a whole unit, which forces `rematch-states.html` to be paired with no
judgement at all and finds `duel-end.html` unprompted (measured: it fires exactly once across the
seven, and truly). Inference was refused on measurement — `rematch-panel.html` draws `Account` and
`Sign out` as an **account-screen stand-in** (`ADR-0145` §2 scaffolding), and inferring a pair from it
fails thirteen of `account-text.ts`'s fifteen. **§5**: `RematchControl.tsx:71`'s literal becomes
`OFFER_STANDING`, carded on `rematch-states.html` — **one string, not a rule**, because *product copy
a card draws belongs in a text module* has no gate and nothing reads JSX. **§6 re-arms the trigger
narrower**: a stale sentence on a card a row **already names**, that is no module's export. Costs
named: the register **roughly doubles** — three rows and ~21 lines against a bootstrap of 26 — and
its second half is duller, one default sentence claiming something about twenty-three exports at
once; **N cards means N partitions per module, forever**; the two whole-unit assertions **will**
false-positive on one-word strings (`Rematch`, `Cancel`, `Sign in`, `Account`) as `ADR-0145` §2
scaffolding accretes, and the gate cannot tell a stand-in from the thing it stands in for;
**§5 removes the one live instance of the hole §1 declines to close**, so §6's count restarts at
zero and the evidence is deferred, which is said out loud; six `NO_MODULE` reasons nobody must defend
beyond §4b; and `ADR-0142` gains a second document. **Registers no decision. Blocks nothing** — the
work is one ticket against `web-client/src/design/card-text.test.ts`,
`web-client/src/result/rematch-text.ts` and `web-client/src/result/RematchControl.tsx`.
`DEC-154` → [`ADR-0147`](../docs/adr/ADR-0147-the-held-room-is-abandoned-after-the-seat-under-the-joiners-own-stripe.md)
on 2026-09-10 — **the held room is abandoned after the seat, under the joiner's own stripe.**
`ADR-0141` §8 fixed the product half of the release and refused the mechanism because
`RoomRegistry.mutate` runs its block under the **joined** room's mutex and that block is a plain
function type, so a second room's mutex cannot be acquired inside it (`ADR-0133` §4). **§1: the
release is `Room.abandon` through `mutate`, not a removal from `rooms`.** `Room.join`'s first `when`
branch already answers `UNKNOWN_ROOM` for an `ABANDONED` room *"before anything else, so it is
indistinguishable from a room that never existed"*, `heldRoom` already excludes `ABANDONED`
(`ADR-0133` §1), `resume` already answers `null` for it and `isReapable` already collects it — so
`ADR-0141` §2's observable is **inherited from shipped, tested code** and **no refusal path, no
`ProtocolError` value and no `when` arm is added anywhere**. Removal was rejected on `reap`'s own
KDoc, which says why it sits outside `mutate` — *"teaching one shared critical section two different
shapes of 'done'"* — since a release built on it would be the **third** hand-rolled critical section
in the class, written on the product's most-travelled path. **§2: the transition is
`room.abandon(room.lastActivityAt)`**, the room handing its own stamp back, so `ADR-0141` §8's *"no
`lastActivityAt` is written anywhere but in the room being joined"* is honoured **verbatim rather
than amended** — and because `DEFAULT_FINISHED_MILLIS` (5 min) is under `DEFAULT_WAITING_MILLIS`
(10 min), a released room is reapable **strictly no later** than the same room would have been as
`WAITING`. **§3: the rooms are found by a new private `waitingRoomsHeldBy(player, except)`, never by
`heldRoom`** — the trap the register's wording hides: `HELD_ROOM_ORDER` sorts `PLAYING` **first**, so
by release time `heldRoom(player)` answers with **the room just joined**, and the obvious
implementation abandons the duel the player has just sat down in. It returns **codes, not rooms**
(a lock-free snapshot is not a state to act on), ordered by `RoomCode.value`, and a **list**:
`replyToCreateRoom`'s `PLAYING` fall-through makes two held `WAITING` rooms reachable on `develop`,
and `ADR-0141` §7's *"exactly one live seat afterwards"* is false otherwise. **§4: one new public
method, `joinReleasingHeld`, takes `stripeFor(player)` outermost**, then `join`, then a guarded
private `release` per held room — so `ADR-0133` §4's *"a player stripe is always outermost"* is
answered **yes, a stripe is taken**, at the very call site §4 named by name. **No two room mutexes
are ever held at once**, so no room-ordering rule is introduced and none is needed, and `mutate`'s
block stays non-suspending so §4's structural guarantee is untouched. The guard — still `WAITING`,
still seating this player — sits **inside** the critical section that writes, which is §8's
*"conditions … at the moment of the write"* made structural; the unconditional public
`abandon(code)` is deliberately not reused, since a `PLAYING` room reaching it would end a duel.
**§6: `replyToJoinRoom` changes by exactly one identifier**, `join` → `joinReleasingHeld`, with **no
new branch, no second write and no new failure mode**; `resume` and `ALREADY_SEATED` are untouched
(`ADR-0141` §4's *"the release follows sitting down, never trying to"*), and the write lands in the
registry rather than the socket because `DuelSocket` cannot take a private `RoomRegistry` stripe.
**§7 hands the planner ten test rows**, each naming the `ADR-0141` clause it defends, and requires
**two codes and two players** so `except` cannot pass as a constant. **Registers no decision**, and
**unblocks the ticket that implements `ADR-0141`** — never written, and now writable. Costs named:
**the joiner's first frame now waits on a stranger's lock**, since `RoomJoined` goes out only after
every held room's mutex has been taken and released, so a rival joining the room being released
delays this player's table by that rival's entire `join`; a player stripe is now held **across
suspension**, so `heldOrOpen`'s second KDoc property has to be narrowed to `heldOrOpen` itself;
`RoomState.ABANDONED` stops meaning *a duel ran here*; `abandon(room.lastActivityAt)` passes a
non-`now` value into a parameter documented as `now`, guarded only by a comment; the two-rooms-live
window is **narrowed, not closed**, and still unmeasured; a `WAITING` and a `PLAYING` room can still
be held at once, which is `ADR-0105` §1's decided-and-unbuilt ticket and **nothing fails if it is
never written**; and `RoomRegistry` gains its first method that touches two rooms.

`DEC-157` → [`ADR-0146`](../docs/adr/ADR-0146-a-handle-is-held-not-spent.md)
on 2026-09-10 — **no: *give this profile a password* owes no irreversibility sentence, because the
act takes nothing from the player, and a handle is *held*, not *spent*.** Registered by `ADR-0143`
§4, which found the gap by measuring rather than by being asked and deliberately declined to close
it. **Derived from the vision** — *Positioning*'s *"The reference points are **Lichess** and
**Chess.com**, not PokerStars. **Dark, quiet, fast, minimal.**"*, read as *a quiet screen does not
warn about a cost it does not impose* — with most of the work done by merged ADRs: `ADR-0030`
(*a claim adds a credential and moves nothing*), `ADR-0039`, `ADR-0031` §1 and `ADR-0130` §2
between them already settle what signing up costs, which is nothing. **§1: no sentence is added to
the sign-up form, `SignUpForm` gets no second press, and no copy anywhere asserts that a handle is
permanent or that it is now unavailable to anybody else.** **§2 applies `ADR-0143` §2 rather than
narrowing it, and writes down the test its §4 table left implicit**: an act owes those obligations
when **a player who performs it ends up with less than they had and cannot get it back**, never
because the row it writes cannot be deleted — under `ADR-0039` almost nothing in this product can
be deleted, and a rule firing on permanence alone would put a warning under every control on every
screen, which is how the three sentences that already ship would stop being read. **§3 answers the
register's second half — *held*.** `ADR-0130` §2's word describes a string that has **left** its
owner (*"Nobody else may take it, ever"*); a handle has left nobody and goes on naming the player
who chose it for as long as the row exists, so the two words are opposite states of the same unique
index. §2's derivation cannot transfer either, because it is entirely about visibility: measured,
`ProfileResponse` carries **seven** fields and **no handle**, and `ADR-0031` §1 reads *"a handle is
**never shown to anybody**"* and *"The handle appears in no response body and in no `ServerMessage`,
ever"* — so a reissued handle would print a second player onto **no** row. The stranger who can no
longer claim the string **is not owed a sentence on somebody else's form**, and is the one subject
this product refuses to discuss (`ADR-0029` §5's refused availability endpoint; the sign-up `409`
conflating *the handle is taken* with *this player already holds a password*, on `ADR-0027` §6).
**The decisive fact is a merged refusal to decide**: `ADR-0031` §1's *"Nothing here changes a
handle. No endpoint updates `credential.identifier`; the row is written once, at sign-up. **That is
not foreclosed** — an `UPDATE` under the same unique constraint would do it — but it is not built,
and no story should assume it."* So *you cannot change this later* would settle handle permanence
in shipped copy rather than in an ADR — the exact move `DEC-157` was registered to prevent — one
document after `ADR-0130` §5 paid to delete the identical sentence for names (`PERMANENCE_LINE`,
measured **0** occurrences in `web-client/src` today, and since `ADR-0142` §5 a shipped string is a
register entry too). **§4 completes `ADR-0143` §4's table** — the fourth row is *nothing to say*,
the first three are untouched — and measures the screen both ways: `ANONYMOUS_WAY_OUT` is **already
the last line before the panel** in the state `account.html`'s *No password yet* frame draws, while
`AccountScreen.tsx:106`'s `!signedIn` gate against `:133`'s `!hasPassword` gate leaves one reachable
state in which the form renders with none of the three anonymous sentences above it and can only
ever answer the `409`. **§5: the card owes one margin sentence and no change to any frame**, and the
honest note is that a margin sentence is **not a gate**. **§6** leaves *can a handle ever be
changed* exactly where `ADR-0031` §1 left it — open in both directions — and names, **deliberately
without registering it**, whether the account screen should ever show a player their own handle;
the render-state question is named as adjacent to `DEC-090` and not answered. **§7:** reversed by a
merged ADR **deciding** a handle is permanent, by one player asking to change theirs or asking what
theirs is, or by the human in one sentence; another agent re-reading `ADR-0143` §4's table is
explicitly not evidence. **Supersedes nothing**, and **corrects one word** — *spends* in `ADR-0143`
§Consequences 5 — in a clause that decided nothing. **Changes no wire, no server file, no engine
file, no migration, no string, no token and no control.** Costs named: **a mistyped handle is still
forever and nothing says so while it can still be avoided** (`ADR-0029`'s own *"A typo is forever…
a player named `Bobb` who meant `Bob`"*, removed for names by `ADR-0130` and left standing here);
a player with no recovery address who forgets their handle has no path back and is not told to
remember the one string the product will never show them again; **the answer is conditional on a
decision nobody has made, and no gate, test or trigger detects the condition changing**; and the gap
is closed by ruling rather than by shipping, so a reader comparing the four acts still sees one form
saying less than the other three.
`DEC-160` → [`ADR-0145`](../docs/adr/ADR-0145-a-cards-scaffolding-is-composing-and-the-transcriber-names-the-mint.md)
on 2026-09-09 — **a design card's own scaffolding is composing, and the criterion is the
transcriber, never the level of detail.** Raised while landing `TASK-141502`, whose `light` review
returned `fail` on the classification rather than on the artifact. **§1 settles the reading the
driver could not**: `ADR-0091` §3 splits on what a card **creates for the product**, and has never
been about what the human has not yet seen — §3's own *"**Either way** the verdict that matters
stays the human's visual one … and it may **trail the merge**"* puts the human's sight downstream of
**both** branches, so the not-yet-seen reading empties the composing branch and contradicts the
paragraph it sits in; `ADR-0091`'s first named cost says the same from the other side, that a merged
card ticket *"reads as* the look was approved *when it only means* the look was recorded and is
structurally sound."* **§2: a card holds two kinds of drawing** — **subject**, which a coder
transcribes into `web-client/`, and **scaffolding** (`.wrap`, `.eyebrow`, `.lede`, `.frames`,
`.frame`, `.note`, `.stub`, `.screen`), which nothing transcribes — and the split-time test is
**name the file that will transcribe it**. If a client file will, the ticket mints that drawing; if
the answer is *nothing — it exists so the subject reads*, it is scaffolding and **composing at any
level of detail**. That is `ADR-0033` §1's consumer test (*"The test is consumers, not units"*)
lifted from values to drawings, with §3's *promotion is one shared consumer away* carried over
intact, and it has a **direction**: a card copying **from** shipped code mints nothing. **§3's
mechanical floor** is three clauses read from the ticket's `## Files` table plus `ls`, on
`ADR-0091` §2's floor-plus-judgment pattern: writing `design/tokens/tokens.css`; creating a file
under `design/components/` or `design/graphics/`; or scaffolding that depicts a **named product
screen with no card under `design/screens/`**. The third is what actually decides `TASK-141502` —
`leaderboard.html` and `account.html` both exist, so the stand-in cannot claim an authority already
occupied, and had either card been missing the same drawing would have been **minting**. **The axis
both parties argued — specificity — is explicitly rejected**, because *abstract enough* cannot be
judged from a ticket, which moves the argument to the merge where it just cost a round trip; the
reviewer's instinct is answered rather than overruled, since detail is dangerous exactly where
authority is vacant. Measured on `develop` at `702408ed`: `.wrap` in **20 of 21** cards, `1020px` in
**19**, `.frame` and `.note` in **all 12** screen cards, and **406 raw `px` lengths outside
`:root`** — so scaffolding-is-minting reclassifies all **86** `module: design` tickets, including
the 43 the human has already accepted. **§4: `TASK-141501` and `TASK-141502` both stand and nothing
is owed** — `.panel` is subject and was labelled `minting` correctly; the driver's conclusion is
upheld and its **stated ground corrected**, since four raw lengths landed rather than two and
`ADR-0103` §1 fixes 390 × 664 and **720 × 900**, leaving `960px` and `420px` with no external
source. The `fail` was a finding against the **split**, not the diff, and a `DEC` is where such a
finding goes. **`TASK-141511` is composing**, tier and dispatch unchanged: it writes no `tokens.css`,
creates no canonical file, depicts no screen, and copies *from* `RematchControl.tsx`. **§5 does not
settle what minting costs** and registers **`DEC-161`** instead. **§6** records that `ADR-0091` §2's
rule was **never written into `.claude/agents/planner.md`**, the one file §2 names, and states what
that file owes. Costs named: the floor misses `.panel`, the very case that produced this story's
minting label; licensed scaffolding accretes with **no gate comparing it to the canonical it
depicts**; scaffolding is licensed to reproduce shipped strings and `ADR-0142` §4's one-module-one-card
pairing is what contains that, not this rule; *nothing will transcribe this* is a prediction nothing
detects; a **third** classification vocabulary; and `DEC-161` blocks nothing.
`DEC-159` → [`ADR-0144`](../docs/adr/ADR-0144-a-negative-route-test-is-a-control-and-the-predicate-resolves-the-device-once.md)
on 2026-09-09 — **none of the three merged sources yields, because they were never jointly
unsatisfiable.** The prohibition was never in an ADR: `ADR-0049` §5 specifies `DELETE
/api/me/device` and **contains no sentence constraining any other method on that path** — the
only occurrence of the word *verb* in it is *"The `auth` family is verbs about a session —
sign-up, sign-in, sign-out — and the point of this decision is that revocation is not one"* —
so *"only DELETE was asked for"* is the **test comment's** summary of what an ADR had asked for
in 2026-08, turned into *"only DELETE may ever be installed"*, which nothing decided. **§1
withdraws the citation** and supersedes, amends and narrows no clause of `ADR-0049`: a route is
forbidden by an ADR saying so and by nothing else. **§2 states the rule** — a negative route
assertion is a **control for a positive row in the same file** and must name a verb no merged
ADR has asked for; it is never a prohibition, and when a later ADR asks for the verb it names,
the control **moves**. So `theDeviceRouteIsNotInstalledOnAnyOtherVerb` becomes
`…OnUnaskedVerbs` and issues `client.put`, keeping **both** inequalities (Ktor answers `404` or
`405` and the assertion is agnostic on purpose) and keeping `TASK-040611`'s own reason for it,
*"a `get(...)` typed beside the `delete(...)` would otherwise be invisible"*. `PUT` because `PUT
/api/me/name` is live under the same prefix, which makes the control **more** discriminating
than the `GET` it replaces, not less. **§3: `ADR-0068` does not yield either — it already
decides this, and it says *split*.** §4's *"a scope that grew after the ticket was written… it
is still a split, still today"* is this case exactly, so `ADR-0135` §8's *not `atomic:`*,
`ADR-0068`'s cap and `MAX_FILES_TOUCHED = 3` **agree** rather than collide, and **no linter
constant moves**; §1's *"A budgeting rule never rewrites a correctness gate"* is met, because
the control is re-pointed for a changed **premise** and not to fit a budget. **§4: the edit
lands before the route**, which is the only direction in which every assertion in the
intermediate commit is true when it lands — a row claiming the `GET` **is** installed could not
be — and the `GET` row has been stale since `ADR-0135` merged on **2026-09-07**, not since
`TASK-141003`. **No replacement positive row** is added, and the argument is given rather than
the omission left silent. **§5 corrects `ADR-0135` §4, which was wrong, and says so.**
`namesPlayer` answers one boolean; §5's table needs *resolves to nobody* and *resolves to
somebody else* to **differ**; **no sign repairs it** — `namesPlayer` inverts rows three and
four, `!namesPlayer` inverts rows one and two. The function is **deleted** and the predicate
comes from the shipped `resolve(token = null, deviceId)`, whose `Identity.Device`,
`UnknownDevice` and `Anonymous` are §5's rows one-and-two, four and three: **no new public
surface**, `IdentityResolver` back to one public suspend function, and the two impossible
branches answering `false` because `ADR-0135` §6 makes every unknown a keep. §4's **prose**
predicate, §5's table, §6 and §8 all stand. **Ticket edits registered**: new **`TASK-141015`**
(three files, ordered before `TASK-141003`), and `TASK-141003` stays at **three** files and a
plain ticket, its `!namesPlayer` expression replaced by §5's `when`. Costs named: a ticket on
the critical path of eleven that ships no behaviour; four merged unit tests deleted, their
properties surviving one level further out; **`TASK-141002` becomes a done ticket whose
`verify:` block no longer passes** — written into its successor's Notes rather than left to be
found; one merge with no `GET` assertion and no composition-level one ever; a second `resolve`
call per request; and an ADR corrected by the story implementing it, for the second time in
this epic. **Registers no decision.** The recurrence is named as a **trigger**, honestly:
**nothing detects an ADR asking for a verb a merged control denies** —
`HttpEndpointDocumentationTest` reads the document against the DTOs and never enumerates
installed routes — so what changes is the **cost**, from a contradiction needing an ADR to a
one-line move with a named shape. **The second occurrence, on this path or any other, makes the
verb list a mechanism decision**, on `ADR-0142` §7's precedent, where a named trigger fired and
became `DEC-158`.
`DEC-156` → [`ADR-0143`](../docs/adr/ADR-0143-irreversibility-is-said-last-and-never-coloured.md)
on 2026-09-09 — **yes, an irreversible act gets weight, and the weight is words, position and
a press, never a treatment.** Raised while landing `TASK-140909` by the coder that had just
drawn the name form: four panels drawn alike, and only one of them spends something forever.
**Derived from the vision** — *Positioning*'s *"The reference points are **Lichess** and
**Chess.com**, not PokerStars. **Dark, quiet, fast, minimal.**"* — after correcting the
premise by measurement: the account surface carries **three** irreversible acts, not one. The
name is one; `REVOKE_PERMANENT` is *"This device will never sign in to this account again.
**This cannot be undone.**"*; `SIGN_OUT_WARNING` says a duel left this way can be lost. The
product's shipped grammar for the other two is `RevokeControl`'s and `SignOutControl`'s
**in-page second press, in plain type** — *"never a native dialog… one shape for both
confirmations on this screen"* — so this question had an answer in the code before it had one
in an ADR. **§1: no danger register.** No accent, tint, border, box, rule, icon, badge, size or
weight step, capitalisation or motion marks an act as irreversible, on any screen, and none in
the assistive register either — **no `role="alert"`**, no `aria-live` urgency, no hidden
*Warning:* prefix. Measured, non-test files only: colour in this client names **what a thing
is** — a result (`text-win`/`text-loss`), a clock running out (`text-warn`), whose turn it is,
your own ranked row, a standing rematch offer, a control — and has **never** named a
consequence; all **eight** `role="status"` refusal sentences across seven components are
plain, seven `text-small` and one unclassed. `design/tokens/tokens.css` reserves the only
urgency hue in its own comment — *"Amber exists only for the turn clock running down."* — so a
treatment would need a **new** hue, and minting is worked with the human (`ADR-0091` §3), not
chosen in an ADR. **§2: three obligations**, binding a screen of controls and stopping at the
table's edge — the act says it cannot be undone; that claim is the **last thing read before
the control**, with no other sentence between them; and a **second press wherever no merged
ADR has refused one**. **§3:** the name form takes the first two and **cannot** take the
third — `ADR-0130` §1's *"no confirmation step"* and §7's *"no confirmation press"* are
**applied, not reopened** — which leaves the display name the one irreversible act in this
product whose entire guard is a sentence, said plainly rather than smoothed over. **§4** tables
every irreversible act and finds one unguarded: *give this profile a password* claims a handle
nothing gives back and says nothing at all, **registered as `DEC-157`** rather than answered,
because a second answer would have been two decisions in one run. **§5:** `TASK-140909`'s two
name-form frames **already conform** — the *gone for good* line is last, with only the field
between it and the button — so the card owes **one margin sentence** recording that the
flatness is the decision, and **no frame changes**. **No gate is bought**, and `ADR-0126`
§Consequences 5's price is paid again knowingly: *"A refusal that leaves nothing behind is
undone by the next coder with a spare CSS class."* **§7** states what reverses it: **one player
asking for a name back** — which needs no instrument, because `ADR-0130` §2 leaves no mechanism
that could serve them — or the human at the pane in one sentence. Another agent re-reading the
card is explicitly **not** evidence. It **supersedes nothing** and narrows `ADR-0130` §5's
grant of layout to the card **by exactly one relation**. Costs named: the skimmer is not
helped and §7's trigger fires **after** somebody has lost a name, so the price is denominated
in one player's name; the least-guarded act carries the largest permanent loss; the refusal has
no gate behind it; and a product-wide rule rests on a base of three acts.

`DEC-155` → [`ADR-0142`](../docs/adr/ADR-0142-a-text-module-is-checked-against-the-rendered-card.md)
on 2026-09-08 — **a text module is checked against the rendered card, by a test in the client's own
suite.** It **amends nothing**: it is `ADR-0091` §4's own sentence — *"Adoption is gated where
consumption happens — the client's own CI job"* — carried out on a second fact, so the gate is
`web-client/src/design/card-text.test.ts` reading each card with `readFileSync` off
`import.meta.url`, exactly as `styles/tokens.test.ts` already reads `design/tokens/tokens.css`.
**No workflow line and no dependency**: `vitest run` globs it, and `ADR-0088` §1 forbids the
second. The placement is evidence-led — **`design/check-frame-cards.sh` runs in no workflow at
all**, three ticket `verify:` blocks and zero CI, so *a gate only a ticket runs* has already
happened here once, to a design gate, unnoticed. **§2 reads the card's rendered text**, comments
stripped **first** so a marker can never satisfy the gate, splitting on a **NUL** sentinel and not a
newline — measured: a newline separator also splits the card's own line wraps, which cut
`account.html`'s three `<p class="line">` sentences in half and made all three read as absent from
the card that plainly carries them. **§3 deletes** `account.html`'s three `<!-- ANON-BLOCK: … -->`
markers instead of extending them to every card: nothing ever compared a marker to the `<p>` beside
it (`TASK-140810`'s gate counts three), so building on them would certify code against an unchecked
duplicate while the render the human accepted drifts — and normalisation dissolves the line-wrapping
problem they were invented for. **§4 pairs a module to a card, not to a frame** — `name-ask.html`'s
four frames carry different subsets of the six strings, the fourth carrying two, so frame scope
needs a per-string frame register. **§5 checks the partition both ways**: each pair declares
`carded` and `notCarded` export names, an unclassified new export fails and a register entry naming
a deleted export fails, and each carded string is matched by its **imported value**, never a `grep`
of source — `SIGN_OUT_WARNING` and `SIGN_UP_THROTTLED` are hand-concatenated and occur **0** times
in their own file as a literal, so a grep gate is blind to the two longest sentences there.
**§6 globs `*-text.ts`** so a new module cannot escape the register. **§7 states the direction
out loud**: names both ways — which `HttpEndpointDocumentationTest` gives **no** DTO — values one
way, and **card ⇒ module deliberately unenforced**, because a card is mostly headings, `<p
class="note">` commentary and placeholder data, with the trigger to close it named. Costs named:
**~26 bootstrap register lines** a reviewer must actually read (15 `notCarded` for `account-text.ts`
— 27 exports, 12 on its card — and 11 `NO_CARD` modules), one classification per future export
forever, **no carded sentence may span an inline element**, and a green gate over a card nobody has
looked at proves only that two copies agree. Forecloses marker comments as a card convention, which
is additive to reopen. Honest about its own yield: it would have been **green throughout** at
`TASK-140710`'s merge — all six strings matched the card then and match now — so it fixes no live
defect and its whole value is against the next edit.

`DEC-145` → [`ADR-0141`](../docs/adr/ADR-0141-taking-a-seat-elsewhere-releases-the-room-you-were-holding.md)
on 2026-09-08 — **the seat elsewhere is taken, and the room they were holding is released in the
same act.** `ADR-0124` §7's *"last route by which one player can end up in two duels at once"*,
answered from `docs/vision.md` in **two halves from two sentences**. **The seat** is licensed by the
first success condition — *"Send a link. She opens it in a browser. We play a full heads-up match.
Someone wins. We hit Rematch."*, with *"Everything else is downstream of that moment."* — the
sentence `ADR-0094` §1 already read to ship this exact path, so the invite gains **no second
qualification** and refusing it, being more friction than the confirming click `ADR-0094` refused,
is out. **The room** is licensed by *"One duel coin per win… A counter of duels won."* with *"Two
people, **one link**, one heads-up poker match"* and *"Not a multi-table poker room."* — `ADR-0124`'s
own three sentences for `DEC-111`, applied to the state it left open: a room that can settle a coin
for a duel its holder never saw is that counter counting a duel nobody played. **§2: a released room
is indistinguishable from a reaped one** — the code answers `UNKNOWN_ROOM`, which `ADR-0022` §3
already makes *"indistinguishable from a room that never existed"*, and `Lobby.tsx:436` prints the
string it already prints, so **no new string, no wire, no `PROTOCOL_VERSION` step, no client file
and nothing stored**; the whole footprint is one server write on one path. **The trigger is taking a
seat, not leaving one**: the room is released because its holder has sat down in another duel, which
is the one act after which *"and it brings you back"* is something the product will not do for them.
Bounded on three sides — **§3** releases only a room still `WAITING` and holding nobody else, so a
duel already running in it is left alone; **§4** releases nothing on a join that seats nobody, so a
mistyped or dead code costs the player their room not at all; and **§5**, the larger half, releases
nothing when the holder merely left, dropped, closed the tab or pressed create again — **a duel can
still begin into a seat that is already away**, and that stays `ADR-0073` §3's promise working as
intended. **§6: nothing is said, to anyone**, on *"Dark, quiet, fast, minimal."* and because the only
surface the player is on at that instant is a dealt table with a decision due, which `ADR-0105` §4
forbids a notice to take away. **Measured, not assumed** (`develop` at `7016b5b4`): the rival who
clicks the abandoned code today gets **no presence frame at all** at the join — `withFreshRunner`
builds none, and the host's `WAITING`-room drop built none because there was no guest — then up to
**3 min 30 s** of `turnMillis` plus `timebankMillis` counting down against a table that has been told
nothing, then `giveUpTurn` latching the seat into `absentSeats` (*timed out*, `ADR-0046`), then
instant folds to a coin. **Supersedes two sentences of `ADR-0073`** — the header's *"which gives a
`WAITING` room one way to end, an idle timeout, and no other"* and §3's *"the room ends the one way
`ADR-0022` gives it — the idle timeout"*, both **only** as statements about a `WAITING` room in
general; `ADR-0022`'s own Decision is untouched. **Amends `ADR-0105` §2's `WAITING` row on its
`JoinRoom` half only** — still **not refused**, the reason still holds, and *as today* is what
changes. **Registers `DEC-154`**, the architect's, for the release's call site and lock order —
**answered on 2026-09-10** by
[`ADR-0147`](../docs/adr/ADR-0147-the-held-room-is-abandoned-after-the-seat-under-the-joiners-own-stripe.md),
above. Costs
named: **a rival sent a live code can meet a dead one, and it is the product that killed it** —
`ADR-0124`'s Alternatives rejected that outcome on its own route, and the difference is that here
the rival's only alternative is a duel against a seat nobody occupies ending in a coin; a shipped
on-screen line can be made false by its reader's own next act; **the first act in the product that
ends a room ahead of its clock**; **two duels at once is narrowed to a window, not closed**; the
player is not told; and `replyToJoinRoom` grows a write on its success path.

`DEC-150` → [`ADR-0140`](../docs/adr/ADR-0140-the-clipboard-api-is-the-only-copy-this-client-attempts.md)
on 2026-09-08 — **the Clipboard API is the only copy this client attempts, because it is the
only one that reports its result.** The answer to *by what mechanism may the client copy where `navigator.clipboard` is undefined* is **none**: `navigator.clipboard.writeText` stays the only clipboard mechanism, and where the API is absent the press performs `ADR-0128` §3's hand-over — focus the invite-link box, select the whole link, render `Copy it from the box above.` — and attempts nothing else. **The reason is the missing read-back, not the deprecation.** `document.execCommand("copy")`'s boolean reports that the command was supported and enabled for that invocation, not that the system clipboard now holds the link; the one in-page read-back, `navigator.clipboard.readText`, lives on the same `[SecureContext]` interface that is undefined by hypothesis — so it is unavailable precisely and only where it would be needed, and a running client cannot tell a browser whose `true` is truthful from one whose is not. `ADR-0128` §4 — *"never of an attempt whose success the client cannot read"* — is therefore unmeetable by any available mechanism, and `Link copied.` stays gated on a resolved `writeText`. **The silent copy is the option that nearly won**, and it is refused on **§3 rather than §4**: attempting a copy and claiming nothing over-claims nothing, but §3's promise is *"one press leaves the link either in your clipboard or selected under your cursor, and the screen says which"*, and a silent success prints `Copy it from the box above.` on exactly the devices where the mechanism fires — so the screen says the wrong one. Making it honest needs a third outcome, a third outcome needs a string, and `ADR-0110` §6 routes that to the **product owner**, not to an architect and not to a coder. **Measured this run rather than assumed**: under the pinned jsdom 24.1.3 `document.execCommand` is **absent** (`"execCommand" in document` is `false`), as are `queryCommandSupported` and `queryCommandEnabled`, so a test of an `execCommand` mechanism could only install a fake of a function jsdom does not have and assert the code called it; and `ADR-0117` §1 puts the proofs of record on `http://localhost:4173`, a secure origin where the no-clipboard branch is unreachable. Before any evidence could exist, two things would have to change, and §6 names them: the bundle served to the drive on a **non-secure** origin (the host's LAN address, moving `scripts/qa/stack.sh` and the origin `ADR-0117` §1 fixed), and a **read-back verb** in `scripts/qa/drive.mjs` that pastes from the browser side, since `readText` does not exist there. **No file changes and no ticket is owed** — `web-client/src/table/InvitePanel.tsx` already behaves exactly this way, and the negative is pinned three times already, including `Lobby.test.tsx`'s seven-string enumeration of the host-alone table. Costs named, the honest one first: **nobody measured whether `execCommand` actually works on the author's two devices**, so a press that would probably have delivered the link is refused on an argument about knowledge rather than on evidence; `ADR-0128` §3's *"one gesture, not zero"* stops being provisional, and the iPhone in `EPIC-14`'s own capture table will not see `Link copied.` until `DEC-126` or a product answer; the rule shows up in the client as an absence no grep finds; and reversal now costs an instrument before it costs code. **Supersedes nothing**, registers no decision, unblocks no ticket, and leaves `DEC-126` untouched in both directions.

`DEC-153` → [`ADR-0139`](../docs/adr/ADR-0139-the-read-beat-is-spent-once-and-the-store-already-remembers.md)
on 2026-09-07 — **the read beat is spent once per hand, and the store already remembers.**
`ADR-0120` §3's rule is *"a hand face up the viewer **has not been shown before**"*; the sentence restating it *"as the client can evaluate it"* drops the word **before**, and `ADR-0136` §1 shipped the restatement while calling it *"`ADR-0120` §3's sentence and not a paraphrase of it"* — a claim this ADR **withdraws**. `layOutReveal` gains a third parameter `held: PlayerView | null`, the `Snapshot` case passes `state.view` (still the *previous* view on that path), and the final step answers `"read"` only when the rival's cards are **newly** shown: `rivalCardsShown(view) && !(held.handNumber === view.handNumber && rivalCardsShown(held))`. **Nothing new is remembered** — `DuelState` gains no field, `state.view` was already there, is written only by the `Snapshot` case, is cleared by nothing, and holds a `COMPLETE` view only by having **painted** it, since a frame arriving while a reveal stands is queued rather than applied. So `ADR-0136` §1 is amended **in three sentences and in those three only**, and all three of its stated reasons survive: the guard sits in the reducer precisely because `advanceReveal` folds queued frames back through `applyServerMessage`, which a guard in `duel-store.ts` would miss, and the predicate still reads `view.viewerSeat` rather than `state.mySeat`. Two ways it could be wrong are closed by merged facts rather than by care — `HandRevealed` fires only in `reachShowdownAndSettle`, which settles in the same engine step, so rival cards cannot appear before `COMPLETE`; and `held.handNumber === view.handNumber` is what keeps the **next** hand a read. **The symptom is measured unreachable**, which is why this was a question and not a repair: **0** hand-completing `Snapshot`s in **2,220** resume frames over 60 duels, against **602** from the same probe asked what a runner would hand back if `act` did not call `advance`. The second answer — *leave the predicate and write the invariant down* — was real, and lost on where its pin would live: `EPIC-14` forbids `poker-server`, and a test deferred to a story outside the epic is a test nobody writes. The invariant is written down anyway, as a **fact with no test**, because after the guard nothing depends on it. Costs named rather than argued away: `layOutReveal` is no longer a pure function of one frame; a one-day-old ADR is amended by the story implementing it; **the change is unfalsifiable in the product**, since a symptom no path reaches is one no path disproves; a reload still forgets, so *before* means *in this store's lifetime*; and the repository now carries a client guard against a server behaviour that does not occur. **Registers no decision.** Unblocks `TASK-141108`, which stands **exactly as written** — two files, `92 → 95`, both mutations, every literal reproduced

`DEC-144` → [`ADR-0138`](../docs/adr/ADR-0138-the-panel-mounts-beside-the-lobby-and-the-dismissal-lives-in-the-mount.md)
on 2026-09-07 — **the panel mounts beside the lobby, and the dismissal lives in the
mount.** A props-less `RematchNotice` is mounted by `App` as the **last child of `<main>`**, beside `<Lobby />` — so `Lobby.tsx` is not edited, the panel's gate is a **second call** of `ADR-0114`'s `shown`/`ruling` **with no effect of any kind**, and the dismissal is one `useState` boolean whose lifetime is the mount. The mount point is decided by `ADR-0123` §7's dismissal lifetime: state inside the cascade dies on one press of *Back*, so the surface must sit above the thing that changes. Three of §2's prohibitions then hold **structurally** — outside every `<form>` so no `Enter` reaches it, no timer because it has no effect, and out of flow so it cannot move the screen underneath. It reads `roomStanding`/`rulingOn` and writes nothing, so it cannot race the layout effect, and the agreeing `Snapshot` empties it in the same commit that seats the table. The dismissal is cleared **during render** when the offer ends — without that clear, one *Not now* would hide every later offer for the life of the tab. Two components sharing a new `rematch-text.ts`, with *Not now* **copied rather than imported** because `ADR-0125` deletes `account-offer-text.ts`. `role="status"`, never `alert`/`dialog`/`alertdialog`, no focus call, no focus trap, no key handler. Nothing crosses the socket, so `STORY-1415`'s implementing ticket is **not** `atomic:` — it still waits on its minting card (`ADR-0123` §8). One inexactness named rather than registered: `ADR-0123` §3 equates *shown* with *rendered*, and `ADR-0114` §2 keeps a fall-through; in the shipped client they coincide, and §9 fences it on `ADR-0105` §6's route

`DEC-142` → [`ADR-0137`](../docs/adr/ADR-0137-a-name-suggestion-is-drawn-in-the-browser.md)
on 2026-09-07 — **a name suggestion is drawn in the browser, and the vocabulary's size is what
makes a refusal rare.** The mechanism `ADR-0119` §4 registered when it fixed what a suggestion *is*
and chose no generator. **The forces:** `ADR-0119` §4 asked for a suggestion a player can normally
accept first try, and the same paragraph refused every instrument that could guarantee it —
`ADR-0029` §5 refuses an availability endpoint, `ADR-0051` §9 refuses an enumeration surface, and
`ADR-0119` §5 forbids pre-registering, because `ADR-0051` §1 lets nothing out of `name_registry` and
§9 builds no un-retire — so **rarity cannot be produced by a check** and has to come from the size
of the set drawn from; meanwhile a suggestion behind a request can fail to arrive on the one path
`ADR-0119` §1 will not let stall, between a press and the room, where the only answers to *the
endpoint is down* are the empty field `ADR-0119` rejected or a local fallback that is this decision
plus a request. **§1: client-side, and nothing crosses the wire** — no socket frame, no HTTP route,
no `docs/protocol.md` edit, no Kotlin file, no migration; `ADR-0047` §2's fingerprint cannot move,
**`STORY-1407`'s tickets are not `atomic:`** under `ADR-0068` §3 and take no branch lock, and a
suggestion **cannot fail to arrive**, so the ask has no loading state and no empty-field window.
**§2: `suggestName(random, replacing?)` is a pure function of a random source** — it takes no
`PlayerProfile`, device id, session, room code, clock or `Storage`, so `ADR-0029` §6's refusal of a
minted `Player-3F2A` and `ADR-0067`'s *no id turns into a profile* hold **by the signature** rather
than by care; `random` returns `[0, 1)` and its default `Math.random` lives at the call site, which
is `reconnecting.ts`'s shipped `jitter` idiom; the index is clamped (`Math.min(len - 1, …)`) so a
source returning `1` cannot join `undefined` into a **permanent** name; `replacing` redraws at most
four times so a stuck source cannot spin; and the draw is **deliberately not cryptographic** —
`RandomRoomCodeSource` needs `SecureRandom` because a room code *is* an invite, while a display
name is half of nothing (`ADR-0029` §7) and the vocabulary ships in the bundle anyway. **§3: the
vocabulary is one import-free data module** at `web-client/src/profile/name-vocabulary.ts` — an
ordered array of word lists, one entry drawn from each and joined by a single `U+0020`, **bundled
and synchronous** (no JSON asset, no dynamic import, no fetch, which is what §1's *cannot fail*
rests on), with the **arity as data**, so two lists or four is a curator's edit that changes no code
and needs no ADR. **§4: three tests are the whole guarantee** — the product of the list lengths
`>= SUGGESTION_SPACE_FLOOR = 1_000_000` (with `k` strings spent from inside it a draw is refused
about `k / N` of the time, and `k` grows on every rename too since `ADR-0130` §2, so a million keeps
first-try refusal near 1% at ten thousand names ever written; three lists of a hundred words meet
it), every entry non-empty, NFC and matching `/^[^\p{Cc}\p{Cf}\s\p{Z}]+$/u` — **no whitespace of
any kind**, which is stronger than `ADR-0029` §3 needs and is what makes the join total — and the
longest-entry sum plus separators `<= 32` **code points**. Together: a suggestion can be refused
with `409` and **can never be refused with `400`**, so the product never offers a string its own
write path rejects, and tests 2 and 3 are `O(entries)` rather than an enumeration of a million
combinations. **§5: one draw per mount, as the field's initial state, stored nowhere** — no storage
key, so `ADR-0086`'s rule and `one-module-owns-each-storage-key.test.ts` are untouched — and a
fresh suggestion is placed **on `conflict` alone** and only into a field still holding, unedited,
the string the generator put there, which is `ADR-0119` §4's *never overwrite what the player typed*
as an equality rather than a dirty flag. The reroll is **free against the write budget**, because
`ADR-0134` §6 meters the string **spent** rather than the attempt made and refunds a `409` —
without that clause `ADR-0119` §4's *costs one press* would have become false at the fifth refusal.
**§6: `name_registry` is never consulted** — no query, no endpoint, no filter, no cache, no hint;
`ADR-0029` §5 and `ADR-0051` §9 are applied, not reopened. **§7: one surface has a suggestion
today** — `ADR-0119` §1's ask; `NameSurface` and the account screen's change form (`ADR-0130` §6)
keep their empty initial value, because a mechanism does not place itself where no product decision
asked for one — and a drive whose recorded frames reach the ask must inject `random`, since an
un-injected draw can never be byte-identical twice (`ADR-0100` §3). **Costs named rather than
hidden:** hundreds of words must be authored before `STORY-1407` can merge and **this ADR ships
none**; the list is public in the bundle and therefore farmable at `ADR-0134` §6's rate with
`ADR-0012`'s mintable device ids, which is `ADR-0029`'s already-accepted *second irreversible thing
to farm* rather than a new exposure; the floor decays as `k` grows and **nothing counts `k`**; the
three tests hold shape and never meaning, so a suggestion is exactly as safe as the list somebody
typed, with `ADR-0051` §5's blocklist still empty; every player downloads the list including the
invited rival who never sees the ask; and no pair or triple is reviewed, so a combination can read
oddly. **The reversal direction is the argument:** there is no deadline here — no row, no wire, no
`V<n>`, no key — and client-now → server-later stays open at one call site, while withdrawing a
shipped endpoint does not, which is why the endpoint that filters against the registry, the
strongest alternative, is refused today rather than in principle. **Registers no decision.**

`DEC-146` → [`ADR-0136`](../docs/adr/ADR-0136-a-beat-declares-its-own-length-and-zero-silences-every-beat.md)
on 2026-09-07 — **a beat declares its own length, and zero silences every beat.** The mechanism
`ADR-0120` §3 registered rather than assumed when it spent `ADR-0102` §4's licence on **one beat**
instead of on the step. **The forces:** three things had to be true at once — the number stays at the
boot seam (`ADR-0102` §4 calls that the part *not* free to move), the beat's kind is a property of a
**frame** rather than of a clock (§3 says the two seats answer differently for the same hand), and
**zero has to stay zero for every beat**, because the reveal's timer is what *drains the queue*: a
read beat that could arm a real `setTimeout` inside a synchronous `act()` would not merely slow the
four recorded-frame suites, it would stop the next hand ever reaching the screen, and the symptom
reads like a flake. **§1: the beat carries its *kind*, not its length** — `RevealStep` gains
`hold: "step" | "read"`, set once in `layOutReveal` from `ADR-0120` §3's own predicate
(`view.seats.some(seat => seat.index !== view.viewerSeat && seat.holeCards.length > 0)`), read off
**`PlayerView.viewerSeat`** rather than `state.mySeat`, so the classification is a pure function of
one frame, a queued hand-ending `Snapshot` classifies itself on drain, `mySeat`'s `null` case cannot
arise, `layOutReveal`'s signature does not change, and a fold answers `"step"` with no special case.
**§2: the store owns both millisecond figures** — `DuelStoreOptions.readMillis`, **absent meaning
`stepMillis`**, mapped in `armTick` off the beat standing at the head of the queue, with
`REVEAL_READ_MS = 2000` named beside `REVEAL_STEP_MS = 600` at `boot.ts`'s seam — so `ADR-0102` §4's
*named once, at the boot seam* is **extended, not amended**, `advanceReveal` is byte-unchanged, and
moving 2,000 stays the one-line change §3 promised it would be. **§3: `stepMillis === 0` silences
every beat**, gated in `duel-store.ts` rather than in `boot.ts` because that is the one place both
numbers meet and the only one covering all 113 store call sites: **`drive-duel.tsx` is not edited**,
is deliberately not given a redundant `readMillis: 0` — a redundant zero can drift out of agreement
with the gate that makes it redundant — no file under `web-client/src/e2e/` moves, and
`scripted-duel.gen.json` is byte-unchanged. What the driver proves is **the frames**; it never
proved a duration and does not begin to, because at `0` nothing stands. **§4: no component learns
the hold** — the widened step is assignable to `DuelTable`'s narrower `revealStep` prop (checked
under `tsc --strict`, not assumed), so `Lobby.tsx`, `DuelTable.tsx` and `PotStrip.tsx` are untouched,
**the DOM is byte-identical at 600 ms and at 2,000 ms**, and `no-derivation.test.tsx` stays
byte-unchanged. **§5: nothing crosses the socket** — the predicate reads `PlayerView.viewerSeat` and
`SeatView.holeCards`, both already shipping and already filtered per recipient by `PlayerView.of`'s
`showCards`; no declaration changes, so `ADR-0047` §2's fingerprint cannot move, and **`STORY-1411`'s
implementing ticket is not `atomic:` and takes no branch lock**. The non-negotiable is met head on:
the client *reads* hole cards to choose a schedule and can only see what the projection already sent,
and **the worst this mechanism can do wrong is stand a beat for the wrong length**. **§6: reduced
motion cannot reach it, by construction rather than by exemption** — the hold has **no CSS**, mints no
`--pd-motion-*` token, edits neither `tokens.css` nor the sheet's one `prefers-reduced-motion` block,
and lives in an injected `schedule`, so `ADR-0115` §3's *a step is not motion* needs no ticket to
remember it; the design card draws a **still** state, and the hold is never a transition, because
frame delivery would then sit behind layout. **§7** names what a test must prove — the same frame
answering differently for the two seats, a fold's ending staying `"step"`, only the last step being
long, the injected schedule's **`delayMillis` arguments** rather than a call count, and
`stepMillis: 0` scheduling nothing with `readMillis` explicitly non-zero. Costs named: a **second
dial** at the boot seam where `ADR-0102` §4 kept one; **`stepMillis: 0` overloaded** from *steps are
instant* to *the reveal has no clock*, so instant streets with a paced read beat cannot be expressed;
`RevealStep` wider than anything consumes it; and **no browser-visible proof of the hold at all**, so
§7's delay assertion is the only guard and the first thing to break when auditing this. **§8** names,
without registering (`ADR-0105` §6), the two things that follow arithmetically from `ADR-0120` §3 as
written: a showdown reached from preflop takes **3.8 s**, not 4.4, and the **deciding hand of a duel
holds too**, so the result screen arrives 2 s later — one clause in the predicate if the product owner
wants it sooner, and no change to this mechanism. **Registers no decision.**

`DEC-152` → [`ADR-0135`](../docs/adr/ADR-0135-the-server-says-which-sign-out-this-is-and-the-browser-forgets-one-key.md)
on 2026-09-07 — **the server says which sign-out this is, and the browser forgets one key.** The
mechanism `ADR-0131` §7 fenced with six constraints, three unchosen shapes and no repair written.
**The forces:** §3 forbids every server act that *would* be the abandonment — no revoke, no delete,
no move, no write — so subtract them and the only lever left is **what the browser holds**, which
`ADR-0002` licenses pulling; but §7 says the **case** is a server fact, and it is —
`IdentityResolver.resolve` returns `Identity.Session` without ever calling `PlayerDirectory`, and
`Identity.Session`'s own KDoc makes that structural. So the act is the client's and the knowledge is
the server's, the two halves are separated by a network and by time, and the only question that
matters is **which way they fail when they disagree**. **§1: `player.device_id` does not exist** —
`V7__device_binding.sql` ran `DROP COLUMN device_id` after migrating every row (`ADR-0049` §1, §7) —
and nothing is written to `device_binding`, `player`, `credential`, `duel` or `duel_result` on this
path; the row left behind keeps `revoked_at IS NULL`. **§2: the browser stops owning a profile by
forgetting `pd.deviceId`**, through a new `forgetDeviceId` in `device-id.ts` — the one module that
may hold that literal — reachable from `signOut` and nowhere else, in the abandoning case alone,
before `reload()` and regardless of what the server answered, which is the rule the token already
follows. **Exactly three keys move**; `ADR-0119` §5's skipped bit and every key added later are kept
(`ADR-0131` §6), and `ADR-0030` §8 still governs every other moment in the life of the browser.
**§3: the replacement profile is minted by `Identity.Anonymous` on the next `Hello`** — `ADR-0027`'s
third path, already shipped and tested — so it is created **at that handshake and not at the press**,
and a player who signs out and closes the tab writes **no rows at all**: the accumulation
`ADR-0131` named is one row per sign-out **that reconnects**, not per press. **§4: the case is
`GET /api/me/device`**, session-required on `ADR-0049` §5's reasoning, answering
`DeviceStandingResponse(signOutHandsANewProfile)` — `true` **exactly when the `X-Device-Id`
presented on that same request does not resolve, through a live binding, to a player other than the
caller**. The comparison is one new `IdentityResolver.namesPlayer`, so **`resolve` and all five
`Identity` cases stay byte-unchanged**, `ADR-0027` §4's precedence is untouched and the socket gains
no query. No `409` credential guard is needed: a session is issued only by sign-in, so **`ADR-0131`
§2's invariant is a property of the route's identity rule** rather than a rule to remember. **§5:
`true` is unreachable while any live binding names another player**, so the one outcome §2 forbids
**cannot be expressed** — which is why the two framings that read more naturally were refused, and
why the revoked-binding browser, already handed a fresh profile silently today, now reads `true` and
is told the truth. **§6: everything unknown is a keep** — an absent field, a `401`, an `unavailable`
read, a read that has not returned and a client too old to ask all read `false` — and the read
**must not go through `readFromApi`**, whose no-device short circuit answers *unknown* for exactly
the browser whose answer is `true`. **§7: `signedIn` decides whether a sign-out exists, never which
one it is**; `signOut` takes the boolean as a **required** field; the words stay the card's and this
ADR mints no string. **§8: nothing crosses the socket** — `Hello`, `Welcome`, `protocol.gen.ts` and
`PROTOCOL_VERSION` are byte-unchanged, `ADR-0047`'s ledger gains no row, no schema or migration
moves, and **`STORY-1410`'s implementing ticket is not `atomic:`** under `ADR-0068` §3. Costs named
rather than argued away: **the abandonment is not durable** — a restored `localStorage` keeps the
device route, which `ADR-0131` §3 already stated as a property of the decision — and **no undo, no
grace window and no *sign back in as you were* path is added**, so the forgotten-password cost is
exactly as sharp as `ADR-0131` left it and nothing here makes it look recoverable; a second request
on the account screen; a new public route carrying one bit; a fact read on one screen and spent at a
later press; a third route resolving identity without the shared helper; and `STORY-1408` serialised
against `STORY-1410`, since both edit `docs/protocol.md`'s `/api/me` section and the client's
profile module. **Registers no decision.**

`DEC-151` → [`ADR-0134`](../docs/adr/ADR-0134-a-rename-spends-before-it-replaces.md) on
2026-09-07 — **a rename spends before it replaces, and no name is ever released.** The mechanism
`ADR-0130` registered and designed none of, answered whole. **The permanence trigger becomes the
never-released trigger**: `player_display_name_is_permanent()` is dropped and re-created as
`player_display_name_is_never_released()`, carrying `ADR-0051` §3's body **minus one disjunct** — a
name may leave a player only if the registry has already spent it — so the clause `ADR-0051` had to
cut as an exception is now the whole rule, and `NULL → name`, the identical-name no-op and
`name → NULL` are untouched, which keeps `retire_display_name` the only route to nameless and leaves
it unedited. **The names change** because a schema telling a `psql` operator that a column is
*permanent* is `PERMANENCE_LINE` with a smaller and more consequential audience. **The write is four
statements in one transaction** — `SELECT … FOR UPDATE`, the `TAKEN` insert, the retirement of the
string being left, then the column — with the profile locked **before any registry row**, the order
`retire_display_name` already takes; that lock is what replaces `AND display_name IS NULL` as the
interlock, and it is why two concurrent `PUT`s from one player have no fifth outcome to invent.
`SetNameResult.AlreadyNamed` is **deleted**, `ADR-0051` §2's rollback now protects **two** strings
rather than one, and statement 3 deliberately carries **no** `AND reason = 'TAKEN'`, because an
impossible row must raise rather than pass in silence. **The vacated row moves `TAKEN → REPLACED`**,
a fourth `reason`, with **`retired_from` populated**: the string is spent for everyone under a fold
that ignores `reason`, and the row records *which* of the two ways the name left its holder — the
fact `ADR-0130`'s own reason for preferring retiring to holding depends on, and the one thing that
cannot be recovered if it is not written today. The word is not `RELEASED`, because that is the
answer `ADR-0130` §2 refused. **`ADR-0052` §6's derived bit reads false for a renamer for two
independent reasons**, and `PostgresProfileReads` and `PostgresProfileWrites` change not at all.
**`PUT /api/me/name` answers `200 | 400 | 401 | 409 | 429`**: `403` has no cause and is deleted
rather than repurposed, and a player changing only the **case** of their own name is `409` forever.
**Yes, the write needs a budget**, and it **meters spending rather than trying** — `AttemptBudget`
keyed by `PlayerId`, `ADR-0074` §2's reserve-then-refund, every refusal refunded because `ADR-0051`
§2 says a refused claim burns nothing, and numbers chosen to be unreachable by a human so it stays a
rate limit rather than the quota `ADR-0130` §1 refuses. **Nothing crosses the socket**:
`PROTOCOL_VERSION` does not move, `protocol.gen.ts` is not regenerated, and `STORY-1409`'s tickets
are **not** `atomic:` under `ADR-0047`'s lock. Costs named rather than argued away: a player can
**never** change the case of their own name; the write holds a `player` row lock across a registry
insert, so a coin write for that player can wait behind it; one statement became four; an idempotent
retry spends budget; and a fourth `reason` is a schema word every future query has to keep true.
**`STORY-1409` is unblocked.**

`DEC-132` → [`ADR-0131`](../docs/adr/ADR-0131-signing-out-of-your-own-account-hands-the-browser-a-new-profile.md)
on 2026-09-07 — **signing out of your own account hands the browser a new profile, and leaves the
old one whole.** Last of `EPIC-14`'s original eleven. **Yes**, when the account being left is the
profile this browser owns (a promotion, `ADR-0030` §1) — the browser becomes a **new, empty**
anonymous profile; **no**, when it is somebody else's account — the browser returns to the profile
it owns, unchanged; and a browser that never had a profile stays a first-time visitor, which
already ships. **Both halves are the human's and neither is chosen here**: the abandoning half
verbatim on 2026-09-06 — *"if logout you comeback to new annonumus profile"*, the fourth clause of
an item whose every other clause is about **this browser's own profile and its promotion**, and
which never mentions signing into an account the browser does not own — and the keeping half
verbatim on 2026-08-14, *"i would like to keep anonimous accout if user log out"*, recorded in
`ADR-0030` §2. What the ADR chooses is **which sentence governs which case**, and it says so as a
**reading**, with the one sentence that would correct it left for the human. A browser is handed a
fresh profile **at most once per promotion** — the second sign-out is the second case, because the
session's player is no longer the profile the browser owns. **The invariant is what answers this
epic's own framing of the risk**: to be in the abandoning case you signed in to that profile, so it
holds a password — **nothing is ever abandoned that cannot be signed back into**, and `DEC-152` may
not build a route that is. **Nothing is revoked, deleted, moved or destroyed**: `ADR-0049` §2's
`UPDATE` never runs on this path, its trigger never fires, §3 is not entered and the binding left
behind stays **live** — so what `ADR-0049`'s finality means for a browser about to be somebody else
is **nothing, because a sign-out is not a revocation**, on `ADR-0050`'s own *"Revocation is final
forever; signing out is the cheapest act in the product."* One abandonment is licensed, at one
confirmed press, and `ADR-0027` §5's stale-client harm stays forbidden. **The player is told before
it happens**, on the confirmation `SignOutControl` already renders — no new surface, two
obligations, and `ADR-0125` §3's three refusals re-applied — which makes the shipped
`SIGN_OUT_WARNING` (*"This browser goes back to the profile it had before."*) **false the day the
behaviour lands**, so copy and behaviour ship in one diff. `ADR-0119` §5's skipped bit is **weighed
and left unamended**: a new profile is not an answer, so a player who skipped is not asked again
and plays as `No name`. **Supersedes `ADR-0030` §3's outcome clause and its *keeps its device id*
bullet, and §8's *not on sign-out* and *clears the token and only the token* — for the abandoning
case only**, standing verbatim everywhere else; supersedes nothing in `ADR-0012`, `ADR-0027`,
`ADR-0037`, `ADR-0049` or `ADR-0050`. Costs named rather than argued away: a player watches their
coin count fall to **zero** with one sentence of explanation; **a forgotten password now loses the
account in one press that doing nothing used to undo**, a weaker form of the risk `ADR-0037`
refused to force on everybody, and the strongest argument against the decision; empty profiles
accumulate with no deletion (`ADR-0039`); and two sign-outs behave differently under one label.
**Registers `DEC-152`**, the architect's, fenced by §7 — **answered 2026-09-07 by [`ADR-0135`](../docs/adr/ADR-0135-the-server-says-which-sign-out-this-is-and-the-browser-forgets-one-key.md)**. `STORY-1410` is splittable.

`DEC-131` → [`ADR-0130`](../docs/adr/ADR-0130-a-name-can-be-changed-and-the-name-it-leaves-is-spent.md)
on 2026-09-07 — **a name can be changed, and the name it leaves behind is spent.** Two halves from
two places, and only one is chosen: ***that* a name may be changed is the human's, recorded
verbatim** — *"player name can be changes at any time in accounts settings"* — reversing their own
`DEC-017` answer (*"unique and permanent … chosen once … and never changed"*), and the ADR does not
choose, argue or soften it. ***What becomes of the string* is derived** from *"**A leaderboard.**
Ranked results over a season."* read as `ADR-0067` §1 already read it: a row is **text and leads
nowhere**, so the display name is the whole of a row's identity, and `ADR-0039` makes history a
**live join** — so a reissued string would print a second player onto the first player's finished
duels, on the opponent's own history screen, retroactively. **§1: a change is a `set`** under every
rule a first name is set under — `ADR-0029` §2's canonicalisation, §3's characters, §1's fold,
`ADR-0038`'s blocklist — with **no quota, no cooling-off, no confirmation press and no *changes
remaining* counter**, because the human's phrase is *at any time*; and it **never leaves the player
nameless**, so `ADR-0051` §3's takedown stays the only route back to `null`. **§2: the name given up
is retired — not released, not held.** It is spent forever for everyone **including the player who
gave it up**, which makes `ADR-0038`'s *retired forever, including the player it was taken from* one
rule covering both ways a name can leave a player; `ADR-0051` §1's *no string ever leaves this
table* is what it rests on and is **not amended**. **§3: history shows the name as it *is***, not as
it was — the ladder, every finished duel and the strip print the current string, **no row is
rewritten because no row holds a name**, the old name appears nowhere (no *formerly known as*, no
tooltip, no `aria-label`), and **nothing marks a row that changed**, extending `ADR-0052` §5's
asymmetry one step. Measured on `develop` at `e7061e7b`: the blast radius is **three surfaces**
(`LadderScreen`, `HistoryScreen`, `ProfileStrip` — the only non-test callers of `nameOrNone`) and a
rename is **invisible at the table**, since no `ServerMessage` carries a display name and the
opponent is the fixed string *Your rival*. **§4: nobody is told** — not the rival, not then, not
later — and a player who renamed themselves is **never** shown `ADR-0052` §1's removal notice,
whatever the mechanism writes to the registry. **§5**: `PERMANENCE_LINE` **leaves the product**,
replaced by two obligations said **before** the send — the name can be changed later, and the one
given up is gone for good — on `STORY-0411`'s reasoning that *a player is entitled to know that at
the moment they can still avoid it*; `refusalSentence`'s `permanent` sentence goes with it, because
**no refusal in this product says a name cannot be changed**. **§6: one form, on the account
screen**, doing both acts — which answers the seam `ADR-0125` deliberately left open, moves
`NameSurface` off the front door, and carries `ADR-0052`'s notice with it; `ADR-0119` §1's
first-press ask is untouched, and the change form **offers no suggestion**. **Supersedes**
`ADR-0029` §4's *`name → a different name` raises*, its title's *permanent* and §5's `403` for an
already-named player; `ADR-0051` §3's *no exception at all*; and `ADR-0119` §3's obligation 2,
exactly as `ADR-0119` said it would be. **Costs named rather than argued away**: a rename is
**one-way** and the commonest outcome will be a player who cannot change back; the namespace now
burns **one string per `PUT`** rather than one per profile, with no brake in the product; a rival who
beat `Ace` can never find `Ace` again; other players' records change under them silently and often;
and two merged migrations encode the clause §1 makes wrong. Registers **`DEC-151`**, the
architect's, for the mechanism — due **before** `STORY-1409` ships, not after. **`DEC-151` is
answered the same day** by [`ADR-0134`](../docs/adr/ADR-0134-a-rename-spends-before-it-replaces.md),
which carries the brake, so *no brake in the product* stops being true when that migration lands.

`DEC-110` →
[`ADR-0133`](../docs/adr/ADR-0133-the-room-a-player-holds-is-scanned-for.md) on 2026-09-07 —
**the room a player holds is scanned for, and only the `PLAYING` refusal spends a frame.** The
architect's mechanism question, open since 2026-09-01 and widened to the `WAITING` case on
2026-09-06, answered as **one lookup over both live states**: `RoomRegistry.heldRoom(player)`,
non-suspending, returning the whole `Room`, where **held** means `seatOf` non-null **and** state
`WAITING` or `PLAYING` — `FINISHED` and `ABANDONED` are excluded **by the method, not by its
callers**, because a caller that forgot would break `ADR-0105` §2's finished row and
`Room.offerRematch`. The answer is **total and independent of iteration order** (`PLAYING` first,
then oldest `lastActivityAt`, then lowest code). **It is a scan and not an index**: it reads each
`Holder.room` through the same `@Volatile` field `get(code)` and `reap`'s unlocked pass already
read, so the rooms stay the one truth about where a player sits (`ADR-0002`) — an index would be a
second, whose leak is a player who can never open a room again, the failure `recording`'s own KDoc
documents — and the swap to one later is a one-body change behind the same method, which is why the
method is the seam. **Opening becomes find-or-create**: `heldOrOpen(host, format)` is
`heldRoom ?: create` under a **per-player striped `Mutex`**, `SeatOwnership`'s shape in a second
array, without which find-then-create is check-then-act across `ADR-0018`'s asynchronous eviction;
with it, `ADR-0124` §1's *one waiting room* is **true rather than usually true**, since `create` is
a `WAITING` seat's only source. **The deadlock question is closed by construction**: the scan takes
**no lock**, `heldOrOpen` takes exactly one across **no suspension point**, the rule is *a player
stripe is always outermost* — and it is structurally enforced today, because `mutate`'s `block` and
`absent` are not `suspend` while `Mutex.withLock` is, so nothing inside a room's critical section
can acquire a lock at all. Two things an implementer would get wrong are fixed: the seat is
`Room.seatOf` read off the returned snapshot and **never the literal `0`** (true of a fresh and of a
returned `WAITING` room, false of a `PLAYING` one), and `RoomMembership.code` is repointed at the
room the answer names — which **disarms `ADR-0118`'s window at the source it named**. The `JoinRoom`
guard fires only when the held room is `PLAYING` **and its code differs from the code asked for**,
or every reconnect breaks. **The `WAITING` half moves no wire and the `PLAYING` half does**:
`RoomJoined(code, seat)` alone hands back a waiting room, while the refusal takes one new
`ProtocolError` value, `ALREADY_IN_DUEL` — `ProtocolError` is a generated declaration, so
`ADR-0047` §2's fingerprint moves and `PROTOCOL_VERSION` with it, and no existing value can carry
it without asserting something false about the room that was **asked for**, which `ADR-0022`'s
no-oracle rule forbids. Frames arrive `RoomJoined` → `resume`'s frames → `Failure` **last**, on
`ADR-0044` §5's precedent, and `resume`'s `reconnect().touch()` is checked clause by clause against
`ADR-0105` §6's *nothing moves* and passes. **The split**: `STORY-1416` builds the lookup, changes
no client file, moves no wire, is **not** `atomic:`, and **must leave the `PLAYING` case falling
through to `create` exactly as today** — handing it back silently would ship `ADR-0105` §1's
refusal without §4's words; `ADR-0105` §1's own ticket is `atomic:` under `ADR-0047`/`ADR-0068`
and sized by `ADR-0070`'s probe. Registers nothing, and names two questions unregistered on
`ADR-0105` §6's rule. Costs named: a linear scan on the product's primary press with nothing
measuring it; a second lock class in a class where breaking the rule hangs the server; `create`
stays reachable behind a KDoc note rather than a fence; the answer can name a room the reaper takes
a moment later, and no lock closes that window; the `PLAYING` half's version step and its hold on
`ADR-0047`'s lock; a found-and-ignored branch in the merged tree between the two tickets; and one
more `ProtocolError` whose absence from the client's reducer shows as nothing happening.

`DEC-138` → [`ADR-0123`](../docs/adr/ADR-0123-a-standing-rematch-offer-follows-the-rival.md)
on 2026-09-06 — **a standing rematch offer follows the rival, and it never takes the screen.** Two
halves, and only one is chosen: the ***whether*** is the human's, recorded verbatim (*"it shoud
appear on any screen"*), and the ***what*** is derived from the success condition everything is
downstream of — *"We play a full heads-up match. Someone wins. **We hit Rematch.**"* — fenced by
*Positioning*'s *"Dark, quiet, fast, minimal."* An offer **from the rival** follows a player onto
every screen they can be on while they still hold the room — every `Screen` but `first`, **no
carve-out**, mailed screens included — while a player who **left** the room (`forgetRoom()`, the
page load behind *Back to the lobby*) is followed nowhere: walking to another screen is not
leaving (`ADR-0112` §3 keeps the seat, the memory and the socket). The surface is a **panel above
the screen and never a modal**: nothing beneath it is scrimmed or disabled, it takes no focus,
**no keypress the player was already making can answer it**, it makes no sound, and it never
retires itself on a timer — *cannot miss* is bought by standing there, not by blocking, because
unrequested seizure of a screen is the casino's move. Only an **incoming** offer follows (a
player's own standing offer trails them nowhere), and only where the room's own screen is not
showing, so **one fact never has two live surfaces**. **No string is minted**: *Your rival offers
a rematch*, `Rematch`, *Not now* (`OFFER_DISMISS`), the dealing sentence and *That duel room is
gone.* are all merged. It stands until accepted, dismissed, or refused with `UNKNOWN_ROOM` —
`ADR-0044` §6's *frame that ends a rematch* — and a countdown is impossible honestly, since the
wire carries no deadline. **Dismissal hides the surface, never the offer**: nothing is sent, the
rival is told nothing before or after, silence stays the decline, and **nothing is stored**, so a
reload brings the panel back on the offer the server restates (`ADR-0044` §5). The accept needs no
new design — the agreeing `Snapshot` moves the standing to `running`, `rulingOn` refuses the
chosen screen and the layout effect restores `/` in the same commit (`ADR-0112` §4, `ADR-0114`
§3), which is what the product already does today. **Pays `ADR-0112`'s own named cost** — *"a
finished player on the ladder does not see a rematch offer arrive. No notice exists, and none is
designed here."* — and amends nothing: no wire, no `PROTOCOL_VERSION`, no server file, no engine,
no schema, no stored key, and `ADR-0112` §2's *no notice, no dialog* stands untouched because it
governs a **refused** ask over a **running** duel. **Costs named**: one fact with two surfaces
that can drift; the product acquires an overlay pattern whose only fence is this ADR's prose; *Not
now* may read as *I told them no* and tells them nothing (`RematchDeclined` stays one bump away,
unbuilt); the panel can point at a reaped room until it is pressed; a dismissal does not survive a
reload; and on `verify`/`reset` a deliberate accept walks out of a half-filled recovery form. The
card is **minting** under `ADR-0091` §3 — worked with the human, not dispatched — and draws every
state including the reduced-motion form (`ADR-0115`). Registers **`DEC-144`**, the architect's,
for the mechanism, and unblocks `STORY-1415`.

`DEC-139` and `DEC-111` →
[`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md) on 2026-09-06 —
**a player holds one waiting room, and pressing play again hands it back.** Two questions answered
together because they are one question read from opposite ends. `DEC-139`'s **direction is the
human's**, recorded verbatim and not chosen here — *"i go back to lobby and press play again. in
this case i shoud be back to the same room"*; `DEC-111` is derived from `docs/vision.md`'s own
one-sentence version, *"Two people, **one link**, one heads-up poker match, one winner"*, with
*"Not a multi-table poker room"* and *"A counter of duels won."* **While a player holds a `WAITING`
seat, a `CreateRoom` from them opens no second room**: they are handed back the one they hold —
same code, same seat, room left exactly as found — and **nothing is refused**, which is what keeps
`ADR-0073` §3's promise true rather than breaking it. *"…and it brings you back"* is now true twice
over. **The browser goes on forgetting.** `ADR-0072` §3 stands byte-unchanged and there is **no
client change at all**: a remembered code is what boot rejoins on, so keeping it across
`Back to the lobby` would put the waiting table straight back up and restore `ADR-0073`'s room with
no door; the seat is the server's fact (`ADR-0002`); and the server can answer a device that has
never heard of the room, which a browser's memory cannot. **Nothing is said and no string is
added** — `ADR-0110` §6's enumeration is untouched, on *"Dark, quiet, fast, minimal."* — with the
observation that would reopen it written down. The room's ten minutes still runs from `Room.open`
and a return does **not** restart it. `DEC-111` is answered **by construction**: a `WAITING` seat
has one source (`Room.open` is its only constructor, `RoomRegistry.create` has one production call
site, no transition returns a room to `WAITING`), so what a second `CreateRoom` does *is* how many
waiting rooms a player may hold. Amends `ADR-0105` §2's `WAITING` row only; extends `DEC-110` to
the `WAITING` case, which now also blocks `STORY-1416`; registers **`DEC-145`** — the one route
left into two duels at once, a holder of a waiting room taking a seat in another by invite (**answered
2026-09-08** by [`ADR-0141`](../docs/adr/ADR-0141-taking-a-seat-elsewhere-releases-the-room-you-were-holding.md): the seat is
taken and the held room is released in the same act). Costs
named: **a host cannot get a fresh code for up to ten minutes** and the product still has no way to
close a room; a press that appears to do nothing; two live states answering the same press
differently; `replyToCreateRoom` grows a read; and `ADR-0105` §2's table stops being a complete
statement on its own.

`DEC-130` →
[`ADR-0125`](../docs/adr/ADR-0125-the-account-screen-names-the-anonymous-profile-and-owns-the-door.md)
on 2026-09-06 — **the account screen names the anonymous profile, and it is the only door to a
password.** Answered in **two parts from two places**, marked separately. *That* the post-win offer
goes is **the human's, recorded verbatim and not chosen**: on `edits4.png` the **entire** block —
heading, paragraph, *Keep them with a password* and *Not now* — is struck through with the single
word ***remove***, while `Rematch` and `Back to the lobby` beside it carry no mark. What the screen
then says, and what the front door stops saying, is **derived** from *Positioning* — *"Dark, quiet,
fast, minimal"* — read with ***"One duel coin per win. Not chips, not currency, not a balance. A
counter of duels won."***: a counter does not need protecting at the instant it increments, and
dressing a victory in a loss warning is the register the vision refuses. **§1 retires the offer
whole** — the surface and its strings, `offerAccount`'s three terms, `DuelResult`'s `offer` prop,
the key `pd.accountOfferSettled`, its module **and its row in
`one-module-owns-each-storage-key.test.ts`**, which go in one diff or the row reddens naming a file
that no longer exists. **§2: nothing replaces it** — no banner, no badge, no reminder, on any
screen; the standing door is the `Account` control the front door already renders unconditionally,
so a win is **two presses** from the password form, and §2 refuses to shorten it because a pointer
on the result screen is the struck block in a smaller font. **§3: the screen names the profile
*anonymous*** and says three things — what the profile is, that it lives in this browser, and that
a password keeps **every duel coin and every duel** because nothing moves (`ADR-0030` §1, which is
item 1e's *sentence on a screen*) — with the words the card's, inside three refusals: **not a
tier** (no badge, no *real* account, no rank a password promotes you to), it states rather than
urges, and the heading stays `Account`. **§4: the screen asserts the state only when it has been
told** — measured, `GET /api/me` carries no credential field and `signedIn` is a false positive for
anonymity on a `201` whose follow-up sign-in failed and on a player signed out here — which
registered **`DEC-147`** for the architect and held §3's block until it landed, while §§1, 2, 5 and 6
need no fact the client does not already hold. **`DEC-147` is answered** on 2026-09-07 by
[`ADR-0132`](../docs/adr/ADR-0132-the-profile-says-whether-it-holds-a-password.md): `GET /api/me`
gains a seventh field, `hasPassword: Boolean`, computed on every response that carries a
`ProfileResponse` and told only to the player it is about; a body missing it reads `unavailable`
rather than `false`, so §4's *not told* case stays reachable; and the provider re-reads after
`signUp`'s `signed-up`, the one profile-changing outcome that does not already reload. No wire
move — `PROTOCOL_VERSION` stays put — so `STORY-1408`'s anonymous-state ticket is startable and
**not `atomic:`**. **§5: the front door stops describing an absence** —
`ProfileStrip`'s `no-profile` branch renders nothing, exactly as its `unavailable` sibling already
does, so `No profile yet.` leaves the product; the `profile` branch and the duel coin count are
untouched, because the box the human struck held one sentence about nothing. **§6: *anonymous* is a
state, never a name** — `ADR-0058`'s `No name` and `ADR-0029` §6 stand, and after `ADR-0119` a
player may be **named and anonymous** or **nameless and claimed**. **The first supersession this
repository has made**: `ADR-0085`, `ADR-0086` and `ADR-0116` in full, and `ADR-0036`'s offer block
and three of its Consequences — with its **first paragraph standing and re-applied**, *an account
is never required to play and anonymous play stays fully ranked*, which was the human's own
`DEC-025` call. **The cost is stated rather than argued away**: this is the *"optional forever, with
no prompt"* shape `ADR-0036` §Alternatives rejected, so a player who never opens the account screen
is never told, and *"a player who has never been told their coins are device-bound learns it by
losing them"* is now a thing that can happen. Free to do **today** only because nothing is hosted
(`EPIC-07` *not written*), so every browser holding the orphaned key is the driver's own. It
**strikes `DEC-089` by deletion** — with no nudge, the verdict is the only thing on the screen — and
leaves the credential's shape to the human: the human's *"set email and password"* is read as the
ordinary phrase for signing up, not as an instruction that reverses `ADR-0041`.

`DEC-134` →
[`ADR-0126`](../docs/adr/ADR-0126-the-table-shows-the-cards-and-marks-none-of-them.md)
on 2026-09-06 — **the table shows the cards and marks none of them.** Open for hours, and answered
before `STORY-1412` was split and before `STORY-1411`'s showdown card was drawn, which is the only
moment that card is free. The ask is one clause of three on one arrow over the duel table —
*"winning combination higlited, winning cards + board card that make combination"* — and the other
two are already placed: the reveal is `DEC-133`, answered by `ADR-0120`, and the pot's travel is a
card under `ADR-0115`. **The answer is no.** Every card the table draws is drawn like every other
card of its kind: at a showdown, at a fold, at every street, in either seat, on the board, and for
**both** winners of a split, **the five that made the winning hand are drawn exactly as the two that
did not**. A mark is anything that separates one drawn card from another by *what it did* rather
than *what it is* — ring, border, glow, tint, shadow, lift, scale, opacity, reordering, badge,
connector, a class a stylesheet can key on, a `data-*`, a `title`, or anything in an `aria-label`
beyond the rank and suit `card-text.ts` already builds — and **dimming the cards that did not play
is the same statement and the same refusal.** This is `ADR-0095` §3 **applied, not reopened**: a
highlight prints no string, so the merged `HAND_TALK` matcher could never have seen it, but the fact
it states is the same kind — a reading of seven public cards, performed by the product, at the
table, live — and *these five played* sits one string away from *two pair, kings and jacks*. The
licence is the vision's *What it is* — ***"a match can be replayed and analysed afterwards"*** —
read as `ADR-0095` read it when it refused the name: **the explaining surface is afterwards**, dated
**v0.4** by the roadmap while v0.1 is *"one complete duel, rematch"*. The human's annotation is the
**report, not the ruling**, which is how `ADR-0120` read the same drawing on the same day. **Nothing
moves on the wire**: no field, no `PROTOCOL_VERSION` step, no server file, no engine file, and
`no-derivation.test.tsx` is byte-unchanged — the client could never have computed the five anyway
(`ADR-0002`). `BestHand.cards` **keeps its value and loses its stated reason**: the KDoc's *"exists
because the client highlights the winning five at showdown"* has described a consumer that does not
exist since August, the ADR is the correction of record, and the one-line repair is **not
`EPIC-14`'s** — that epic's *Out of scope* says nothing here opens `poker-engine`. `STORY-1412`
loses its wire move, its `atomic:` label and `poker-server`, and becomes **one gate** — *the drawing
of a card does not depend on whether it played* — plus one line on `STORY-1411`'s showdown card;
**`STORY-1413` no longer waits on it**, because what it waited for was `ADR-0047`'s lock. **Costs
named rather than implied**: the player who cannot read seven cards is charged a **second** time —
`ADR-0095` took the name, this takes the mark, and `ADR-0120` gave two seconds to work it out
unaided; something the human asked for **in writing** is declined; a deferred wire field is a dearer
wire field, which is this answer's own strongest objection; and a refusal that leaves nothing behind
is undone by the next spare CSS class, so a gate has to be written to hold a nothing. It **registers
nothing** — a *no* has no wire question, so the reserved `DEC-148` is not opened (`ADR-0095`'s
precedent, `ADR-0105` §6) — and it decides nothing about the hand's **name** (`ADR-0095` §3 stands),
about marking the five **afterwards** in replay or `EPIC-08`'s analysis board, or about the 2 s hold,
the pot's travel and the rival hand's width. Reversal keeps `ADR-0095` §6's trigger — the first
duels played by people who are not the author — and must settle **the mark and the name together**,
one field and one turn of the version lock, rather than spending that lock twice on two halves of
one question.

`DEC-135` and `DEC-108` →
[`ADR-0127`](../docs/adr/ADR-0127-a-control-stands-only-for-a-decision-the-server-has-opened.md) on
2026-09-06 — **a control stands only for a decision the server has opened.** `DEC-135` is answered
**no**: when it is not the player's turn the action bar does not stand as disabled controls. It
keeps its box, keeps both rows reserved (`ADR-0103`'s quoted promise is untouched) and holds **one
still sentence**; outside a turn it draws no button, no sizing chip and no amount field — not live,
not faint, not outlined, not as a placeholder.

**Why, measured.** `PendingTurn.legalActions` is written by `case "YourTurn"` alone
(`duel-state.ts:282-292`) and cleared by the next `Snapshot`, so **the client holds no legal
actions outside a turn**. Heads-up, the coming decision is `{CHECK, BET}` or
`{FOLD, CALL, RAISE}` and the client cannot know which — so verbs-without-amounts is the **bigger**
claim, not the smaller one: a faint `Fold` where nothing is owed teaches the interface wrong, and a
faint `Call 400` is a number for a turn that does not exist (`ADR-0111`, `bar-no-derivation.test.tsx`).
The faint treatment already ships carrying one meaning — the card's *Sent — waiting on the server*
— and keeps it, so two different facts are never drawn alike. Licensed by `docs/vision.md`'s
*Positioning* — ***"The reference points are Lichess and Chess.com, not PokerStars. Dark, quiet,
fast, minimal."*** — applied through `CLAUDE.md`'s *a client may never assert a game fact*, **which
actions are legal** being the fact.

**The human's annotation is recorded verbatim and is not followed** — *"show disabled controllers"*,
`edits3.png`, 2026-09-06 — because `EPIC-14` routed it as a `DEC` for the product owner rather than
as a specification. §7 makes the overrule two sentences: which of the two action shapes a faint bar
draws, and what stands where an amount goes.

**`DEC-108` is answered on both halves by §4, not merely mooted.** *May* wait *and* you may act *be
sayable at one moment?* **No.** *Which gives way?* **The notice, never the bar** — the bar is drawn
from the server's own `LegalActions` for an open decision, so a notice contradicting it is copy
about a state that no longer holds. That is the route the product already took: `ADR-0113` §7
deleted the pause and `TASK-130911` removed the line that named it, and `DUEL_PAUSED` is verified
absent from every `.kt`/`.ts`/`.tsx` and from `ProtocolError`. No ticket falls out — nothing
exhibits the case today — and the rule makes the next such notice a defect on the day it is
written. §5 leaves one edge open by name: the sweep race, where a late press may still be
`Rejected`, is a boundary no screen can be ahead of rather than a standing offer the product has
decided to refuse.

**What it costs `EPIC-14`.** `STORY-1406` — *The bar stands disabled where it printed a sentence* —
**has no work in it** and is retired rather than split; item 3c shrinks to the timebank figure and
the pot's chip pile. The sentence stays the third statement of a fact the rival's plate already
makes twice (`seat-status.ts:24`, the accent border, the countdown), kept because the alternative
on offer was worse and not because the redundancy is good. Amends nothing, moves no wire, registers
no `DEC`.

`DEC-129` → [`ADR-0119`](../docs/adr/ADR-0119-the-name-is-asked-at-the-first-press-and-skipping-plays.md)
on 2026-09-06 — **the name is asked at the player's own first press, the suggestion promises nothing,
and skipping plays.** Answered in **three parts from three places**, marked separately rather than
blended. *That* a player is asked at all is **the human's own instruction, recorded verbatim** —
*"First time you play dialog "choose your name" appears with sugestion(before duel)"* — and the ADR
does not choose it. *When* the ask stands, and *whether* it can be skipped, are derived from the
vision's first success condition — ***"Send a link. She opens it in a browser. We play a full
heads-up match."*** That sentence is about the **invited** player, and she presses nothing: measured
on `develop`, `main.tsx:160` reads the code out of the URL and `store/boot.ts` sends `JoinRoom` on
the socket's `Welcome`, outside React, so *"before duel"* has **no press to hang an ask off** on the
one path the vision tests itself against. So the ask stands at the player's **own** first press of
*Create a duel room* or *Join the duel*, in this browser, holding no name — a screen in place of the
front door, after which the press goes through and the room is created — and **never** on the invite
path, never before a rematch, never on a resume, never over the table, and never to a player whose
name was **removed** (`ADR-0052` §1 keeps that conversation on the name surface and nowhere else).
It is deliberately **not a modal**: this product has none and `DEC-138` owns whether it gets one.
**It may be dismissed** — skip is one of two equally reachable ways out, it sends nothing and writes
nothing, and no refusal `set-name.ts` can settle ever prevents a duel. That half is load-bearing:
it is what keeps `ADR-0036`, `ADR-0058` and `ADR-0063` §2 describing a state players are really in,
rather than leaving three merged ADRs formally accepted and quietly false. The **suggestion is an
offer to type** — the field's initial value, never a reservation and never a claim that the name is
free, because `ADR-0029` §1 makes freedom a database fact and §5 refuses the endpoint that would
answer it; a refusal costs one press, and the string encodes nothing about the player (`ADR-0029` §6,
`ADR-0067`). **Nothing is written unless the player takes it**, which is the only irreversible thing
in the whole area: `ADR-0051` §1 lets no string leave `name_registry` and §9 builds no un-retire, so
an ask that pre-registered or auto-accepted a suggestion would burn one name per browser that ever
reached the screen. Two costs are named rather than hidden — **the invited rival is not asked before
her first duel**, which in a two-player product is a coin flip and not a corner case, and the host
meets one interstitial in a product whose positioning sentence is *fast*. **Supersedes and amends
nothing.** Registers **`DEC-142`**, the architect's, for the generator.

`DEC-133` →
[`ADR-0120`](../docs/adr/ADR-0120-a-showdown-shows-the-hands-the-rules-showed-and-the-beat-that-shows-them-stands.md)
on 2026-09-06 — **a showdown shows the hands the rules showed, and the beat that shows them
stands.** Open for hours, and answered before `STORY-1411` was split, which is the only moment the
design card is free. **The measurement moved the question.** `EPIC-14` recorded that no client file
outside `protocol.gen.ts` mentions `HandRevealed` and inferred that a shown hand *"is drawn by
nothing"*; the first half is exact and **the inference is wrong**. The reveal reaches the table
through the **snapshot**, not the event: `Addressed.kt:52,62` computes `revealedSeats(handEvents)`
into `PlayerView.of`, which populates a revealed seat's `holeCards`; `DuelTable.tsx:76-78` draws the
rival with `cards={rival.holeCards}`; `Hand.tsx` turns face up every place the view carries — and
`duel-secrecy.test.tsx`'s *"do reach it once the reveal has arrived"* has pinned it over recorded
frames all along. What was missing was **time**: a hand called down to the river carries no
`StreetDealt`, so `ADR-0102` gives its ending **one 600 ms step** before `advanceReveal` releases
the queued next hand, on a rival hand `ADR-0103` §3.2 draws at **24–40 px**. And in the hands the
reporter *won* — they won 7–0 — the rival's cards were never sent at all: `Showdown.kt:61-76`'s
`revealOrder` returns **the winners**, one seat or both on a split, exactly as `ADR-0008` says.
**The answer, both halves.** The table shows **exactly what the rules showed** — the winner's hand,
both on a split, no hole card on a fold — with **no post-hand disclosure**, `ADR-0008` whole and
unamended, and **no wire move**: *you always see the hand that beat you, and you never see what you
beat.* And **the last beat of a hand that turned a hand face up the viewer had not seen stands 2,000
ms**, every other ending keeping 600 ms, the hold ending when the next hand's frames are applied
rather than on a fade — `ADR-0095` §4 untouched. It is drawn in the seat's own two card places: **no
panel, no overlay, no new string, no *mucked* line**, and the faces come from the snapshot and never
from `HandRevealed`, because `ADR-0102` §5 sends a reconnecting client no events at all. **Costs
named rather than implied**: the **winner still learns nothing**, so on the commonest shape of the
very session that raised this only the pacing changes; a held player spends **2 s of a 30 s
allowance**, because a queued `YourTurn` anchors to arrival (`ADR-0113` §6); the two tabs step ~1.4
s apart; and a **second duration number** exists where `ADR-0102` §4 pinned one. It decides nothing
about the winning five (`DEC-134`), the pot's travel, or the rival hand's width (`DEC-136`) — *two
seconds of a card nobody can read is still not showing it*. It **registers `DEC-146`** for the
architect, and **names without registering** (`ADR-0105` §6) the widening it would take first: a
hand **all in and called** turning both hands face up, as live poker requires and
`docs/duel-rules.md`'s *"The loser may muck"* does not.

`DEC-137` → [`ADR-0122`](../docs/adr/ADR-0122-call-names-the-price-and-raise-to-names-the-total.md)
on 2026-09-06 — **`Call` names the price, and `Raise to` still names the total.** Reported as a bug
and answered as a decision, because there was no arithmetic to be wrong: the button printed a field
the server sent. It now prints `callTo − committedThisStreet` — `ADR-0101` §1's `toCall` character
for character, and the engine's own term recovered rather than a number manufactured — so the
human's own frame, 100 in against a raise to 300, reads `Call 200`. The other three verbs keep
their totals, told apart by the game's own preposition, and `ADR-0109`'s mark **goes bare on
`Call`** because `PlayerCalled` carries only `to`: after the act the caller's `committedThisStreet`
*is* `to`, so the price is unrecoverable and keeping the total would put two figures on one act a
tick apart. That amends `ADR-0109` §2 in one clause and leaves §§1, 3, 4 and 6 byte-unchanged.
**No architect `DEC` was owed** — `ActionBar.tsx:255` already computes the difference for the
sizing base — so no wire moves and item 4 does not serialise against the showdown under
`ADR-0047`'s lock. Named uncovered: `All in`'s bare total. Measured for the implementing ticket:
`ActionBar.test.tsx` commits zero in seventeen places, so the change lands green either way unless
the fixture gives the two seats different commitments.
...
Of the two readings the row carried, the answer takes the first: this is a **defect against
`ADR-0103` §1's instrument**, not a reason to change its mechanism. §1's promise is upheld and its
instrument is qualified in exactly three ways — it watched **one axis**, at **one assumed number**,
in a browser with **no chrome**. A uniform scale **is** still one table (it removes nothing,
reorders nothing, renames nothing, and passes all three of `ADR-0096` §4's second-surface tests), so
picking between the mechanisms was the product owner's; it is refused on **`R3`** — every amount at
or above the body type size — with `ADR-0106` §1's rule that **the property governs the
instrument**, since a transform passes `getComputedStyle` and fails the eye, and on `ADR-0103` §3's
give order, which spends whitespace, then the rival's card backs, then the hero's hand, then the
board, and **never the type**. The reference point is the vision's own: on Lichess the **graphic**
scales and the **type** does not. `ADR-0103` §§2–5 stand byte-unchanged. The fit is now measured at
**the smallest viewport the browser presents** — iOS Safari with the bars fully expanded, read as
the smaller of `documentElement.clientHeight` and `visualViewport.height` — and a fit there is a fit
at every larger height, so the client owes **no `visualViewport` listener and no refit**. The
contract becomes **two numbers, one per axis**: `scrollHeight ≤ clientHeight` **and**
`scrollWidth ≤ clientWidth`, with `ADR-0106`'s sub-pixel reading and one-pixel fence applying to
each. That adds **no criterion** — `R2` says *"at once, without scrolling"* and names no axis, `R3`
already says *not clipped* — so the photographed plates are `R3` `not met` today and three of the
human's four annotations were never blocked on a decision at all. **390 × 664 stays**, with
`ADR-0096` §4's provenance, as an **assumption about a device**: a hand-checked reading
(`ADR-0088`) on a real phone that is smaller on either axis **replaces** it, for the round's shape,
the headless override and `duel-table.html`'s `.viewport.phone` box — and it **never moves up**,
because §4's own text says a taller value *"would let `R2` pass for a player who does not have it"*.
Below the judged shape nothing is promised, which is the human's *"scroll … only if table area reach
some reasonable min heigt/width"* in their own structure. The **cause is not diagnosed** and the ADR
does not pre-judge it; if the merged give list runs out at the measured shape, `ADR-0103` §3's stop
rule fires. **Named as the human's**: relaxing `R3` to permit the scale, raising the judged shape,
and any list of devices the product commits to.

`DEC-127` → [`ADR-0118`](../docs/adr/ADR-0118-a-recovering-browser-shows-nothing-it-was-not-told.md)
on 2026-09-04 — **a recovering browser shows nothing it was not told, and a late screen is not a
false one.** Open for a few hours, and answered as **two decisions rather than one**, because
`STORY-1310`'s three sightings — `P2` mid-runout, `P4` a plain reload, `P6b` a mailed link with no
reload at all — are **two windows of different kinds** sharing one sequence. The rule that decides
both, in opposite directions: **the client shows what the server told it, and before it is told it
shows nothing.** **Window one, before any frame** (`ADR-0114` §5's `unknown`): the client paints the
front door, a screen the server stated nothing to support. What turned that from an ugly flicker into
a defect was read out of `DuelSocket.kt` rather than argued — **`replyToCreateRoom` performs no seat
check**, `DEC-110` being open precisely because that guard does not exist, so *Create a duel room*
pressed inside the window is honoured, repoints this connection's `RoomMembership`, and `boot.ts`
overwrites the remembered code on the `RoomJoined` that comes back; the player's duel is still theirs
on the server and their tab no longer knows its code, and `ADR-0113`'s sweep plays the seat from
there. **Wrong-because-false**: `/` now renders **no element** — the dark `App.tsx` shell, empty, no
wordmark, no doors, no profile strip, no room code, no spinner, **no new string** — and the silence
ends on the first frame that answers the ask, `RoomJoined` (the room's screen) or `Failure` (the
front door **carrying** *"No duel room has that code."*). **Window two, after `RoomJoined` and before
the frame that states the duel**: the stale *Waiting for your rival* is **acceptable-because-late**,
and the row asked for that to be said out loud rather than left to fall out, so it is — accepted
product behaviour, not an unrepaired defect, and no ticket repairs it. Three reasons: it is a state
the server **did** state (`ADR-0102` §5's standard, met); the exposure is one delivery's frame gap,
which shrinks with the artifact rather than growing; and **the client cannot do better** —
`replyToJoinRoom`'s `ALREADY_SEATED` branch answers a host rejoining a still-`WAITING` room with
`RoomJoined` **and nothing after it, ever**, so any rule that waits for a second frame hangs that
host for the whole of their wait, and closing the window exactly needs the server to name the room's
state, a wire change and the architect's. **No interstitial**, on *"Dark, quiet, fast, minimal."* plus
`ADR-0105` §4's merged discipline — and the answer **says what it depends on**: the `built` re-read
`ADR-0117` §6 already owes is the test, and `/` empty for **longer than about a second** on a normal
connection makes the interstitial question live again as its own ADR. **Qualifies `ADR-0114` §2 by
one condition** — the fall-through is reached on the `none` standing, not on `unknown` — and moves
nothing else: no wire, no `PROTOCOL_VERSION`, no server file, no `Screen` member, no card owed
(`ADR-0091` §2: nothing is drawn). **Costs named rather than implied**: a recovering browser is a
**black rectangle**, strictly worse than today on a slow connection where the front door at least
loaded the profile strip, and **permanently** so against a server that answers neither frame — named
and deliberately **not registered**, `ADR-0105` §6's route, *a `DEC` nobody is working is noise in the
open table*; and the accepted stale waiting screen ships with a **destructive control** on it, *Back
to the lobby* forgetting the room over a running duel. `STORY-1311`'s tickets owe the silence, the
room-free boot that proves it is a rule and not a deletion, both frames that end it, and a test that
**pins the accepted half** so no later ticket answers `DEC-127` the other way.

`DEC-087` → [`ADR-0117`](../docs/adr/ADR-0117-the-proofs-of-record-load-the-built-bundle.md)
on 2026-09-04 — **the proofs of record load the built bundle, on an origin of its own.** Open since
2026-08-30, when `ADR-0093` §1a made *ready for real users* conditional on it and its row said it
blocked nothing; `TASK-131006` falsified that the day before this answer, turning the question from
*which artifact a record describes* into *whether a class of drive exists at all* — under
`npm run dev` the browser's connection to Vite carries the app's `/ws` **and** Vite's HMR client, so
`P3`'s transport cut takes both and the page reloads before `reconnecting.ts` can be seen recovering
in place (4 of 4, observed by navigation type and by a marker gone at `t+25ms`, inside
`retryDelayMillis`'s first attempt). **`vite preview` serves `dist/` on `http://localhost:4173`.**
Vite is already pinned by `ADR-0026`, so **no dependency is added** and `ADR-0088` §1's refusal
stands; `vite.config.ts` gains `preview: { port: 4173, strictPort: true }` and deliberately **no
`preview.proxy`**, because Vite resolves `preview.proxy ?? server.proxy` and one table cannot
disagree with itself — `dev-proxy.test.ts` asserts all three, the `undefined` proxy being the one
with teeth. `strictPort` is load-bearing: a leftover listener becomes a refusal to start rather than
a silent origin nobody named. **The mode is proved off the artifact, never off the port** — the dev
server injects `/@vite/client` into its HTML and the built `index.html` never names it, so the
marker *is* the mechanism that broke `P3`. `scripts/qa/stack.sh` gains `build-web` (foreground, so a
failed build is an exit code rather than a background task that never listens) and `web-origin`, and
**`wait-web [built|dev]` defaults to `built`** and fails on a marker mismatch — the check goes there
because every caller already invokes it. **`ADR-0088` §2 step 3 alone is superseded**, through that
ADR's own §5 route: `npm ci && npm run build && npm run preview`, with the marker as a third failure
condition; steps 1, 2 and 4–11 stand byte-unchanged, and step 4's two-`pd.deviceId` check is
unaffected because it compares two partitions with each other, never with a remembered one.
**`npm run dev` stays for development and loses only the proofs of record**: the hand-check is
`built` always, every QA/UAT/audit round started after this merges is `built` by default, a `dev`
round names itself and may not be offered toward `ADR-0093` §1a, and **every record names its
artifact beside its network layout** — `built, delayed 300ms` is complete, `bare` alone is not. §6
extends `ADR-0089` §4 by one clause in the same shape: **a `dev` finding whose conclusion turns on
load latency is re-read on `built` before it is filed**, because the dev server answers a page in
~102 requests where the bundle answers it in ~3 — `102 / 6 × 600 ms ≈ 10 s` is the arithmetic behind
`P2`'s observed 11.8 s first paint, a property of the instrument and not of the client. **No gate is
created and none may be read into it**: `ADR-0089` §2b stands unamended, `build.yml` keeps its two
jobs, and no PR, `verify:` block or ticket waits on a drive. **Re-drives**: `TASK-131006` / `P3` on
`built`; `P4` finishes on the mode it started on; `P1`, `P2` and `P5` do not re-drive for this ADR,
though `P2`'s and `P5`'s `delayed` findings against `ADR-0112` §6 fall under §6 and are a separate
ticket. **Costs named**: a **stale `dist/` is undetectable** — the marker proves the mode, not the
freshness, and the mitigation is ordering, which is discipline rather than a mechanism; the origin
move makes every literal `5173` outside production code wrong and leaves `ServerConfig`'s
`DEFAULT_BASE_URL` naming an origin no proof uses, harmless while `NoRecoveryMailer` is bound and a
trap for the first drive that can follow a link; the production build drops `StrictMode`'s
double-invoke, so a round may disagree with its own history for neither a defect nor a repair, and
`ADR-0089` §4's classifier has no term for it; and the proxy stays in the path, so this proves the
**bytes** and not the serving path. The **storage-partition move is explicitly acceptable**, checked
rather than assumed: `ADR-0089` §3 and the `qa-cycle` skill make a fresh `mktemp -d` profile per
round mandatory, so no round has ever carried a partition across an origin — only a human's own
browser loses one, and arriving as a device the server has never seen is what a real user is.
Falsifies `ADR-0088`'s *"the built bundle stays unproven"* and spends `ADR-0096`'s *"the audit walks
`npm run dev` until `DEC-087` is answered"*; `docs/test-plan.md`'s *"every case runs against
`npm run dev`"* is corrected by the answering change. Registers **`DEC-126`** — what serves the
bundle in the **offered deployment** — which the ADR declines to pre-decide, because
`docs/architecture.md` says the deployment is *"Not decided yet."*
...
serving mechanism is **`DEC-087`, the architect's**, registered open by the same PR and
**answered on 2026-09-04 by
[`ADR-0117`](../docs/adr/ADR-0117-the-proofs-of-record-load-the-built-bundle.md)** — `vite preview`
serves `dist/` on `http://localhost:4173`, so fact a became meetable rather than met: the
proofs of record now *can* load the bundle, and whether a given record did is a thing that record
says. **Fact b: recovery is completable by a real user in
...
the phrase the human aimed it at; the bar waited on `DEC-087` (answered by `ADR-0117`) and
still waits on a transport, and on `DEC-126` for the deployment's own serving; §1b may cost a
...
| **[STORY-1216](stories/STORY-1216-round-1-epic-14-the-stack-served-a-tree-three-days-old.md)** Round 1 (EPIC-14) — the stack served a tree three days old, so nothing at `ea4bd05c` was measured — *schema 2; **the first round under the `qa` focus since `STORY-1206`, and the first round in this ledger that measured a different commit than it reported**. The dev server on `:5173` and the JVM on `:8080` were both rooted in `.claude/worktrees/agent-af32ec8cf22f8819c`, whose branch is at `c3e1a830` (2026-09-07) — **92** commits behind `ea4bd05c` on `web-client/` and `poker-server/`, and missing on disk the three files every failing finding is about (`RematchNotice.tsx`, `NameAsk.tsx`, `device-standing.ts`). Eight of nine findings are **harness** under [`ADR-0089`](../docs/adr/ADR-0089-a-browser-drives-this-client-for-a-qa-round-never-for-a-gate.md) §4 and excluded from `B(1)`; the ninth (date locale) contradicts no merged source and matches one, [`ADR-0061`](../docs/adr/ADR-0061-a-season-is-a-calendar-month-and-the-coin-never-resets.md) §Consequences. `B(1) = 0`, `B(0)` n/a, **no** baseline round — and `PASS` is **refused**, because `B(1) = 0` here means nothing was measured, not that nothing is wrong. Verdict **`STOP_INFRA`**. Registers `DEC-162` (the human's) for a finding made outside the round* | | | **ready** |
| | [TASK-121601](tasks/TASK-121601-the-harness-names-the-tree-it-is-serving.md) The harness names the tree it is serving — *harness fix set 1 of 3; a `served` verb resolving `:5173` and `:8080` to a checkout root and a short commit, read-only through `lsof`/`ps` and hermetically self-tested with no stack up. Measured red at `ea4bd05c`: `stack.sh served` exits **2** today* | S | done |
| | [TASK-121602](tasks/TASK-121602-a-round-records-the-tree-it-drove.md) A round records the tree it drove, not its own `HEAD` — *harness fix set 2 of 3; `manual-verify`. `.claude/agents/qa.md`'s `COMMIT: <git rev-parse --short HEAD>` is **removed**, not supplemented, and the gate is a count of **0** against **1** today, so it cannot be satisfied by adding text. The `qa-cycle` skill refuses a round whose served commit is not the commit under test. **Serialised behind `TASK-121601`**, which it reads* | XS | done |
| | [TASK-121603](tasks/TASK-121603-one-database-wherever-stack-sh-is-run-from.md) One database, wherever `stack.sh` is run from — *harness fix set 3 of 3; `compose` derives its project from the checkout basename and `docker-compose.yml` publishes a fixed `"5432:5432"`, so a worktree's `db-up` opens a second project against a bound port and dies. Three postgres containers exist on this machine, one per checkout that ever ran the verb. `docker-compose.yml` is **not** opened — `ADR-0089` §4 forbids a production change for a harness defect. **Serialised behind `TASK-121601`**, which touches the same two files* | XS | done |
| | [TASK-121604](tasks/TASK-121604-three-browsers-launched-back-to-back-all-bind.md) Three browsers launched back to back all bind — *deferred to the backlog by the round, not by the eight-cap: `qa` itself called it local contention and retried past it, and it is not what cost round 1* | XS | backlog |
| | [TASK-121605](tasks/TASK-121605-the-other-two-drivers-name-the-tree-too.md) The other two drivers name the tree they drove too — *filed 2026-09-10 while landing `TASK-121602`, whose coder reported the deferral without a ticket behind it. Measured: `.claude/agents/uat.md:118` and `.claude/agents/audit.md:107` still carry `COMMIT: &lt;git rev-parse --short HEAD&gt;`, the exact line that let a round report `ea4bd05c` while driving a tree 92 commits behind. Both agents drive the running product — `uat` walks the screens, `audit` walks a duel beat by beat — so either can be handed a stale stack by the same chain. **Named cost:** the repair is prose in three agent files rather than one gate, so `ADR-0091` §2's warning about a second copy drifting applies; the ticket's answer is to copy `qa.md`'s landed shape rather than invent a second phrasing* | XS | done |
| | [TASK-121607](tasks/TASK-121607-the-chip-flight-is-read-from-the-animation-not-the-frame.md) The chip flight is read from the animation, not the frame — *filed 2026-09-10 to give `EPIC-13`'s one open Definition-of-done box a mechanical route. Four of that box's five clauses were observed on the 2026-09-05 drive at `866ebc21` over 31 hands; the fifth — **chips that move** — is not drivable by screenshot, and the box says so itself: `--pd-motion-chip-flight` is a 420 ms one-shot, so its absence from a frame log is **not evidence of absence**. The box names the way out — *"a future driver can read `document.getAnimations()` immediately after an act"* — and this is that. Ships an **instrument**, not a claim: no gate names the motion token, because a gate pinning a motion's presence would be a coverage claim `ADR-0089` §§2b–2c refuses from a browser. The human's eye at `design/components/seat-and-pot.html` stays a valid route (`ADR-0115` §4); this stands beside it. **The mutation to probe:** make the verb return `[]` unconditionally and the positive control must go red — the same defect that produced this story's rejected `P3` row* | S | ready |
...
decisions. **`STORY-1301`–`STORY-1306` are cut into tickets** — three, seven, four, eight, six and
eight; the first five are done and the sixth is startable. The other five are not cut, because
...
| | [TASK-130407](tasks/TASK-130407-one-mark-at-the-seat-the-act-names.md) One mark, at the seat the act names, and it moves when the other seat acts — *`DuelTable` hands the mark to the plate whose `seat.index` the event names and `null` to the other — `ADR-0109` §1's *exactly one mark, never one per seat*, which §Alternative 2 forecloses rather than defers. The replacement test re-renders with a second act at the **other** seat and asserts one `.last-act`, inside the right plate, with the first act's figure nowhere on the table: a per-seat implementation fails all three. `seat: 1` and `seat: 0` both read 0 in this file today, so both literal gates are real. 24 → **27**, with `no-derivation` pinned at 7 and `SeatPlate` at 12 to prove neither was reached* | S | done |
| | [TASK-130408](tasks/TASK-130408-the-screen-feeds-the-mark-and-there-is-none-before-the-first-snapshot.md) The screen feeds the mark, and there is none before the first snapshot — *`review: deep` — the second story to amend `EPIC-13`'s standing contract, which `TASK-130206` landed under a `deep` review because `ADR-0110` §3 is `ADR-0002` applied. One attribute in `Lobby.tsx` and one test in `null-view.test.tsx`, in **one** diff because that test is the wiring's only proof: its positive control applies an `Events` act and a `Snapshot` to a real store under the real `Lobby`, so deleting the attribute reddens it. Both open questions are written down here — nothing on the null view, printed or spoken, and no mark across a refresh until the next act. `spoken(` pinned at ≥ 12, `digitBearing(` at ≥ 4, `acting-mark` at ≥ 3, `Lobby.test.tsx` at 80. 5 → **6*** | XS | done |
| **[STORY-1305](stories/STORY-1305-a-bet-amount-can-be-typed.md)** A bet amount can be typed, and an illegal one is refused in the server's own numbers — *item 7; `ADR-0111`; **split into six tickets on 2026-09-03**, one chain, `TASK-130501` startable. The split probed rather than reasoned, and measured four things. **Exactly one merged assertion is in the blast radius** — `ActionBar.test.tsx:141`'s `expect(container.querySelector("input")).toBeNull()`, `ADR-0100`'s *no slider* guard — found by planting the field and running the whole client suite: 1 failure, **1033 passes**, so `Lobby.test.tsx` (80), `whole-duel.test.tsx` (8), `no-derivation.test.tsx` (7) and `null-view.test.tsx` (6) are all untouched and gated to prove it. `TASK-130504` owns that line and replaces it with the narrower guard it was always trying to be. **The field is `type="text"` with no `min` and no `max`**: planting `max={allInTo}` reddens `bar-no-derivation.test.tsx` (`expected [ 175, 175, 13400, 175 ] to not include 13400`), so that file's 3 is a live gate against a bound-carrying control — while `min={floor}` reddens nothing, which is why the field's own test asserts `min`'s absence directly. **The refusal shares the bar's existing notice line and the local sentence wins while it stands**, so no fourth row is added and `ADR-0103` §1's 390 × 664 fit is out of the diff by construction; moving `<Notice>` inside `Live` was probed and left **1034 of 1034** green. **The typed field does not exist when `view === null`**, for three independent reasons — `Lobby.tsx` mounts `WaitingTable` and no bar at all, the field lives in `Live` which needs a turn, and inside `Live` it needs `amountFloor(actions) !== null` — so a `Snapshot` alone is not enough, which is the boundary `TASK-130506` pins. The cards keep the stepper's ± buttons (`DEC-102` is open and stays the product owner's), so the client ships a **subset** of what the phone frame draws and the fit judgement is conservative in the right direction* | | | **done — all six tickets merged on 2026-09-03.** A bet amount can be typed: `readTypedAmount` decides, the bar carries the field, and an illegal entry is **refused in the server's own numbers** — never clamped, never sent, with the player's own text left exactly as typed. `"1,200"` is accepted, because the field displays grouped figures and the product must not refuse the text it just showed. `ADR-0100` §5 held throughout: no `data-testid`, no test-only prop, and the e2e driver still presses what a player presses. **Open gap, found by the last deep review and recorded rather than lost:** a `YourTurn` whose `allowed` excludes both `RAISE` and `BET` makes `amountFloor` null, so the typed field should be absent **even though a turn was named** — but `ActionBar.test.tsx`'s test for that state asserts only that the sizing chips are empty, never that the `the total` textbox is gone, and no test in either file does. Reachable, pre-existing, and one contract over from `TASK-130506`'s. Worth a follow-up ticket. |
| | [TASK-130501](tasks/TASK-130501-the-action-bar-card-draws-the-typed-total-and-its-three-refusals.md) The action-bar card draws the typed total, and the three refusals it can express — *design first (`ADR-0091` §2, `ADR-0111` §7, `EPIC-13`'s rule). Three new frames on `design/components/action-bar.html` — `500` under the minimum, `20,000` over the stack, `1o0` not an amount — each showing the entry **exactly as typed**, a `Raise to` button with **no figure**, and the merged sentence on a notice line the card has never drawn. The field takes the stepper's readout slot and the ± buttons stay, because `DEC-102` is open; the client ships the field alone, so the row it must fit is narrower than the row drawn here. Look, border, padding and width stay the human's (`ADR-0024` §3). `Raise to <span class="amt">` pinned at 2 so a corrected total can never come back through the paint* | S | done |
| | [TASK-130502](tasks/TASK-130502-the-two-table-cards-carry-the-typed-total-in-place.md) The two table cards carry the typed total in place, at the phone as well as the laptop — *`.total` copied character for character into both screen cards, as `.last-act` and `.seat.on-turn` were — gated by grepping the rule out of all three files, `sort -u`, and failing unless exactly one line comes back. Two fields on `duel-table.html` (the laptop and the 390 × 664 phone frame, `value="3,650"`) and three on `duel-table-states.html` (`value="3,250"`, in `bar off` rows whose hidden sizing row must keep mirroring the live one). `aria-label="the total"` is the client's own name, so card and control answer to the same one. No frame added: `viewport phone` 5, `class="frame"` 6/3, `class="chip` 10/15, `@keyframes` 1/1 all pinned* | S | done |
| | [TASK-130503](tasks/TASK-130503-what-the-bar-makes-of-what-the-player-typed.md) What the bar makes of what the player typed — *`review: deep`; `readTypedAmount(entry, floor, ceiling)` returning a discriminated `TypedAmount`, and the one place `ADR-0111` §Consequences' unpoliced direction lives: a reading stricter than the server's rule refuses a legal amount and **no server message ever says so**. `parseInt`, `parseFloat` and a `Number(entry)` reading are refused by name in the gates — `1e3` is 1000 and `0x10` is 16 to all three — so the shape is read by regular expression first. The sentences are **shared, not copied**: the module calls `rejectionText` with a locally built `Rejection` (a formatting argument, never a frame), and gates refuse the templates appearing in the file at all. Eight tests, both interval endpoints among them* | S | done |
| | [TASK-130504](tasks/TASK-130504-the-bar-carries-a-typed-total-and-a-refused-one-sends-nothing.md) The bar carries a typed total, and a refused one sends nothing and says why — *the entry becomes the bar's only amount state, presets write into it, and a `BET`/`RAISE` press with an unsendable entry returns before `setSent` and before `send` — the other four buttons are untouched, which its fourth test proves by folding through a garbage entry. The button prints the proposal or nothing, never a different amount (`ADR-0111` §7, `ADR-0100` §2). `<Notice>` moves inside `Live` and gains an `entryRefusal` that wins while it stands: no fourth row, so the phone fit stays out of the diff. Owns and replaces `ActionBar.test.tsx:141`. 26 → **30*** | S | done |
| | [TASK-130505](tasks/TASK-130505-both-ends-of-the-interval-the-repeated-refusal-and-the-button-that-never-lies.md) Both ends of the interval, the repeated refusal, and the button that never prints a different amount — *test-only, one file, split from `TASK-130504` for size and nothing else. Six tests: **six sends across two intervals** driving `floor` and `allInTo` exactly (the ADR's named unpoliced direction, driven through the real bar), the over-stack sentence, the plain `0` against an empty field and a negative, a refused press repeated with no lock taken, a `Raise to` that carries no digit while refused and its figure back when fixed, and the server's own word surviving beside the local one. 30 → **36*** | S | done |
| | [TASK-130506](tasks/TASK-130506-there-is-no-typed-total-before-the-server-has-named-a-turn.md) There is no typed total before the server has named a turn — *`review: deep` — the third amendment to `EPIC-13`'s standing contract, after `TASK-130206` landed it and `TASK-130408` amended it, both deep for the same reason: `ADR-0110` §3 is `ADR-0002` applied to the empty table. One test walking three states, and the middle one is the point — **a `Snapshot` alone is not enough**, because this surface waits for a `YourTurn` and not for a view. `spoken()` is closed to `[]` on the null view only; past that it stays `> 0` probes, which is the trap `STORY-1304` recorded. `YourTurn` reads 0 in this file today, so the gate is real. 6 → **7*** | XS | done |
| **[STORY-1306](stories/STORY-1306-a-stack-is-chips-and-chips-move.md)** A stack is chips, and chips move — *item 6; the chip is **minted** interactively (`ADR-0091` §3); **split into eight tickets on 2026-09-03**, one chain, `TASK-130601` startable. `DEC-124` was answered and merged on 2026-09-02, so **nothing here is blocked**. The split probed rather than reasoned and settled six things the story left open or got wrong. **The flight's duration is a `--pd-motion-*` token, not a store parameter**: `ADR-0115` §4 is merged and later than the story's note, and its §3 draws the line — *a step changes what the screen states; motion is how the change is drawn*. So the flight is a *how*, takes no place in `ADR-0102` §1's queue, needs no seam, and `boot.ts`, `drive-duel.tsx` and the four recorded-frame suites are untouched **by construction** rather than by a `0`. `ADR-0102`'s trap is escaped by its own named escape and no architect's `DEC` is owed. **The blast radius is empty, measured**: a silent `aria-hidden` pile planted in `SeatPlate`, `PotStrip` *and* `DuelTable`'s bet line at once left the client suite at **1053 of 1053** green, so no merged test file is in any ticket's budget; a pile that *spoke* the seat's own stack reddened exactly one test (`SeatPlate.test.tsx`'s `speaks the mark to nobody`) and one that spoke a **derived** figure reddened `no-derivation.test.tsx` twice — both are gated because both are live. **A pile is never a count**: three discs at every amount, because a pile sized from a figure invents a denomination the server never sent; a growing pile is a repair ticket *and* a product owner's `DEC`. **`ADR-0107` §4 is half-answered by `ADR-0115` §1** — the stack numeral cannot go, only the rival's bet-line is open, and `TASK-130603` draws the retired alternative while the client ships the additive half. **The chip is minted in the sheet, not as an SVG**, following the half of the coin's precedent the client already uses (`--pd-coin-face`); an SVG would owe `check-drift.sh` clause 4 pairs and a manifest entry no ticket can make. Also recorded: `ADR-0091` §4's **fourth client guard does not exist yet** — nothing refuses a raw length in a Tailwind arbitrary value, and `DuelTable.tsx` carries several today* | | | **done — all eight tickets merged on 2026-09-03.** A stack is chips and chips move: the chip minted (`EPIC-13`'s only new art), both table cards carrying piles, and `ChipPile` drawn at the seat's stack, the pot and the rival's bet line. **`ADR-0115` is satisfied structurally rather than by a second drawing:** `pd-chip-flight` runs from an offset to `transform: none`, so the final frame *is* the no-animation frame, and **nothing is ever animated out** — chips sitting where they no longer are cannot arise. Every pile sits beside the numeral that states the amount, so the fact never lives in the motion. The pile is a **fixed three discs**: a pile varying with the amount would imply chip **denominations**, which the split refused to invent and recorded as needing a product owner's `DEC`. **Recorded for the next story that adds a chip surface:** the null-view control is deliberately aggregate — `> 0`, never a count — because a per-surface count would couple this nine-story contract file to each surface's pile composition. Per-surface regressions belong in each surface's own test file, and `SeatPlate.test.tsx` caught one during review. **Open for the human's eye:** `TASK-130603` drew both answers to `ADR-0107` §4 — whether `committed N` survives once chips sit beside it — and the client ships the bet line as it stands until that verdict. |
| | [TASK-130601](tasks/TASK-130601-mint-the-chip-and-draw-the-pile-at-rest-in-flight-and-stilled.md) Mint the chip, its flight values, and the pile at rest, in flight and stilled — *minting worked with the human (`ADR-0091` §3), the sheet and the component card in one diff for `TASK-130301`'s reason: a size and a duration chosen before anything is drawn at them cannot be corrected by a card ticket that does not hold the sheet. Six tokens — `--pd-chip-face`/`-edge`/`-size`, `--pd-motion-chip-flight`/`-ease`/`-travel` — and **no second reduced-motion block**, because the merged one already stills `animation` and `transition` product-wide. Seven rows on `seat-and-pot.html`, two of them the same pile at `13,400` and at `150`, which is `ADR-0115` §1 drawn: a pile is never a count. `@keyframes pd-chip-flight` runs **from** an offset **to** the resting frame and nothing is ever animated out — the one shape whose stilled form would be wrong. `class="chip"` is the sizing button on both screen cards (10 and 15), so the drawn chip is `.disc` inside `.pile`* | S | done |
| | [TASK-130602](tasks/TASK-130602-the-two-table-cards-carry-the-pile-and-the-host-alone-frames-carry-none.md) The two table cards carry the pile, and the host-alone frames carry none — *transcription. Eight piles on `duel-table.html` (both live frames: stack, bet line, pot, stack), seven on `duel-table-states.html` — three at rest in the waiting frame, and **two `pile flying` at the hero's stack** in the two ending frames, which is the story's *stack receiving an award*. Those two frames get **no pot pile**: `You win 4,850` stands where the figure was and the chips have already left. The refusal is a region gate — `awk` counts `class="pile` from the first `Host alone` heading to end of file and requires zero, which is `ADR-0110` §3 made mechanical. Nothing moves: `class="frame"` 6/3, `class="chips"` 4/6, `class="pot"` 2/3, `class="bet-line"` 2/3 all pinned* | S | done |
| | [TASK-130603](tasks/TASK-130603-the-card-asks-whether-the-bet-line-survives-the-chips.md) The card asks whether the bet-line survives the chips, and answers half of it already — *one new 390 × 664 frame, `Phone — the bet-line retired`, inserted before the first `Host alone` frame so the region gate stays true. **`ADR-0115` §1 settles half of `ADR-0107` §4 without a verdict**: a stack numeral cannot go, because a stack stated only as a drawing is a fact living only in a drawing. Only the bet-line is open, and only because its 400 is already inside `Pot 2,850`. The load-bearing gate is `class="bet-line"` staying at **2** — a frame that kept the div and hid its figure would look right in the pane and answer nothing* | S | done |
| | [TASK-130604](tasks/TASK-130604-a-pile-of-chips-is-a-drawing-and-it-says-nothing.md) A pile of chips is a drawing, it arrives, and it says nothing — *`ChipPile` takes **no props at all**, which is what makes a denomination unrepresentable rather than merely forbidden. `app.css` gains `@keyframes pd-chip-flight` and three `@utility` rules **outside** `@theme static` (`theme.test.ts` reddens on anything else in that block). **A bundle grep for a class name proves the class exists, never that it styles anything** — `TASK-130406`'s counterfactual — so the gates count token **occurrences** in the built CSS and require ≥ 2: one is the sheet's declaration, the second can only come from a body that emitted. Measured on the merged precedent at this commit: `--pd-motion-turn-period` and `pd-acting-seat` each read exactly 2* | S | done |
| | [TASK-130605](tasks/TASK-130605-a-seats-stack-is-drawn-in-chips-and-the-numeral-still-says-how-many.md) A seat's stack is drawn in chips, and the numeral still says how many — *one keyed `<ChipPile />` before the stack span; the key is the only mechanism, so a frame that leaves the stack alone interrupts nothing. `> 0`, not truthiness: a busted seat draws no pile and still reads `0`. Three tests, and the second is the denomination gate — `stack: 150` and `stack: 13400`, **neither the fixture's default of 500**, each rendering exactly three discs. 12 → **15**, with `no-derivation` 7, `DuelTable` 27 and `null-view` 7 pinned unmoved* | S | done |
| | [TASK-130606](tasks/TASK-130606-the-pot-is-drawn-in-chips-and-the-figure-still-names-the-total.md) The pot is drawn in chips, and the award line takes them away again — *the pile's condition is `awardLine === null && total > 0`, driven with a **non-zero** `pot: 4850` so the guard is load-bearing and deleting it puts a pile back. The sum is hoisted to one `const total` feeding both the figure and the key, so `potCommittedToTheHand` stays at 2 occurrences and `ADR-0107` §5's *exactly one* holds. The third test carries both halves — award line present, then the same view with no narration — so it cannot pass because the selector matches nothing. 7 → **10*** | S | done |
| | [TASK-130607](tasks/TASK-130607-a-bets-chips-stand-at-the-bet-line-and-there-is-still-only-one.md) A bet's chips stand at the rival's bet line, and there is still only one bet line — *`p .chip-pile` is an exact selector because `DuelTable` renders exactly one `<p>`. The hero has no bet line and gains none: their commitment is inside `ADR-0107`'s total, and a second line would be a new surface on a column standing 0.09375 px from the fence. The third test pins `querySelectorAll("p")` at 1 so the asymmetry is a decision on record, not an omission somebody repairs. `whole-duel.test.tsx` (8) is pinned by name — no frame is re-recorded, because the flight is CSS over a change that already happened. 27 → **30*** | S | done |
| | [TASK-130608](tasks/TASK-130608-no-chip-before-the-server-has-named-a-stack.md) No chip before the server has named a stack, and the pile is a drawing after — *`review: deep` — the fourth amendment to `EPIC-13`'s standing contract, after `TASK-130206`, `TASK-130408` and `TASK-130506`, all deep because `ADR-0110` §3 is `ADR-0002` applied. **A stack in chips is a game fact**, and it renders nothing before the first `Snapshot` because `Lobby.tsx` mounts `WaitingTable`, which mounts none of the three components that draw one. Unlike the typed total there is **no middle state** — the pile waits on the view and nothing later — and the ticket says so rather than leaving it unchecked. It also takes the narrow closure `STORY-1304` recorded as the right shape: after the Snapshot, no `.chip-pile` or descendant carries an `aria-label` or `title`. The general `spoken()`-after-Snapshot repair stays a follow-up. 7 → **8*** | XS | done |
| **[STORY-1307](stories/STORY-1307-the-turn-clocks-card.md)** The turn clock's card — regular, running out, on timebank, expired — *item 4a; `ADR-0108` §5; the half no decision blocks; **split into four tickets on 2026-09-03**, one chain, `TASK-130701` startable, **no `DEC` raised**. The split settled the story's hardest question and measured five things. **The still form of a ticking clock is the clock.** [`ADR-0115`](../docs/adr/ADR-0115-motion-never-carries-a-fact-and-reduced-motion-stills-every-surface.md) §3 already names the clock by name — *"each second's numeral is a step. A smooth sub-second depletion drawn between them is a how, and is what a reduced form skips"* — so a numeral changing each second is information arriving, not motion, and a reduced-motion player reads the same figure at the same second as everyone else. Taken **structurally**, as `STORY-1306` took `ADR-0115`: the clock **animates nothing at all**, with `@keyframes`, `animation:`, `animation-` and `transition:` pinned at their present counts on all three cards, so no keyframe, shorthand or longhand can reach it and the still form is byte-identical by construction rather than by care. `TASK-130701` draws the *at rest* row anyway, because a reader who does not know this assumes a clock must tick. **The clock composes; minting is contingent, not assumed.** `design/tokens/tokens.css` already declares `--pd-warn: #c99a4a` under *"Amber exists only for the turn clock running down"* and already says the clock is mono with tabular figures — the vocabulary was minted before the clock existed. `--pd-warn` appears in **no** card and **no** client file today, so this card is its first use and a gate requires it; the two sheets stay in `TASK-130701`'s budget only in case the pane wants a value the sheet lacks. **`ADR-0108` §5 and `ADR-0113` §6 word *expired* differently** — the story glosses §5 as *the state after the frame*, while §6 says the seat *"reads as expired… until a server frame carries the consequence"*. They name two consecutive moments, so `TASK-130702` draws **both** and captions which clause each row is, rather than picking one silently. **The merged `reconnecting…` row is wrong twice** and is deleted, not edited around: `reconnecting…` is not a string this product ships (`ADR-0046` §1 ships `Away` and `seat-status.ts` returns it), and its caption cites `ADR-0013`'s grace window, which `ADR-0113` §7 deletes outright. `\.seat\.away` is then pinned at **2** — the two merged rules that dim a name and a stack — which is `ADR-0108` §1's *"dropping the connection is never a way to gain time"* expressed as a count, because a third rule reaching `.clock` would say in CSS that a dropped socket changes what a seat is owed. **Counts were re-measured, not carried over:** `duel-table.html` is 7 frames and 6 `class="chips"` today, not `TASK-130602`'s 6 and 4 — `TASK-130603` added a frame after that ticket was written. Also settled: the bank's label is **`Timebank`**, the human's own word from the feedback, in the shipped `Pot 2,850` idiom; bare `Bank` is refused because `docs/vision.md` *Positioning* refuses *bankroll* and a lone `Bank` beside a chip stack reads as money. The replacement for *"The duel is paused."* is **not** this story's — `ADR-0108` §5 gives it to the story that lands it, against this card. And the 390 × 664 fit is a PR statement and a pane verdict, never a gate (`ADR-0089` §2b): the plate now carries seven children, and running out of `ADR-0103` §3's give list is a `DEC` to register, never a figure to shrink* | | | **done — four tickets, one chain, four merged.** The 390 x 664 fit was *measured* at landing, not asserted: inside the card's own `.viewport.phone` all twelve phone seats compute `flex-wrap: nowrap`, nothing escapes the plate and nothing drops to a second line; forcing a 21-character name makes the **name** ellipsise while the clock and both banks stay inside the plate and its height holds at 67px. So `ADR-0103` §3's give list is not exhausted and no `DEC` is owed. One drift left for the record: this row says the plate carries **seven** children, and the shipped card renders **five** at your seat and **six** at the rival's, the dealer button being the sixth — the prose over-counted, the card is right. No `DEC` raised |
| | [TASK-130701](tasks/TASK-130701-the-clocks-vocabulary-and-the-fresh-allowance.md) Draw the turn clock, its two allowance states, and the timebank at both seats — *the vocabulary ticket, minting-capable and worked with the human (`ADR-0091` §3), with both token sheets in the budget in case the drawing needs a value the sheet lacks — if nothing is minted they are untouched and the gates pass either way. Four rows on `seat-and-pot.html`: *regular*, *running out*, *at rest* and *not on turn*, the last carrying a different bank figure so one value cannot pass for two seats. `.clock` / `.clock.running-out` / `.timebank` are born here; `@keyframes` 2, `animation:` 4, `animation-` 4 and `transition:` 0 are pinned, which is the whole of `ADR-0115` §1 for a clock* | S | done |
| | [TASK-130702](tasks/TASK-130702-the-bank-spending-zero-and-the-frame-that-follows.md) Draw the clock on the timebank, holding at zero, and the plate after the server acted — *three rows: `on timebank` with the countdown and the bank figure as one number, `expired` holding at zero with the turn **still** on the seat, and the plate after the frame lands, marked `Check`. Two zero rows because `ADR-0108` §5 and `ADR-0113` §6 name two consecutive moments. No word for zero, no colour change on the status line, no control that becomes disabled — `ADR-0046` §3 re-applied* | S | done |
| | [TASK-130703](tasks/TASK-130703-an-away-seats-clock-and-an-absent-seats-lack-of-one.md) Draw ADR-0108's presence table, and retire the grace window's row — *deletes the `reconnecting…` row and its `ADR-0013 grace window` caption and draws §4's three regimes in their place: away **on** turn with a countdown byte-identical to a present seat's, away **not** on turn with none, and `Timed out` with none and a spent bank. `Fold` beside `TASK-130702`'s `Check`, because one act cannot show the conduct is read from the legal set* | S | done |
| | [TASK-130704](tasks/TASK-130704-the-two-table-cards-carry-the-clock-and-both-timebanks.md) The two table cards carry the clock and both timebanks, and the host-alone frames carry none — *transcription: the five `.clock`/`.timebank` rules copied into both screen cards, one countdown at the seat on turn, both banks at every live frame, and two region gates counting zero of each from the first `Host alone` heading — `ADR-0110` §3 made mechanical. The laptop and phone frames stay the same markup (`ADR-0103` §4). This is the last card in the epic's sequence and it proves `ADR-0103` against everything merged before it; the fit is stated in the PR and judged at the pane, and a plate that will not hold is a `DEC`* | S | done |
| **[STORY-1308](stories/STORY-1308-the-server-states-a-deadline-and-plays-the-expired-seat.md)** The server states a deadline and plays the seat whose clock ran out — *item 4b; the only wire move in this epic; `ADR-0113`; **split into twelve tickets on 2026-09-03**, one chain, `TASK-130801` startable, **no `DEC` raised**. The split ran `ADR-0070`'s probe to **green** before writing a line of it, and the run corrected two guesses. **The bump is thirty-three files, and it is one ticket.** `TASK-130805` carries the wire and nothing that sends it: the frame is declared, `graceRemainingMillis` and the `DUEL_PAUSED` enum entry are removed, the version steps, and the refusal leaves `Room.act`. The stub was `ADR-0113` §9's four declarations applied together; the loop took **seven** Gradle rounds and **eleven** client rounds, and two paths surfaced only after an earlier failure was cleared — `web-client/src/e2e/scripted-duel.gen.json`, held by a `:poker-server:verifyDuelScript` byte-compare nothing in `ADR-0113` mentions, and `web-client/src/table/PresenceNotice.tsx`, reddened by TS2741 only once `Lobby.tsx` had already been fixed. A red run named a prefix every time. **The client half of the radius is real and was not obvious**: eleven merged client tests die with the field that fed them — six in `Lobby.test.tsx`, four in `PresenceNotice.test.tsx`, one in `ActionBar.test.tsx` — and the ticket names each and pins the per-file counts at 74 / 4 / 35, with `duel-state.test.ts` held at 75 because no test there is removed. **`docs/test-plan.md` is its own ticket** (`TASK-130812`): `CORE-23` asserts the very `DUEL_PAUSED` refusal this story deletes, no test reads that document, so no gate would ever redden — and the split found `CORE-22` cites `RoomTimeouts.DEFAULT_DISCONNECT_GRACE_MILLIS`, a constant `TASK-130810` deletes, which `ADR-0113` §9 did not name. **Two further tickets are `atomic:` and say plainly that they are not probe-green**: `TASK-130809` (9) and `TASK-130810` (17) are renames whose counts are `grep -rl` reference sets, and neither may be corrected under `ADR-0070` §4, whose closing sentence excludes *a rename, a refactor* by name — the planner's split pointed both at §4 and the driver corrected them on landing. The gap is structural, not an omission: each ticket's baseline is the tree its predecessor leaves, which does not exist at planning time, so no probe could have reached green, and `ADR-0113` §9's stub is the **wire's** declarations only. **The driver re-runs `ADR-0070` §1's loop against that baseline and replaces each table before dispatch**, as `ADR-0070` §5 re-sized `TASK-021301` after it was written; a coder finding a path the table lacks stops and says so rather than widening it. `ADR-0047`'s one-bumping-branch lock was checked against the whole board and is **uncontended**: no open PR and no branch holds a bump, and the ledger's last row is 5* | | | **done — twelve tickets, twelve merged.** The epic's only wire move: `PROTOCOL_VERSION` 5 to 6, spent once. **Five file tables were short**, and how each was found is the story's own lesson: `TASK-130805` 33 to 35 and `TASK-130809` 9 to 10 and `TASK-130810` 17 to 18 were caught by re-probing before dispatch, at the cost of a grep apiece; `TASK-130806` 2 to 4 and `TASK-130807` 3 to 5 were caught by a blocked coder, at the cost of a dispatch apiece. A count computed against one baseline is short by whatever landed after it, and `TurnClockFramesTest` alone broke two of them because `TASK-130806` created it after both were written. **Two exact-count gates actively hid defects.** `TASK-130809`'s pin of 19 made its coder delete a test it had just used to verify a fix, and the hazard went unpinned until review found a worse one beside it: a `FINISHED` room's stale runner reached `Room.finish`, threw, and leaked `recording[code]` set on the line before — leaving the room un-reapable and rematch-refusing whenever a player closed the tab within four minutes of a duel ending. The gate went to 21. `TASK-130810`'s acceptance let a coder retire a test its own gate pinned; settled before dispatch. **Two findings from `TASK-130804` were carried rather than lost.** **No test in this story yet separates a *derived* deadline from a field written once by a correct caller** — the coder said so plainly instead of overclaiming, and review confirmed it by mutation: removing the unchanged-guard reddens exactly one test and no other. `clocked` takes `runner` explicitly, so the risk `ADR-0113` §3 actually guards — a caller that forgets to re-derive once the runner advances — is only reachable from `RoomRegistry`, and **`TASK-130806` and `TASK-130807` owe that test**. Second, `TASK-130804` handed the coder an engine-typed parameter (`DuelRunner?`) with no test-construction guidance, forcing four reads outside its *Files* budget for API shape alone; the diff stayed clean, but a split naming an engine type should name the file that shows how to build one |
| | [TASK-130801](tasks/TASK-130801-one-seats-given-up-decision-is-one-function.md) Extract one seat's given-up decision from foldAbsent's loop — *pure refactor, no wire, no clock: `ADR-0113` §4's `giveUpDecision(step, seat, seeds)`, so the expiry reuses `ADR-0023`'s conduct as code rather than re-implementing it. Gated on `legalActions(` staying at exactly **1** in the file, which is the second implementation refused as a count* | XS | done |
| | [TASK-130802](tasks/TASK-130802-room-timeouts-carry-the-turn-allowance-and-the-timebank.md) RoomTimeouts carries the turn allowance and the timebank — *additive: `turnMillis` (30 s) and `timebankMillis` (3 m), both required positive, both defaulted so a reaping test states no clock. `disconnectGraceMillis` stays until `TASK-130810`, so nothing merged moves* | XS | done |
| | [TASK-130803](tasks/TASK-130803-the-turn-allowance-and-the-timebank-are-configuration.md) The two numbers are read from configuration at startup — *`duel.turnMillis` / `TURN_MILLIS` and `duel.timebankMillis` / `TIMEBANK_MILLIS`, resolved through the helper the window already uses. The environment-outranks-key test states **both** numbers at **different** values, because one resolver serving both cannot be told from two by a single value* | S | done |
| | [TASK-130804](tasks/TASK-130804-the-room-derives-the-deadline-and-debits-the-bank.md) The room derives the deadline and debits the bank by arithmetic — *`TurnDeadline`, `turnDeadline`, `timebankRemainingMillis` and `Room.clocked(runner, now, turnMillis)` — `ADR-0113` §3's two expressions and nothing else. Additive with defaults, so every merged `Room` test stands. Two seats and two overruns, deliberately: one seat and one value cannot tell a debit from a constant. `review: deep`, and a refusal gate proves `Room.kt` still names no clock source* | S | done |
| | [TASK-130805](tasks/TASK-130805-the-wire-states-the-turn-clock-and-the-version-takes-its-step.md) TurnClock reaches the wire, the pause leaves it, and PROTOCOL_VERSION takes its step — ***`atomic:` 33, and the number is a green probe's `git status`*** — `ADR-0070` §§1-2 run to exit 0 on 2026-09-03 with Docker up so `-PrequireDocker=true` skipped nothing. Ten gate classes are named, including two byte-comparing Gradle tasks, ktlint's unused-import rule and `tsc`'s TS1360/TS2322/TS2339/TS2353/TS2367/TS2741. It sends nothing: `TASK-130806` is the first ticket that emits a frame. Neither the version number nor the fingerprint is written anywhere in advance (`ADR-0047` §6) — the ticket runs the ledger test **expecting red** and pastes what the failure hands it | S | done |
| | [TASK-130806](tasks/TASK-130806-every-decision-point-restarts-the-clock-and-states-it.md) Every act write-back restarts the clock and states it to both seats — *one `TurnClock` per write-back, to **both** seats, **last** in the batch — a batch that plays three absent seats through carries one clock, for the decision the duel came to rest on. Gated on `clocked(` appearing exactly **once** in `RoomRegistry.kt`, so the deadline is restarted in one place and never re-derived beside it* | S | done |
| | [TASK-130807](tasks/TASK-130807-a-fresh-duel-refills-both-banks-and-a-resume-restates-the-clock.md) A fresh duel refills both banks, and a resume restates the clock — *`withFreshRunner` refills both banks and clears the deadline — a rematch is a new duel; `resume` states the live deadline again from the same `now` its presence frames already read. Two merged assertions move and the ticket names both: `RoomResumeTest`'s outbound size 2 → 3, and `RoomRematchTest`'s whole-list comparison, rewritten rather than deleted* | S | done |
| | [TASK-130808](tasks/TASK-130808-the-room-gives-up-the-turn-of-a-seat-out-of-time-or-absent.md) The room gives up the turn of a seat that is out of time or absent — *`foldAbsentSeats` → `giveUpTurn(now, seeds)`, widened. The timed-out seat is **not** put into `foldAbsent`'s absent set — *an expiry costs the seat one decision* is enforced by which argument the seat is passed as. Presence decides the latch, not the clock. `review: deep`* | S | done |
| | [TASK-130809](tasks/TASK-130809-the-sweep-expires-turn-clocks-in-one-pass.md) The sweep expires turn clocks in one pass through act — ***`atomic:` 9, measured and not probed*** — `expireGracePeriods` → `expireTurnClocks`, `GraceExpiry` → `TurnClockExpiry`, two passes collapsed into one through `act`, so a duel that ends on a timeout reaches the coin by the same single path a played one does. `now` read once, so an expiry can only trail its deadline. The count is a `grep -rl` reference set and the ticket says so; ktlint's filename rule forces both file renames | S | done |
| | [TASK-130810](tasks/TASK-130810-the-grace-window-leaves-the-room-and-the-configuration.md) The grace window leaves the room and the configuration — ***`atomic:` 17, measured and not probed*** — `gracePeriods` → `awaySeats`, `isPaused` and `expireGrace` deleted, `presenceOf` and `disconnect` lose their time arguments, `disconnectGraceMillis` leaves `RoomTimeouts` and `ServerConfig`. Presence stops being time-derived. **No wire moves**: `PROTOCOL_VERSION` stays put and the ledger gains no row. A gate holds the `Thread.sleep` scan at exactly **5** source-scanning test files, so `ADR-0013`'s inject-time rule cannot be lost in the file rename | S | done |
| | [TASK-130811](tasks/TASK-130811-the-store-anchors-the-clock-when-the-frame-arrives.md) The store anchors a TurnClock when the frame arrives — *`ADR-0113` §6's anchor-on-arrival, paint-on-apply: the third parameter is read **before** the reveal queue, so a clock held behind a paced runout is already reduced when it is drawn. `performance.now()` and never `Date.now()`, gated. Two dwells and two anchors, because one cannot tell an anchor from a constant. Draws nothing — `STORY-1309` does, and takes `presence-countdown.ts` → `countdown.ts` with it* | S | done |
| | [TASK-130812](tasks/TASK-130812-the-test-plan-retires-the-pause-and-the-grace-window.md) The test plan retires the pause case and the grace window it measured against — *the artifact no gate holds (`ADR-0113` §9). `CORE-23` is **rewritten to its inverse**, not deleted, and `CORE-22` loses a citation to a constant that no longer exists — a drift the ADR did not name. `CORE-18` and `CORE-19` survive untouched, and the greps assert the file exists before asserting what it does not contain, so none can pass vacuously* | XS | done |
| **[STORY-1309](stories/STORY-1309-the-table-counts-down-and-the-pause-leaves-the-screen.md)** The table counts down, and the pause leaves the screen — *item 4c; **split into twelve tickets on 2026-09-04**, one chain, `TASK-130901` startable, **no `DEC` raised**. The split answered the question the story could not: **what makes the countdown tick is the store, not a component.** `ADR-0113` §6 puts the injected monotonic clock in **the store**, at *"the seam `ADR-0102` §4 already established for `REVEAL_STEP_MS`"* — which is `Schedule`, documented in `duel-store.ts` as *"the store's only door onto a clock"* — and `advanceReveal`/`armTick` is that mechanism's merged first instance, already moving the store's state on a timer and no message at all. So the tick re-arms `schedule` once a second while a clock is live, `DuelState` gains one `nowMillis` reading, `useSyncExternalStore` redraws, and **no component owns a timer** (`ADR-0032` §3). The stop condition is the sharp part and is one clause: `tickClock` compares the reading it **already holds** against `expiresAt`, so the update that first crosses zero still lands and the next does not — a store comparing the incoming reading would never show a player the zero, and one that never stopped would re-render a finished duel's result screen once a second forever, because nothing clears `turnClock` on `DuelFinished`. **`ADR-0113` §3's bank expression is re-applied client-side rather than re-derived**: `expiresAt − max(now, turnEndsAt)`, clamped, reproduces all four of the card's bank cells including the one that matters — *on timebank* draws `.clock` `2:47` and `.timebank` `Timebank 2:47`, the same figure twice, because once the fresh allowance is gone the bank is what falls. **The blast radius was probed, not reasoned about**: two unconditional spans planted in `SeatPlate` reddened exactly two tests, both in `no-derivation.test.tsx`, out of 1 062 — so an optional prop keeps that guard green, and `TASK-130909` then widens it honestly with a closed `toEqual` on the clock's own digit runs at two readings rather than leaving the file claiming a property of a screen it no longer draws. Removing the pause clause reddens exactly three tests in two files, measured the same way. **Two counts were not computed**: every gate is a measured per-file lower bound (`awk` over `Tests N passed (N)`), never an equality, because `STORY-1308` had two exact pins actively hide defects — one made its coder delete the test that had just verified a fix. **One deviation is named rather than hidden**: the merged card draws `.clock` at `font-size: 1rem` and the type scale has no `1rem` step, so `TASK-130906` composes `text-large` from the settled vocabulary — a raw length in an arbitrary value is the shape `ADR-0091` §4 refuses, and minting a step is the human's (`ADR-0091` §3) — and the pane may overrule it in one line* | | | **done — twelve tickets, twelve merged.** The countdown ticks: the store owns the tick through `ADR-0102` §4's seam, `boot.ts` is the only place the real clock is read, and the pause's sentence left with the pause. **A finding this split turned up, in a card `STORY-1307` already merged.** `design/components/seat-and-pot.html` line 79 sets `.clock { font-size: 1rem }`, and **1rem is not a step in the sheet** — `--pd-fs-body` is `0.9375rem` and `--pd-fs-large` is `1.125rem`. `ADR-0024` §2 says every size is born in the sheet, so the card states a size the vocabulary does not have. `TASK-130701`'s gates checked that tokens were **used**; none checked that raw lengths were **absent**, which is why it landed green. This story does not widen into it — `TASK-130906` composes from `--pd-fs-large`, the nearest merged step, and keeps the property the drawing makes. **The card needs a repair ticket**, and whether the answer is minting a `1rem` step or redrawing at `1.125rem` is `ADR-0091` §3 minting, which is the human's **A second finding, from `TASK-130901`, and this one has no owner.** Renaming `presence-countdown.ts` to `countdown.ts` left **four references in merged ADRs pointing at a file that no longer exists**, and review sorted them from the merely historical ones: `ADR-0108` §5 (*its arithmetic is the pattern*), `ADR-0108` §6 (*its input is the architect's to dismantle*) and `ADR-0113` §6 (*this is its existing arithmetic*) are **live instruction**, not record. Mentions in those ADRs' Context and Alternatives sections are correct as history and stay. This repository treats ADRs as immutable, which is right and is also why nothing will ever catch this: a rename cannot redden a document no gate reads. **It needs a decision, not a cleanup** — whether a merged ADR's live instruction may be corrected when the file it names moves, and if so by what mechanism. Recorded rather than fixed, because a coder quietly rewriting the record is the worse outcome **A third, smaller finding, from `TASK-130902`.** `bankFigure` renders minutes unpadded, so an hour would read `60:00` — a shape the card never draws. Review called it unreachable; it is **config-reachable**, because `TASK-130803` made the bank a deployment value (`duel.timebankMillis` / `TIMEBANK_MILLIS`). Nothing crashes and nothing is wrong at the shipped three minutes, so this is not a defect today — but a deployment that lengthens the bank past 59:59 gets a figure no card specifies, and the drawing would owe an answer before it did. Recorded, not fixed **A gap `TASK-130905`'s coder declared rather than papered over.** No test exercises a **pending tick firing after the clock goes away** — on `DuelFinished`, or on a `RoomJoined` naming a different room. `Schedule` is `(run, delayMillis) => void` and gives the store no cancel handle, a pre-existing constraint of `ADR-0102` §4's seam that the reveal pacing already lives with, so the callback always fires. The coder read the code and reported that it fires once, gets the identical state back from `tickClock`, and exits `clockTick`'s `if (next === state) return;` without notify or re-arm — then said plainly that this is **code-reading, not a passing assertion**. Review confirmed the reading is correct. So the behaviour is right and unpinned: a later ticket that wants it held should name the test, since nothing here will redden if it changes **A fourth finding: one shared class has now bitten three consecutive tickets, twice invisibly.** `PotStrip`'s pot figure wears the same `font-mono text-large tabular-nums` as a countdown. `TASK-130907` caught it **over-counting** — a whole-container *exactly one countdown* assertion silently became *exactly two, one of which is the pot*. `TASK-130908` caught it **over-stripping** — an unscoped selector removed the pot from a before/after comparison, narrowing a whole-screen assertion without reddening anything. Both coders found their own error mid-task, which is luck rather than a system. The current separator is `classList.length > 3`: a clock carries a fourth colour class and the pot carries three. Review judged it **brittle but intentional** — documented in the test and in scope — and named the failure mode plainly: **if the pot ever gains a fourth class the test narrows silently rather than failing.** Distinguishing by class *count* rather than by meaning is the weakness; a marker the countdown owns would end the collision. Recorded, not fixed, because changing it is a drawing question **A technique ruled out before it spread, from `TASK-130909`.** Needing a prop's type from `DuelTable.tsx`, a file its *Files* table did not name even as `read`, that coder wrote a throwaway `.tsx` importing the component and forced `tsc` errors to reveal the public signature, then deleted it — and flagged the manoeuvre itself for judgement. Deep review ruled **against reusing it**, on the strongest available ground: it was **unnecessary**. Writing the real test with a guessed prop and running the ticket's own `npm run check` gate produces the identical compiler diagnostic, with no extra file created and none to remember to delete. `CLAUDE.md`'s *read only what the ticket names* is unqualified, and routing around it through a different channel is still routing around it. Recorded because techniques propagate: the ruling is worth more than the ticket. **Also noted, not blocking:** the golden literals fail loudly if a fixture moves, never silently — but the failure output prints the exact array a human could paste back without re-deriving it. That hazard is file-wide and pre-existing (`6625` in the pot test has the same shape), not introduced here **One dead string remains, confirmed twice and judged cosmetic both times.** `ActionBar.test.tsx` line 937 keeps an alternation whose `paused` branch can never match, since both the word and the digit after it were deleted. The test is **not** vacuous — its other two branches are live — so it is a dead branch rather than a green test guarding nothing, and it is left rather than widened into |
| | [TASK-130901](tasks/TASK-130901-the-countdown-loses-its-presence-prefix.md) `presence-countdown.ts` becomes `countdown.ts` — *`ADR-0113` §7's own row, and the whole ticket: `secondsRemaining`'s body is gated as a fixed string so it cannot move, and the citation re-points from `ADR-0028` §3 to `ADR-0108` §5* | XS | done |
| | [TASK-130902](tasks/TASK-130902-the-clocks-two-figures-are-the-cards-own.md) The clock's figure and the bank's figure are the two shapes the card drew — *bare seconds under a minute, `m:ss` above; the bank always `m:ss`, because the merged card draws `Timebank 0:00` where the clock draws `0`. Where `TASK-130701`'s prose and the drawing disagree, the drawing is what the human accepted* | S | done |
| | [TASK-130903](tasks/TASK-130903-one-seat-draws-the-clock-and-two-numbers-choose-its-treatment.md) One seat draws the clock, and two server-stated numbers choose its treatment — *`ADR-0113` §6's licence exactly: a comparison of two anchored instants to pick a style. `RUNNING_OUT_SECONDS = 10` is bounded by the card's own `24` and `6` and named once, `ADR-0102` §4's feel-number precedent* | S | done |
| | [TASK-130904](tasks/TASK-130904-the-reducer-keeps-a-second-hand-and-knows-when-to-stop.md) The reducer keeps a second hand, and stops it once the clock can no longer move — *`nowMillis` in `DuelState`, `tickClock` beside `advanceReveal`; the stop compares the **held** reading so one tick lands at zero before it stops* | S | done |
| | [TASK-130905](tasks/TASK-130905-the-store-owns-the-tick-and-boot-names-the-clock.md) The store arms the second hand, and boot names both the clock and the period — *`CLOCK_TICK_MS = 1000` and `now: () => performance.now()` at `boot.ts`, `ADR-0113` §6's injection seam; one-shot re-arming and no `setInterval`, so a missed tick costs an update and never compounds* | S | done |
| | [TASK-130906](tasks/TASK-130906-the-seat-plate-draws-the-clock-and-the-bank.md) The seat plate draws the countdown and the seat's timebank, and speaks neither — *the card's four colours through a `Record<ClockTreatment, string>`; zero `aria-label`, zero `title`, zero motion utility, so `ADR-0115` §3's still form is the form by construction. The bundle's emitted rules are read out of `dist/assets`, not grepped in source* | S | done |
| | [TASK-130907](tasks/TASK-130907-the-table-draws-one-countdown-at-the-acting-seat.md) The table draws one countdown, at the acting seat, and both seats' banks — *the seat argument is the plate's own index, never `viewerSeat`; asserted with the turn on each seat and at each `viewerSeat`, so a hard-coded seat fails half of four. `DuelTable` reads no clock — re-deriving the anchor here would restart a queued countdown at thirty* | S | done |
| | [TASK-130908](tasks/TASK-130908-the-four-states-are-reachable-and-zero-is-not-an-event.md) The four states are each reachable on the table, and reaching zero changes nothing else — *the enforced expiry trails the visible zero on **every** timeout, so this is a second that always happens. Two renders, clock spans stripped from both, remaining `innerHTML` byte-identical — naming what did not change is how such a test rots* | S | done |
| | [TASK-130909](tasks/TASK-130909-the-derivation-guard-admits-the-clocks-own-figures.md) The derivation guard admits the clock's own figures and no other new number — *`review: deep`. `allowedNumbers` is untouched, so the seven merged tests stay exactly as strict; the widening is one closed `toEqual` on `[3, 12, 24]` and `[3, 12, 19]`, literals and never computed from the code under test* | S | done |
| | [TASK-130910](tasks/TASK-130910-the-duel-screen-hands-the-table-its-clock.md) The duel screen hands the table its clock, and it visibly changes each second — *the human's own sentence, checkable at last: three figures across two fired ticks, driven through the store's injected `schedule` and `now` so nothing sleeps and no timer is installed. Pays `null-view.test.tsx` the line every table surface owes it* | S | done |
| | [TASK-130911](tasks/TASK-130911-the-pause-leaves-the-line-that-named-it.md) *The duel is paused.* leaves the line that named it — *derived under `ADR-0046` §2's register and written down: the first sentence states the fact, the second answered *what does this mean for me*, and `ADR-0108` §4 says the rival's clock answers that now. Not a new string, not a deleted row, and not an answer to `DEC-108`* | S | done |
| | [TASK-130912](tasks/TASK-130912-the-notice-stops-testing-a-sentence-that-left.md) The presence notice stops testing a sentence that left — *`/paused\.\d/` can now match nothing whatever the code does: green by construction, with fake timers installed for a component that has no timer. A vacuous assertion that reads like a guard is worse than none* | XS | done |
| **[STORY-1310](stories/STORY-1310-the-refresh-paths-nobody-drove.md)** The refresh paths nobody drove, driven and written down — *item 8a; `ADR-0112` §6's six paths; no browser drive may be a `verify:` gate* | | | **done 2026-09-04** — decision-free, waits on `STORY-1301` **Settled 2026-09-02 while landing `TASK-130205`, so this story does not hunt a phantom:** a host who reloads while `WAITING` **does** return to the table. `boot.ts` re-sends `JoinRoom` on the next `Welcome`; `replyToJoinRoom` tries `resume()` — which is `null` for a `WAITING` room — and then **falls through to `join()`**, which succeeds and sends `RoomJoined`, so `roomCode` is set. A coder claimed the opposite by reasoning from the `resume()` path alone; the epic's driven measurement was right. **Split into nine tickets on 2026-09-04**, one chain, `TASK-131001` startable. The split's own finding is that **five of the seven readings are races a bare localhost stack cannot resolve**: `drive.mjs` samples `#root` every 250 ms and `record` cannot be armed across a page load, so *no lobby appeared* on loopback is a statement about the sampler — the sentence `EPIC-13` already wrote down. So the first two tickets build one instrument, `scripts/qa/delay.mjs`: a Node-builtins loopback relay that delays both directions and can **cut** every live pair on command. The cut is what makes `P3` — a socket dropped without the page dying — drivable at all, since `close` closes the tab and `open` is a navigation, and both destroy `reconnecting.ts` with the document. The layout was measured rather than guessed: **Vite moves to `5273` and the relay takes `5173`**, because `drive.mjs` hard-codes `localhost:5173` in `APP` and in `close`'s tab lookup, and a moved origin would silently change which `pd.deviceId` and `pd.roomCode` the drive is about. **No `verify:` block anywhere in the story runs a browser** (`ADR-0089` §2b) and one gate checks that across all nine tickets by reading their frontmatter `verify:` blocks — the only gate here that checks a claim rather than a document's shape, and the tickets say so instead of dressing it up. The rest are honest shape checks: seven `P` rows stand, this ticket's row is no longer the placeholder, and the placeholder count is an **upper** bound so a drive that answers more than it was asked is never forbidden — `STORY-1308`'s exact pin made a coder delete a test to satisfy it. **One half is undrivable and is recorded as a result, not a gap**: *the token is still usable afterwards* cannot be observed, because `NoRecoveryMailer` sends nothing (`ADR-0031` §7), the tokens are stored only as `BYTEA` hashes, and seeding one is `ADR-0089` §3's forbidden write. What **is** drivable is whether `VerifyScreen`'s mount effect runs at all over a held room, which a fabricated token answers through `VERIFY_LINK_DEAD`. **An operating note for the drives, measured at `TASK-131001`.** The relay's `--selftest` cannot observe teardown: it destroys its own client after the echo, so a relay wiring only `data` and dropping `end` / `close` / `error` would pass every assertion while leaking one outgoing socket **per accepted connection**. The shipped relay wires all four, so this is a gap in the guard rather than a defect — but it was sized rather than shrugged at: about **ten accepted connections per page load**, so roughly **140 to 300 descriptors per drive cycle** against a typical limit of 1024. One cycle is safe; three or four without a restart approaches exhaustion. **Restart the relay between long validation runs.** Two further blind spots confirmed by mutation: the timing assertions are lower bounds with no ceiling, so a relay multiplying its delay by ten still prints *passed*; and — the useful negative — a relay delaying only **one** direction **is** caught, because a timer cannot fire early, so under-delaying either way drops the round trip below the floor  **Closed 2026-09-04. All seven paths driven, no placeholder left.** Six produced a reading; `P3` produced an instrument fault instead — under `npm run dev` Vite's HMR client shares the app's port, so cutting a socket reloads the page and `reconnecting.ts` cannot be watched recovering in place. That landed on `DEC-087`, answered the same day by [`ADR-0117`](../docs/adr/ADR-0117-the-proofs-of-record-load-the-built-bundle.md), which moves the proofs of record onto `vite preview` and `dist/`; `P3` therefore owes a **re-drive, not a decision**. **Two findings against `ADR-0112` came out of the drives.** §6's *no lobby on the way* fails under latency, observed **three independent times by three routes** — `P2` mid-runout, `P4` on reload, `P6b` through a mailed link — each painting a confident *Waiting for your rival* over a room that was live or already finished; whether a self-correcting false screen is acceptable-because-late or wrong-because-false was **`DEC-127`, the product owner's**, **answered the same day by [`ADR-0118`](../docs/adr/ADR-0118-a-recovering-browser-shows-nothing-it-was-not-told.md)**: both, in different windows — the front door before any frame is wrong-because-false and `/` now renders nothing, and the stale waiting screen after `RoomJoined` is acceptable-because-late and is accepted out loud. And §5's *a mailed link refused mid-duel must not spend its token* fails at `delayed 300ms`: the mailed screen mounts and the dead-link sentence renders, so **the client submits**. That the *server* would consume a live token is inferred and not testable on this stack, for the same three reasons the undrivable half cites, and the record says so rather than claiming more. `P1`'s two contradictory readings were **narrowed, not settled** — a fresh bare re-drive reproduced the surviving result screen twice and never the lobby, explaining one reading and leaving the other open rather than tidying it away|
| | [TASK-131001](tasks/TASK-131001-a-loopback-relay-puts-milliseconds-in-front-of-the-stack.md) A loopback relay puts milliseconds in front of the stack — *the instrument, first, because five readings are races loopback cannot resolve. `node:net` only, no dependency (`ADR-0089` §2a); Vite moves to `5273` so the relay can take `5173` and leave every hard-coded port in `drive.mjs` alone. `--selftest` runs **two** relays, at 50 ms and 200 ms, and asserts the difference — one delay is passed by a hard-coded sleep* | S | done |
| | [TASK-131002](tasks/TASK-131002-the-relay-learns-to-cut.md) The relay learns to cut, so a socket drops without the page dying — *`P3` has no verb today: `close` closes the tab, `open` is a navigation, and `eval` cannot reach the socket without writing application state. A control port that destroys every live pair is the network going away under a page that keeps running. The self-test echoes **before** the cut, or it would pass against a relay that never connected* | S | done |
| | [TASK-131010](tasks/TASK-131010-the-stack-asks-compose-what-it-named-the-container.md) The stack asks compose what it named the container — *inserted 2026-09-04: `TASK-131003`'s coder failed at the drive's first command and changed no files. `stack.sh:23` writes `poker_duels-postgres-1` down as a literal, but compose names containers after the project, which defaults to the checkout directory's basename — measured in this worktree, `compose up -d` created `agent-a73380fa3b5353c90-postgres-1`, so the readiness loop polls a name nobody has and dies naming the wrong cause. **`db-up` cannot succeed in any worktree**, which is where every drive in this story runs. Three derivations were run against a live daemon before one was chosen: `compose ps -q postgres` wins because it exits **0 with empty stdout** when nothing is up, so it is safe under `set -euo pipefail` and `status` still prints `down`; `compose exec -T` gives the same answer but pays service resolution on all sixty polls and leaves no id for `docs/test-plan.md`; the directory basename re-implements compose's normalisation from outside and breaks under `COMPOSE_PROJECT_NAME`, `-p` or a top-level `name:`. It must go through the existing `compose()` shim — **`docker compose` is not installed on this machine**, only standalone `docker-compose`, so a literal `docker compose ps` would fix nothing here. The gate is `scripts/qa/stack-selftest.sh`: a stub `docker` first on `PATH`, six assertions, **no daemon** — so it is a real red/green with Colima stopped and in CI. Two different ids across two runs, because one canned id cannot tell a derivation from a new constant, and the two runs also cover both `compose()` branches. Its negative control was run before the ticket was written and prints `docker exec poker_duels-postgres-1 pg_isready` from the recorded call log. It is a **separate file** for a design reason and a hard one: `TASK-131001`, `TASK-131002` and `TASK-131003` carry a merged gate failing any `verify:` line under `TASK-1310*.md` that names the stack script, so naming it would turn three merged tickets red — `stack-selftest.sh` does not contain that substring, and §2b's substance holds because the harness starts no browser. **`docker-compose.yml`'s fixed `5432:5432` stays**: drives are serial for five other reasons, and a dynamic port would have hidden the orphan that prompted this rather than colliding with it. The real second-order cost is the **volume**, not the port — four `*_poker-duels-postgres` volumes were measured on this machine, three belonging to worktrees that no longer exist, because `db-down` is `compose down` and keeps the volume. Not ticketed; written down* | S | done |
| | [TASK-131003](tasks/TASK-131003-p1-a-refresh-on-the-result-screen.md) `P1` — a refresh on the result screen — *the path `EPIC-13`'s five did not cover. `open`'s own output is the first paint and the only pre-frame observation a navigation allows; `record` is armed immediately after it. Driven bare **and** delayed, because a negative on loopback is a statement about the sampler* | XS | done |
| | [TASK-131004](tasks/TASK-131004-p5-the-account-offers-accept-is-observed.md) `P5` — the account offer's accept, observed rather than derived — *`ADR-0112` §Context calls this one *derived, not driven* in its own words, and §5 resolved it in `ADR-0086`'s favour. The anchor is **clicked**, never navigated to: an `open` skips the handler that settles the offer and would prove something else* | XS | done |
| | [TASK-131005](tasks/TASK-131005-p2-a-refresh-during-a-runout.md) `P2` — a refresh during a runout — *`ADR-0102` §5 already explains the jump to the end from `resumeFrames` sending no `StreetDealt`; the undriven half is `ADR-0112` §6's *confirm no lobby shows on the way*. The undisturbed runout is recorded first, so the reloaded one has a baseline* | XS | done |
| | [TASK-131006](tasks/TASK-131006-p3-a-genuinely-dropped-socket.md) `P3` — a genuinely dropped socket — *the one path where `record` genuinely works, because nothing navigates and the document survives. Both screens are recorded across the cut. The PR body must show the `cut <n>` answer, or the row is indistinguishable from a `close`-and-`open`. **Was `blocked` 2026-09-04 to 2026-09-10 for a reason that was never true**: the `P3` row asked for a `DEC` to be registered about the instrument, none ever was, and none was needed — [`ADR-0117`](../docs/adr/ADR-0117-the-proofs-of-record-load-the-built-bundle.md) §1 merged earlier the same day (PR #1389, to this row's PR #1392) already decides the first of the three options the row lists. The real blocker was that `ADR-0117`'s own two lines were never cut into a ticket; that is now `TASK-131011`, and this is an ordinary dependency. **Blocked again 2026-09-10, and by a real question this time**: the drive found the relay cannot reach `vite preview` at all — `delay.mjs` dials `127.0.0.1`, `preview` has no `host` and so binds `::1` only — and reported `cut 0`, no socket to sever. Registered as `DEC-169` and answered the same day by [`ADR-0156`](../docs/adr/ADR-0156-the-relay-dials-by-name-and-holds-both-loopback-families.md): the relay changes, not the origin. No decision blocks this ticket now; it waits on that ADR's §6 ticket (`delay.mjs` + `drive.mjs`'s `PD_APP_ORIGIN`), and drives on §5's layout — preview on `4273` behind the relay on `4173`* | XS | done |
| | [TASK-131011](tasks/TASK-131011-the-proof-origin-adr-0117-described-is-built.md) The proof origin `ADR-0117` described is built — *`ADR-0117` §1 merged 2026-09-04 and nothing implemented it: `web-client/package.json` has no `preview` script and `vite.config.ts` has no `preview` key, both files last touched 2026-08-14. So every `P3` drive cut a dev-server socket carrying Vite's HMR client, the page reloaded, and `reconnecting.ts` could never be seen recovering in place — reproduced 4/4. Two lines, verbatim from the ADR. **The load-bearing gate** is `npm run build && ! grep -q '@vite/client' dist/index.html`: a `preview` script that served the dev bundle passes every other gate and still ships the client the cut severs* | XS | done |
| | [TASK-131012](tasks/TASK-131012-the-relay-reaches-the-origin-it-is-pointed-at.md) The relay reaches the origin it is pointed at — *filed 2026-09-10 from [`ADR-0156`](../docs/adr/ADR-0156-the-relay-dials-by-name-and-holds-both-loopback-families.md) §6, which answers `DEC-169`. `TASK-131011` removed the HMR client and the very next `P3` drive still could not measure: `delay.mjs:66` dials `127.0.0.1` while `preview` binds `localhost` → `::1`, so the relay accepts the browser and cannot forward. The drive reported **`cut 0`** and a first pass wrote a row from it anyway — every reading in that row is what an **undisturbed** page produces, so it could not tell *reconnected in place* from *never cut*. Rejected. **Why the failure is silent rather than loud:** `[::1]:P` and `127.0.0.1:P` do not collide, so a browser pointed at a relay that never forwards still reaches preview on the same port **directly**, and the drive proceeds measuring a page nothing is cutting. **The mutation the gate must be shown red against**, named by the ADR rather than chosen by the coder: restore the `127.0.0.1` dial and `--selftest-unreachable` must fail against a `::1`-bound echo* | S | done |
| | [TASK-131007](tasks/TASK-131007-p4-the-rejoin-round-trip-made-visible.md) `P4` — the rejoin round trip made visible — *the path the instrument was built for. Three delays — 0, 300, 1000 ms — because a flash that grows with the delay is the round trip and one that does not is something else. `ADR-0114` §5 introducing `roomAwaited` to tell *no room* from *not told yet* is evidence a flash exists today; the ticket forbids writing the row from that paragraph* | XS | done |
| | [TASK-131008](tasks/TASK-131008-p6-a-mailed-link-over-a-held-room.md) `P6a`/`P6b` — a mailed link opened while a room is held — *a page load to `#/verify/<token>`, never a `location.hash` write: `Lobby.tsx` reads the token once at mount, so a hash write in a running tab captures nothing. *Token survives* is **undrivable** and says why; what is drivable is whether `VerifyScreen`'s mount effect ran, which a fabricated token answers through `VERIFY_LINK_DEAD`. Ends the story's placeholders at zero* | S | done |
| | [TASK-131009](tasks/TASK-131009-the-record-read-whole.md) The record read whole, and every finding given an owner — *`ADR-0112` §6: *a dismissal without the attempt does not satisfy the DoD row* — and seven rows nobody joins up is a table, not a conclusion. Each finding gets a **non-browser** gate named as a file and a test name; the ticket files nothing, because a variable number of new files cannot be a fixed *Files* table. `ADR-0114` §6's one-render `waiting` residual is answered whether or not it was seen* | S | done |
| **[STORY-1311](stories/STORY-1311-only-a-running-duel-refuses-another-screen.md)** Only a running duel refuses another screen, and the refusal restores the address — *item 8b; `ADR-0114` the mechanism, `ADR-0118` the recovery, `ADR-0116` the offer's accept; **split into fourteen tickets on 2026-09-04**, one chain, `TASK-131101` startable, **no `DEC` raised**. The split is long because the change is a **relocation**: `ADR-0114` §2 moves six chosen-screen branches above three store branches, and a pure move of 120 lines is 240 diff lines, so the block moves one contiguous piece per ticket with an `awk` order gate on each. `ADR-0118` grew the story by a silence and by the test that **pins the accepted half** of `DEC-127`, so no later ticket answers it the other way. Two questions were judged rather than inherited: `ADR-0118` §5's unreachable server is **deliberately unregistered** by the ADR itself (`ADR-0105` §6 — *a `DEC` nobody is working is noise in the open table*) and is refused a ticket here in as many words, so no coder invents a timeout for it; and `ADR-0118` §4's one-second `built` reading is a **browser drive** (`ADR-0089` §2b) whose subject does not exist until this story merges, so it belongs with `ADR-0117` §7's owed re-read of `P2`/`P5` in a story of its own. Every ticket carries `STORY-1310`'s gate that no `verify:` block names `drive.mjs`, `stack.sh` or `vite preview`* | | | **done 2026-09-05** — all fourteen tickets merged. `ADR-0114` §2's relocation landed in four contiguous pieces and closes on a count gate: zero `if (screen === "` lines, exactly six `if (shown === "`, so a chosen screen cannot render without consulting `rulingOn` first. Two defects `STORY-1310` drove in a browser now carry regression tests — the mailed token spent while the room was unknown (`TASK-131107`) and the account door that opened onto nothing (`TASK-131110`, proven red before the fix). `ADR-0118`'s silence ships: `/` renders no element until the server speaks, with no timeout and no interstitial, and the half `DEC-127` answered *acceptable* is pinned so no later ticket answers it the other way. Two gaps are recorded open rather than closed quietly: the token-effect coupling, where three separate predictions were each contradicted by a run, and `ADR-0112` §3's *frames keep applying* |
| | [TASK-131101](tasks/TASK-131101-room-standing-reads-the-room-off-the-frames.md) `roomStanding` reads the room off the frames the server sent — *the pure module `ADR-0114` §1 quotes verbatim, in the `screen.ts` tradition: no `window`, no React, type-only imports, all three gated. Seven tests, two of them deliberately not tautologies — `finished` while `view` still stands, and `running` while a reveal is mid-paint* | S | done |
| | [TASK-131102](tasks/TASK-131102-ruling-on-answers-every-ask.md) `rulingOn` answers every ask, and the two mailed screens wait — *`spendsOnArrival` is an **exported, tested predicate**, never a literal inside a condition, so *does mounting this screen send something the server cannot be asked twice?* is a question a future screen has to answer. The `hold`/`honour` pair over `unknown` is what stops the hold becoming *hold everything** | S | done |
| | [TASK-131103](tasks/TASK-131103-boot-says-whether-this-tab-is-awaiting-a-room.md) Boot says whether this tab is awaiting a room — *computed **once at construction**, not in the `Welcome` handler, and the load-bearing test is the fourth: a later `RoomJoined` does not move it. `readRoomCode`, never `localStorage` — `ADR-0086` §2 keeps the global in `main.tsx`* | XS | done |
| | [TASK-131104](tasks/TASK-131104-the-provider-carries-room-awaited.md) The provider carries `roomAwaited`, and `main.tsx` hands it over — *an optional prop defaulting to `false`, exactly `forgetRoom`'s shape, so the other ninety-odd tests are untouched. Two gates keep the flag **out** of `src/e2e/`: those harnesses replay frames with no round trip, and `drive-arc.tsx` reuses one `Storage` across two boots, so wiring it there would silence a second boot for a reason unrelated to the product* | XS | done |
| | [TASK-131105](tasks/TASK-131105-the-lobby-rules-on-the-ask.md) The lobby rules on the ask, and the refusal restores the address in one act — *`useLayoutEffect` keyed on `ruling` is the whole of `ADR-0114` §3, and swapping it back breaks nothing any test can name — the reason lives in a comment. The token effect **must** be re-keyed to `shown` in the same diff: the restore is now a layout effect and the token effect is passive, so keyed on `screen` it would write `#/verify` back over the `/` just restored* | S | done |
| | [TASK-131106](tasks/TASK-131106-the-mailed-screens-move-above-the-room.md) The two mailed screens move above the room and read `shown` — *one contiguous cut and paste, plus two `awk` order gates asserting each moved `if` precedes `if (state.outcome !== null)`. Closes the window `STORY-1310`'s `P6a`/`P6b` observed, where the mailed screen mounted over a live duel and the call completed inside a 0.2 s gap* | S | done |
| | [TASK-131107](tasks/TASK-131107-a-mailed-token-is-not-spent-before-the-server-answers.md) A mailed token is not spent before the server has answered — *test-only, and the fixture goes through the **tab's memory** rather than the provider prop — `ADR-0114` §7 requires it, and it is the only thing proving memory → `boot.ts` → provider → `Lobby` is one wire and not three halves. **Zero, zero, one**: counts, never the argument of a call* | S | done |
| | [TASK-131108](tasks/TASK-131108-the-record-and-the-ladder-move-above-the-room.md) The record and the ladder move above the room — *the `historyRead` spy is the sharp assertion: the settled DOM cannot tell a correct first pass from a wrong one the same ticket's effect corrects, and the record's own fetch firing is the witness a wrong branch order leaves behind* | S | done |
| | [TASK-131109](tasks/TASK-131109-a-look-away-takes-nothing-with-it.md) A look-away takes nothing with it, and a frame that seats a duel overrules it — *test-only. *The socket stays open* is asserted as *`forgetRoom` was never called*, which is the only part of it a component test can see, and the ticket says that rather than claiming more* | S | done |
| | [TASK-131110](tasks/TASK-131110-the-account-moves-above-the-room.md) The account moves above the room, and the offer's accept has somewhere to land — *`ADR-0116`'s regression test, red at `1a09fb46`. Its second owed fact is **already merged** — `offers an account after a win, and after nothing else` ends on `expect(offerWiring.settle).not.toHaveBeenCalled()` — so the ticket cites it instead of writing a fourth render. Two gates refuse the repair `ADR-0116` forecloses: `settleOfferHere` once in `Lobby.tsx`, never in `AccountScreen.tsx`* | S | done |
| | [TASK-131111](tasks/TASK-131111-the-sign-in-screen-moves-above-the-room.md) The sign-in screen moves above the room, and the six branches are one block — *the closing gate on the relocation: **six** `if (shown === "` lines and **zero** `if (screen === "` lines, which is what keeps `ADR-0114` §2's order true when the next screen is added* | S | done |
| | [TASK-131112](tasks/TASK-131112-slash-renders-nothing-until-the-server-has-spoken.md) The address `/` renders nothing until the server has spoken — *`ADR-0118` §2. The pair is the ticket: the same tree, the same address, one boolean different — silence with a room awaited, the front door without one, because one fixture cannot tell a rule from a deletion. Out of scope **in as many words**: the unreachable server (§5, deliberately unregistered), an interstitial of any kind (§4), and the one-second `built` reading* | S | done |
| | [TASK-131113](tasks/TASK-131113-the-late-waiting-screen-is-pinned.md) The late waiting screen is pinned, and the recovery lands where the frames say — *test-only, and the unusual one: it pins the half of `DEC-127` answered **acceptable**, so a later “repair” withholding the waiting screen goes red here while the merged host-alone test stays green. The two recovery frames go in **separate** `act()` calls — the subject is a sequence delivered as two socket messages* | S | done |
| | [TASK-131114](tasks/TASK-131114-the-record-is-read-against-the-repair.md) The record is read against the repair, one row per driven path — *seven rows, each naming a `TASK-1311NN` or `NO-REPAIR-OWED` with a merged source cited, so `EPIC-13`'s refresh DoD row closes on evidence. The gates check shape only and the ticket says so, exactly as `TASK-131003` did. `STORY-1310` is not edited — a record is a statement about a commit (`ADR-0089` §2c)* | XS | done |

## EPIC-14 — The name, the showdown and the fit

Opened **2026-09-06** on the human's feedback after they brought the stack up on the local network
and played the product on a **laptop and an iPhone** — the first report from a player rather than
from a driver, and the first that a rubric could not have produced: no criterion asks whether a
player knows who they are, and none asks what a showdown looks like. It opens **`backlog`**, with
nothing startable, because **eleven product decisions** stand in front of it — **all eleven**,
`DEC-129`–`DEC-139`, now answered, along with the `DEC-140` and `DEC-141` the planner raised
while splitting item 3b.

**Six items, sixteen story seams.** Three came in the first message — the account flow, the
showdown, and the screens — and three the same day: the `Call` button's figure, the rematch offer's
surface, and the room's lifetime. Five annotated screenshots are the source for item 3 and are
**transcribed into the epic** rather than committed; the repository holds no binaries and keeps
none, by the human's choice.

**Five decisions contradict a merged ADR, and one re-opens an open one.** `DEC-130` **has** deleted
the surface `ADR-0036`, `ADR-0085`, `ADR-0086` and `ADR-0116` were written about —
[`ADR-0125`](../docs/adr/ADR-0125-the-account-screen-names-the-anonymous-profile-and-owns-the-door.md)
supersedes the last three in full and `ADR-0036`'s offer block, the first supersession this
repository has made; `DEC-131` **has made**
changeable a name whose ADR title says **permanent** —
[`ADR-0130`](../docs/adr/ADR-0130-a-name-can-be-changed-and-the-name-it-leaves-is-spent.md)
supersedes `ADR-0029` §4's rename clause, its title's *permanent* and §5's `403`, plus `ADR-0051`
§3's *no exception at all* and `ADR-0119` §3's obligation 2, and the name given up is **retired**,
not released; `DEC-132` would have handed a browser a new profile
against `ADR-0012`'s device binding, and is **answered** by
[`ADR-0131`](../docs/adr/ADR-0131-signing-out-of-your-own-account-hands-the-browser-a-new-profile.md),
which supersedes **`ADR-0030`** instead — §3's outcome clause and §8's *not on sign-out*, for the
one case where the account being left is the profile this browser owns — and supersedes nothing in
`ADR-0012`, `ADR-0027` or `ADR-0037`, because a browser handed a new profile is still a profile
bound to a device and a sign-out is not a revocation; `DEC-133` asked to widen what a showdown shows against `ADR-0008`'s
ceiling and was answered without widening it; `DEC-136` would have replaced the reflow `ADR-0103` §2 fixed with scaling, and is
**answered** by `ADR-0121`, which keeps the reflow and supersedes no clause — it qualifies
`ADR-0103` §1's instrument and `ADR-0096` §4's number, both in the direction those sections'
own text asks for. `DEC-139` carried
**`DEC-111`**, open since 2026-09-01, and
[`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md) struck both on
2026-09-06 — a player holds one waiting room, and pressing play again hands it back. Each answering
ADR must name the clause it supersedes; `ADR-0125` did, and **struck `DEC-089` by
deletion** in the same PR — with no nudge on the result screen there is no second headline to rank
against the verdict.

**What the epic measured, so that no story re-discovers it.** The duel coins the human asked to see
*transferred* on promotion **never move** — `ADR-0030` §1 makes a claim one credential `INSERT`, so
item 1e owes a test and not a migration. The showdown is a **client** gap before it is anything
else: the engine emits `HandRevealed` for every hand actually shown and `EventRedaction` passes it
to both seats, it is on the wire, and no client file outside `protocol.gen.ts` mentions it — though
[`ADR-0120`](../docs/adr/ADR-0120-a-showdown-shows-the-hands-the-rules-showed-and-the-beat-that-shows-them-stands.md)
measured that the shown hand **is** drawn all the same, through the snapshot's own `holeCards`, and
stands for one 600 ms step — while `BestHand.cards`, whose KDoc says it exists *"because the client
highlights the winning five at showdown"*, reaches nothing outside `poker-engine` — and by
[`ADR-0126`](../docs/adr/ADR-0126-the-table-shows-the-cards-and-marks-none-of-them.md) it stays
there: **the table marks no card**, so that KDoc sentence is the stale one and not the client. Chips never cross
the table: `ChipPile` is three discs that animate on mount and stand still. The timebank's
`1:58.623999999999995` has one cause — `turn-clock.ts:105` hands `bankFigure` a fraction where its
own KDoc asks for whole seconds, on the plate of the seat the clock does not name. The `Call` button
does no arithmetic, because `action-text.ts` refuses it in as many words. And the room the human
wants alive for *"3-5 min"* already lives **ten**; what does not survive is the tab's memory of it.

| Seam | Title | Waits on |
| --- | --- | --- |
| `STORY-1401` | The timebank figure is whole seconds at every seat — *item 3c's defect* | **done** — one ticket |
| `STORY-1402` | The front door stops covering its own wordmark, and `Play duel` says what it does — *item 3a* | **split** — four tickets |
| `STORY-1403` | The room code field takes the link as well as the code — *item 3a* | nothing |
| `STORY-1404` | The waiting table copies its link, says the duel starts itself, and stops labelling the host — *item 3b* | **nothing** — split raised `DEC-140` and `DEC-141`, both answered 2026-09-06 by [`ADR-0128`](../docs/adr/ADR-0128-the-copy-control-is-never-absent-and-a-press-that-cannot-copy-hands-over-the-selection.md) and [`ADR-0129`](../docs/adr/ADR-0129-the-host-is-told-the-duel-starts-by-itself.md); split into three tickets on 2026-09-07 |
| `STORY-1405` | The table fits the phone it is played on — *item 3d* | nothing — `DEC-136` → `ADR-0121`; **split into three tickets on 2026-09-07**, and the cause `ADR-0121` §5 deliberately left undiagnosed was **measured** in the split: `SeatPlate.tsx:50`'s `min-w-0` generates **no CSS** because `app.css:55` sets `--spacing: initial;`, so the name block cannot give ground and the plate's tail — the **stack figure** — is painted up to **53 px past** the 390 px edge whenever a seat carries the last-act mark and the timebank together. The merged card already specifies the opposite in words and delivers it in CSS, so **no design ticket is owed** |
| ~~`STORY-1406`~~ | ~~The bar stands disabled where it printed a sentence — *item 3c*~~ — **retired, not split**: [`ADR-0127`](../docs/adr/ADR-0127-a-control-stands-only-for-a-decision-the-server-has-opened.md) answers `DEC-135` **no**, so the story's title states the opposite of the decision and there is no work in it | — |
| `STORY-1407` | A player names themselves before their first duel — *item 1a; [`ADR-0119`](../docs/adr/ADR-0119-the-name-is-asked-at-the-first-press-and-skipping-plays.md) is the answer* | ~~`DEC-142`~~ — **answered 2026-09-07** by [`ADR-0137`](../docs/adr/ADR-0137-a-name-suggestion-is-drawn-in-the-browser.md); **nothing blocks it**, and its tickets are **not** `atomic:`. **Written and split into sixteen tickets on 2026-09-08**, and the split measured three things rather than assuming them: the ask must **not** print `PERMANENCE_LINE`, because `ADR-0130` §5 supersedes `ADR-0119` §3's obligation 2 and replaces it with a pair (*it can be changed later*, *the name you give up is gone for good*); every merged test that presses a front-door control renders **without** a `ProfileProvider`, so all 104 of `Lobby.test.tsx` stay green; and `App.test.tsx` mocks `./main` with an **explicit factory**, so the first new export `Lobby` imports from it fails **24 of that file's 36 tests** — probed, and the reason `TASK-140714` lands the binding one ticket before the wiring instead of claiming `atomic:` for a four-file diff |
| `STORY-1408` | The account screen says what an anonymous account is, the post-win offer goes, and the front door stops printing `No profile yet.` — *item 1b; [`ADR-0125`](../docs/adr/ADR-0125-the-account-screen-names-the-anonymous-profile-and-owns-the-door.md) is the answer* | **Nothing** — `DEC-147` is answered by [`ADR-0132`](../docs/adr/ADR-0132-the-profile-says-whether-it-holds-a-password.md) (`hasPassword` on `GET /api/me`, no wire move), so the anonymous block is startable too |
| `STORY-1408` | The account screen says what an anonymous account is, the post-win offer goes, and the front door stops printing `No profile yet.` — *item 1b; [`ADR-0125`](../docs/adr/ADR-0125-the-account-screen-names-the-anonymous-profile-and-owns-the-door.md) is the answer* | **Nothing** — `DEC-147` is answered by [`ADR-0132`](../docs/adr/ADR-0132-the-profile-says-whether-it-holds-a-password.md) (`hasPassword` on `GET /api/me`, no wire move), so the anonymous block is startable too; **split into fourteen tickets on 2026-09-08**, deletions first because `ADR-0125` §§1, 2, 5 and 6 need no new fact, with three `atomic:` tickets sized by `ADR-0070` probes run to `exit 0` and the server/client coupling honoured by **ordering** on `ADR-0132`'s own one-directional failure |
...
| `STORY-1410` | Signing out hands the browser a new anonymous profile, and the confirmation says so — *item 1d; [`ADR-0131`](../docs/adr/ADR-0131-signing-out-of-your-own-account-hands-the-browser-a-new-profile.md) is the answer, and `SIGN_OUT_WARNING`'s second sentence moves in the same diff as the behaviour* | — `DEC-152` answered 2026-09-07 by [`ADR-0135`](../docs/adr/ADR-0135-the-server-says-which-sign-out-this-is-and-the-browser-forgets-one-key.md): a new session-required `GET /api/me/device` answers `signOutHandsANewProfile`, `signOut` forgets `pd.deviceId` on `true` and on nothing else, the new profile comes from the next `Hello`, and **no wire, schema or `PROTOCOL_VERSION` moves** — so the implementing ticket is a plain ticket, not `atomic:`; **written and split into fourteen tickets on 2026-09-08**, and the split **discharged the serialisation rather than inheriting it**: `DEC-147` is answered by `ADR-0132` and `TASK-140808`/`TASK-140809` have landed `hasPassword` in both contested files, while this story opens neither — a **new** `### Device standing` section and a **new** client module, because `ADR-0135` §Alternatives 1 refused a seventh `ProfileResponse` field on a measured fact. **Two `atomic:` tickets, both probed to `exit 0`** — `signOut`'s required field (5) and `AccountCalls.signOut`'s parameter (4, including `account-provider.test.tsx:198`, which a reading would have missed) — and **three groupings that looked atomic are not**, each falsified by probing. **The abandoning branch is unreachable until the last ticket**, because every intermediate seam passes a literal `false`, so behaviour and copy become true in one merge. `SIGN_OUT_WARNING`'s **value does not move**: `ADR-0131` §5 keeps the returning sentence, and what was false was saying it unconditionally — which is why `claimed-here-recovered-there.test.tsx` and `account-server.ts` are measured **out** of the story, that arc being `ADR-0131` §1 row two. **No decision was raised** |
| `STORY-1411` | The showdown shows the hands it reached, and stands long enough to read — *item 2a; the product half is [`ADR-0120`](../docs/adr/ADR-0120-a-showdown-shows-the-hands-the-rules-showed-and-the-beat-that-shows-them-stands.md), the mechanism [`ADR-0136`](../docs/adr/ADR-0136-a-beat-declares-its-own-length-and-zero-silences-every-beat.md)* | nothing — `DEC-146` answered 2026-09-07 by `ADR-0136`: the beat carries its **kind** and the store owns both lengths, `stepMillis === 0` silences every beat so **no e2e file is edited or re-recorded**, the hold has **no CSS** so reduced motion cannot shorten it, and **nothing crosses the socket** — the implementing ticket is a plain ticket, not `atomic:` |
| `STORY-1412` | No card on the table is marked, and a gate says so — *item 2b, answered `no` by [`ADR-0126`](../docs/adr/ADR-0126-the-table-shows-the-cards-and-marks-none-of-them.md): no wire move, one gate and one line on the showdown card* | nothing — **written and split into two tickets on 2026-09-08**, exactly what `ADR-0126` §4 leaves: one gate and one line on the showdown card. **Where the gate lives was decided rather than assumed** — `ADR-0142` §1 restates `ADR-0091` §4 as a constraint, so a shell gate under `design/` may not read `web-client/`, and it becomes a vitest file beside `no-derivation.test.tsx` that `vitest run` globs into existence with **no workflow edited**. `design/check-frame-cards.sh` reads one card and **does** run in CI (`tickets.yml`, `TASK-140719`). Measured at `c6a41e6d`: **5** frames, **38** `class="pc`, **4** `back mucked`, **2** mentions of `ADR-0126` — and the frame-scoped `awk` is red on exactly the two frames that owe the line and green on the two that carry it. The gate is shown red against the defect it names by **two `verify:` mutations of `PlayingCard.tsx` itself**, both run here and both restoring the file. **No decision was raised** |
| `STORY-1413` | The pot travels to the winner — *item 2c* | nothing — `STORY-1412` no longer holds the version lock; **written and split into five tickets on 2026-09-07**, and the defect was measured rather than assumed: the award already remounts **only the winner's** pile (keyed on `stack`) and the pot's already leaves with the banner, so what is wrong is the **direction** — `pd-chip-flight` falls in from above at every seat, which is right below the middle and backwards above it, at ImKate's plate, which is the plate the human drew the arrow to. Three cards then two client files: two keyframes, two utilities and `ChipPile`'s `from?: "above" | "below"`, **no token minted**, **nothing on the wire**, no store file opened. `ADR-0136` is answered *no* in writing — the 420 ms flight fits inside the 600 ms step, and a beat lengthened **for** an animation would be a hold reduced motion ought to collapse and by §6 cannot. The reduced-motion gate reads the **built stylesheet**: measured, renaming `@utility chip-flight-up` away removes the rule from `dist/assets/*.css` while all five class-name tests stay green |
| `STORY-1414` | `Call` says what it costs — *item 4; `ADR-0122`* | nothing |
| `STORY-1415` | The rematch offer finds the rival wherever they are — *item 5; `DEC-138` answered by [`ADR-0123`](../docs/adr/ADR-0123-a-standing-rematch-offer-follows-the-rival.md) and `DEC-144` by [`ADR-0138`](../docs/adr/ADR-0138-the-panel-mounts-beside-the-lobby-and-the-dismissal-lives-in-the-mount.md); **written and split into eleven tickets on 2026-09-09**, the first two its **minting** card and none of them `atomic:`* | nothing — the split raised `DEC-158`, the architect's, on `ADR-0142` §7's named trigger firing for the first time; it blocked no ticket and is **answered on 2026-09-10** by [`ADR-0148`](../docs/adr/ADR-0148-a-module-names-every-card-that-draws-it-and-every-card-is-registered.md) |
| `STORY-1416` | Play again returns the host to the room they still hold — *item 6* | **split** — four tickets, server only; `DEC-139` and `DEC-111` by [`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md), `DEC-110` by [`ADR-0133`](../docs/adr/ADR-0133-the-room-a-player-holds-is-scanned-for.md), whose §9 says what this story builds (`heldRoom`, the per-player stripes, `heldOrOpen`, `replyToCreateRoom`'s `WAITING` branch), that it moves **no wire** and is **not** `atomic:`, and that it **must leave the `PLAYING` case falling through to `create`** — that half is `ADR-0105` §1's own ticket, and it **is** `atomic:`, because its refusal spends a new `ProtocolError` value, `ALREADY_IN_DUEL`, which moves `ADR-0047` §2's fingerprint and forces a `PROTOCOL_VERSION` step; that is the one place `ADR-0133` **disagrees** with `ADR-0124` §6's prediction |

**Every seam `STORY-1401`–`STORY-1416` is now written and split**, `STORY-1415` last, on
2026-09-09. The
first ticket of every story that puts a new surface in front of a player is its **design card**
(`ADR-0091` §2) — which `STORY-1401` owes none of, because the merged card already draws
`Timebank 3:00` and the client is what fails to transcribe it, and which `STORY-1414` owes none of
either, because it puts no new surface up: it corrects four nodes on cards that already exist, and
`ADR-0122` §7 says so. `STORY-1404` **does** owe one, and its card is the hardest kind: `ADR-0129`
§4 pins no literal, so the sentence's glyphs are the card's and no ticket under that story can write
the assertion until the card merges. `STORY-1411` owes **three**, because `duel-table-states.html`
has never drawn a showdown the viewer lost and one frame of that card is about sixty lines.
Four seams — `STORY-1401`–`STORY-1404` — wait on no decision and are what this epic starts with.
...
| **[STORY-1402](stories/STORY-1402-the-front-door-stops-covering-its-own-wordmark.md)** The front door stops covering its own wordmark, and the door says `Play duel` — *item 3a's overlap and rename, split into four tickets on 2026-09-07. Both halves were **measured** before the split rather than reasoned about. The overlap: `clearance` between the wordmark's rect and the primary control's is a flat **0.000 px** at 390 × 664 **and** at 720 × 900 — the boxes touch, stacked at phone width and side by side at laptop width — and it is **not occlusion**, `elementFromPoint` returning a node inside the `<h1>` in all forty probes. The rename: renaming `Lobby.tsx` alone took the suite from `1159 passed` to `17 failed \| 1142 passed`, all 17 inside two files, which is exactly where the ticket boundary falls* | | | ready |
| | [TASK-140201](tasks/TASK-140201-the-cards-primary-control-says-play-duel.md) The cards' primary control says `Play duel` — *the card first (`ADR-0091` §2). **Three** design files, not one: the screen card's button, the same button on the component card `flow-actions.html` that says of itself "change it here, then the copies", and one phrase in `enter-code.html`'s lede. `create-duel.html`'s `.frame` is **not** touched — it already draws the column — and its `<title>` and `<h1>` keep reading `Create a duel`, because `design/README.md` says a `<title>` names the **card** and the flow is still creating a duel. A gate pins that at exactly 2 occurrences* | XS | done |
| | [TASK-140202](tasks/TASK-140202-the-front-doors-primary-control-says-play-duel.md) The front door's primary control says `Play duel` — *`Lobby.tsx` ×1, `App.test.tsx` ×18, `Lobby.test.tsx` ×10, and **no `atomic:`**: the probe showed `npm run check` refuses every proper subset of exactly these three, which is the ordinary cap. `Play duel` is nine characters shorter, so Prettier reflows the call sites and `format:check` fails until it is run — the ticket says so, having measured it. A gate renames the source back and requires `17 failed \| 121 passed (138)`* | S | done |
| | [TASK-140203](tasks/TASK-140203-the-two-names-no-gate-holds-say-play-duel.md) The two names no gate holds say `Play duel` — *`e2e/whole-duel.test.tsx` and `docs/test-plan.md` (three rows, **four** occurrences — `05-01` names it twice on one line). Separate because the same probe left the e2e file **green**: its reference is a negative observation, `createRoom: false`, which goes stale into a tautology that would hold for a client painting the whole front door over the table. Renaming it is what restores the assertion* | XS | done |
| | [TASK-140204](tasks/TASK-140204-the-front-door-stands-in-the-cards-column.md) The front door stands in the card's column — *one attribute on `Lobby.tsx:388`, the class list its six sibling branches already carry, plus two new tests with **one mutation each**: stripping the column reddens only `stands in the card's column`, wrapping the button in a `<div>` reddens only `are that column's own children`. `ADR-0089` bars a browser from any `verify:`, so the number is the implementer's reading — `clearance` **0.000 → ≥ 8.000**, predicted 12.000 — with a stop rule if the defect does not reproduce. The card's 16 px against the client's 12 px is named and left alone* | XS | done |
| **[STORY-1403](stories/STORY-1403-the-room-code-field-takes-the-link-as-well-as-the-code.md)** The room code field takes the link as well as the code — *item 3a's `"shoud also accept link"`, split into two tickets on 2026-09-07. The parse already exists for the tab that **navigates** to the invite; these two tickets give the person who **pastes** it the same answer, through the same function. No new string, no new visible state, no card* | | | ready |
| | [TASK-140301](tasks/TASK-140301-one-function-reads-a-code-out-of-a-pasted-invite-link.md) One function reads a room code out of whatever was pasted into the field — *`roomCodeFromField` delegates to `roomCodeFromSearch` rather than generalising it: that function's `null` is load-bearing at `boot.ts:84,99`, where `joinRoomCode ?? remembered` would take `"HTTPS://…"` over the remembered code. A `verify:` gate deletes the one line that reads the link and requires the two link tests to go red at `HTTP://192.168.0.142:5173/?ROOM=918RH…`, while the bare-code, no-code and empty tests stay green* | S | done |
| | [TASK-140302](tasks/TASK-140302-the-front-doors-field-joins-the-room-a-pasted-link-names.md) The front door's field joins the room a pasted link names — *two lines in `Lobby.tsx` and three tests through the rendered door. The empty-code refusal is untouched, the field keeps showing what was pasted, and nothing is said about what the text was taken for (`ADR-0022`). A `verify:` gate returns line 73 to today's behaviour and requires exactly the two link tests to fail* | S | done |
| **[STORY-1404](stories/STORY-1404-the-waiting-table-copies-its-link-and-says-the-duel-starts-itself.md)** The waiting table copies its link, says the duel starts itself, and stops labelling the host — *item 3b, split into three tickets on 2026-09-07 against the merged [`ADR-0128`](../docs/adr/ADR-0128-the-copy-control-is-never-absent-and-a-press-that-cannot-copy-hands-over-the-selection.md) and [`ADR-0129`](../docs/adr/ADR-0129-the-host-is-told-the-duel-starts-by-itself.md), which unblocked it. The card is first and carries all three changes at once — the retired no-clipboard frame, the sentence's literal, the bare host plate — because `ADR-0110` §8's own argument is that deciding after the card is drawn costs the drawing twice. `ADR-0129` §4 pins no literal, so the card is a **hard** dependency, not a courtesy: `TASK-140403` transcribes the sentence out of the merged card with a `verify:` gate rather than repeating it* | | | ready |
| | [TASK-140401](tasks/TASK-140401-the-card-redraws-the-waiting-table-three-frames-no-you-and-the-sentence.md) The card redraws the waiting table — three frames, no `You`, and the sentence — *one file. The `Host alone — no clipboard API` frame is retired with the state it drew (`ADR-0128` §6), the surviving three lose `You` while the live table keeps all three of its own, the hand-over frame shows the link selected off `--pd-accent-subtle`, and the sentence is written once under `ADR-0129` §§2–4's two facts and six refusals. Gates pin one distinct spelling, the words *rival* and *duel*, and no digit or refused word — the mechanical half of §3; the rest is the review's and the human's* | S | done |
| | [TASK-140402](tasks/TASK-140402-the-copy-control-is-never-absent-and-a-press-that-cannot-copy-hands-over-the-selection.md) The copy control is never absent, and a press that cannot copy hands over the selection — *`CopyLink` loses its `!navigator.clipboard` early return and its `\| null` return type; both no-copy routes share one `handOver` that focuses the box and selects the whole link. jsdom implements no Clipboard API, so a test that installs none **is** the browser `ADR-0128` was written for — the path `ADR-0117`'s localhost proofs can never reach. The hand-over tests break the box's own `autoFocus`/`select()` first, or they would pass on a press that did nothing* | S | done |
| | [TASK-140403](tasks/TASK-140403-the-host-plate-goes-bare-and-the-table-says-the-duel-starts-itself.md) The host's plate goes bare and the table says the duel starts by itself — ***`atomic: 4`***, the one on this story: a prototype measured that removing `You` and adding the sentence reddens exactly four tests across three merged test files, and `npm run check` refuses every intermediate. Four `verify:` gates extract the literal from the merged card and require all four files to carry it byte-for-byte, so no coder can invent the sentence `ADR-0129` §4 declined to pin* | S | done |
| **[STORY-1414](stories/STORY-1414-call-says-what-it-costs.md)** `Call` says what it costs — *item 4, split into five tickets on 2026-09-07 against the merged [`ADR-0122`](../docs/adr/ADR-0122-call-names-the-price-and-raise-to-names-the-total.md). The change itself is one subtraction that already ships at `ActionBar.tsx:255`; the story is the tests, because `ActionBar.test.tsx` commits **zero in seventeen places** under a `?? 0` default and all three of its `Call` assertions sit at that zero, where `callTo − 0` and `callTo` are the same number — so the repair lands green against today's suite either way. Every ticket names its fixture's two commitments and proves its tests red by mutation. Chosen shape: `actionText` gains a required, undefaulted fourth parameter, which the type checker makes a three-file ticket, so **nothing here is `atomic:`*** | | | ready |
| | [TASK-141401](tasks/TASK-141401-the-cards-draw-the-call-mark-bare.md) The cards draw the call mark bare — *the card goes first (`ADR-0091` §2, `ADR-0107` §6's shape). `ADR-0122` §7 named one node; **four** were measured — `duel-table-states.html:298`, `seat-and-pot.html:252` and its caption, and the `.last-act` comment saying two of six acts are bare in **three** cards. Gates pin the eight `Call` **button** nodes as unchanged, because those frames commit nothing and 400 is already the price* | XS | done |
| | [TASK-141402](tasks/TASK-141402-the-call-button-prints-the-price-the-press-takes.md) The `Call` button prints the price the press takes from the stack — *`actionText` gains a required `committedThisStreet` and returns `callTo − committedThisStreet` for `CALL`; the KDoc that refuses the netting by name is corrected; `action-text.test.ts` moves off zero at all ten call sites, including for the three verbs whose figure must **not** move. A gate deletes the subtraction and requires exactly one of eleven tests to fail* | S | done |
| | [TASK-141403](tasks/TASK-141403-the-bar-is-pinned-where-the-two-commitments-differ.md) The bar is pinned at a frame where the seat has already committed — *test-only, and the reason it exists is that a caller passing `0` would satisfy every unit test in `action-text.test.ts`. Two frames, 600/200 and 925/75, asserting the button row **and** the absence of the total; the never-derives guard admits `ADR-0122` §5's one further quantity and is made to refuse `callTo`. The gate mutates the wiring back to `0` and requires both files red* | S | done |
| | [TASK-141404](tasks/TASK-141404-the-last-act-mark-goes-bare-on-a-call.md) The last-act mark goes bare on a call — *`ADR-0109` §2's figure list loses `Call`: `PlayerCalled` carries only `to`, and after the act the caller's `committedThisStreet` **is** `to`, so the price is gone by the time the mark exists. It reddens exactly **two** merged tests in two files, counted rather than guessed — `DuelTable.test.tsx`'s call fixture asserts placement, never the mark's text. `SeatPlate.tsx` is not opened: it already draws a null amount bare* | S | done |
| | [TASK-141405](tasks/TASK-141405-adr-0109-records-the-clause-adr-0122-amended.md) `ADR-0109` records the clause `ADR-0122` amended — *bookkeeping `ADR-0122`'s own PR owed and did not write: `ADR-0109` still reads `Status: Accepted` and its index row still summarises the figure the mark no longer prints. `Amended by 0122` appears nowhere in `docs/adr/`. The body is annotated, never rewritten (`ADR-0068`'s precedent), and line-count gates enforce it* | XS | done |
| **[STORY-1411](stories/STORY-1411-the-showdown-shows-the-hands-it-reached-and-stands-long-enough-to-read.md)** The showdown shows the hands it reached, and stands long enough to read — *item 2a, split into six tickets on 2026-09-07 — a **seventh**, `TASK-141107`, was added the same day, after review showed `TASK-141102`'s 48 whole-file gates all stay green when two frames' rival card slots are swapped — against the merged [`ADR-0120`](../docs/adr/ADR-0120-a-showdown-shows-the-hands-the-rules-showed-and-the-beat-that-shows-them-stands.md) and [`ADR-0136`](../docs/adr/ADR-0136-a-beat-declares-its-own-length-and-zero-silences-every-beat.md). The epic's premise was **corrected downward before the split**: the shown hand is not "drawn by nothing" — it reaches the table through the **snapshot** (`PlayerView.of`'s `showCards` → `SeatView.holeCards` → `DuelTable.tsx:76-78`) and `duel-secrecy.test.tsx` has pinned it since it was written. What is missing is **time**: a hand called down to the river carries no `StreetDealt`, so its ending is **one** step at 600 ms. So this is a card, a hold, and the tests that pin both — no second drawing path, no server file, no engine file, and **nothing on the wire**, so nothing here is `atomic:`* | | | ready |
| | [TASK-141101](tasks/TASK-141101-the-card-strikes-the-muck-line-and-both-second-lines.md) The card strikes the muck line and both banners' second lines — *the card first (`ADR-0091` §2), and this ticket is the arrears: `ImKate mucks` is struck by `ADR-0120` §1 (*"no line is added to say a hand was mucked"*, and `BetLine` prints a committed figure and nothing else), while `Two pair, aces and sevens` and `Nobody shows —…` are the two `ADR-0095` §3 struck **by name** and whose *"own `module: design` ticket"* was never written. `ImKate folds` **stays**, gated at 1: `ADR-0120` §1 strikes the muck line and says nothing about a fold line* | XS | done |
| | [TASK-141102](tasks/TASK-141102-the-card-draws-the-showdown-the-viewer-lost.md) The card draws the showdown the viewer lost — *the state this card has never had and the one the human was looking at: ImKate's two cards **face up in her own seat** at 40 px, `Your rival wins 4,850`, chips to her. Drawn as a plain `.pc`, **not** `.pc mini` — that rule is inlined at `:182` and used by nothing, and `PlayingCard.tsx`'s `CardFace` has no mini variant, so the card states what the client draws. Gates split `class="pc` into **16 + 13 = 29** so a third class on any card fails (`ADR-0126` §1), pin `animation:` at 3 and `--pd-motion` at 12 (`ADR-0136` §6 — the hold has no CSS), and refuse the hand-name vocabulary in the frame **and its margin*** | S | done |
| | [TASK-141103](tasks/TASK-141103-the-card-draws-the-split-where-both-hands-are-shown.md) The card draws the split where both hands are shown — *`ADR-0120` §1's third row, finishing the table: `A♣ K♦` against the hero's `A♥ K♠` on `A♠ 7♦ 2♣ J♥ 7♣` so the two hands are indistinguishable rank for rank, `Split pot — you win 2,425` stating the **viewer's own** share (`ADR-0095` §2), both piles flying, and 5,375 + 14,625 = 20,000. Built on an **even** pot so no odd chip arises; the note states the out-of-position rule and the frame does not draw it* | S | done |
| | [TASK-141104](tasks/TASK-141104-a-beat-carries-its-kind-and-one-frame-decides-it.md) A beat carries its kind, and one frame decides it — *`RevealStep.hold: "step" \| "read"`, set once in `layOutReveal` from `ADR-0120` §3's predicate over `view.viewerSeat` and `seat.holeCards`. **Three** files, and the third was measured rather than grepped: `src/routing/room-standing.test.ts:58` builds a `RevealStep` literal without ever spelling the type (`TS2741`), and `tsc` reports no other. It owns the two merged `toEqual` assertions it invalidates — both gain `hold: "step"`, the honest value, and the **`"read"` mutation reddens both**, which is what proves they were tightened rather than relaxed. Two mutations: `2 failed \| 90 passed (92)` and `4 failed \| 88 passed (92)`* | S | done |
| | [TASK-141105](tasks/TASK-141105-the-store-holds-the-read-beat-and-zero-silences-every-beat.md) The store holds the read beat, and zero silences every beat — *`readMillis` absent meaning `stepMillis`, mapped in `armTick` **after** the `stepMillis === 0` return. Proved by the injected `schedule`'s `delayMillis` **arguments** — `[600, 600, 600, 2000]` and `[600]` — never by a call count, which is `ADR-0136` §7.4's own instruction and is load-bearing because §4 keeps the DOM byte-identical at both lengths. The second mutation deletes the zero gate and requires `src/e2e` to fall from `55 passed (55)` to `24 failed \| 31 passed (55)` across **4** files: `drive-duel.tsx` is not edited, gains no redundant `readMillis: 0`, and a `git status` gate says so* | S | done |
| | [TASK-141106](tasks/TASK-141106-boot-names-the-read-beats-length-beside-the-steps.md) Boot names the read beat's length beside the step's — *`REVEAL_READ_MS = 2000` beside `REVEAL_STEP_MS = 600`, `BootOptions.readMillis`, one `??` line. `bootDuelClient` builds its own schedule, so `setTimeout` is the only seam a duration is observable through: fake timers, a spy, two figures (the constant and 50) because one fixture default cannot tell a pass-through from a hard-coded 2,000. Deleting the pass-through reddens **both** — `2 failed \| 26 passed (28)`* | XS | done |
| | [TASK-141108](tasks/TASK-141108-the-read-beat-is-spent-once-and-a-repeat-is-a-step.md) The read beat is spent once, and a repeat of the same hand is a step — *added 2026-09-07, **unblocked 2026-09-07 by [`ADR-0139`](../docs/adr/ADR-0139-the-read-beat-is-spent-once-and-the-store-already-remembers.md)**, which answers `DEC-153` for the guard and says this ticket stands **exactly as written**. `ADR-0120` §3's rule is *a hand face up the viewer **has not been shown before***, but the sentence restating it *as the client can evaluate it* drops the word **before**, and `TASK-141104` merged the restatement: every delivery carrying rival hole cards answers `"read"`, because `PlayerView.of` refills a revealed seat's `holeCards` on every projection of the hand, `duel-state.ts` re-lays a reveal on every `COMPLETE`, and `duel-store.ts` keeps one state across sockets. **Latent, not live, and measured rather than argued:** a throwaway `poker-server` probe over 60 duels found **0** hand-completing `Snapshot`s in **2,220** resume frames and **0** repeats across **604** hand endings — `act` calls `advance` in the same call, so a live hand is never over — while the same probe, asked what a runner would hand back without that call, found **602**. `ADR-0139` §§2–4 take the guard anyway and amend `ADR-0136` §1 in three sentences: `layOutReveal` gains `held: PlayerView \| null`, the call site passes `state.view`, and **`DuelState` gains no field** — the memory was already there. Prototyped and reverted: 2 files, **92 → 95** tests, mutations `2 failed \| 93 passed (95)` and `1 failed \| 94 passed (95)`, each gate proving with `! cmp -s` that the literal it mutates was there* | S | done |
| **[STORY-1405](stories/STORY-1405-the-table-fits-the-phone-it-is-played-on.md)** The table fits the phone it is played on — *item 3d, split into three tickets on 2026-09-07 against the merged [`ADR-0121`](../docs/adr/ADR-0121-the-table-reflows-never-scales-and-the-shape-is-measured-on-the-device.md). The ADR §5 deliberately did not diagnose the cause; the split did, headless at 390 × 664 against the client's own built stylesheet. **All 57 recorded beats fit on both axes** — and that is the finding, not a clean bill: the committed script carries no `TurnClock`, so no recorded frame has ever drawn a plate at the width `EPIC-13` gave it. The real `SeatPlate` overflows by **+21.36 to +53.19 px** the moment a last-act mark and a timebank sit on it together; the plate as `ADR-0106` measured it fits, and the plate as `ADR-0113` measured it fits. The cause is one utility that compiles to nothing — `min-w-0` needs `--spacing`, which `app.css:55` sets to `initial` — so the name block keeps flexbox's automatic minimum and the stack figure is pushed off screen. **No design ticket:** the merged card says *".who gives ground by truncating the name first"* and measures 390 / 390 at the widest strings. The device reading is the **human's** and no ticket pretends otherwise* | | | ready |
| | [TASK-140501](tasks/TASK-140501-both-axes-are-read-at-the-phone-and-the-reading-is-written-down.md) Both axes are read at the phone, and the reading is written down — *the reading comes before the repair (`ADR-0121` §4). Three named beats on the **running** client at `size 390 664`, both tabs, both axes, plus each plate's tail overhang, written into `EPIC-14`'s Definition of done as an `ADR-0088` §3 receipt with the **device** row left for the human. `ADR-0089` §2 keeps every number out of `verify:`, so the gates check the receipt's shape — a commit, three rows, both axis pairs each — and that the epic's checklist is otherwise untouched at 6 and 9. B3 is the reproduction and carries the stop rule: `scrollWidth ≤ clientWidth` there means the defect did not reproduce, and the ticket stops rather than looking for something else to fix* | XS | done |
| | [TASK-140502](tasks/TASK-140502-the-seat-plates-name-block-gives-ground-again.md) The seat plate's name block gives ground again — *one attribute: `min-w-0` → `min-w-[0px]` at `SeatPlate.tsx:50`, measured to take the document from `scrollWidth` **443** to **390** against 390 while the height axis does not move. The spelling is **forced by a merged gate** — `theme.test.ts` admits only resets and `var(--pd-*)` references inside `@theme static`, so a `--spacing-0` step would need a minted token. Two tests with one mutation each, and a third gate that reads the **built stylesheet**, because a class-name assertion passes equally for a utility that generates nothing — which is the entire defect. The epic is the third file: the ticket that moves the number records the move* | S | done |
| | [TASK-140503](tasks/TASK-140503-the-second-inert-min-width-is-spelled-so-it-generates.md) The second inert min-width is spelled so it generates, and a gate closes the class — *`ActionBar.tsx:149`, the only other `min-w-0` in the tree, inert for the same reason and with **no measured consequence** — the bar and the pot strip read 390 / 390 at the widest figures `DuelFormat.DEFAULT` can produce. It is its own ticket because the gate that makes the class impossible — the string `min-w-0` appears nowhere under `web-client/src/` — can only be written once both sites are clean. Prettier moves the class; the ticket says where it lands* | XS | done |
| | [TASK-141107](tasks/TASK-141107-the-showdown-card-is-gated-frame-by-frame-not-file-by-file.md) The showdown card is gated frame by frame, not file by file — *added 2026-09-07 after `TASK-141102` merged: every one of that ticket's 48 gates is a **whole-file aggregate count**, and a swap of the showdown-lost and fold `oppcards` slots leaves all of them green — re-confirmed on the merged card at `96b9a278`, where **41 of 41 count gates, all 5 absence gates and `check-drift.sh` still pass** on the swapped file while the fold frame draws a folded hand face up, which `CLAUDE.md` forbids outright. `design/check-frame-cards.sh` cuts the card into `<div class="frame">` blocks, finds each by its own `<h2>` — never by line or ordinal, and a gate inserts a frame **before** every existing one to prove it — and counts inside that frame's own rival slot: fold **0/0/2/2**, showdown-lost **1/1/0/0**, all four measured. The reviewer's brief put those numbers at frame scope, where they are false (the fold frame holds seven face-up cards); they are exact one level in. Two mutation arms, both falsified against a `exit 0` stub. No frame markup changes* | S | done |
| **[STORY-1416](stories/STORY-1416-play-again-returns-the-host-to-the-room-they-still-hold.md)** Play again returns the host to the room they still hold — *item 6, split into four tickets on 2026-09-07 against the merged [`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md) and [`ADR-0133`](../docs/adr/ADR-0133-the-room-a-player-holds-is-scanned-for.md). The epic's premise was **corrected before the split and holds**: the ten-minute `WAITING` life already ships, so what the human met was a **second room**, not a short-lived one. One linear chain, **server only** — no client file, no string, no design card, no wire and no `PROTOCOL_VERSION`, so nothing here is `atomic:`. The one thing every ticket repeats: the `PLAYING` case **keeps falling through to `create`**, because handing that room back silently would ship `ADR-0105` §1's refusal without §4's words; that half, and `ADR-0133` §6's `JoinRoom` guard with its load-bearing *code differs* comparison, are `ADR-0105` §1's own `atomic:` ticket, outside this epic and not yet cut* | | | ready |
| | [TASK-141601](tasks/TASK-141601-the-registry-answers-which-live-room-a-player-holds.md) The registry answers which live room a player holds — *`heldRoom(player)`, non-suspending, returning the whole `Room`, where **held** is `seatOf` non-null **and** state `WAITING` or `PLAYING`, filtered **in the method** because a caller that forgot would break `ADR-0105` §2's finished row and `offerRematch`. It reads the `@Volatile` `Holder.room` that `get(code)` already reads and takes no mutex — the `fun`, not `suspend fun`, declaration is the proof, since `Mutex.withLock` is suspending. Six tests, one of them the room a **guest** sits in at seat **1**, so the lookup is provably not host-only; two gates pin `.withLock {` at **2** and `tryLock` at **0*** | S | done |
| | [TASK-141602](tasks/TASK-141602-the-held-room-answer-is-total-and-not-the-maps-order.md) The held-room answer is total, and not the map's order — *`PLAYING` first, then ascending `lastActivityAt`, then ascending `RoomCode.value`, because an answer that depends on a hash map's iteration order is neither testable nor explicable. Three tests, and each arranges the **next** key pointing the other way: the `PLAYING` room is the newer in one registry and the older in another; the winning `WAITING` room carries the **higher** code in one and the lower in the other; the tie-break runs at one frozen stamp with the codes minted in both orders. Its scripted source is named `HeldRoomCodes` because `RoomRegistryTest.kt` already declares a `private class ScriptedCodes` in the same package, and per-file privacy does not cover top-level classes* | S | done |
| | [TASK-141603](tasks/TASK-141603-opening-a-room-is-find-or-create-serialised-per-player.md) Opening a room is find-or-create, serialised per player — *`heldOrOpen(host, format)` = `heldRoom ?: create` under a **second** striped `Mutex` array, `SeatOwnership`'s shape and never pruned, for `SeatOwnership`'s own reason. The critical section holds **no suspension point**, which is the whole deadlock answer. Five tests: the fresh open, the hand-back proved by a stamp of `1_000` against a clock left at `61_000`, `ADR-0105` §2's finished row, **100 concurrent calls for one player → one room and 100 for 100 players → 100 rooms** (one input cannot tell a stripe from a global lock), and `ADR-0133` §4's **tripwire** kept as a test — `mutate`'s blocks stay plain function types, so nothing inside a room's critical section can take a lock. Gates pin `.withLock {` at **3*** | S | done |
| | [TASK-141604](tasks/TASK-141604-pressing-play-again-hands-back-the-room-the-host-holds.md) Pressing play again hands back the room the host holds — *`replyToCreateRoom` calls `heldOrOpen`, answers `RoomJoined` for a `WAITING` room and **falls through to `create` for anything else**, repointing `RoomMembership` at the room the answer names — which disarms `ADR-0118`'s window at the source it named. `DuelSocketRoomTest` goes **10 → 14** with none of the ten edited. The seat is `seatOf` off the returned snapshot and the shipped literal `0` is gated out — but the pin is **structural on purpose and the ticket says so**: `Room.open` seats the host at 0, so a returned `WAITING` room and a fresh one carry the same seat and no behavioural test in this story can tell a derived seat from the constant* | S | done |
| **[STORY-1413](stories/STORY-1413-the-pot-travels-to-the-winner.md)** The pot travels to the winner — *item 2c, split into five tickets on 2026-09-07 against the merged [`ADR-0115`](../docs/adr/ADR-0115-motion-never-carries-a-fact-and-reduced-motion-stills-every-surface.md) and [`ADR-0136`](../docs/adr/ADR-0136-a-beat-declares-its-own-length-and-zero-silences-every-beat.md). The human's arrow ran from the pot's pile to **ImKate's** seat plate; measured, the award already remounts only the winner's pile and the pot's already leaves with the banner, so the one defect is that `pd-chip-flight` arrives **from above at every seat** — from the pot below the middle, from off the top of the table above it. The flight gains a side and nothing else: no token minted, no store file opened, no Kotlin file, nothing on the socket. `ADR-0136` is answered **no** in writing — 420 ms fits inside 600 ms, and a beat lengthened for an animation would be a hold reduced motion ought to collapse and by §6 cannot* | | | ready |
| | [TASK-141301](tasks/TASK-141301-the-component-card-draws-both-directions-and-the-frame-they-share.md) The component card draws both directions and the still frame they share — *the card first (`ADR-0091` §2), and under `ADR-0115` §1 the **still form** is the thing that carries the fact, so the `stilled` row is relabelled as the frame **both** flights end on. `.pile.flying` becomes `.pile.flying-down` and `.pile.flying-up` over one new keyframe; the three merged `--pd-motion-chip-*` values are reused, so this is composing rather than minting. Gated at `--pd-motion` **15**, `pd-chip-flight` **5**, `animation:` **4** unchanged, `class="pile` **7**, and a mutation that collapses the two directions* | S | done |
| | [TASK-141302](tasks/TASK-141302-the-duel-table-cards-copy-of-the-flight-follows.md) The duel table card's copy of the flight follows the component card — *the card's own comment calls the block a **faithful copy**, and it draws no flying pile at all: the copy is of the block, not of the used subset. Two gates `sed` both keyframes out of both files and compare the **text**, with a non-empty test so an empty extraction cannot pass* | XS | done |
| | [TASK-141303](tasks/TASK-141303-the-rival-takes-the-pot-from-below-on-the-states-card.md) The rival takes the pot from below on the states card — *the frame the human annotated. Depends on `TASK-141103`, which adds two more flying piles to the same file, so the gate is **frame-relative rather than a total** (`TASK-141107`'s lesson): tracking the last `opp`/`hero` marker, every `flying-up` sits in an `opp` block and every `flying-down` in a `hero` block. Measured `up=1 down=2 bad=0` exit 0; swapped, `up=0 down=0 bad=3` exit 1* | XS | done |
| | [TASK-141304](tasks/TASK-141304-the-flight-has-a-side-and-the-built-sheet-proves-it.md) The flight has a side, and the built stylesheet proves it — *two keyframes, two utilities, `ChipPile`'s optional `from`, and gates that `npm run build` then `grep -F` over `dist/assets/*.css`, because a class-name assertion passes identically for a rule that generates nothing — this epic's third encounter with that defect class. Three mutations, all run at planning time: the utility renamed away (built rule gone, **`5 passed (5)`** still green), the reduce block turned to `no-preference` (reduce grep red, `src/table` **253** green), and the side table collapsed (**`1 failed | 4 passed (5)`**). A **trap is written into the ticket**: spelling the class dynamically does **not** drop the rule, because Tailwind scans the test file's literals too* | S | done |
| | [TASK-141305](tasks/TASK-141305-the-rivals-seat-takes-its-chips-from-below.md) The rival's seat takes its chips from below — *one expression, `from={props.isViewer ? "above" : "below"}`, read off the plate's own prop rather than threaded from `DuelTable`, whose KDoc already fixes the arrangement. The test renders **both** seats — one input cannot tell an expression from a constant — and **both** hard-codings redden it, each `1 failed | 19 passed (20)`. `PotStrip` and `DuelTable` are gated as unopened* | XS | done |
| **[STORY-1407](stories/STORY-1407-a-player-names-themselves-before-their-first-duel.md)** A player names themselves before their first duel — *item 1a, split into **sixteen** tickets on 2026-09-08. The product's **first dialog**: a screen in place of the front door at the player's own first press, a field holding a browser-drawn suggestion, and two ways out that both end in the duel the press asked for. `ADR-0119` is the product half and `ADR-0137` the mechanism; nothing crosses the wire, no Kotlin file is opened and **no ticket is `atomic:`**. Four of the sixteen are word lists — `ADR-0137` ships none, and prettier prints one string per line, so 350 words is four tickets* | | | done |
| | [TASK-140701](tasks/TASK-140701-the-card-draws-the-ask-four-frames-and-two-ways-out.md) The card draws the ask — four frames, two ways out, and the words — *the card first (`ADR-0091` §2), and the fourth frame is the one that would otherwise never be drawn: a refusal `mayTryAgain` closes, the form withdrawn and the **skip still standing**, which is `ADR-0119` §2's *no state leaves a player unable to duel* made visible. Two controls and **no third**: the *suggest another* question `ADR-0137` hands to the card is answered **no**, because the field is editable and a `conflict` already redraws. Gated at 4 frames, 7 buttons, 0 links, one distinct spelling for each of six strings, the two shipped refusal sentences, and the word *permanent* nowhere* | S | done |
| | [TASK-140702](tasks/TASK-140702-the-vocabulary-module-and-the-tests-that-make-a-400-impossible.md) The vocabulary module, its first list, and the tests that make a `400` impossible — *`name-vocabulary.ts` imports nothing and opens with fifty qualities; six `O(entries)` tests close every branch of the server's own `canonicalDisplayNameOrNull` — the regex for `Cc`/`Cf`/whitespace, NFC per entry, ten code points per entry, the longest-join sum, a **case-fold** uniqueness rule that makes the arity a count of reachable names rather than of strings, and the arity itself. Five mutations, each naming the one test it reddens* | S | done |
| | [TASK-140703](tasks/TASK-140703-the-second-list-is-a-hundred-substances.md) The second list is a hundred substances — *the middle word of `Quiet Iron Raven`. The five shape tests range over the whole vocabulary and are **not edited**; one line moves, to `toEqual([50, 100])`* | S | done |
| | [TASK-140704](tasks/TASK-140704-the-third-list-takes-its-beasts.md) The third list takes its beasts — *a hundred birds, animals and fish; `toEqual([50, 100, 100])`, a product of `500_000`, which is correct at this point because the floor is not asserted until the vocabulary is whole* | S | done |
| | [TASK-140705](tasks/TASK-140705-the-third-list-takes-its-offices-and-the-arity-reaches-a-million.md) The third list takes its offices, and the arity reaches a million — *a hundred roles, ranks and trades; `50 × 100 × 200 = 1_000_000`, **exactly** `ADR-0137` §4's floor, so one deleted word puts the vocabulary under it. The fold test now spans both halves of the list* | S | done |
| | [TASK-140706](tasks/TASK-140706-the-floor-and-the-draw.md) The floor and the draw — *`SUGGESTION_SPACE_FLOOR = 1_000_000` and `suggestName(random)`: one entry per list, in order, joined by one `U+0020`, with `ADR-0137` §2's clamp written verbatim so a source returning `1` cannot join `undefined` into a permanent name. The floor becomes a test; a fourth test runs the server's whole rule set over the **joined** string. The module takes no player fact — gated at one import and zero storage or globals* | S | done |
| | [TASK-140707](tasks/TASK-140707-the-redraw-is-bounded-and-only-replaces-what-it-drew.md) The redraw is bounded, and a broken source cannot spin — *`replacing`, `const MAX_DRAWS = 4;`, and two tests that count the calls to `random` because the returned string alone cannot tell one draw from four. Raising the bound reddens one test; lowering it to 1 reddens both* | S | done |
| | [TASK-140708](tasks/TASK-140708-one-module-owns-the-skipped-key.md) One module owns the key that says this browser skipped the ask — *`pd.nameAskSkipped`, `account-offer-settled.ts`'s shape as `ADR-0119` §5 requires, with **no export that clears it** and a fifth row in `one-module-owns-each-storage-key.test.ts`. A mutation writes the literal into `name-text.ts` to prove the row bites* | S | done |
| | [TASK-140709](tasks/TASK-140709-one-predicate-decides-who-is-asked.md) One predicate decides who is asked for a name — *`askForName({ profile, skipped })`: true only for a `profile` read holding no name and no removal, in a browser that has not skipped. **No `signedIn` input at all** — gated at zero occurrences — because `ADR-0119` §1's condition is holding no name, not holding no account. Six tests, each flipping exactly one field of one asking input* | S | done |
| | [TASK-140710](tasks/TASK-140710-the-asks-words-are-transcribed-from-the-card.md) The ask's words are transcribed from the card — *six constants, each gated against `design/screens/name-ask.html` by extracting the card's own strings and matching them as fixed patterns, so the planner never had to know the words. No `PERMANENCE_LINE`, no *permanent*, and no refusal sentence: those stay `name-text.ts`'s* | S | done |
| | [TASK-140711](tasks/TASK-140711-the-ask-draws-itself-and-its-two-ways-out.md) The ask draws itself, and both ways out end in the duel — *`NameAsk.tsx`: `const { random = Math.random } = props;` at the call site (`ADR-0137` §7), one draw per mount in the `useState` initialiser — a **source** gate, because a controlled field's value comes from state either way — the skip **outside** the `<form>` so `Enter` cannot reach it, and `onNamed` only after the server has said yes. A skip that also called `setName` reddens the one test that guards *nothing is written unless the player takes it** | S | done |
| | [TASK-140712](tasks/TASK-140712-a-refusal-reads-in-the-words-the-product-already-uses.md) A refusal reads in the words the product already uses, and never blocks the skip — *`refusalSentence` and `mayTryAgain` imported, **no branch on an outcome kind**, so `STORY-1409` deleting the `permanent` kind changes this file not at all. The form is **hidden, not unmounted**; the skip is never hidden and never disabled; two presses send one write, which matters more here than on `NameSurface` because under `ADR-0130` §1 a second write is a **rename*** | S | done |
| | [TASK-140713](tasks/TASK-140713-a-conflict-redraws-only-what-the-generator-put-there.md) A conflict redraws, and only over what the generator put there — *an equality rather than a dirty flag (`ADR-0137` §5), passing `replacing` so the refused string cannot come back. The typed-name test asserts the field **and** that the source was not called again; widening the condition to every refusal reddens exactly one test* | S | done |
| | [TASK-140714](tasks/TASK-140714-the-browsers-answer-is-bound-at-the-boot-seam.md) The browser's answer is bound at the boot seam, and the fake keeps up — *`main.tsx` gains `nameAskSkippedHere` / `skipNameAskHere` over a `Storage` binding of its **own** (not `offerStorage`, which `ADR-0125` retires with the offer in `STORY-1408`), and `App.test.tsx`'s explicit factory names both. Measured: without this, the first new `../main` import fails **24 of that file's 36 tests** — so the split is two green tickets instead of one four-file `atomic:` one* | XS | done |
| | [TASK-140715](tasks/TASK-140715-the-press-is-held-until-the-ask-is-answered.md) The press is held until the ask is answered, and then it goes through — *`useState<CreateRoom | JoinRoom | null>` so the ask can only release a duel-starting frame, one `startDuel` helper behind both controls, and the branch placed **below** the room branch — a line-number gate holds that order, which is what makes *never over the table, the waiting room or the result* structural rather than conditional. All 104 merged tests stay green, measured, because every merged press test renders without a `ProfileProvider`* | S | done |
| | [TASK-140716](tasks/TASK-140716-the-moments-the-ask-never-stands.md) The moments the ask never stands, pinned one at a time — *five tests and **no production edit**, proved by a clean working tree over every production path: a named player, a removed name, a browser that skipped, a profile read that has not landed, and a `RoomJoined` frame taking the screen from a standing ask. Each asserts the ask is absent **and** that the press went out, because *no ask, no duel* would satisfy an absence check on its own* | S | done |
| | [TASK-140717](tasks/TASK-140717-a-refused-player-may-press-again.md) A refused player may press again, and a test says so — *found while reviewing `TASK-140712` and **measured, not suspected**: deleting the in-flight guard's reset leaves all eight of that ticket's tests green, so a build in which one refusal locks a player out of naming themselves ships on a full green run. That ticket is not at fault — its `Tests` table names no retry case, and its *Out of scope* refuses an **automatic** resend, which is a different thing from the player pressing again. One test, `NameAsk.tsx` byte-unchanged* | XS | done |
| | [TASK-140718](tasks/TASK-140718-a-held-press-does-not-outlive-its-screen.md) A held press does not outlive the screen it was made on — *found by the coder implementing `TASK-140715` and **traced, not guessed**: `Lobby`'s six chosen-screen branches render above the ask, so navigating to one leaves `Lobby` mounted and `heldPress` set, and returning brings the ask back still holding a press the player made before they left. It never fires on its own — the player must still answer — so what is wrong is an intent outliving its screen. The unmount case is already correct; only the in-app hash router leaks. `TASK-140715` is not at fault: its scope covers holding and releasing the press, and `ADR-0119` §5 governs what **sets** it, not when it clears* | S | done |
| | [TASK-140719](tasks/TASK-140719-the-frame-card-gate-runs-in-ci.md) The frame-card gate runs in CI — *found by the architect answering `DEC-155`, and the reason that decision refused a new shell gate: the failure `DEC-155` names — a check living only in ticket `verify:` blocks, not outliving the merge — **has already happened here, to a design gate, unnoticed**. `design/check-frame-cards.sh` is named by three tickets and by no workflow, so it has guarded the three commits whose authors ran it by hand. One step in `tickets.yml`; the script is not edited, and criterion 3 requires the step be shown **able to fail*** | XS | done |
| | [TASK-140720](tasks/TASK-140720-the-frame-gate-says-which-card-it-guards.md) The frame gate says which card it guards — *found wiring the gate into CI, by asking what it does with a card that has no `<h2>` frames: it does not look at it. The script reads **one** of eleven cards, `duel-table-states.html`, and one slot inside it. That is right for what `TASK-141107` built it for — but **two landings have already cited it alongside claims about `account.html`'s frames**, claims a reviewer verified by hand while the script never opened the file. Header and success line say the scope; the logic is byte-unchanged, and widening it is a separate question* | XS | done |
| **[STORY-1408](stories/STORY-1408-the-account-screen-names-the-anonymous-profile-and-the-post-win-offer-goes.md)** The account screen names the anonymous profile, the post-win offer goes, and the front door stops describing an absence — *item 1b, split into fourteen tickets on 2026-09-08 against the merged [`ADR-0125`](../docs/adr/ADR-0125-the-account-screen-names-the-anonymous-profile-and-owns-the-door.md) and [`ADR-0132`](../docs/adr/ADR-0132-the-profile-says-whether-it-holds-a-password.md). **The deletions come first and wait on no fact** — `ADR-0125` §§1, 2, 5 and 6 need nothing the client does not already hold, so the four largest, most mechanical tickets merge before the field's chain begins. **Three `atomic:` tickets, each sized by an `ADR-0070` probe run to `exit 0`, not remembered**: the offer's render path (5), its modules and browser key (13), the server field (6) and the client field (7 — six probed, one for the parser's own proof). The server-to-client coupling `ADR-0132` §7 calls *"one diff"* is honoured by **ordering** instead, on the ADR's own asymmetry — a server that sends an unread field breaks nothing, a client that requires an unsent one breaks every profile read — and no CI gate boots the two against each other. The split also **measured a pre-existing order dependence**: with production sources untouched, deleting the offer test `ADR-0125` §1 deletes makes `Lobby.test.tsx`'s attach test fail, because an `<a href="/">` click in an earlier test leaves jsdom a refused navigation it retires in a later task, and that task drops a fragment set before rendering. The repair is written into `TASK-140801` and was verified against the whole gate set; a literal `setTimeout` is unavailable, `virtual-time.test.ts` being the gate that says so. **`STORY-1408` and `STORY-1410` must not run at the same time** — both edit `docs/protocol.md`'s *Profile endpoint* section and `web-client/src/profile/profile.ts`* | | | done |
| | [TASK-140801](tasks/TASK-140801-the-offer-leaves-the-result-screen.md) The offer leaves the result screen — ***`atomic: 5`***, probed: `DuelResult`'s `offer` prop drags `DuelResult.test.tsx` through `TS2322`, `Lobby.tsx`'s five now-unused imports fail `--max-warnings 0`, and six merged tests in two files assert the offer renders. It **owns the test its change invalidates**: `Lobby.test.tsx`'s `puts the attach form…` passes today only because the offer test running before it absorbs a stray jsdom navigation, and the ticket carries the verified repair — reach the account screen through the product's own `Account` door rather than staging the address before the render* | S | done |
| | [TASK-140802](tasks/TASK-140802-the-offers-modules-and-its-browser-key-leave-the-product.md) The offer's modules and its browser key leave the product — ***`atomic: 13`***: eight files deleted, `main.tsx` and `drive-arc.tsx` failing `TS2307`, both e2e harnesses failing `TS2353` on an `ArcWiring` two fields shorter, and the one-module-owns-each-key gate reddening while it names a file that no longer exists — which is `ADR-0125` §1's own reason for one diff. `pd.accountOfferSettled` is **orphaned, not cleared**, and the ticket forbids a cleanup* | S | done |
| | [TASK-140803](tasks/TASK-140803-the-app-test-stops-mocking-two-exports-that-are-gone.md) The app test stops mocking two exports that are gone — *`App.test.tsx` names `offerSettledHere` and `settleOfferHere` in a `vi.mock` factory after `main.tsx` stops exporting them, and **nothing fails**: measured green on the probe tree, so no gate names the file and it cannot be a row in the ticket above. A dependency, not a coupling — `TASK-041641`'s shape* | XS | done |
| | [TASK-140804](tasks/TASK-140804-the-front-door-stops-describing-an-absence.md) The front door stops describing an absence — ***`atomic: 4`***: `ProfileStrip`'s `no-profile` branch returns `null`, which reddens its own test and the arc case that awaits the string, plus `docs/test-plan.md` — an obligation `ADR-0125` §Consequences puts on **this** diff by name, *"and not before"*, because the plan's claim and `04-01`'s fails-if clause are both true until the branch changes* | S | done |
| | [TASK-140805](tasks/TASK-140805-the-profile-says-whether-it-holds-a-password.md) The profile says whether it holds a password — ***`atomic: 6`***, probed with `./gradlew check -PrequireDocker=true` run to `exit 0` over three rounds: two main sources, two test sources, then five golden JSON strings no compiler could see. A fourth correlated `EXISTS` in `PROFILE_OF_SQL` **and the same one in both `PostgresProfileWrites` statements**, with `CredentialKind.PASSWORD` **bound** rather than spelled — and the parameter indices move, which JDBC will not tell you. Names the `hasRecoveryEmail` literal defect beside it and refuses to fix it* | S | done |
| | [TASK-140806](tasks/TASK-140806-the-password-flag-is-that-players-and-only-a-password-sets-it.md) The password flag is that player's, and only a password sets it — *the read statement, against the container. Its own ticket because no gate names `PostgresProfileReadsTest` — it constructs no `ProfileResponse` — which is exactly how `TASK-041616` shipped a flag whose four mutations all built green. Two players in one database asserting opposite values, and a second credential **kind** that must not count* | S | done |
| | [TASK-140807](tasks/TASK-140807-a-name-write-answers-with-the-password-flag-it-read.md) A name write answers with the password flag it read — *both write statements, and they are two code paths: `SET_NAME_SQL` on the happy path and `CURRENT_PROFILE_SQL` on `ADR-0051` §2's idempotent retry. Neither test catches the other. Forbids any assertion about `hasRecoveryEmail`, because asserting its literal `false` would freeze a known defect as intended behaviour* | S | done |
| | [TASK-140808](tasks/TASK-140808-the-document-names-the-password-field.md) The document names the password field — *one row and one reflection test. It lands **after** the field and separately, because `theDocumentedFieldNamesAllExist` checks *documented ⇒ exists* and not the converse — measured green on the probe tree with the field undocumented. The row may not contain the word `null`, or `theDocumentDoesNotCallANonNullFieldNullable` fails* | XS | done |
| | [TASK-140809](tasks/TASK-140809-the-client-requires-the-password-field-or-reads-nothing.md) The client requires the password field, or reads nothing — ***`atomic: 7`***, six probed and one named by no gate: `profile.test.ts` builds every body through `meBody`, so `ADR-0132` §4's whole requirement — a missing field reads `unavailable`, **never `false`** — is unproven without two cases nothing forces. `hasPassword` sorts **before** `hasRecoveryEmail` in the fixture key lists* | S | done |
| | [TASK-140810](tasks/TASK-140810-the-card-draws-the-anonymous-block.md) The card draws the anonymous block — *the card first (`ADR-0091` §2), and the words are its decision: `ADR-0125` §3 puts three obligations and three refusals on it and pins no literal. Gates forbid *real account*, *upgrade*, *promote*, *you should* and *anonymous account*, hold the heading count at one, and require three one-line `<!-- ANON-BLOCK: … -->` markers that the next ticket's `awk` reads back out* | XS | done |
| | [TASK-140811](tasks/TASK-140811-the-words-the-anonymous-block-says.md) The words the anonymous block says — *`account-text.ts` copies the card's three sentences character for character, and the gate is a single `awk` reading **both files**: every `ANON-BLOCK` sentence must appear verbatim in the source and again in the test. The ticket chooses no word and says so — a card sentence that reads wrongly is reported, never edited here* | XS | done |
| | [TASK-140812](tasks/TASK-140812-the-account-screen-names-the-anonymous-profile.md) The account screen names the anonymous profile — *one derived value, `profile.kind === "profile" && !profile.profile.hasPassword`, and five tests that make the derivation impossible to fake: the crossed pair (`signedIn: true` with no password **shows** it, `signedIn: false` with one **hides** it) is what forbids the converse-of-`signedIn` defect `TASK-120601` already found here, and the named/nameless pair is `ADR-0125` §6 after `ADR-0119`. `aProfile()` now defaults to `hasPassword: false`, so the block renders in most merged cases — none of their assertions may be weakened* | S | done |
| | [TASK-140813](tasks/TASK-140813-the-profile-provider-can-be-told-to-read-again.md) The profile provider can be told to read again — *`refresh()` re-runs `read` and adopts **whatever** it answers, including `unavailable`: `ADR-0132` §6's rule is that the client never edits a field of the profile it holds. Three tests, one of them an absolute call count of `1` then `2`, because `toHaveBeenCalled` cannot tell a re-read from a mount* | S | done |
| | [TASK-140814](tasks/TASK-140814-a-confirmed-sign-up-re-reads-the-profile.md) A confirmed sign-up re-reads the profile — *the transition, not the field. `ProfileProvider` reads once and `sign-up.ts` passes `noReload` on purpose, so a successful sign-up is **the one transition that flips the answer with nothing re-asking** — and without this the screen prints *anonymous* beside `This profile now has a password.` One mount, two different `GET /api/me` answers, the form submitted in between; the refused-outcome counterpart pins the read count at **1**, without which a wrapper that re-read on every outcome would pass* | S | done |
| **[STORY-1409](stories/STORY-1409-a-name-can-be-changed-in-account-settings.md)** A name can be changed in account settings, and the name it leaves is spent — *item 1c, split into **eighteen** tickets on 2026-09-08 against the merged [`ADR-0130`](../docs/adr/ADR-0130-a-name-can-be-changed-and-the-name-it-leaves-is-spent.md) and [`ADR-0134`](../docs/adr/ADR-0134-a-rename-spends-before-it-replaces.md). **`ADR-0134` §7's ordering was checked rather than trusted**: the migration alone reddens `1794 tests completed, 3 failed`, all three asserting the **old** trigger's name, message or refusal, and no shipping writer of `player.display_name` moves — `SET_NAME_SQL` still carries `AND display_name IS NULL` so a named player matches zero rows, `retire_display_name` promotes the registry row before it nulls the column, and the coin write is excluded by `OF display_name`. So it is behaviour-preserving for the **product** and not for the **suite**, and one merged assertion *inverts*: `aRetiredNameStillCannotBecomeADifferentName` is what `ADR-0134` §1's deleted disjunct used to forbid, and `TASK-140901` rewrites it into the positive form of the new rule. **Five `atomic:` tickets, each sized by an `ADR-0070` probe run to `exit 0`, never remembered**: the migration (4), the four-statement write (4 — including `retiredFromIsReadInExactlyOneFile`, an exact set equality the mandated `retired_from` write breaks), `AlreadyNamed`'s deletion (4 — three named by the compiler and one by **reflection**, which no compiler can see), the route's budget (7 — *No value passed for parameter 'budget'* at 69 call sites in five test files) and the client's outcome kinds (5). **The budget needed no new machinery**: `AttemptBudget.admit`/`refund` already ship and `AuthRoutes` already reserves before the Argon2 verification, so metering *spending* rather than *trying* is the same two calls with the refund on the other branch — a fifth `AttemptLimits`, a fifth instance, one call site. `PERMANENCE_LINE`'s blast radius was **measured**: four files under `web-client/src`, none under `design/` or `poker-server/`, four ADRs under `docs/` that keep it, and no unrelated test broken — so it leaves in **three plain tickets** instead of one `atomic:` four-file one. The form moves in four steps because the whole-client arc names its profile at the front door in ten places; the replacement pattern — press `Account`, name inside the `account` region, press `Back` — was run on this tree against one of the four failing cases. **`STORY-1409` must not run beside `STORY-1407`, `STORY-1408` or `STORY-1410`**: it edits `Lobby.tsx`, `AccountScreen.tsx`, `PostgresProfileWrites.kt` and both sections of `docs/protocol.md`. The split raised **no decision*** | | | done |
| | [TASK-140901](tasks/TASK-140901-the-trigger-stops-saying-permanent.md) The trigger stops saying permanent, and the registry gains a fourth reason — ***`atomic: 4`***, probed with the full gate set run to `exit 0`: one migration file drops `player_display_name_permanent`, which `MigrationsTest` selects by literal name, replaces the exception message `SchemaConstraintsTest` asserts, and **inverts** `DisplayNamePermanenceTest`'s refusal for a player whose held string is already spent. The body is `ADR-0051` §3's minus one disjunct; `V3` and `V5` are byte-identical to `develop`, and a `verify` command says so. Two new tests use `RETIRED` and `REPLACED` separately, so an `IN` list written as a single comparison reddens exactly one of them | S | done |
| | [TASK-140902](tasks/TASK-140902-a-rename-spends-before-it-replaces.md) A rename spends before it replaces — ***`atomic: 4`***, probed: four statements under `SELECT … FOR UPDATE`, `AND display_name IS NULL` gone, and the vacated row `REPLACED` with `retired_from` set. The gate nobody would guess is `retiredFromIsReadInExactlyOneFile`, an **exact set equality** over every main source naming that column. A `RETIRED` written where `REPLACED` belongs passes the trigger and every behavioural test, so the reason is asserted directly; and `retired_from` is proved with **two players whose ids disagree**, because a constant satisfies one row | S | done |
| | [TASK-140903](tasks/TASK-140903-already-named-is-deleted-and-403-leaves-the-route.md) `AlreadyNamed` is deleted, and `403` leaves the route — ***`atomic: 4`***: three files named by `compileTestKotlin` and one by **reflection** over `sealedSubclasses`, invisible to every compiler and red only at test execution. The `when` keeps no `else`, and a `verify` command greps for one — a route that answered `403` from a fallback would pass every test in the suite, because no port left can produce a third case | XS | done |
| | [TASK-140904](tasks/TASK-140904-the-name-writes-budget-has-two-numbers.md) The name write's budget has two numbers — *a fifth `AttemptLimits` pair on `ServerConfig`, both defaulted so no construction site moves. Three tests whose numbers **disagree with each other and with the defaults** (`7/222000` from the file, `4/444000` from the environment on top of it), so no value a bug leaves unchanged satisfies any of them* | XS | done |
| | [TASK-140905](tasks/TASK-140905-a-fifth-budget-stands-beside-the-four.md) A fifth budget stands beside the four — *one field, one construction, one test. The test exhausts `nameWriteBudget` and then admits on `signInBudget` **under the same key**, which is the only shape that tells its own instance from an alias; the two maxima in the config differ, or the two bundlers would be indistinguishable* | XS | done |
| | [TASK-140906](tasks/TASK-140906-the-write-is-admitted-before-it-spends.md) The write is admitted before it spends, and refunded when it does not — ***`atomic: 7`***, probed: a fourth required parameter fails *No value passed for parameter 'budget'* at **69** call sites in five test files at once. `ADR-0134` §6's inversion has exactly one gate — six refusals against a budget of two, then a write that must still answer `200` — because a budget that metered attempts passes every other test in the file. Two players prove the key is a `PlayerId` and not a constant | S | done |
| | [TASK-140907](tasks/TASK-140907-the-document-answers-429-and-no-longer-answers-403.md) The document answers `429` and no longer answers `403` — *the shipped status loop is a **contains** check, so leaving the `403` row in place passes it; the ticket adds the negative assertion and greps the section independently. Also repairs the two sentences that gave *permanence* as a rule's reason — the doubled-space rule and `DisplayName.kt`'s KDoc — keeping both rules and changing only why they exist* | XS | done |
| | [TASK-140908](tasks/TASK-140908-the-removal-bit-reads-false-for-a-renamer.md) The removal bit reads false for a renamer, for two independent reasons — *`ADR-0134` §4 warns that a test pinning the conjunction alone passes on a mechanism that wrote `RETIRED`; that is exactly right, so there are two tests and each carries **two subjects that disagree** — the renamer and a taken-down player, in one database. `PROFILE_OF_SQL` is unchanged and a `verify` grep pins it character for character; the comment that called the `reason` clause redundant is corrected, because a `REPLACED` row now writes into the partial index* | S | done |
| | [TASK-140909](tasks/TASK-140909-the-card-draws-the-name-form-on-the-account-screen.md) The card draws the name form on the account screen — *two frames and three one-line `NAME-FORM:` markers the next two tickets read back out, so no copy ticket chooses a word. The marker count is an **equality** at three, so a fourth invented obligation fails too. The permanence greps are narrowed to the three phrasings that make the promise: line 126 already uses *permanent* correctly, about account ownership* | S | done |
| | [TASK-140910](tasks/TASK-140910-403-leaves-the-client-and-429-arrives.md) `403` leaves the client and `429` arrives — ***`atomic: 5`***, probed: `refusalSentence` and `mayTryAgain` switch exhaustively with no `default`, so moving the union fails both with `TS2366`, and five literals typed as that union fail `TS2322`/`TS2345`. One failure is **invisible to the compiler** — `NameSurface.test.tsx`'s case list retypes cleanly while still holding the old sentence. `NameSurface.tsx` is measured **out**: it branches on no outcome kind | S | done |
| | [TASK-140911](tasks/TASK-140911-the-two-obligations-arrive-in-words.md) The two obligations arrive, in the card's words — *`CHANGEABLE_LINE` and `SPENT_LINE`, gated by a loop that reads each marker out of the **merged card** and requires it verbatim in the source *and* the test. Two files agreeing is not evidence; three, one of them the card that decided it, is* | XS | done |
| | [TASK-140912](tasks/TASK-140912-the-surface-says-them-and-stops-saying-permanent.md) The surface says them, and stops saying a name is permanent — *two files, because `PERMANENCE_LINE` stays exported for one more ticket. Keeps the merged `compareDocumentPosition` assertion, which is `ADR-0130` §5's whole point — *before the send* — and clears the last two lowercase *permanent*s in the directory, which is why the sweeping grep can run here and not earlier* | XS | done |
| | [TASK-140913](tasks/TASK-140913-permanence-line-leaves-the-product.md) `PERMANENCE_LINE` leaves the product — *the constant **and** the sentence, gated separately: a deletion of a name is not a deletion of a string. Measured: four files under `web-client/src`, none under `design/` or `poker-server/`, four ADRs under `docs/` that keep it and are not edited* | XS | done |
| | [TASK-140914](tasks/TASK-140914-the-account-screen-carries-the-name-form.md) The account screen carries the name form — *an **optional** prop, so the ticket is green with nothing passing it. Three tests: the same profile with and without the prop (differing in one input, so neither constant passes both), and all four non-`profile` states enumerated rather than sampled* | XS | done |
| | [TASK-140915](tasks/TASK-140915-the-front-door-hands-the-form-to-the-account-screen.md) The front door hands the form to the account screen — *the front-door render **stays** for one ticket: deleting it here drags the e2e arc in, and deleting it without the wiring fails `TS6133` — both measured. The test drives a real submit and asserts the spy took the typed string, because a handler that ignored its argument satisfies `toHaveBeenCalled`* | XS | done |
| | [TASK-140916](tasks/TASK-140916-the-arc-names-its-profile-where-the-form-now-stands.md) The arc names its profile where the form now stands — *ten call sites move behind the `Account` door. The pattern was **run on this tree** against one of the four cases that break, with the front-door render already deleted. Green before and after the deletion, which is the risk: a query against the whole container would find the front door's copy today and fail tomorrow* | S | done |
| | [TASK-140917](tasks/TASK-140917-the-form-leaves-the-front-door.md) The form leaves the front door — *it owns the two `Lobby.test.tsx` cases its change invalidates and rewrites both rather than deleting them; the `TestPlayer` count moves 2 → 1, and 1 is not a value the bug leaves unchanged. `TASK-140915`'s wiring test is named must-stay-green, because a deletion ticket whose gates are all negative can delete a feature instead of moving it* | XS | done |
| | [TASK-140918](tasks/TASK-140918-a-player-who-holds-a-name-is-offered-the-form-that-changes-it.md) A player who holds a name is offered the form that changes it — *last on purpose: landing it before `TASK-140917` would put a rename control on the front door, the one placement `ADR-0130` §6 refuses. The no-suggestion gate renders a **named and a nameless** profile and asserts the field is empty in both — on the nameless render alone, an echoing field and an empty one are the same. The fake answers a different string from the one typed, or the canonical-answer test is vacuous* | S | done |
| | [TASK-140919](tasks/TASK-140919-a-thrown-write-does-not-keep-its-reservation.md) A thrown write does not keep its reservation — *found by the deep review of `TASK-140906`, probing whether any path leaves a reservation unrefunded: `ProfileRoutes` has no `try/finally` around the write, and `PostgresProfileWrites` rethrows any `SQLException` that is not the unique violation, so the refund never runs. **Not a regression** — the same shape already ships in `AuthRoutes`, which `TASK-140906` was told to reuse — and `ADR-0134` §6 speaks only in `SetNameResult` terms, so that ticket was right to pass. Worth fixing because a leaked reservation throttles exactly the player §6 protects, after a fault they did not cause* | S | done |
| **[STORY-1410](stories/STORY-1410-signing-out-hands-the-browser-a-new-anonymous-profile.md)** Signing out hands the browser a new anonymous profile, and the confirmation says so — *item 1d, split into **fourteen** tickets on 2026-09-08 against the merged [`ADR-0131`](../docs/adr/ADR-0131-signing-out-of-your-own-account-hands-the-browser-a-new-profile.md) and [`ADR-0135`](../docs/adr/ADR-0135-the-server-says-which-sign-out-this-is-and-the-browser-forgets-one-key.md). **The serialisation against `STORY-1408` is discharged and the split says why**: `DEC-147` was answered by `ADR-0132` and `TASK-140808`/`TASK-140809` have landed `hasPassword` in both contested files, and this story reopens neither — its document change is a **new section**, `### Device standing`, and its client read is a **new module**, because `ADR-0135` §Alternatives 1 refused a seventh `ProfileResponse` field on a measured fact about `PUT /api/me/name`'s second producer. **Two `atomic:` tickets, both probed to `exit 0` and neither a grouping of convenience**: `signOut`'s required field (5 — `tsc` names `main.tsx`, `drive-arc.tsx`, seven call sites in `sign-out.test.ts` and one in `no-secret-in-a-url.test.ts`) and `AccountCalls.signOut`'s parameter (4 — including `account-provider.test.tsx:198`, which a reading would have missed). **Three groupings that looked atomic are not**, and probing is what said so: `SignOutControl`'s prop names three files, `AccountScreen`'s names three, and the provider's mount names two because the module can land unused first. **The abandoning branch is unreachable until the last ticket** — every seam from `TASK-141007` to `TASK-141013` passes a literal `false`, which is `ADR-0135` §6's keep — so behaviour and copy become true in one merge, which is what `ADR-0131` §Consequences asks for. `SIGN_OUT_WARNING`'s **value does not move**: `ADR-0131` §5 says the returning sentence stays, and what was false was saying it unconditionally, so a second export and a `signOutWarning(handsANewProfile)` selector are the change — which leaves `claimed-here-recovered-there.test.tsx` and `account-server.ts` untouched, both measured, because that arc is `ADR-0131` §1 **row two**. The split raised **no decision*** | | | done |
| | [TASK-141001](tasks/TASK-141001-the-account-card-draws-both-sign-out-confirmations.md) The account card draws both sign-out confirmations — *the card first (`ADR-0091` §2): `account.html` draws the `Sign out` button and has **never** drawn the confirmation step, so both of `ADR-0131` §1's rows are new frames. The words are the card's and `ADR-0135` mints none. Nine `awk` gates, all frame-scoped: the returning frame carries the shipped `SIGN_OUT_WARNING` verbatim, the abandoning one carries its first sentence and **not** `goes back to the profile it had before` — the exact sentence `ADR-0131` §Consequences calls false. The three refusals cannot be gated and the ticket says so rather than pretending* | S | done |
| | [TASK-141002](tasks/TASK-141002-the-resolver-answers-whether-a-device-names-a-player.md) The resolver answers whether a device names a player — *one function, `namesPlayer`, `players.findOrNull(deviceId)?.id == playerId`, reusing the shipped `revoked_at IS NULL` lookup. Three `grep -c` structural gates are what say `resolve` did not move: five `Identity` cases (measured 5), two public suspend functions (1 → 2) and two `players.findOrNull` sites (1 → 2), so a refactor routing one through the other drops the last back to 1. Dead code for exactly one merge* | XS | done |
| | [TASK-141015](tasks/TASK-141015-the-resolver-stops-answering-a-comparison-and-the-smoke-test-names-an-unasked-verb.md) The resolver stops answering a comparison, and the smoke test names a verb nobody asked for — *inserted 2026-09-09 by [`ADR-0144`](../docs/adr/ADR-0144-a-negative-route-test-is-a-control-and-the-predicate-resolves-the-device-once.md), which is why it is numbered fifteenth and runs **third**. Two removals, no behaviour: `namesPlayer` goes, because one boolean cannot make `ADR-0135` §5's *resolves to nobody* and *resolves to somebody else* differ and no sign repairs it, so `IdentityResolver` returns byte-for-byte to what it was before `TASK-141002` — `TASK-141002`'s own three `grep -c` gates read backwards (1, 1, 5) are what say so. And `theDeviceRouteIsNotInstalledOnAnyOtherVerb` is **re-pointed, not retired**: renamed `…OnUnaskedVerbs` and issuing `client.put`, keeping both inequalities and `TASK-040611`'s own reason — *"a `get(...)` typed beside the `delete(...)` would otherwise be invisible"* — while the comment stops citing `ADR-0049` §5, which never forbade a verb. `PUT` because `PUT /api/me/name` is live under the same prefix, so the control also refuses a path-matching mistake. It lands **before** the route so every assertion in it is true at its own commit; a row claiming the `GET` is installed could not be. Three `awk index` gates pin put=1, delete=1, get=0, `IdentityResolverTest` 12 → 8 and `DuelServerRoutesTest` 9 → 9. **Named cost:** `TASK-141002` becomes a done ticket whose `verify:` block no longer passes — nothing re-runs it, and the reason is written into the ticket's Notes and `ADR-0144` §Consequences rather than left to be discovered* | XS | done |
| | [TASK-141016](tasks/TASK-141016-the-forget-is-pinned-ahead-of-the-reload.md) The forget is pinned ahead of the reload — *inserted 2026-09-09 while landing `TASK-141007`, whose review found that moving `forgetDeviceId` to after `request.reload()` left all nine tests in `sign-out.test.ts` green. The coder reported the ordering as unobservable under Vitest because `reload` is a no-op mock; review disproved that by writing the assertion with the pattern the file already contains — the `reload` mock body, where `reloads once, after the local half is done` already observes the session token — and it failed against the reordered code. The shipped order is correct, so this is a coverage gap and not a bug, filed rather than folded in because `TASK-141007` enumerates its tests exactly and its `atomic:` set is the five files `tsc` names. **Named cost:** for one merge `ADR-0135` §6's safe direction rests on an ordering nothing observes* | XS | done |
| | [TASK-141017](tasks/TASK-141017-the-warning-is-the-last-thing-read-before-the-press.md) The warning is the last thing read before the press — *inserted 2026-09-09 while landing `TASK-141008`, whose coder reported unprompted that its diff pins none of `ADR-0143` and deferred it to `TASK-141009` — which mentions neither `ADR-0143` nor ordering anywhere, so the deferral had nowhere to land. Grepping all of `web-client/src` for `ADR-0143` returns no test: §2's obligation 2, that the claim is the last thing read before the control with no sentence between, is asserted in an ADR and enforced by nobody. It binds harder now that `TASK-141008` added a second sentence and `TASK-141009` chooses between them at runtime. **Named cost:** the same obligation binds `RevokeControl` and the name form, and this closes neither — both are outside `STORY-1410` and are named in the ticket's Out of scope rather than fixed by implication* | XS | done |
| | [TASK-141018](tasks/TASK-141018-the-storage-reads-sit-inside-the-catch.md) The storage reads sit inside the catch — *inserted 2026-09-09 while reviewing `TASK-141011`. That provider calls `read().then(...)` with no `.catch`, on the stated contract that `readDeviceStanding` never rejects. The contract holds for everything its KDoc enumerates — every one of those is inside the `try` — and fails for one path it does not: `readSessionToken` and `readDeviceId` are called **before** the `try` and both call `storage.getItem` unguarded, so a browser with site data blocked makes the promise reject. **Named cost:** the consequence today is benign, since the provider holds `false` until the read lands and a rejected read never moves it — `ADR-0135` §6's safe direction — so what is being fixed is that the safety comes from the initial value rather than from the function honouring its own contract* | XS | done |
| | [TASK-141019](tasks/TASK-141019-the-boot-nests-the-provider-above-the-profile.md) The boot nests the provider above the profile — *inserted 2026-09-09 after `TASK-141012`'s review. Its gates count occurrences, and a provider mounted as a **sibling** of `ProfileProvider` rather than around it has exactly the same counts: coder and reviewer both mutated the boot that way and nothing reddened — 1264 tests green, all four counts unchanged. The ticket's own *What would still pass* section anticipates three defects and all three assume correct placement; placement is the shape it does not cover. `TASK-141014` does not close it either, checked rather than assumed: its tests wrap `Lobby` in a provider of their own and never boot `main.tsx`'s tree. **Named cost:** a misnested provider makes every consumer read the context default `false`, which is `ADR-0135` §6's safe direction, so the failure is a feature silently not working rather than a player losing a profile* | XS | done |
| | [TASK-141020](tasks/TASK-141020-a-required-prop-is-required-in-the-typechecker.md) A required prop is required in the typechecker — *inserted 2026-09-09 after `TASK-141013`'s review. `ADR-0135` §7 — *"a component that can be mounted without it is a component whose absence is invisible from the outside"* — is obeyed by both `SignOutControl` and `AccountScreen` and enforced by nothing: coder and reviewer each changed the declaration to `?: boolean` with a `= false` default and it passed `tsc`, `npm run check` and all 1265 tests, because no grep looks for optional syntax and every fixture supplies the prop. In both `TASK-141009` and `TASK-141013` the check was a human reading the declaration. **Named cost:** `false` is not a neutral default — `ADR-0135` §6 makes it the reassuring answer, so an unwired caller would be told silently and wrongly that signing out is safe* | XS | done |
| | [TASK-141003](tasks/TASK-141003-the-server-says-which-sign-out-this-is.md) The server says which sign-out this is — *`DeviceStandingResponse` with no default, and `get("/api/me/device")` beside the `delete` that already lives there; `Application.kt:103` already passes `identities`, so no wiring file moves. Six tests, 10 → 16, and the body is asserted as the **whole string** `{"signOutHandsANewProfile":true}` through `module()`'s own `Json` — a DTO unit test cannot see `encodeDefaults = false`, which is the trap. `theStandingRouteNeverAsksAboutCredentials` asserts an **empty** `holdsCalls`, because copying `DELETE`'s `409` guard leaves every status test green* | S | done |
| | [TASK-141004](tasks/TASK-141004-the-document-names-the-device-standing-route.md) The document names the device-standing route — *a new `### Device standing` section and the section re-chaining it forces: `deviceSection` ends at the new heading and a `deviceStandingSection` begins there, or every merged `deviceSection` assertion could be satisfied by the new section's text. First section in that file to get **both directions** of the DTO check, which `ADR-0142` §Context names as the file's standing defect. 44 → 49* | S | done |
| | [TASK-141005](tasks/TASK-141005-the-device-id-can-be-forgotten-by-its-own-module.md) The device id can be forgotten, by its own module and no other — *`forgetDeviceId` in the one file `one-module-owns-each-storage-key.test.ts` allows the literal in. Four seeded keys and a `storage.length` of 3, because one survivor cannot tell a targeted `removeItem` from a `clear()` that missed; and the raw `getItem`, because `readDeviceId` reports `""` as `null` too. 4 → 7* | XS | done |
| | [TASK-141006](tasks/TASK-141006-the-browser-asks-which-sign-out-this-is.md) The browser asks the server which sign-out this is — *its own module over `plainFetch`, **never** `readFromApi`: that helper answers without a request when the browser holds no device id, which is precisely the browser whose answer is `true` (`ADR-0135` §6). It does short-circuit on the **other** credential — no session, no request, `false` — which is `revoke-device.ts`'s shipped shape and the same answer a `401` gives. The control that tells the two apart asserts a **call count** of 1 for a browser with a token and no device id. 7 tests* | S | done |
| | [TASK-141007](tasks/TASK-141007-signing-out-forgets-the-device-id-when-it-is-told-to.md) Signing out forgets the device id when it is told to, and never otherwise — ***`atomic: 5`***, probed to `exit 0`: a **required** field — `ADR-0135` §7 refuses an optional one in writing — fails `TS2345` at `main.tsx:133`, `drive-arc.tsx:127`, `no-secret-in-a-url.test.ts:226` and seven sites in `sign-out.test.ts`. Two merged tests change input and neither loses an assertion; `asks nothing of the server when no session is held` flips to `true` and keeps its assertion, which **strengthens** it into the early-return guard. A mutation strips the `if` and requires the file to redden. 6 → 9* | S | done |
| | [TASK-141008](tasks/TASK-141008-the-confirmation-has-a-sentence-for-each-sign-out.md) The confirmation has a sentence for each of the two sign-outs — *`SIGN_OUT_WARNING` keeps its value character for character and loses only its unconditionality; a second constant and a `signOutWarning(handsANewProfile)` selector on `deviceRouteLine`'s own model. The new literal must be **one string on one line**, because the module-to-card gate is a `sed` extraction then a `grep -F` against the card, and `ADR-0142` §Context measured that a hand-concatenated string appears **0** times in its own source. 7 → 8* | S | done |
| | [TASK-141009](tasks/TASK-141009-the-confirmation-states-the-sign-out-it-is-about-to-make.md) The confirmation states the sign-out it is about to make — *the sentence and the call take the **same expression**, so no state exists where the words and the act disagree. `signedIn` still gates only whether the control exists (`ADR-0135` §7), and the merged not-signed-in test flips its input to `true` to say so. A mutation pins the sentence to `signOutWarning(false)` and requires the file to redden. Three files, probed — `AccountScreen.test.tsx` is **not** among them. 5 → 8* | S | done |
| | [TASK-141010](tasks/TASK-141010-the-account-calls-carry-which-sign-out-this-is.md) The account calls carry which sign-out this is — ***`atomic: 4`***, probed to `exit 0`: `AccountCalls.signOut` gaining a parameter fails `TS2554 Expected 1 arguments, but got 0` at `account-provider.test.tsx:198` and `TS2353` at both bindings. The gate nobody would guess is not a test at all — a binding that takes the parameter and passes `handsANewProfile: false` typechecks and passes every test in the client, so four `grep` gates pin the literal gone and the identifier at exactly two occurrences per file. 5 → 6* | S | done |
| | [TASK-141011](tasks/TASK-141011-a-provider-holds-the-answer-and-false-until-it-lands.md) A provider holds the answer, and `false` until it lands — *`ProfileProvider`'s shape with a `createContext<boolean>(false)` whose default **is** `ADR-0135` §6's rule. Lands unused, which is what keeps `TASK-141012` to two files. Five tests, and the one a copy of that file is likeliest to drop is the `live` guard's — asserted as *the value did not change*, never as the absence of a console warning* | S | done |
| | [TASK-141012](tasks/TASK-141012-the-boot-reads-the-device-standing-once.md) The boot reads the device standing once, beside the profile — *`App.test.tsx` is in the budget because it pins a **count of `main.tsx`'s own source**: `fetch: plainFetch` is **7** at `c6a41e6d` and this binding makes it 8, while `fetch: apiFetch` stays at 5. `plainFetch` is not a preference — `readDeviceStanding` sets its own bearer header and the wrapper would attach a second. Two-sided regex, because a count alone would also pass for some third fetch. 36 → 37* | S | done |
| | [TASK-141013](tasks/TASK-141013-the-account-screen-is-told-which-sign-out-it-offers.md) The account screen is told which sign-out it offers — *probed: a required prop on `AccountScreen` names **nine** render sites in its own test and one in `Lobby.tsx`, and nothing else; `App.test.tsx` escapes because it wraps the component rather than rendering it. The new test holds `signedIn` fixed and varies only the new prop, because the two are adjacent booleans and confusing them is the mistake. 20 → 21* | S | done |
| | [TASK-141014](tasks/TASK-141014-the-lobby-hands-the-screen-the-answer-the-server-gave.md) The lobby hands the screen the answer the server gave — *the merge at which the behaviour and the copy both become true. None of the 113 merged `Lobby.test.tsx` tests is edited: every one renders without the provider, so the hook answers its default. The two new ones wrap `Lobby` in a provider resolving `true` and `false` and assert the sentence **and** `toHaveBeenCalledWith`, and a mutation replaces the hook read with `false` and requires the file to redden. Row two's proof — `readDeviceId(storageB)` unchanged in the arc test — is pinned as a must-not-move. 113 → 115* | S | done |
| **[STORY-1412](stories/STORY-1412-no-card-on-the-table-is-marked-and-a-gate-says-so.md)** No card on the table is marked, and a gate says so — *item 2b, split into **two** tickets on 2026-09-08 against the merged [`ADR-0126`](../docs/adr/ADR-0126-the-table-shows-the-cards-and-marks-none-of-them.md), which answers the human **no** for v0.1 and dates the reversal at v0.4. Exactly what `ADR-0126` §4 says is left: one gate and one line on the showdown card — and the reason a decision to build nothing still owes a test is in the same sentence, *"nothing in the repository today would notice a mark appearing."* **Where the gate lives was decided, not assumed**: `ADR-0142` §1 restates `ADR-0091` §4 as a constraint — a shell gate under `design/` may not read `web-client/` — so it is a vitest file beside `no-derivation.test.tsx`, which `vitest run` globs into existence with **no workflow edited**. `design/check-frame-cards.sh` reads one card and **does** run in CI (`tickets.yml`, `TASK-140719`), which is where `ADR-0142` §1's remark about an unwired gate is true of an older tree and not this one. Measured at `c6a41e6d`: the card carries **5** frames, **38** `class="pc`, **4** `back mucked` and **2** mentions of `ADR-0126` — the two frames missing it are the mucked showdown and the fold, and the frame-scoped `awk` is red on exactly those two and green on the other two. Nothing crosses the wire, `BestHand` stays inside `poker-engine`, and **nothing here is `atomic:`*** | | | done |
| | [TASK-141201](tasks/TASK-141201-two-more-frames-say-no-card-is-picked-out.md) Two more frames say in their margin that no card is picked out — *`ADR-0126` §1 names a fold as well as a showdown, so both frames get the line. Four frame-scoped gates, not one file-wide count: the two that must gain it are **red today** and the two that already carry it are **green today**, so a coder cannot satisfy the ticket by moving an existing sentence. Three markup counts (5 frames, 38 `class="pc`, 4 `back mucked`) say no card moved* | XS | done |
| | [TASK-141202](tasks/TASK-141202-a-gate-says-the-drawing-of-a-card-does-not-depend-on-whether-it-played.md) A gate says the drawing of a card does not depend on whether it played — *a **drawing signature** over every face-up card at a completed showdown, normalising away exactly three things — rank glyph, suit glyph, suit-colour class — and seeing everything else. Three defences against a gate that holds nothing: a **count of nine** before any comparison, **four mutation controls** with their own untouched-clone control, and the whole property run from **both** viewer seats. `aria-label` is the one hole a signature cannot close, so a fifth test pins each label to exactly what `card-text.ts` produces. Two `verify:` mutations edit `PlayingCard.tsx` itself — a `" (winning)"` label and a `ring-2` class on one card — prove with `! cmp -s` that the file changed, require the suite to fail, and restore it; both were run at `c6a41e6d` against a stub, both exited 0 with `git status` clean. Expected **green on the day it lands**, which is `ADR-0126` §Consequences' *"a gate must be written to hold a nothing"** | S | done |
| | [TASK-141203](tasks/TASK-141203-the-card-text-register-ADR-0142-owes.md) The card-text register `ADR-0142` owes — *found by the planner splitting `STORY-1410`/`STORY-1412`, which looked for the ticket owing `ADR-0142`'s register and found **none anywhere**. The architect's report said the planner would write it; the planner was briefed on two other stories, so it fell between them — which is the very failure `ADR-0142` is about, an obligation recorded where nothing executable enforces it. The NUL sentinel is load-bearing: a newline separator halves wrapped sentences and reports them absent, the bug that produced the `ANON-BLOCK` markers this ticket deletes* | S | done |
| **[STORY-1415](stories/STORY-1415-the-rematch-offer-finds-the-rival-wherever-they-are.md)** The rematch offer finds the rival wherever they are — *item 5, the last unwritten seam of `EPIC-14`, **written and split into eleven tickets on 2026-09-09** against the merged [`ADR-0123`](../docs/adr/ADR-0123-a-standing-rematch-offer-follows-the-rival.md) (the product half, the human's "it shoud appear on any screen") and [`ADR-0138`](../docs/adr/ADR-0138-the-panel-mounts-beside-the-lobby-and-the-dismissal-lives-in-the-mount.md) (the mechanism, answering `DEC-144`). **No ticket is `atomic:`** — nothing crosses the socket, and `ADR-0138` §7 says so. Five things were **measured rather than recalled** and each changed the split: `react-hooks/set-state-in-render` is live at `error` and **passes** the guarded render-phase clear, so the ADR's conditional set owes **no suppression** and a ticket that adds one is adding noise; **jsdom performs no implicit form submission**, so `ADR-0138` §8's "a real `Enter` in the password field" cannot be written with `fireEvent` and a `keyDown` version would pass against a panel **inside** the form — the proof becomes structural; assigning `window.location.hash` inside `act()` does **not** settle a `useScreen` consumer, so a *still hidden after a screen change* test passes on a navigation that never happened, and every mid-test move must await the destination screen's own heading; applying the three offer-ending frames in **one** `act()` batches away the render the clear needs, so the reset test fails against a **correct** component; and every `-0` spacing utility generates **no CSS** under `app.css:55`'s `--spacing: initial`, which is `TASK-140502`'s defect waiting for the first `fixed` panel. `ADR-0138` §4 mandates the clear for `dismissed` only, and the split applies it to `accepted` too, for §3's own stated reason. The mount is the **eighth** ticket, not the fourth, so `develop` never carries a mounted panel a player cannot dismiss. `TASK-141203` merged **mid-split** as `f20d07ed`, so `ADR-0142` §6's register now exists and `TASK-141503` opens three files rather than two; every count was re-measured at that head and none moved. The split raised **`DEC-158`**, the architect's, and it blocks nothing — **answered on 2026-09-10** by [`ADR-0148`](../docs/adr/ADR-0148-a-module-names-every-card-that-draws-it-and-every-card-is-registered.md), which finds the drift was `ADR-0142` §4's cardinality rather than §7's direction* | | **done** |
| | [TASK-141501](tasks/TASK-141501-the-card-mints-the-panel-and-its-three-states.md) The card mints the panel, and draws its three states and the screen without it — ***minting*** (`ADR-0123` §8), so it is worked with the human and the visual verdict may trail the merge. Four frames, one file, and **no motion at all**: `@keyframes`, `animation`, `transition` and `z-index` are each gated at 0, which makes §8's *at rest under `prefers-reduced-motion`* true by there being nothing to still — an entrance, if the human wants one, is a follow-up that mints a token pair in **both** sheets. `--pd-shadow-pop` gets its **first use in the product**: declared at `tokens.css:117` for what floats, and used by no component at `f20d07ed`. Every gate was run against a prototype card and `check-drift.sh` passed at 21 cards* | S | done |
| | [TASK-141502](tasks/TASK-141502-the-card-sets-the-panel-on-two-screens-at-two-widths.md) The card sets the panel on two screens, at the phone and on the laptop — *`ADR-0123` §8's last bullet, four frames appended. `class="frame"` stays **4** and the new ones carry `frame over phone`/`frame over laptop`, because `class="frames"` — the container — shares a prefix with `class="frame"` and a prefix gate counts 9 where a reader counts 8; measured. `<h2>` 4 → 8 and `class="panel"` 3 → 7 are what stop four empty frames satisfying it* | S | done |
| | [TASK-141503](tasks/TASK-141503-one-module-holds-the-words-both-rematch-surfaces-say.md) One module holds the words both rematch surfaces say — *six exports, no import, and `RematchControl`'s edit is **imports only**. Five zero-count gates on the old literals are paired with five positive gates on the interpolations that replaced them: neither half alone is enough, since the zeros pass on a component that deleted the sentences. `Rematch` cannot be gated by substring — it is in the KDoc, in `rematchStand` and in `RematchOffered` — so the gate is a **line that is nothing but the word**, measured at 2. `Rematch offered — waiting for your rival` does **not** move: `ADR-0138` §4's table has four rows and the panel never says it. `ADR-0142` §6's register is handled in both landing orders by `npm run check` alone* | XS | done |
| | [TASK-141504](tasks/TASK-141504-the-notice-stands-where-an-incoming-offer-stands.md) The notice stands where an incoming offer stands, and nowhere else — *the component, mounted by nothing. Every source gate **skips comment lines**, measured: the KDoc this component owes says "no `useEffect`, no `setTimeout`" and "takes no props" in as many words, and the naive substring form fails on a correct file. Four of the five tests are negatives, so the fifth is their positive control, and `follows only an incoming offer` passes the **same** `rematchOffers: [0]` at both seats. Mutations run and their failure counts recorded rather than predicted: dropping the `shown` guard fails **1**, dropping `!theirs` fails **3*** | S | done |
| | [TASK-141505](tasks/TASK-141505-the-accept-is-one-press-one-frame-and-the-span-it-opens.md) The accept is one press, one frame, and the span it opens — *`accepted` gets `ADR-0138` §3's clear even though §4 only says "exactly as `RematchControl.tsx:32` does", because that sentence is about where the fact lives and `RematchControl`'s own flag is stale. Two harness facts measured against a prototype: `getByText(DEALING_TAIL)` **throws** — the two lines share one element across the `<br />`, so the merged regex over the whole node is the matcher — and the three offer-ending frames must arrive in **three separate `act()` calls**, or React batches them, the clear never fires, and the test fails against a correct component* | S | done |
| | [TASK-141506](tasks/TASK-141506-a-gone-room-retires-the-accept-and-says-so.md) A gone room retires the accept and says so, and no other refusal touches it — *`UNKNOWN_ROOM` gated at 1 and `REMATCH_UNAVAILABLE` at 0. `a transient refusal leaves the accept standing` is the positive control that stops the branch being written as `refusal !== null`, and `the panel is still there after the room is gone` is what stops it being written as `return null`* | XS | done |
| | [TASK-141507](tasks/TASK-141507-not-now-hides-the-surface-and-the-next-offer-brings-it-back.md) Not now hides the surface, sends nothing, and the next offer brings it back — *one `useState` whose lifetime is the mount, cleared during render, **no `eslint-disable`** and a gate refusing one. `Not now sends nothing` asserts a **count of 0 on both sides** rather than *not called with a decline*, because sending `OfferRematch` on dismiss is the plausible mistake and it passes the weaker form. The reload proof is a fresh `render()`, deliberately **not** an assertion over `Storage` keys — Node's `localStorage` shadows jsdom's under vitest and reads `undefined`* | S | done |
| | [TASK-141508](tasks/TASK-141508-the-app-mounts-the-panel-and-it-follows-onto-every-screen.md) The app mounts the panel beside the lobby, and it follows onto every chosen screen — *one import and one element in `App.tsx`, with two gates on **line order** because DOM order is the tab order. A **new** test file, not `App.test.tsx`'s 1261 lines, copying its `vi.mock` factory for `./main`. The six-screen enumeration asserts each screen's **own heading** beside the sentence: without that half it proves *the panel shows*, not *the panel follows onto that screen*. The mount mutation fails **exactly one** of the two tests, and the ticket says so rather than claiming both* | S | done |
| | [TASK-141509](tasks/TASK-141509-the-dismissal-outlives-the-screen-and-the-agreement-takes-it.md) The dismissal outlives a walk through the lobby, and the agreement takes the screen — *test-only, `review: deep`, and four `git diff --quiet` gates make repairing what it tests a failure rather than a choice. The **trip through the room's own screen** is `ADR-0138` §8's own assertion that discriminates the mount point: a panel inside `Lobby`'s cascade passes the first half and fails this one. The three awaits are positive controls, not waits. The agreement's three literals are the merged ones from `App.test.tsx:967`* | S | done |
| | [TASK-141510](tasks/TASK-141510-what-the-panel-takes-from-the-player-is-nothing.md) What the panel takes from the player is nothing — *test-only, `review: deep`. Enter is proved by **position** because jsdom performs no implicit submission and a `keyDown` test would pass against a panel inside the form. The submit test asserts two counts — `send` at 0 **and** `signIn` at 1 — because a submit that never fired satisfies the zero. The focus test **pre-focuses** a real control, since `document.activeElement` defaults to `<body>` and the assertion is otherwise vacuous. `ADR-0138` §8's front-door case is **not written**: the front door is the room's own screen, so there is no panel there to press into* | S | done |
| | [TASK-141511](tasks/TASK-141511-the-result-screens-card-says-the-words-the-result-screen-says.md) The result screen's card says the words the result screen says — *the drift this split found, closed. Measured at `f20d07ed`: `design/screens/rematch-states.html` says `ImKate` twice where `RematchControl.tsx:65,74` say *your rival*, and draws no frame for `That duel room is gone.`, shipped since `TASK-030909`. Which side is stale is not a judgement — `ADR-0123` §4 calls the client's spelling "the result screen's own line". The `ImKate` gate is **file-wide**, so a name moved into a margin note fails too. `ADR-0142` §7's register question was `DEC-158` and is not this ticket's — **answered on 2026-09-10** by [`ADR-0148`](../docs/adr/ADR-0148-a-module-names-every-card-that-draws-it-and-every-card-is-registered.md)* | XS | done |
| | [TASK-141512](tasks/TASK-141512-the-gate-refuses-a-key-listener-too.md) The gate refuses a key listener too — *inserted 2026-09-09 while landing `TASK-141504`. `ADR-0138` §6 forbids three things — taking focus, setting a tab stop, and **listening for a key** — and that ticket's awk gate enumerates nine strings covering the first two and none covering the third. Probed in its own worktree: `tabIndex={0}` on the root reddens the gate (exit 1), `onKeyDown={() =&gt; {}}` does not (exit 0); both reverted, file byte-identical. The component is correct and the gate is incomplete against the clause it cites. **Named cost:** the repair is a test rather than a tenth string in the enumeration, because a source grep cannot see a listener registered through a helper — so `TASK-141504`'s gate stays incomplete on its own terms and the test is what carries §6* | XS | done |
| | [TASK-141513](tasks/TASK-141513-the-module-stops-offering-the-label-the-adr-says-to-copy.md) The module stops offering the label the ADR says to copy — *filed 2026-09-10 from a reviewer's aside while reviewing `TASK-141511`, that `rematch-text.ts`'s `NOT_NOW` is defined but unused. Measured: nothing imports it, `RematchNotice.tsx:17` declares its own, and `ADR-0138` §4 says outright *"`Not now` is copied, not imported"* while listing five constants that do not include it. **Named cost:** the dead line is harmless; what is not is that the shared module now offers exactly the import §4 closed off, so a later coder reaching for the dismiss label finds it in the module both surfaces share and couples them by the obvious route rather than the forbidden one* | XS | done |
