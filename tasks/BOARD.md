...
| DEC-145 | **The product owner's** — may a player who holds a `WAITING` room take a seat in **another** room by invite, and what becomes of the room they hold? Registered open 2026-09-06 by [`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md) §7, which answered `DEC-111` for the *create* route — a second `CreateRoom` hands back the room already held — and names this as **the one route left** into two duels at once: they hold a waiting room, click a friend's link, `Room.join` seats them and starts that duel, and their own code goes on resolving for whoever has it. If a rival then clicks it, `ADR-0013`'s window, `ADR-0023`'s absent seat and `ADR-0014`'s coin settle a duel they never saw. Not answered with `DEC-111` because the forces differ: the invite winning over the memory is a shipped principle (`boot.ts` — *"a player who has just followed a link to a new room means that room"*), and every alternative to allowing it reaches a **third party who did nothing** — refusing the join lands on the friend whose link was clicked, and closing the held room lands *No duel room has that code.* on whoever was sent it. **Blocks nothing** | [`ADR-0124`](../docs/adr/ADR-0124-one-waiting-room-and-play-again-hands-it-back.md) §7 | before the first public link |
...
left into two duels at once, a holder of a waiting room taking a seat in another by invite. Costs
