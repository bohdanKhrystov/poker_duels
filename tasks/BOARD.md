...
| DEC-072 | **The architect's** — the shape of the mail seam, and its failure, lifetime and observability semantics. `ADR-0031` §7 declares a build with **no sender configured** a valid state without saying what the wiring holds in it; §5 requires the `202` written before any mail work with delivery *"on a detached coroutine"* and settles nothing about that coroutine's parent, a failed send after the token row is committed, retries, or **what a test can await** — which decides whether *a mail was sent* is assertable at all. Also where `baseUrl` lives and what an unconfigured one does. **Choosing a relay or provider is explicitly excluded** — that is `EPIC-07`'s, and a bill, and the human's | [`STORY-0416`](stories/STORY-0416-the-recovery-email-and-the-password-reset.md) — blocks `TASK-041625`, `TASK-041626`, `TASK-041627` | before `TASK-041625` |
