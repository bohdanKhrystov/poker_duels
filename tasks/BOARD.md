...
| DEC-158 | **The architect's** — [`ADR-0142`](../docs/adr/ADR-0142-a-text-module-is-checked-against-the-rendered-card.md) §7's named reopening trigger has fired: does the **card text ⇒ module** direction now get a register, and of what shape? `ADR-0142` enforces *carded value ⇒ card text* and says the reverse is not enforced, on the reasoning that a card is mostly **not** product copy and that separating copy from card furniture needs the marker register its §3 refuses. Its trigger for reopening is **the first time a stale sentence is found on a card**, and one was found on 2026-09-09 while splitting [`STORY-1415`](stories/STORY-1415-the-rematch-offer-finds-the-rival-wherever-they-are.md): measured on `develop` at `f20d07ed`, `design/screens/rematch-states.html` says `ImKate offers a rematch` and `Rematch offered — waiting for ImKate` where `RematchControl.tsx:65,74` say *your rival*, and it draws no frame at all for `That duel room is gone.`, a state shipped since `TASK-030909`. Which side is stale needed no decision — `ADR-0123` §4 calls the client's spelling *"the result screen's own line"* — so `TASK-141511` repairs the sentences and this decision is only about the **gate**. The counter-case is `ADR-0142` §7's own: the cost falls on every card and one instance in twenty cards may not pay for it; a third option is to check `PAIRS` **both ways** for the strings a module already declares and leave the rest of a card unchecked. **Blocks nothing** | [`ADR-0142`](../docs/adr/ADR-0142-a-text-module-is-checked-against-the-rendered-card.md) §7 | before the next card is drawn against a shipped text module, or before a second stale sentence is found |
