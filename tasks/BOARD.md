...
| DEC-166 | **The product owner's** — when the room behind a standing rematch offer is gone and its holder is standing on the **front door**, which of two merged sentences does the player read: the front door's *No duel room has that code.* (`Lobby.tsx:494`, rendered whenever `state.refusal !== null`) or the panel's *That duel room is gone.* (`ADR-0138` §4)? Registered 2026-09-10 by [`ADR-0151`](../docs/adr/ADR-0151-the-press-is-one-fact-and-the-way-back-stops-navigating.md) §7. Until `ADR-0150` §2 the two could not share a screen, because the front door was unreachable while a room was held; they can now — a refused rematch press, or a press of the way back after one — and the first is addressed to a player who typed no code. `Failure` sets one field and the client cannot see which frame it answered (`ADR-0072` §7, `ADR-0044` §3), so this is a choice between two merged sentences and not a mechanism. **Blocks nothing**: the mechanism ships either way and the repair is one conditional wherever the answer puts it | [`ADR-0151`](../docs/adr/ADR-0151-the-press-is-one-fact-and-the-way-back-stops-navigating.md) §7 | with the story that implements `ADR-0150`, or the first UAT round that reaches the front door with a room held |
