...
| --- | --- | --- | --- |
| `DEC-101` | What amount does each named sizing preset set — what does a chip labelled `pot` (and `⅓`, `½`) promise a player in a heads-up duel? | the **product owner's** | the rewritten `TASK-120908`, and nothing else |

`DEC-101` was registered open on 2026-08-31 by
[`ADR-0100`](../../docs/adr/ADR-0100-the-driver-reaches-an-amount-by-pressing-what-a-player-presses.md)
§6, which settled the technical half of the same question and refused this one. The half it settled:
**a named preset computes the quantity it is named for**, and the pot reaches `ActionBar` as
`view.pot + both seats' committedThisStreet` through `Lobby.tsx`, needing no wire change — so
`TASK-120908`'s `floor × 4/3`, `× 1.5`, `× 2`, labelled as pot fractions, does not ship. The half it
refused: the merged card admits **two** formulas that agree on the one frame it draws and disagree
in play, and excludes the textbook pot-sized raise a poker player expects, so no technical fact
chooses and the choice decides what a player believes when they press a label. **Three things follow
for this epic.** (1) It was raised by a ticket, not promoted at a triage, so it is **not** one of
`ADR-0092` §5's three slots — the `DEC-092`/`DEC-095`/`DEC-099` precedent — and no cap is touched.
(2) It **gates a member of no live fix set**: `TASK-120908` is `backlog` and the cycle ended `PASS`
at round 3, so `STOP_BLOCKED` does not fire (`ADR-0092` §5) and nothing here waits. (3) The ticket it
yields is the **rewritten `TASK-120908` itself**, not a new one — this is a `medium` product defect
already counted in `B(1)`, not a decision-born improvement, so `ADR-0092` §5's third exclusion does
**not** apply and no count moves.
