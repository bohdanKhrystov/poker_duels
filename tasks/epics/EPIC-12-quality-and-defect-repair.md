...
**One: `DEC-097`, the architect's** — by what mechanism does
[`ADR-0096`](../../docs/adr/ADR-0096-the-audit-judges-a-whole-duel-against-a-frozen-rubric.md)'s
**audit focus** run under `ADR-0089` §2's three conditions? Two pieces: **(a)** `ADR-0090` §2's
declared-file set is **four**, and an audit observer that says the cycle owns the stack lifecycle
would be a **fifth** — is the set amended again on the `ADR-0092` §2 precedent, or does the focus
run with no new agent file? **(b) — now the load-bearing half.** `ADR-0096` §4 walks the whole duel at
**390 × 664** and re-answers `R2`/`R3` at **720 × 900** in **one round**, so *"launch the browser
at a size"* no longer answers it: relaunching mid-duel re-seats the device (`ADR-0018`) and deals a
different board, so the shapes cannot be two passes over the same beat. Either the harness resizes a
**live tab** — `Emulation.setDeviceMetricsOverride`, which a player's hands arguably reach by
dragging a window but which also carries `deviceScaleFactor` and a mobile flag that make the client
believe it is on hardware it is not — or §4's second shape must move to its own round, which is a
**product** change and returns to the product owner. The product halves are settled by `ADR-0096` and are not reopened. **It blocks the
first audit round and every ticket that builds the focus**, because the planner cannot write a
`Files` table without (a) and cannot walk a beat at two shapes without (b).
...
registered and answered in the same PR and never appeared in an open table. It registers
`DEC-097` above.)
