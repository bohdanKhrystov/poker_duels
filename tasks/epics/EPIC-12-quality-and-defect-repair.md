...
| [STORY-1216](../stories/STORY-1216-round-1-epic-14-the-stack-served-a-tree-three-days-old.md) | Round 1 (EPIC-14) — the stack served a tree three days old, so nothing at `ea4bd05c` was measured | ready — **the first round under the `qa` focus since `STORY-1206`, the first round of the `/qa-cycle epic EPIC-14` invocation, and the first round in this ledger that measured a different commit than the one it reported**. The dev server on `:5173` and the JVM on `:8080` were both rooted in a stale agent worktree at `c3e1a830` (2026-09-07) — **92** commits behind `ea4bd05c` on `web-client/` and `poker-server/` — and three of the files every failing finding is about do not exist on disk in it. `wait-server` and `wait-web` passed because both ask only whether *something* answers on the port, and the report's `COMMIT:` line is the agent's own `HEAD`, never the browser's. Eight of nine findings are **harness** under `ADR-0089` §4 and §Termination rule 6, excluded from `B(1)`, with **no production code changed**; the ninth contradicts no merged source and matches `ADR-0061` §Consequences. `B(1) = 0`, `B(0)` **n/a**, **no** baseline round — and **`PASS` is refused**: `B(1) = 0` here means *nothing was measured*, not *nothing is wrong*, and emitting it would be the coverage inflation `ADR-0089` §2c forbids. Verdict **`STOP_INFRA`**, this section's own exit state, which `.claude/agents/qa-manager.md`'s Step 6 table does not carry — recorded there as an observation rather than filed. It is the **eighth** shift of the round-id convention and it moves the round stories' next free id to **`STORY-1217`** |
