...
| 8 | A refresh leaves the player where they were | `web-client` | **Answered** — [`ADR-0112`](../../docs/adr/ADR-0112-only-a-running-duel-refuses-another-screen.md): only a running duel refuses another screen and the refusal restores the address; a waiting or finished room honors the ask. The mechanism is answered too — [`ADR-0114`](../../docs/adr/ADR-0114-one-predicate-answers-every-ask-and-a-mailed-screen-waits.md): one predicate over the frames rules `honour`/`refuse`/`hold`, the chosen-screen branches move above the store's, the restore is a `useLayoutEffect`, and a mailed screen holds rather than spends |
...
**All eleven are answered and merged.** Nine of them on 2026-09-02, and the last two on 2026-09-04.

The last two were raised differently from all of them: not by planning, but by **driving**.
`STORY-1310`'s `P5` pressed the account offer's accept and found it took the offer and
navigated nowhere — neither outcome `ADR-0112` names — on both layouts, with three independent
signals showing no navigation at all. A player who accepted lost the offer and was told nothing.
`DEC-125` blocked nothing here — the row recorded what was observed, and the story's remaining
paths never depended on the answer — and it is now answered by
[`ADR-0116`](../../docs/adr/ADR-0116-the-accept-is-a-door-and-a-door-that-does-not-open-spends-nothing.md).
The **eleventh** came out of reading that record whole rather than out of any one drive: three
independent routes — `P2`, `P4`, `P6b` — painted a confident *Waiting for your rival* over a room
that was live or already over, which is `ADR-0112` §6's *no lobby on the way* failing while
`ADR-0102` §5 still holds. `DEC-127` blocked nothing today either, and
[`ADR-0118`](../../docs/adr/ADR-0118-a-recovering-browser-shows-nothing-it-was-not-told.md) answers
it before `STORY-1311` is split, which is the only moment it is free.
...
answering ADRs then registered **two** for the architect: `DEC-120`, the turn clock's mechanism
(`ADR-0108`), and `DEC-123`, the navigation guard's (`ADR-0112`). Splitting the epic raised the
**ninth**, `DEC-124`, back to the product owner — whether a surface this product animates owes a
still form under `prefers-reduced-motion`, which nothing in the repository had ever settled.

All eleven are listed under *Answered*, below, and struck in
[`docs/adr/README.md`](../../docs/adr/README.md). Nothing in this epic now waits on a decision.
...
| `DEC-123` | [ADR-0114](../../docs/adr/ADR-0114-one-predicate-answers-every-ask-and-a-mailed-screen-waits.md) | **The architect's, and the only one of the two that item 8 was waiting on.** One pure module — `web-client/src/routing/room-standing.ts` — reads the room off the frames the server already sends (`outcome` → `finished`, `view` → `running`, `roomCode` → `waiting`, `unknown` before any frame has arrived) and rules on the ask: `honour`, `refuse` or `hold`. **`running` is `view !== null && outcome === null`** — `ADR-0105` §2's definition read off frames, grace window and runout inside it — and `ADR-0110`'s move of the host changes nothing, because `waiting` is a standing and not a screen. For the split: `Lobby.tsx`'s **six chosen-screen branches move above** the result, table and room branches and test the ruling rather than the address, so `ADR-0076` §3's frame rule and a refused ask become **one predicate** re-evaluated every render; the restore is a **`useLayoutEffect`** calling the merged `leave()`, which is what makes *one act* one commit with no paint in between; and the mailed `verify`/`reset` screens **hold** — rendering the room's screen and writing nothing to the address — while the standing is unknown, because `VerifyScreen` spends its token in a mount effect and a booting tab reads *no room* for the whole rejoin round trip. `roomAwaited` (computed once in `bootDuelClient`, carried on `DuelProvider` like `signedIn`) is what tells *no room* from *not told yet*, and it only ever withholds. The story's proofs are fixed in the ADR's §7 under `ADR-0100` §5 — pressed doors, `location.hash` assignment, a boot with a room in memory, and a **call count** on `verifyEmail` — and `ADR-0112` §6's six undriven paths are neither added to nor reduced. One residual is named rather than assumed away: a resume into a `PLAYING` room reads `waiting` for one render, closable only by a wire change `ADR-0112` §7 forbids, so a drive that observes it opens a new `DEC`. No wire, no `PROTOCOL_VERSION`, no server file, no new string, no new control, no router |
| `DEC-124` | [ADR-0115](../../docs/adr/ADR-0115-motion-never-carries-a-fact-and-reduced-motion-stills-every-surface.md) | **Motion never carries a fact, and reduced motion stills every surface.** For items 1 and 6, the first continuous motion the product has had: every fact is stated by the surface's **still form** — the same surface at rest — and under `prefers-reduced-motion: reduce` continuous and decorative motion does not run, automatically, with no in-product toggle. Item 1's card owes *waiting*, *acting — moving* and *acting — at rest*, and the at-rest mark answers *whose turn* by itself; item 6's flight is garnish — under reduce, chips appear at the bet line and the won pot at the stack, with every amount stated still by the pot figure (`ADR-0107`) and the stack numerals. Motion values are born `--pd-motion-*` in `design/tokens/tokens.css` beside the sheet's one reduced-motion block (minting, worked with the human per `ADR-0091` §3); pulsing versus running circle stays the human's choice between drawings, exactly as this epic placed it. Pacing is not motion: item 4's per-second clock and `ADR-0102`'s steps stand unchanged. Raised by the planner at split time, registered and answered in the same PR |
| `DEC-125` | [ADR-0116](../../docs/adr/ADR-0116-the-accept-is-a-door-and-a-door-that-does-not-open-spends-nothing.md) | **The accept is a door, and a door that does not open spends nothing.** Pressing *"Keep them with a password"* means one thing — *put me on the account screen where a password can be set* — and **the offer is spent where the player is delivered, never by a press that delivers nothing**: where the product does not put them there, the offer stands and is made again after their next win, as an offer nobody answered. `ADR-0085` §2 is narrowed on one clause (*taking* is reaching the screen, not pressing toward it) with *"Not now"* still spending at the press, and `ADR-0086`'s modified-click case untouched, since a second tab is still reaching it. An answered offer is not shown again on the screen behind it. **Nothing is given back**: no un-dismiss, no migration, no key rename — a rename would re-offer everyone who honestly pressed *Not now* — and the affected population is empty, because nothing is hosted and every browser that pressed it is a QA profile. What today's press does is called neither a *spend* nor a *forfeit* (`ADR-0046` §5): the offer is **taken, and nothing is handed over**. For this epic: item 8 is unchanged in scope, `STORY-1310`'s five undriven paths are untouched, and the whole repair rides on `STORY-1311` landing `ADR-0114`'s branch order — the ADR fixes nothing by itself. It hands `ADR-0114` §6 the question that section left to a drive: the repair writes a **regression test** for an observed defect, non-browser, red at `1a09fb46`. Registers no new `DEC` |
| `DEC-127` | [ADR-0118](../../docs/adr/ADR-0118-a-recovering-browser-shows-nothing-it-was-not-told.md) | **A recovering browser shows nothing it was not told, and a late screen is not a false one** — the eleventh, raised by `TASK-131009` reading `STORY-1310` whole. Answered as **two**, because the sequence the drives caught is two windows of different kinds. **Before any frame** the client paints the front door on `ADR-0114` §5's `unknown` standing, a screen the server stated nothing to support — and it is not a harmless one: `replyToCreateRoom` performs **no seat check** (`DEC-110`, open, is that guard), so a press inside the window repoints the connection's membership and `boot.ts` overwrites the remembered code. `/` therefore renders **no element**, and the silence ends on `RoomJoined` or on a `Failure` carrying its refusal. **After `RoomJoined`**, the stale *Waiting for your rival* is **acceptable-because-late**, said out loud: the server did state it, the gap is one delivery wide, and `replyToJoinRoom`'s `ALREADY_SEATED` branch means a client that waited for a second frame would hang a genuinely waiting host forever. **No interstitial** — *"Dark, quiet, fast, minimal."* and `ADR-0105` §4 — with the dependency named: `/` empty for **longer than about a second** on the `built` re-read `ADR-0117` §6 already owes reopens it. **Here**: it qualifies `ADR-0114` §2 by one condition and hands `STORY-1311` four more tests, including one that **pins the accepted half** so no ticket answers it the other way |
...
**Three decisions were raised after the answered eleven** — one by the split, one by the Definition-of-done drive of 2026-09-05, and one by `TASK-131006`'s drive on 2026-09-10. The first two block nothing here. The third gated this epic's **last ticket** and was registered and answered on the day it was found, so nothing in this epic waits on a decision now:
...
| `DEC-169` | By what change does the QA relay reach the preview origin [`ADR-0117`](../../docs/adr/ADR-0117-the-proofs-of-record-load-the-built-bundle.md) §1 established? `scripts/qa/delay.mjs` dials `connect(port, "127.0.0.1")` while `preview` carries no `host` and so binds `localhost` — `::1` first on this machine — so the relay accepts the browser and cannot forward it, and `TASK-131006`'s drive read `cut 0`: no app socket to sever, and `P3` unmeasurable. Raised by driving, not by planning, like `DEC-125` and `DEC-127` before it | The architect's | `TASK-131006`, this epic's last ticket. **Answered 2026-09-10 by [`ADR-0156`](../../docs/adr/ADR-0156-the-relay-dials-by-name-and-holds-both-loopback-families.md)** — the relay dials its target by name and holds both loopback families; the origin does not move and `ADR-0117` is neither amended nor superseded. Its §4 is the guard nothing supplied before: the relay proves it can reach the target at bring-up and says so when it cannot, so a `cut 0` is never again mistaken for a reading. §6 names the follow-up ticket the drive now waits on |
