...
which registered `DEC-154` for the architect in its place — itself answered on 2026-09-10 by
[`ADR-0147`](../../docs/adr/ADR-0147-the-held-room-is-abandoned-after-the-seat-under-the-joiners-own-stripe.md),
so nothing this epic touched is left open. Five more were
...
**Two are open, and neither blocks anything: one this epic's own newest answer spawned, and one the last split raised.** The third — the one [`ADR-0145`](../../docs/adr/ADR-0145-a-cards-scaffolding-is-composing-and-the-transcriber-names-the-mint.md) registered while answering `DEC-160` — was answered on 2026-09-10 by [`ADR-0149`](../../docs/adr/ADR-0149-minting-is-dispatched-and-a-ladder-value-waits-for-the-human.md) and stands under *Answered* below. Every decision this epic
...
`DEC-158`, below, was the second row this epic had left open, and **it too was answered on
2026-09-10** by
[`ADR-0148`](../../docs/adr/ADR-0148-a-module-names-every-card-that-draws-it-and-every-card-is-registered.md)
— **no**, the `card text ⇒ module` direction gets no register; what gets one is the card side of the
pairing, because two of the three findings that fired `ADR-0142` §7's trigger were its §4
one-module-one-card cardinality rather than the direction. The other row that remained in
`docs/adr/README.md` was `DEC-154`, which no answer of this epic's registered — it is `ADR-0141`'s
own, the architect's, and it blocked only the ticket that implements `ADR-0141`. **It is answered on
2026-09-10** by
[`ADR-0147`](../../docs/adr/ADR-0147-the-held-room-is-abandoned-after-the-seat-under-the-joiners-own-stripe.md)
— the held room is abandoned through `RoomRegistry.mutate` after the seat is taken, found by a new
`waitingRoomsHeldBy(player, except)` rather than by `heldRoom` (which sorts `PLAYING` first and would
answer with the room just joined), under the joining player's own stripe outermost with no two room
mutexes ever held at once, so `replyToJoinRoom` changes by exactly one identifier. It **still changes
nothing this epic ships**: `STORY-1416` is `done` and its `Out of scope` already excluded every
change to `replyToJoinRoom`.
...
**`DEC-161` was registered on 2026-09-09, the architect's, and is answered on 2026-09-10** by
[`ADR-0149`](../../docs/adr/ADR-0149-minting-is-dispatched-and-a-ladder-value-waits-for-the-human.md)
— **minting is a dispatched ticket, and only a value that joins a ladder waits for the human.**
`ADR-0091` §3 said a minting card *"is worked interactively with the human, because taste does not
survive a verify block"*. Measured across all **86** `module: design` tickets, four carry the
`minting` label — `TASK-130301`, `TASK-130601`, `TASK-130701`, `TASK-141501` — and **all four are
`review: standard`, were dispatched to a coder and merged as ordinary PRs**, `TASK-130601` quoting
§3's interactive sentence in its own body and being dispatched anyway. **The merged practice is
ratified and §3's minting branch is superseded, that branch alone**: nothing is owed as repair, and
the interactive session is **un-mandated rather than abolished**. Nothing could execute the old
branch — there is no `await human` state, and `ADR-0091` §2 makes the card the split's **first**
ticket, so the stall is the whole story's, which is the trade §3 itself refused one sentence later
for the verdict. **One exception**: a dispatched mint may not add a colour, or a step in a named
ladder, because inserting into a ladder changes what every existing step means while a new group
falsifies nothing — and it blocks nothing, since the ticket draws at the nearest declared step and
says so. **Not in scope, and untouched**: `ADR-0024` §3 assigns the visual verdict to the human on
every card, and `ADR-0091` names delegating taste as the one thing only the human can give away —
what moved is who **drafts** a value, never who judges it. **Here**: `TASK-141501` stands as merged,
and `STORY-1415`'s own record gains nothing; what the epic owes is §6's line — whether the pane
verdict has been given on `rematch-panel.html` — which measured is recorded nowhere since
2026-08-15.

**`DEC-158` was registered on 2026-09-09, the architect's, and is answered on 2026-09-10.** It
was raised while splitting `STORY-1415`, by reading `design/screens/rematch-states.html` beside the
component it was drawn from: measured on `develop` at `f20d07ed`, the card says `ImKate offers a
rematch` and `Rematch offered — waiting for ImKate` where `RematchControl.tsx:65,74` say *your
rival*, and it draws no frame at all for `That duel room is gone.` — a state that has shipped since
...
repairs the sentences as a transcription with nothing to judge. What **was** undecided is the
...
third direction, and this was that first time. It **blocked nothing** — no ticket in `STORY-1415`
waited on it, and `TASK-141503` handles `ADR-0142` §6's register in both landing orders through
`npm run check` alone.

[`ADR-0148`](../../docs/adr/ADR-0148-a-module-names-every-card-that-draws-it-and-every-card-is-registered.md) answers it: **no, the `card text ⇒ module` direction gets no register — and the reason is that
this drift was never an instance of it.** Sorted by hole, the three findings are **two cardinality
failures and one direction failure**. `RIVAL_OFFERS` and `ROOM_GONE` are `result/rematch-text.ts`'s,
and that module spends `ADR-0142` §4's **one card** on `rematch-panel.html` while the same five
sentences are drawn on `rematch-states.html`; replayed at `f20d07ed`, **three of the five pass and
two fail**, so the merged forward direction catches two of the three findings on its own, aimed at
the card it was not allowed to name. Only `Rematch offered — waiting for ImKate` is §7's, and it is
a bare JSX literal at `RematchControl.tsx:71`. The cardinality bug is **already producing a passing
lie** outside this story: `account-text.ts` registers `SIGN_IN_REFUSED` `notCarded` as *"no frame on
this card draws a refused sign-in"*, true of `account.html` and **false** of
`design/screens/sign-in.html`, whose second frame draws that exact sentence. So the amendment lands
on §4, not §7 — a `PAIRS` row becomes one **(module, card)** pair with a module free to hold several
(six rows), `notCarded` becomes a **checked** claim asserted absent by whole-unit equality and
defaultable once per row, coverage becomes **total over cards** with an unpaired card allowed no
carded value at all, and `RematchControl.tsx:71`'s literal becomes `OFFER_STANDING`. The reverse
register is priced rather than feared: **746 text units** across twelve cards, **386** of them on
`duel-table.html` and `duel-table-states.html`, whose modules export **0** string constants. It
**registers no decision and blocks nothing**; the work is one ticket against
`web-client/src/design/card-text.test.ts` and the two `result/` files.
...
for the architect — answered on 2026-09-10 by
[`ADR-0147`](../../docs/adr/ADR-0147-the-held-room-is-abandoned-after-the-seat-under-the-joiners-own-stripe.md).
It gates no story here and is not this epic's to close: `STORY-1416` is `done`,
...
| `DEC-158` | [`ADR-0148`](../../docs/adr/ADR-0148-a-module-names-every-card-that-draws-it-and-every-card-is-registered.md) | **No — the `card text ⇒ module` direction gets no register, and `ADR-0142` §7's third bullet stands.** The drift that fired §7's trigger was two **cardinality** failures and one direction failure: `RIVAL_OFFERS` and `ROOM_GONE` are `result/rematch-text.ts`'s, which spends §4's one card on `rematch-panel.html` while the same five sentences are drawn on `rematch-states.html` — replayed at `f20d07ed`, **three of five pass and two fail**. The same bug already passes a false reason elsewhere: `SIGN_IN_REFUSED` is `notCarded` as *"no frame on this card draws a refused sign-in"*, false of `sign-in.html`, which draws it. **Amends `ADR-0142` §4**: a row is one **(module, card)** pair, a module may hold several (six rows), `notCarded` becomes a **checked** claim — defaultable per row, asserted absent by whole-unit equality — and coverage becomes **total over cards**, six in `NO_MODULE`, none allowed a carded value. `RematchControl.tsx:71`'s literal becomes `OFFER_STANDING`: one string, not a rule. Reverse register priced at **746 text units**, 386 against modules exporting **0** string constants. Trigger re-armed narrower. **Blocks nothing** |
| `DEC-161` | [`ADR-0149`](../../docs/adr/ADR-0149-minting-is-dispatched-and-a-ladder-value-waits-for-the-human.md) | **Minting is a dispatched ticket, and only a value that joins a ladder waits for the human.** `ADR-0091` §3's minting branch is **superseded, and that branch alone** — the rest of §3, `ADR-0091` §§1, 2, 4, 5 and `ADR-0145` §§1–4 stand byte-unchanged, and **no verdict moves**. Measured on `develop` at `cee03146`: all four `minting` tickets are `review: standard` and all four were dispatched and merged as ordinary PRs, so **the practice is ratified and nothing is owed as repair**; nothing could execute the interactive branch, since there is no `await human` state and `ADR-0091` §2 makes the card the split's **first** ticket. **One exception**: no colour and no step in a named ladder may be minted by dispatch — inserting into a ladder changes what every existing step means — and it blocks nothing, because the ticket draws at the nearest declared step and says so. A mint mints only what the same diff draws and states what each value derives from; `module: design` plus `minting` ⇒ `review: standard`, one lint clause that fails no merged ticket; and **a story that merges a mint records whether the pane verdict has been given** — measured, none is recorded anywhere after 2026-08-15, with **15 of 21 cards** created or modified since. **Here**: `TASK-141501` stands, and this epic owes §6's line for `rematch-panel.html` |
