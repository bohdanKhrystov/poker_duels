...
questions had no merged answer when it opened — `DEC-129`, `DEC-130`, `DEC-131` and
`DEC-133`–`DEC-139` now have one, leaving `DEC-132` — five of them **contradict a merged ADR**, a heavier thing than an open question and
...
`DEC-132`, because each later one is written against the screen the earlier one leaves behind.
Three of the four are answered; **`DEC-132` is the one left**, and `ADR-0130` §*What this does not
settle* names the interaction it should be answered against. The
