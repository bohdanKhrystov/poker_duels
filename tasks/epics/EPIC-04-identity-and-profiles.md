...
**One**, raised on 2026-08-17 by [`ADR-0055`](../../docs/adr/ADR-0055-sign-up-is-budgeted-by-address-and-over-budget-says-so.md)
while answering `DEC-048` the same day. It blocks no ticket and no story.

| ID | Question | Whose | Due |
| --- | --- | --- | --- |
| `DEC-049` | When sign-up answers `429` (`ADR-0055` §3), what is the player told and what does the form do? It is the one refusal on this endpoint that names no field and that the player cannot act on except by waiting, and it can land on somebody who did nothing wrong: `ADR-0055` §4 keys the budget by remote address, so a player behind a shared address may be refused because a stranger signed up. `ADR-0048` §7 fixed that the password rule is *"one sentence a player reads once, and never a verdict on what they typed"*; nothing says whether a throttled player gets a named wait with a plain-language reason, the generic treatment any unexpected error gets, or something quieter. `STORY-0405` ships the status regardless and `STORY-0412` renders it, so nothing waits | the **product owner's** | before `STORY-0412` is split |
...
tickets — by
[`ADR-0055`](../../docs/adr/ADR-0055-sign-up-is-budgeted-by-address-and-over-budget-says-so.md). All
six have moved to the table below. **Nothing in this epic is blocked on a decision, and no story
waits on a human** — `DEC-049` above is the product owner's and is due before a story that has not
been split yet.

The sixteen answered ones are kept here with their answers because the story table still cites them,
